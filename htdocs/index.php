...
$sql .= " INNER JOIN ".MAIN_DB_PREFIX."usergroup AS g";
$sql .= " WHERE u.login = '". $usernm."' AND g.nom = 'CLIENTES'";
$resql = $db->query($sql);
$obj = $db->fetch_object($resql);

if ( !isset($_SESSION['redirect_done']) && ($obj->fk_soc != '')){
    $_SESSION['redirect_done'] = 1;
    $redirection = DOL_URL_ROOT.'/societe/consumptionpersonalized.php?socid='.$obj->rowid;
...
if($action == 'change_warehouse'){
	$new_warehouse = GETPOST('fk_warehouse');
	$user -> fk_warehouse = $new_warehouse;
	$user -> update($user);
	$action = '';
}
...
// Default warehouse
if (!empty($conf->stock->enabled) && $user->rights->user->user->modify_warehouse)
{
	print '<div class="center" style="background-color: #ececf4; width: 50%; border: 1px solid #8888;">';
	print '<form name="add_user_ldap" action="'.$_SERVER["PHP_SELF"].'" method="post">';
	print '<input type="hidden" name="action" id="action" value="change_warehouse"/>';
	print '<p class="inline-block" style="font-size:1.1rem; margin-right: 10px;">Seleccione almacén deseado: </p>';
	print $formproduct->selectWarehouses($user->fk_warehouse, 'fk_warehouse', 'warehouseopen', 0);
	print '<input type="submit" class="button center" style="margin-left:10px;" value="Guardar"/>';
	print '</form>';
	print '</div>';
}

// Listado de los productos a caducar
require_once "productos_a_caducar.php";

// // Dolibarr Working Board with weather
// /*
//  * Aqui comienzan los paneles, uno por uno de acuerdo a permisos
// */
...
//     if (!empty($conf->projet->enabled) && ($user->rights->paneles->proyectos || $user->admin)  ) {
//         include_once DOL_DOCUMENT_ROOT . '/projet/class/project.class.php';
//         $board = new Project($db);
//         $dashboardlines[$board->element.'_borrador'] = $board->load_board($user, "draft");
//         $dashboardlines[$board->element] = $board->load_board($user, "opened");
//     }

//     // Products
//     if (!empty($conf->projet->enabled) && ($user->rights->paneles->productos || $user->admin)) { // permisos para modulo productos
//         include_once DOL_DOCUMENT_ROOT . '/product/class/product.class.php';
//         $board = new Product($db);
//         $dashboardlines[$board->element.'_sin_stock'] = $board->load_board($user, "stock");
//         $dashboardlines[$board->element.'_mas_vendidos'] = $board->load_board($user, "mas_vendidos");
//         $dashboardlines[$board->element.'_menos_vendidos'] = $board->load_board($user, "menos_vendidos");
//         $dashboardlines[$board->element.'_por_transferir'] = $board->load_board($user, "por_transferir");
//         $dashboardlines[$board->element.'_alertas_stock'] = $board->load_board($user, "alertas_stock");
//     }

//     // Number of tasks to do (late) TODO: agregar este permiso al modulo paneles en tabla de permisos  y validarlo
//     if (!empty($conf->projet->enabled) && empty($conf->global->PROJECT_HIDE_TASKS)) {
...
//     if (!empty($conf->propal->enabled) && ($user->rights->paneles->prop_comerciales || $user->admin) ) {
...
//     // Number of commercial proposals opened (expired)
//     if (!empty($conf->supplier_proposal->enabled) && ($user->rights->paneles->presupuestos_proveedor || $user->admin)) {
...
//     if (!empty($conf->commande->enabled) && ($user->rights->paneles->pedidos_clientes || $user->admin) ) {
...
//     if (!empty($conf->supplier_order->enabled) && ($user->rights->paneles->pedidos_proveedor || $user->admin)) {
...
//     // Clientes
//     if ($user->rights->paneles->clientes|| $user->admin) { // permisos para modulo productos
//         include_once DOL_DOCUMENT_ROOT . '/societe/class/societe.class.php';
//         $board = new Societe($db);
//         $dashboardlines[$board->element.'_cliente_excedido'] = $board->load_board($user, "cliente_excedido");
//     }

//     // Proveedores
//     if ($user->rights->paneles->proveedores || $user->admin) { // permisos para modulo productos
//         include_once DOL_DOCUMENT_ROOT . '/societe/class/societe.class.php';
//         $board = new Societe($db);
//         $dashboardlines[$board->element.'_proveedor_excedido'] = $board->load_board($user, "proveedor_excedido");
//     }
//     // Number of invoices customers (has paid)
//     if (!empty($conf->facture->enabled) && ($user->rights->paneles->cuentas_cobrar || $user->admin)) {
...
//     if (!empty($conf->supplier_invoice->enabled) && ($user->rights->paneles->cuentas_pagar || $user->admin)) {
...
//     if (!empty($conf->banque->enabled) && ($user->rights->paneles->banco || $user->admin) && !$user->socid) {
...
//     if (!empty($conf->banque->enabled) && ($user->rights->paneles->banco || $user->admin) && !$user->socid && empty($conf->global->BANK_DISABLE_CHECK_DEPOSIT)) {
...
//     if (!empty($conf->expensereport->enabled) && ($user->rights->paneles->gastos || $user->admin)) {
...
//     if (!empty($conf->holiday->enabled) && ($user->rights->paneles->dias_libres || $user->admin)) {
...
//                 'stats' => array('project_borrador', 'project', 'project_task'),
//             ),
//         'product' =>
//             array(
//                 'groupName' => 'Products',
//                 'globalStatsKey' => 'products',
//                 'stats' => array('product_sin_stock', 'product_mas_vendidos', 'product_menos_vendidos',
//                     'product_por_transferir', 'product_alertas_stock'),
//             ),
//         'propal' =>
//             array(
//                 'groupName' => 'Proposals',
...
//                 'stats' => array('societe_cliente_excedido'),
//             ),
//         'proveedores' =>
//             array(
//                 'groupName' => 'Proveedores',
//                 'globalStatsKey' => 'societe',
//                 'stats' => array('societe_proveedor_excedido'),
//             ),
//         'bank_account' =>
//             array(
...
//         /* Aqui los titulos y lo especifico de cada panel */
...
//                     $openedDashBoard .= '			<a href="'.$board->url.'" class="info-box-text">'.$infoName.' : <span class="'.$nbtodClass.' classfortooltip" title="'.$board->label.'" >';
//                     if ($board->nbtodo > 0) $openedDashBoard .=$board->nbtodo;
//                     $openedDashBoard .= '</span>'.$textLate.'</a>'."\n";
