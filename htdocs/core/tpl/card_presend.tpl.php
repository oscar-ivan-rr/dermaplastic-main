...
if ($action == 'presend')
{
	...
	/** Attaching all files to mail  */
	$filedirectory = $diroutput . '/' . get_exdir($object->id, 2, 0, 0, $object, $object->element);
	$filelist = scandir($filedirectory);
	$num = sizeof($filelist); 
	$i = 0;
	while($i < $num){
		$ext = explode('.', $filelist[$i])[1];
		$name = explode('.', $filelist[$i])[0];
		/** If is an image from CFDI we skip it */
		if($ext == 'png')
			unset($filelist[$i]);
		else{
			if($name == $object->ref)
				$filelist[$i] = $filedirectory.$name.'/'.explode('.', $filelist[$i])[0].'.pdf';
			else
				unset($filelist[$i]);
		}
		$i++;
	}
	/** Unsetting . and .. files in dir  */
	unset($filelist[0]);
	unset($filelist[1]);
	...
	$formmail->param['fileinit'] = $filelist;
