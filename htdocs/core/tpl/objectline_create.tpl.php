...
if(($this->element == 'commande') && $this->statut == 0){
    $col =' COLSPAN="2"';
}elseif(($this->element == 'order_supplier' || $this->element == 'supplier_proposal') && $this->statut == 0 && count($this->lines)>0){
	$col =' COLSPAN="3"';
}
else $col = '';
...
if ($nolinesbefore) {
	...
	<tr class="liste_titre<?php echo (($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_add_') ?> nodrag nodrop">
		...
		<td class="linecoldescription minwidth500imp">
			<div id="add"></div><span class="hideonsmartphone" style="font-size: 18px;"><?php echo $langs->trans('AddNewLine'); ?></span><?php // echo $langs->trans("FreeZone"); ?>
		</td>
        <?php
        if($this->element == 'commande' && $this->statut == 0)
        {
        ...
        <?php if($this->table_element_line == 'facturedet' || $this->table_element_line == 'commandedet' || $this->table_element_line == 'propaldet'
        ...
        || $this->table_element_line == 'supplier_proposaldet') { ?>
            <td class="linecolunit right"><?php echo $langs->trans('Unit'); ?></td>
            <?php
        }
		...
		<?php /*if ($object->element == 'propal' || $object->element == 'supplier_proposal') { ?>
			<td class="linecol right"><span id="title_unit"><?php echo $langs->trans('Unit'); ?></span></td>
		<?php }*/ ?>
...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	$coldisplay++;
	?>
	<td class="nobottom linecoldescription" <?=$col?> >
		...
		if (!empty($conf->product->enabled) || !empty($conf->service->enabled))
		{
			...
			else
			{
				...
				if($object->element == 'order_supplier' && $this->statut == 0)
				{
					// Add barcode scanning input
					echo '<div class="barcode-scanner-container" style="margin-top: 10px;">';
					echo '<label for="barcode_input"><i class="fa fa-barcode"></i> ' . $langs->trans('Código de barras') . ':</label> ';
					echo '<input type="text" id="barcode_input" name="barcode_input" class="flat minwidth200" autocomplete="off" placeholder="' . $langs->trans('Utilice un escáner o escriba el código') . '">';
					echo '</div>';
				}
		...
		$doleditor->Create();
        //Solo para pedidos
        if($object->table_element == 'commande' && $this->statut == 0)
        {
            echo '<span class="button" id="showdes">Añadir descripción</span>';
            echo '<span class="button" id="hidedes" style="display: none;">Ocultar descripción</span>';
            print '<script>';
            print '$(document).ready(function(){';
            print ' $("#dp_desc").hide();';
            print ' $("#showdes").click(function(){$("#dp_desc").show();$("#showdes").hide();$("#hidedes").show();});';
            print ' $("#hidedes").click(function(){$("#dp_desc").hide();$("#hidedes").hide();$("#showdes").show();});';
            print '});';
            print '</script>';
        }
        ?>
    <td class="nobottom linecolqty right"><input type="text" size="2" name="qty" id="qty" class="flat right" value="<?php echo (isset($_POST["qty"]) ?GETPOST("qty", 'alpha', 2) : 1); ?>">
        ...
        $coldisplay++;
		//UNIDAD ENTRADA/SALIDA
		print '<td class="nobottom linecoluseunit right">';
            print '<select name="units" id="units" class="flat" style="width: 100px">';
            $sql = "SELECT scale,label FROM " . MAIN_DB_PREFIX . "c_units WHERE active = 1";
            $result = $this->db->query($sql);
			$selected = 7;
            if ($result) {
                while ($unidades = $this->db->fetch_object($result)) {
					if($unidades->scale == 7) $selected = $unidades->scale;
                    print '<option value="' . $unidades->scale . '" ' . ($unidades->scale == $selected ? 'selected' : '') . '>' . $langs->trans($unidades->label) . '</option>';
                }
            }
			print '</select>';
        ...
        //if($object->element != 'order_supplier'  || $object->element != 'invoice_supplier'){
          //  $res=$object->getLinesArray();
            //if ($res>0 && $this->table_element_line == 'propaldet'){
	...
	<!--<td class="nobottom linecolresupplier"><input id="fourn_ref" name="fourn_ref" class="flat minwidth50 maxwidth150" value="<?php //echo (isset($_POST["fourn_ref"]) ?GETPOST("fourn_ref", 'alpha', 2) : ''); ?>"></td>-->
		...
		else echo $form->load_tva('tva_tx', 16, $seller, $buyer, 0, 0, '', false, 1);
		?>
	</td>
	...
	$coldisplay++;
	?>
	<td class="nobottom nowrap linecoldiscount right"><input type="text" size="7" name="remise_percent" id="remise_percent" class="flat right" value="<?php echo (isset($_POST["remise_percent"]) ?GETPOST("remise_percent", 'alpha', 2) : $remise_percent); ?>"><span class="hideonsmartphone">%</span></td>
	<?php
	if ($this->situation_cycle_ref) {
		$coldisplay++;
	...
	$coldisplay += $colspan;
	?>
    <?php /*if ($object->element == 'propal' || $object->element == 'supplier_proposal') { $coldisplay++ ?>
        <td class="nobottom linecol right"><?php $coldisplay++; ?>
            <input type="text" size="5" name="unit" id="unit" class="flat right" value="<?= GETPOST('unit') ?>">
        </td>
    <?php }*/ ?>
...
if ((!empty($conf->service->enabled) || ($object->element == 'contrat')) && $dateSelector && GETPOST('type') != '0')	// We show date field if required
{
	...
	print '</script>';
	print '</td>';
	//if($this->element == 'order_supplier') print '<td> &nbsp; </td>';
	print '</tr>'."\n";
}
...
?>
		...
		if (empty($conf->global->MAIN_DISABLE_EDIT_PREDEF_PRICEHT) && empty($senderissupplier))
		{
			...
			if ((jQuery('#idprod').val() > 0 || jQuery('#idprodfournprice').val()) && ! isNaN(pbq) && pbq > 0)
			{
				...
				$.post('<?php echo DOL_URL_ROOT; ?>/product/ajax/products.php?action=fetch',
					{ 'id': $(this).val(), 'socid': <?php print $object->socid; ?>,'table_element': '<?php print $this->table_element_line; ?>'},
					function(data) {
                        console.log(data);
						console.log("Load unit price end, we got value "+data.price_ht);
						jQuery("#price_ht").val(data.price_ht);
						jQuery("#units").val(data.umed);console.log(data.umed);
						jQuery("#remise_percent").val(data.discount);
						console.log(data.Uentrada);

						<?php
						if($this->table_element_line == 'facturedet' || $this->table_element_line == 'commandedet' || $this->table_element_line == 'propaldet'){ 
						?>
							$("#units option[value="+data.Usalida+"]").prop('selected', true);
						<?php
						}
						...
						?>
							$("#units option[value="+data.Uentrada+"]").prop('selected', true);
						<?php
						}?>

                        //Aqui va lo de claveProdServ y umed
            <?php if($object->element == 'facture'){?>
                        jQuery("#options_claveprodserv").val(data.claveprodser);
                        jQuery("#options_umed").val(data.umed);
						jQuery("#options_noidenticfdi").val(data.noidenticfdi);
						if(data.exentoiva == 1){
								jQuery("#options_exentoiva").prop('checked', true);
						}
						jQuery("#options_objimp").val(data.objimp);

            <?php }?>
						//jQuery("#unit").val(data.umed);
					},
					'json'
				);
			}
	...
	function setforfree() {
		...
		jQuery("#price_ht, #multicurrency_price_ht, #price_ttc, #fourn_ref, #tva_tx, #buying_price, #title_vat, #title_up_ht, #title_up_ht_currency, #title_up_ttc, #title_up_ttc_currency").show();
    ...
    print '<script type="text/javascript">
			$(document).ready(function() {
				$("#idprodfournprice").change(function() {
					var socid = $(this).val();
        			$.ajax("'.DOL_URL_ROOT.'/core/ajax/ajax_price.php", {
                        type: "post",   // Usually post or get
                        async: true,
                        data: {soc: socid},
                        success: function (result) {
                            result = JSON.parse(result);                      
                           $("#price_ht").val(result.precio);   
                        }
				    });
			    });
			});
...
if($object->element == 'order_supplier' && $this->statut == 0) {
	// Add JavaScript for barcode scanning functionality
	print /*html*/'<script type="text/javascript">
	jQuery(document).ready(function() {
		var barcodeInput = $("#barcode_input");
		
		barcodeInput.focus();
		
		var instructionMsg = $("<div>")
			.addClass("barcode-instruction")
			.html("<i class=\"fa fa-info-circle\"></i> Escanee el código de barras o escríbalo y presione Enter")
			.css({
		...
		$(".barcode-scanner-container").after(instructionMsg);
		...
		function processBarcode(barcode) {
			...
			barcodeInput.addClass("searching");
			...
			$.ajax({
				...
				success: function(response) {
					barcodeInput.removeClass("searching");
					barcodeInput.prop("disabled", false);
					
					if (response && response.success && response.product_id) {
						directlySetProduct(response.product_id, response.ref);
					} else {
						console.error("Producto no encontrado: " + barcode);
						barcodeInput.css("background-color", "#f2dede");
		...
		function directlySetProduct(productId, productRef) {
			if (!productId) return;
			
			var isSupplierOrder = $("#idprodfournprice").length > 0;
			
			try {
				if (isSupplierOrder) {
					var productValue = "idprod_" + productId;
					var selectElement = $("#idprodfournprice");
					
					var optionExists = false;
					selectElement.find("option").each(function() {
						if ($(this).val() === productValue) {
							optionExists = true;
							return false;
						}
					});
					...
					if (optionExists) {
						selectElement.val(productValue).change();
						showSuccess("Producto seleccionado correctamente");
					} else {
						var searchField = $("#search_idprodfournprice");
						if (searchField.length > 0) {
							searchField.val("");
							searchField.val(productRef);
							searchField.trigger("keydown").trigger("input").trigger("change");
							waitForDropdownAndSelect(searchField, productRef);
						} else {
							showManualMessage(productRef);
						}
					}
				} else {
					var selectElement = $("#idprod");
					...
					if (optionExists) {
						selectElement.val(productId).change();
						showSuccess("Producto seleccionado correctamente");
					} else {
						var searchField = $("#search_idprod");
						if (searchField.length > 0) {
							searchField.val("");
							searchField.val(productRef);
							searchField.trigger("keydown").trigger("input").trigger("change");
				...
				console.error("Error al seleccionar producto:", e);
				showManualMessage(productRef);
			}
		}
		
		function waitForDropdownAndSelect(searchField, productRef) {
			var waitingMsg = $("<div>")
				.addClass("barcode-waiting")
				.html("<i class=\"fa fa-spinner fa-spin\"></i> Buscando producto: " + productRef)
				.css({
			...
			$(".barcode-scanner-container").after(waitingMsg);
			
			var attempts = 0;
			var maxAttempts = 20; // 20 attempts × 300ms = 6 seconds max
			var checkInterval = setInterval(function() {
				attempts++;
				var dropdownItems = $(".ui-menu-item");
				...
				if (dropdownItems.length > 0) {
					...
					if (dropdownItems.length > 1) {
						...
						dropdownItems.each(function(index) {
							...
							if (itemText.indexOf(productRef) !== -1) {
								var matchScore = productRef.length / itemText.length;
						...
						if (bestMatch) {
							bestMatch.find("a").click();
							showSuccess("Producto \'" + bestMatch.text().trim() + "\' seleccionado");
						} else {
							dropdownItems.first().find("a").click();
							showSuccess("Producto seleccionado (primer resultado)");
						}
					} else {
						dropdownItems.first().find("a").click();
						showSuccess("Producto seleccionado");
					}
		...
		function showSuccess(message) {
			barcodeInput.css("background-color", "#dff0d8");
			barcodeInput.val("");
			
			var successMsg = $("<div>")
				.addClass("barcode-success")
			...
			$(".barcode-scanner-container").after(successMsg);
		...
		function showManualMessage(productRef) {
			barcodeInput.css("background-color", "#fcf8e3");
			barcodeInput.val("");
			
			var message = $("<div>")
				.addClass("barcode-message")
				.html("<i class=\"fa fa-exclamation-triangle\"></i> Por favor seleccione manualmente: " + productRef)
				.css({
					"background-color": "#fcf8e3",
					"border": "1px solid #faebcc",
					"color": "#8a6d3b",
			...
			$(".barcode-scanner-container").after(message);
			...
			if ($("#search_idprodfournprice").length > 0) {
				$("#search_idprodfournprice").focus();
			} else if ($("#search_idprod").length > 0) {
				$("#search_idprod").focus();
			}
		}
		...
		.html("\
		.barcode-scanner-container { margin-bottom: 10px; }\
		#barcode_input { padding-left: 5px; transition: background-color 0.3s ease; }\
		#barcode_input.searching { background-color: #f5f5f5; }\
		")
		.appendTo("head");
	});
	</script>';
