...
<?php
if ($this->element == 'facture' && $this->statut == 0){
	$coldisplay+=2;
	print '<td colspan="2" class="dropdown-toggle" id="extra-toggle-'.$line->id.'" style="curso r: pointer;"></td>';
}
?>
...
?>
    ...
    ?>
<?php if ($this->element == 'propal' && !empty($user->rights->propal->client_to_fourn) && $this->statut == 0) { //Cot a proveedor?>
	<td>&nbsp;</td>
<?php } ?>
	<td>
	...
	/*
	// Unit only for propal
	if ($object->element == 'propal' || $object->element == 'supplier_proposal') {
		$coldisplay++;
		print '<td class="right"><input size="5" type="text" class="flat right" name="unit" value="'.$line->fk_unit.'"></td>';
	}else{
		echo '<td></td>';
	}*/
    $coldisplay++;
	...
	if ($result) {
		while ($unidades = $this->db->fetch_object($result)) {
			if($line->fk_unit == $unidades->scale){
				print '<option value="' . $unidades->scale . '" selected>' . $langs->trans($unidades->label) . '</option>';
			}else{
				print '<option value="' . $unidades->scale . '">' . $langs->trans($unidades->label) . '</option>';
			}
		}
	}
	print '</select>';
	...
	*/
	...
	print '<input type="hidden" name="tva_tx" value="'.price(16).'">';
	...
	<?php $coldisplay++;
	if (($line->info_bits & 2) != 2) {
		print '<input size="2" type="text" class="flat right" name="remise_percent" id="remise_percent" value="' . $line->remise_percent . '"';
		if ($line->fk_prev_id != null ) print ' readonly';
		print '>%';
	} else { ?>
		&nbsp;
	...
	if($this->element == 'invoice_supplier'){
		print '<td></td>';
		print '<td class="right">Descuento Financiero<br>';
		if($line->compta_discount == null){
			$comptadTemporal = 0;
		}else{
			$comptadTemporal = $line->compta_discount;
		}
		print '<input size="2" type="text" class="flat right" name="compta_discount" id="compta_discount" value="' .$comptadTemporal. '">%';
		print '</td>';
		$coldisplay++;
	}
	?>
...
if ($object->element == 'invoice_supplier') {
	print '<input type="hidden" name="notfound" value="'.$line->notfound.'">';

	if ($line->notfound && $user->rights->produit->creer) {
		print '<tr class="oddeven tredited">';
		print '<td colspan="1" class="center valignmiddle">';
		print $form->selectarray('methodOffNotExist', array(1 => 'Relacionar a producto', 2 => 'Crear producto'), 1);
		print '</td>';
		
		print '<td colspan="'. ($coldisplay-($colspan+1)) .'" id="addNotExist" class="center valignmiddle">';
		// Add to product defined.
		print '<div id="containerAddToProduct">';
		if (!empty($conf->global->ENTREPOT_EXTRA_STATUS)) {
			// hide products in closed warehouse, but show products for internal transfer
			$form->select_produits('', 'addToProduct', 0, $conf->product->limit_size, 0, 1, 2, '', 3, array(), 0, '1', 0, 'maxwidth500', 0, 'warehouseopen,warehouseinternal');
		} else {
			$form->select_produits('', 'addToProduct', 0, $conf->product->limit_size, 0, 1, 2, '', 3, array(), 0, '1', 0, 'maxwidth500', 0, '');
		}
		print '</div>';
		print '</td>';

		// Action button
		print '<td colspan="'.$colspan.'" class="center valignmiddle">';
		print '<button type="button" class="button" id="actionNotExist">'.$langs->trans("Add").'</button>';
		print '</td>';

		print '</tr>';
		
		?>
		<!-- Script to handle factureline product -->
		<script type="text/javascript" charset="utf-8">
			jQuery(document).ready(function() {
				// Toggle between addToProduct or createProduct
				jQuery("#methodOffNotExist").change(function() {
					let option = this.value;
					let action = $("#actionNotExist");

					if (option == 1) {
						jQuery('#containerAddToProduct').show();
						action.text("Añadir");
					} 
					else if (option == 2) {
						jQuery('#containerAddToProduct').hide();
						action.text("<?= $langs->trans("Create") ?>");
					} 
				});

				// Action button clicker
				jQuery("#actionNotExist").click(function() {
					let form = $('form#addproduct');
					let option = $('#methodOffNotExist').val();
					// alert(option);
					
					// Add to defined product
					if (option == 1) {
						let prodid = jQuery('#addToProduct').val();
						// Submit
						if (prodid > 0) {
							document.querySelector("input[name='notfound']").value = 0;
							document.querySelector("input[name='productid']").value = prodid;
							form.submit();
						}
						else $.jnotify('Seleccione un producto para relacionar', 'error');
					}
					// Create new product
					else if (option == 2) {
						// Changes
						form.find("input[name='action']").val('create');
						form.attr('action', '<?= DOL_MAIN_URL_ROOT ?>/product/card.php?type=0');
						// Backtopage
						form.append('<input type="hidden" name="backtopage" value="<?= urlencode($_SERVER["PHP_SELF"].'?id='.$object->id.'&action=addprod_toline&lineid='.$line->id) ?>"><input type="hidden" name="origin" value="<?= $object->element ?>">');

						// Transform inputs
						document.querySelector("#fourn_ref").setAttribute('name', 'ref');
						document.querySelector("#product_desc").setAttribute('name', 'label');
						form.submit();
					}
				});
			});
...
if (! empty($conf->service->enabled) && $line->product_type == 1 && $dateSelector) { ?>
...
if ($this->element == 'facture' && $this->statut == 0){
	print '<script>
		$(document).ready(function() {
			$("#extra-toggle-'.$line->id.'").on("click", function(event) {
		$("#extrarow-facturedet_claveprodserv_'.$line->id.'").toggle("slow");
		$("#extrarow-facturedet_umed_'.$line->id.'").toggle("slow");
		$("#extrarow-facturedet_noidenticfdi_'.$line->id.'").toggle("slow");
		$("#extrarow-facturedet_exentoiva_'.$line->id.'").toggle("slow");
		$("#extrarow-facturedet_objimp_'.$line->id.'").toggle("slow");
		//$("#extrafield_lines_area_'.$line->id.'").toggleClass("open");
			});
		});
		</script>';
}
