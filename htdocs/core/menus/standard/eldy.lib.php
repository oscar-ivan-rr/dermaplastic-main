...
function print_eldy_menu($db, $atarget, $type_user, &$tabMenu, &$menu, $noout = 0, $mode = '')
{
	...
	$menu_arr[] = array(
		'name' => 'Compta',
		'link' => '/compta/facture/list.php?mainmenu=billing&amp;leftmenu=',
		'title' =>  "MenuFinancial",
		'level' => 0,
	    'enabled' => $showmode = isVisibleToUserType($type_user, $tmpentry, $listofmodulesforexternal),
		'target' => $atarget,
	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		if ($mainmenu == 'companies')
		{
			...
			if (!empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS))
			{
				...
				$newmenu->add("/societe/list.php?type=c&amp;leftmenu=customers", $langs->trans("Clientes"), 0, $user->rights->societe->lire, '', $mainmenu, 'customers');
				
				$newmenu->add("/societe/list.php?type=c&amp;leftmenu=customers", $langs->trans("Listado"), 1, $user->rights->societe->lire);

				$newmenu->add("/societe/card.php?leftmenu=customers&amp;action=create&amp;type=c", $langs->trans("MenuNewCustomer"), 1, $user->rights->societe->creer);
			...
			if (!empty($conf->societe->enabled) && (!empty($conf->fournisseur->enabled) || !empty($conf->supplier_proposal->enabled)))
			{
				...
				$newmenu->add("/societe/list.php?type=f&amp;leftmenu=suppliers", $langs->trans("Proveedores"), 0, ($user->rights->fournisseur->lire || $user->rights->supplier_proposal->lire), '', $mainmenu, 'suppliers');
				$newmenu->add("/societe/list.php?type=f&amp;leftmenu=suppliers", $langs->trans("Listado"), 1, ($user->rights->fournisseur->lire || $user->rights->supplier_proposal->lire), '', $mainmenu, 'suppliers');
				$newmenu->add("/societe/card.php?leftmenu=suppliers&amp;action=create&amp;type=f", $langs->trans("MenuNewSupplier"), 1, $user->rights->societe->creer && ($user->rights->fournisseur->lire || $user->rights->supplier_proposal->lire));
			}
			...
			$newmenu->add('../custom/import/index.php?element=societe', $langs->trans("Import"), 0, $user->rights->societe->lire, '', $mainmenu, 'receptions');
			$newmenu->add('../custom/import/index.php?element=societe&type=2', $langs->trans("ImportCustomers"), 1, $user->rights->societe->creer);
			$newmenu->add('../custom/import/index.php?element=societe&type=3', $langs->trans("ImportSuppliers"), 1, $user->rights->societe->creer);
            //Export
            $newmenu->add('../custom/export/index.php?element=societe', $langs->trans("Export"), 0, $user->rights->societe->lire, '', $mainmenu, 'receptions');
            $newmenu->add('../custom/export/index.php?element=societe&type=2&action=export', $langs->trans("ExportCustomers"), 1, $user->rights->societe->creer);
            $newmenu->add('../custom/export/index.php?element=societe&type=3&action=export', $langs->trans("ExportSuppliers"), 1, $user->rights->societe->creer);
			// Contacts
		...
		if ($mainmenu == 'commercial')
		{
			...
			{
				$langs->load("propal");
				$newmenu->add("/comm/propal/index.php?leftmenu=propals", $langs->trans("Cotizaciones a clientes"), 0, $user->rights->propale->lire, '', $mainmenu, 'propals', 100);
				$newmenu->add("/comm/propal/card.php?action=create&amp;leftmenu=propals", $langs->trans("Nueva cotización"), 1, $user->rights->propale->creer);
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "propals") {
                    //Borradores
				    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."propal WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Firmados
                    $sql="SELECT COUNT(rowid) as firmados FROM ".MAIN_DB_PREFIX."propal WHERE fk_statut=2";
                    $resql=$db->query($sql);
                    $firmados = $db->fetch_object($resql)->firmados;
                    //no Firmados
                    $sql="SELECT COUNT(rowid) as nofirmados FROM ".MAIN_DB_PREFIX."propal WHERE fk_statut=3";
                    $resql=$db->query($sql);
                    $nofirmados = $db->fetch_object($resql)->nofirmados;
                    $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=0", $langs->trans("PropalsDraft")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->propale->lire);
				    $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=1", $langs->trans("PropalsOpened"), 2, $user->rights->propale->lire);
				    $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=2", $langs->trans("PropalStatusSigned")." <span class=\"badge badge-info classfortooltip\" title=\"Firmados\">".$firmados."</span>", 2, $user->rights->propale->lire);
				    $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=3", $langs->trans("PropalStatusNotSigned")." <span class=\"badge badge-info classfortooltip\" title=\"No Firmados\">".$nofirmados."</span>", 2, $user->rights->propale->lire);
            ...
            {
                $langs->load("orders");
                $newmenu->add("/commande/index.php?leftmenu=orders", $langs->trans("Pedidos de Clientes"), 0, $user->rights->commande->lire, '', $mainmenu, 'orders', 200);
                ...
                if ($usemenuhider || empty($leftmenu) || $leftmenu == "orders") {
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."commande WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Validado
                    $sql="SELECT COUNT(rowid) as validados FROM ".MAIN_DB_PREFIX."commande WHERE fk_statut=1";
                    $resql=$db->query($sql);
                    $validados = $db->fetch_object($resql)->validados;
                    //Expedicion en curso
                    $sql="SELECT COUNT(rowid) as expedicion FROM ".MAIN_DB_PREFIX."commande WHERE fk_statut=2";
                    $resql=$db->query($sql);
                    $expediciones = $db->fetch_object($resql)->expedicion;
                    $newmenu->add("/commande/list.php?leftmenu=orders&viewstatut=0", $langs->trans("StatusOrderDraftShort")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->commande->lire);
                    $newmenu->add("/commande/list.php?leftmenu=orders&viewstatut=1", $langs->trans("StatusOrderValidated")." <span class=\"badge badge-info classfortooltip\" title=\"Validados\">".$validados."</span>", 2, $user->rights->commande->lire);
                    if (!empty($conf->expedition->enabled)) $newmenu->add("/commande/list.php?leftmenu=orders&viewstatut=2", $langs->trans("StatusOrderSentShort")." <span class=\"badge badge-info classfortooltip\" title=\"Expediciones en curso\">".$expediciones."</span>", 2, $user->rights->commande->lire);
				...
				$newmenu->add("/commande/devoluciones.php?leftmenu=orders", $langs->trans("OrdersVsShipmentsReport"), 1, $user->rights->commande->lire);
			...
			{
				$langs->load("orders");
				$newmenu->add("/fourn/commande/index.php?leftmenu=orders_suppliers", $langs->trans("OC a Proveedores"), 0, $user->rights->fournisseur->commande->lire, '', $mainmenu, 'orders_suppliers', 400);
				$newmenu->add("/fourn/commande/card.php?action=create&amp;leftmenu=orders_suppliers", $langs->trans("Nueva Orden de Compra"), 1, $user->rights->fournisseur->commande->creer);
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "orders_suppliers") {
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."commande_fournisseur WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Aprobados
                    $sql="SELECT COUNT(rowid) as aprobados FROM ".MAIN_DB_PREFIX."commande_fournisseur WHERE fk_statut=2";
                    $resql=$db->query($sql);
                    $aprobados = $db->fetch_object($resql)->aprobados;
                    //Pedidos
                    $sql="SELECT COUNT(rowid) as pedidos FROM ".MAIN_DB_PREFIX."commande_fournisseur WHERE fk_statut=3";
                    $resql=$db->query($sql);
                    $pedidos = $db->fetch_object($resql)->pedidos;
                    //recibido parcialmente
                    $sql="SELECT COUNT(rowid) as recibidosp FROM ".MAIN_DB_PREFIX."commande_fournisseur WHERE fk_statut=4";
                    $resql=$db->query($sql);
                    $recibidosp = $db->fetch_object($resql)->recibidosp;
                    $newmenu->add("/fourn/commande/list.php?leftmenu=orders_suppliers&statut=0", $langs->trans("StatusSupplierOrderDraftShort")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->fournisseur->commande->lire);
                    ...
                    $newmenu->add("/fourn/commande/list.php?leftmenu=orders_suppliers&statut=2", $langs->trans("StatusSupplierOrderApprovedShort")." <span class=\"badge badge-info classfortooltip\" title=\"Aprobados\">".$aprobados."</span>", 2, $user->rights->fournisseur->commande->lire);
                    $newmenu->add("/fourn/commande/list.php?leftmenu=orders_suppliers&statut=3", $langs->trans("StatusSupplierOrderOnProcessShort")." <span class=\"badge badge-info classfortooltip\" title=\"Pedidos\">".$pedidos."</span>", 2, $user->rights->fournisseur->commande->lire);
                    $newmenu->add("/fourn/commande/list.php?leftmenu=orders_suppliers&statut=4", $langs->trans("StatusSupplierOrderReceivedPartiallyShort")." <span class=\"badge badge-info classfortooltip\" title=\"Recibidos parcialmente\">".$recibidosp."</span>", 2, $user->rights->fournisseur->commande->lire);
                    $newmenu->add("/fourn/commande/list.php?leftmenu=orders_suppliers&statut=5", $langs->trans("Recibido completo"), 2, $user->rights->fournisseur->commande->lire);
			...
			// Ocultar menu presupuestos de proveedor
			print '<script>';
			print 'window.addEventListener("DOMContentLoaded", function() {';
			print 'var elements = document.querySelectorAll(".blockvmenu.blockvmenuimpair.blockvmenulast");';
			print 'for (var i = 0; i < elements.length; i++) {';
			print 'elements[i].style.display = "none";';
			print '}';
			print '});';
			print '</script>';
		}
		...
		if ($mainmenu == 'billing')
		{
			...
			//Resumen
			$newmenu->add("/compta/index.php?mainmenu=billing&amp;leftmenu=customers_bills", "Resumen", 0, 1, '', $mainmenu, 'customers_bills');
			...
			if (!empty($conf->facture->enabled))
			{
				...
				$newmenu->add("/compta/facture/list.php?leftmenu=customers_bills", $langs->trans("Listado"), 1, $user->rights->facture->lire, '', $mainmenu, 'customers_bills_list');

				if ($usemenuhider || empty($leftmenu) || preg_match('/customers_bills(|_draft|_notpaid|_paid|_canceled)$/', $leftmenu))
				{
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."facture WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Pte. pago
                    $sql="SELECT COUNT(rowid) as pte_pago FROM ".MAIN_DB_PREFIX."facture WHERE fk_statut=1";
                    $resql=$db->query($sql);
                    $pte_pago = $db->fetch_object($resql)->pte_pago;
					$newmenu->add("/compta/facture/list.php?leftmenu=customers_bills_draft&amp;search_status=0", $langs->trans("BillShortStatusDraft")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->facture->lire);
					$newmenu->add("/compta/facture/list.php?leftmenu=customers_bills_notpaid&amp;search_status=1", $langs->trans("BillShortStatusNotPaid")." <span class=\"badge badge-info classfortooltip\" title=\"Pte. Pago\">".$pte_pago."</span>", 2, $user->rights->facture->lire);
...
//				$newmenu->add("/compta/facture/invoicetemplate_list.php?leftmenu=customers_bills_templates", $langs->trans("ListOfTemplates"), 1, $user->rights->facture->creer, '', $mainmenu, 'customers_bills_templates'); // No need to see recurring invoices, if user has no permission to create invoice.
				...
				// Ocultar listado POS
                // $newmenu->add("/custom/pos/backend/listefacticket.php?idmenu=141&mainmenu=pos&leftmenu=", 'Listado POS', 1, $user->rights->facture->creer); //Provisional

				//COBRANZA
				$newmenu->add("/compta/paiement/cobranza.php", "Cobranza", 1, $user->rights->facture->lire);

				$newmenu->add("/compta/paiement/list.php?leftmenu=customers_bills_payment", $langs->trans("PaymentsFromClient"), 1, $user->rights->facture->lire, '', $mainmenu, 'customers_bills_payment');
				...
				$newmenu->add('/product/custom/best_seller.php?leftmenu=product&amp;type=0', $langs->trans("Venta por Producto"), 1, $user->rights->produit->lire);
				...
				// Ocultar Importación de cuentas externas
                // $newmenu->add("/compta/import/import_cxc.php?leftmenu=cuentas_terceros", $langs->trans("Importacion de Cuentas Externas"), 1, $user->rights->facture->lire, '', $mainmenu, 'cuentas_terceros');
			}
			...
			if (!empty($conf->societe->enabled) && !empty($conf->supplier_invoice->enabled))
			{
				...
				$newmenu->add("/fourn/facture/card.php?leftmenu=suppliers_bills&amp;action=create", $langs->trans("NewSupplierBill"), 1, $user->rights->fournisseur->facture->creer, '', $mainmenu, 'suppliers_bills_create');
				$newmenu->add("/fourn/facture/loadXml.php?leftmenu=suppliers_bills", $langs->trans("NewFactureXml"), 1, $user->rights->fournisseur->facture->creer);
				...
				if ($usemenuhider || empty($leftmenu) || preg_match('/suppliers_bills/', $leftmenu)) {
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."facture_fourn WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Pte. pago
                    $sql="SELECT COUNT(rowid) as pte_pago FROM ".MAIN_DB_PREFIX."facture_fourn WHERE fk_statut=1";
                    $resql=$db->query($sql);
					//Cancelado
                    $sql="SELECT COUNT(rowid) as cancelado FROM ".MAIN_DB_PREFIX."facture_fourn WHERE fk_statut=3";
                    $resql=$db->query($sql);
                    $cancelado = $db->fetch_object($resql)->cancelado;
					$newmenu->add("/fourn/facture/list.php?leftmenu=suppliers_bills_draft&amp;search_status=0", $langs->trans("BillShortStatusDraft")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->fournisseur->facture->lire, '', $mainmenu, 'suppliers_bills_draft');
					$newmenu->add("/fourn/facture/list.php?leftmenu=suppliers_bills_notpaid&amp;search_status=1", $langs->trans("BillShortStatusNotPaid")." <span class=\"badge badge-info classfortooltip\" title=\"Pte. Pago\">".$pte_pago."</span>", 2, $user->rights->fournisseur->facture->lire, '', $mainmenu, 'suppliers_bills_notpaid');
					...
					$newmenu->add("/fourn/facture/list.php?leftmenu=suppliers_bills_cancel&amp;search_status=3", $langs->trans("BillShortStatusCanceled")." <span class=\"badge badge-info classfortooltip\" title=\"Cancelada\">".$cancelado."</span>", 2, $user->rights->fournisseur->facture->lire, '', $mainmenu, 'suppliers_bills_cancel');
				}
				...
				// Ocultar Importación de cuentas externas
                // $newmenu->add("/compta/import/import_cpx.php?leftmenu=cuentas_terceros_pagar", $langs->trans("Importacion de Cuentas Externas"), 1, $user->rights->fournisseur->facture->lire, '', $mainmenu, 'cuentas_terceros_pagar');
			}
...
//					}
				...
				{
					$langs->load("banks");
					// $newmenu->add("/compta/bank/various_payment/list.php?leftmenu=tax_various&amp;mainmenu=billing", $langs->trans("MenuVariousPayment"), 1, $user->rights->banque->lire&&$user->rights->banque->show_various_p, '', $mainmenu, 'tax_various');
					if ($usemenuhider || empty($leftmenu) || preg_match('/^tax_various/i', $leftmenu)) {
                        $newmenu->add("/compta/bank/various_payment/card.php?leftmenu=tax_various&action=create", $langs->trans("New"), 2, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
                        $newmenu->add("/compta/bank/various_payment/list.php?leftmenu=tax_various", $langs->trans("List"), 2, $user->rights->banque->lire&&$user->rights->banque->show_various_p);
                    }
				}
			}
            // $newmenu->add("/compta", $langs->trans("Reportes"), 0, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
            // $newmenu->add("/compta/reports/saldo_clientes.php?leftmenu=customers_bills", $langs->trans("Saldo de clientes"), 1, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
			// $newmenu->add("/compta/reports/ventas.php?leftmenu=customers_bills", $langs->trans("Reporte de Ventas"), 1, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
            // $newmenu->add("/compta/reports/saldo_proveedores.php?leftmenu=suppliers_bills", $langs->trans("Saldo de proveedores"), 1, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
            // $newmenu->add("/compta/reports/valuacion_de_inventario.php", $langs->trans("Valuacion de inventario"), 1, $user->rights->banque->modifier&&$user->rights->banque->show_various_p);
            // $newmenu->add("/compta/reports/stamped_invoices_cash.php", $langs->trans("Ventas Efectivo F"), 1, $user->rights->facture->view_invoioces_cash);
            // $newmenu->add("/compta/reports/nostamped_invoices_cash.php", $langs->trans("Ventas Efectivo NF"), 1, $user->rights->facture->view_invoioces_cash);
		}
		...
		if ($mainmenu == 'products')
		{
			...
			if (!empty($conf->product->enabled))
			{
                ...
                $newmenu->add("../custom/price_report/index.php", $langs->trans("Reporte de Precios"), 1, $user->rights->produit->lire);
                ...
                $newmenu->add('/product/index.php?leftmenu=product&amp;type=0', $langs->trans("Reportes"), 0, $user->rights->produit->lire, '', $mainmenu, 'product');
                $newmenu->add('/product/custom/best_seller.php?leftmenu=product&amp;type=0', $langs->trans("Venta por Producto"), 1, $user->rights->produit->lire);
                $newmenu->add('/product/custom/warehouse_report.php?leftmenu=product&amp;type=0', $langs->trans("Reporte de Sucursal"), 1, $user->rights->produit->lire);
                $newmenu->add('/product/custom/monthly_report.php?leftmenu=product&amp;type=0', $langs->trans("Reporte mensual"), 1, $user->rights->produit->lire);
                $newmenu->add('/product/custom/supplier_report.php?leftmenu=product&amp;type=0', $langs->trans("Reporte de proveedor"), 1, $user->rights->produit->lire);
                $newmenu->add('/product/custom/buyprice_report.php?leftmenu=product&amp;type=0', $langs->trans("Reporte de precios de compra"), 1, $user->rights->produit->lire);
			}
			// Services
			...
			$newmenu->add("/product/stock/massstockmove.php?leftmenu=&draft=1", $langs->trans("Transferencias masivas"), 0, $user->rights->stock->mouvement->creer);
			$newmenu->add("/product/stock/massstockmove.php?leftmenu=&draft=1", $langs->trans("Nueva transferencia"), 1, $user->rights->stock->mouvement->creer);
			$newmenu->add("/product/stock/movement_list_draft.php", $langs->trans("Transferencias por aprobar"), 1, $user->rights->stock->mouvement->creer);
			...
			if (!empty($conf->expedition->enabled))
			{
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "sendings") {
				    //Borradores
				    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."expedition WHERE fk_statut=0";
				    $resql=$db->query($sql);
				    $borradores = $db->fetch_object($resql)->borradores;
				    //Validados
                    $sql="SELECT COUNT(rowid) as validados FROM ".MAIN_DB_PREFIX."expedition WHERE fk_statut=1";
                    $resql=$db->query($sql);
                    $validados = $db->fetch_object($resql)->validados;
                    $newmenu->add("/expedition/list.php?leftmenu=sendings&viewstatut=0", $langs->trans("StatusSendingDraftShort")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->expedition->lire);
				    $newmenu->add("/expedition/list.php?leftmenu=sendings&viewstatut=1", $langs->trans("StatusSendingValidatedShort")." <span class=\"badge badge-info classfortooltip\" title=\"Validados\">".$validados."</span>", 2, $user->rights->expedition->lire);
			...
			// $langs->load("receptions");
			$newmenu->add('../custom/import/index.php?element=product&type=1', $langs->trans("Import"), 0, $user->rights->produit->creer, '', $mainmenu, 'product');
			$newmenu->add('../custom/export/index.php?element=product&type=1&action=export', $langs->trans("Export"), 0, $user->rights->produit->lire);
			// $newmenu->add("/reception/card.php?action=create2&amp;leftmenu=receptions", $langs->trans("NewReception"), 1, $user->rights->reception->creer);
			// $newmenu->add("/reception/list.php?leftmenu=receptions", $langs->trans("List"), 1, $user->rights->reception->lire);
		}
		...
		*/
		if ($mainmenu == 'hrm')
		{
			...
			if (!empty($conf->holiday->enabled))
			{
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "hrm") {
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."holiday WHERE statut=1";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Espera de aprobación
                    $sql="SELECT COUNT(rowid) as espera_a FROM ".MAIN_DB_PREFIX."holiday WHERE statut=2";
                    $resql=$db->query($sql);
                    $espera_a = $db->fetch_object($resql)->espera_a;
                    $newmenu->add("/holiday/list.php?search_statut=1&mainmenu=hrm&leftmenu=hrm", $langs->trans("DraftCP")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->holiday->read);
				    $newmenu->add("/holiday/list.php?search_statut=2&mainmenu=hrm&leftmenu=hrm", $langs->trans("ToReviewCP")."<span class=\"badge badge-info classfortooltip\" title=\"En espera de aprobación\">".$espera_a."</span>", 2, $user->rights->holiday->read);
			...
			if (!empty($conf->expensereport->enabled))
			{
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "expensereport") {
                    //Borradores
                    $sql="SELECT COUNT(rowid) as borradores FROM ".MAIN_DB_PREFIX."expensereport WHERE fk_statut=0";
                    $resql=$db->query($sql);
                    $borradores = $db->fetch_object($resql)->borradores;
                    //Validados
                    $sql="SELECT COUNT(rowid) as validados FROM ".MAIN_DB_PREFIX."expensereport WHERE fk_statut=2";
                    $resql=$db->query($sql);
                    $validados = $db->fetch_object($resql)->validados;
                    //Aprobados
                    $sql="SELECT COUNT(rowid) as aprobados FROM ".MAIN_DB_PREFIX."expensereport WHERE fk_statut=5";
                    $resql=$db->query($sql);
                    $aprobados = $db->fetch_object($resql)->aprobados;
                    $newmenu->add("/expensereport/list.php?search_status=0&amp;leftmenu=expensereport&amp;mainmenu=hrm", $langs->trans("Draft")." <span class=\"badge badge-info classfortooltip\" title=\"Borradores\">".$borradores."</span>", 2, $user->rights->expensereport->lire);
				    $newmenu->add("/expensereport/list.php?search_status=2&amp;leftmenu=expensereport&amp;mainmenu=hrm", $langs->trans("ValidatedPay")." <span class=\"badge badge-info classfortooltip\" title=\"Validados\">".$validados."</span>", 2, $user->rights->expensereport->lire);
				    $newmenu->add("/expensereport/list.php?search_status=5&amp;leftmenu=expensereport&amp;mainmenu=hrm", $langs->trans("Approved")." <span class=\"badge badge-info classfortooltip\" title=\"Aprobados\">".$aprobados."</span>", 2, $user->rights->expensereport->lire);
		...
		if (!empty($user->rights->banque->lire) && $mainmenu == 'bank')	// Entry for each bank account
		{
			...
			$sql .= " WHERE entity = ".$conf->entity;
			if(!$user->rights->stock->show_all_warehouses) $sql.= " AND fk_warehouse = ".$user->fk_warehouse."";
			$sql .= " AND clos = 0";
			$sql .= " ORDER BY label";
