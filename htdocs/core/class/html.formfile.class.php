...
class FormFile
{
	...
	public function showdocuments($modulepart, $modulesubdir, $filedir, $urlsource, $genallowed, $delallowed = 0, $modelselected = '', $allowgenifempty = 1, $forcenomultilang = 0, $iconPDF = 0, $notused = 0, $noform = 0, $param = '', $title = '', $buttonlabel = '', $codelang = '', $morepicto = '', $object = null, $hideifempty = 0)
	{
		...
		$printer = 0;
		if (in_array($modulepart, array('facture', 'supplier_proposal', 'propal', 'proposal', 'order', 'commande', 'expedition', 'commande_fournisseur', 'expensereport', 'livraison', 'ticket', 'pos')))	// The direct print feature is implemented only for such elements
		{
		...
		if ($genallowed)
		{
			...
			elseif ($modulepart == 'bank')
			{
				...
				else
				{
					...
					$modellist = ModeleBankAccountDoc::liste_modeles($this->db);
				}
			}
			elseif ($modulepart == 'holiday')
            {
                ...
                {
                    include_once DOL_DOCUMENT_ROOT.'/core/modules/holiday/modules_holiday.php';
                    $modellist = ModelePDFHoliday::liste_modeles($this->db);
                }
            }
			else
			{
			...
			{
				asort($modellist);
				if($modulepart=="supplier_proposal" || $modulepart=="commande_fournisseur" || $modulepart=="commande" || $modulepart=="propal" || $modulepart=="remisecheque" || $modulepart=="holiday"){
                    $out .= '<span class="hideonsmartphone" style="display: none">'.$langs->trans('Model').' </span>';
                }else{
		...
		if (!empty($filedir))
		{
			...
			if (is_array($file_list))
			{
				foreach ($file_list as $file)
				{
					...
					$sign = $conf->global->MAIN_SIGNATURE_MODULE;
					if ($delallowed || $printer || $morepicto || $sign)
					{
						$out .= '<td class="right nowraponall">';
						if ($sign && $modulepart == 'movement' && $mime == 'application/pdf')
						{
							$out .= '<a class="paddingleft" href="'.$urlsource.(strpos($urlsource, '?') ? '&amp;' : '?').'action=digital_sign&amp;module='.$modulepart;
							$out .= ($param ? '&amp;'.$param : '');
							$out .= '">'.'<i class="fa fa-edit" style="color: #454545;"></i>'.'</a>';
						}
						if ($delallowed)
						{
	...
	/**
	 *  Show list of documents public in $filearray (may be they are all in same directory but may not) to external users.
	 *  This also sync database if $upload_dir is defined.
	 *
	 *  @param	 array	$filearray          Array of files loaded by dol_dir_list('files') function before calling this.
	 *  @param   string $upload_dir         Full path directory so we can know dir relative to MAIN_DATA_ROOT. Fill this to complete file data with database indexes.
	 * 	@param	 string	$textifempty		Text to show if filearray is empty ('NoFileFound' if not defined)
	 *  @param	 string	$title				Title before list. Use 'none' to disable title.
	 *  @param	 int	$withoutcover 		Not include cover (public photo) of object in list.
	 * 	@return	 string						Html table of documents
	 *  @see list_of_autoecmfiles()
	 */
	public function list_of_public_files($filearray, $upload_dir, $textifempty = '', $title = '', $withoutcover = 1)
	{
        // phpcs:enable
		global $langs, $dolibarr_main_url_root, $form;
		...
		// Sort array
		$filearray = dol_sort_array($filearray, 'position', 'asc');

		// Show list of existing files
		$out = '';
		if ((empty($useinecm) || $useinecm == 6) && $title != 'none') $out .= load_fiche_titre($title ? $title : $langs->trans("AttachedFiles"));

		$out .= '<table class="striped">'."\n";

		$out .= '<thead><tr">';
		$out .= '<th>'.$langs->trans('File').'</th>';
		$out .= '<th>'.$langs->trans('Size').'</th>';
		$out .= '</tr></thead>';

		$nboffiles = count($filearray);
		if ($nboffiles > 0) include_once DOL_DOCUMENT_ROOT.'/core/lib/images.lib.php';

		$i = 0; $nboflines = 0; $skippedCover = false;
		foreach ($filearray as $key => $file)      // filearray must be only files here
		{
			if ($file['name'] != '.' && $file['name'] != '..' && !preg_match('/\.meta$/i', $file['name']) && $file['share'])
			{
				if (!$skippedCover && $withoutcover) {
					if (image_format_supported($file['name']) >= 0) {
						$skippedCover = true;
						//continue;
					}
				...
				// Show file name with link to download
				$out .= '<a href="'.($fulllink != '' ? $fulllink : "#").'">';
				$out .= img_mime($file['name'], $file['name'].' ('.dol_print_size($file['size'], 0, 0).')', 'inline-block valignbottom paddingright');
				$out .= '<span class="ml-1">'.dol_trunc($file['name'], 200).'</span>';
				$out .= '</a>';
				...
				$out .= '<td>';
				if ($sizetoshow == $sizetoshowbytes) $out .= $sizetoshow;
				else {
					$out .= $form->textwithpicto($sizetoshow, $sizetoshowbytes, -1);
				}
				$out .= '</td>';
				$out .= "</tr>\n";
		...
		if ($nboffiles == 0)
		{
			...
			if (empty($textifempty)) $out .= $langs->trans("NoFileFound");
			else $out .= $textifempty;
			$out .= '</td></tr>';
		}
		$out .= "</table>";
		$out .= '</div>';
