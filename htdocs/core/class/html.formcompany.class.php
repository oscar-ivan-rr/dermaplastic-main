...
class FormCompany extends Form
{
	 ...
	 *	@return	void
	 */
	public function form_prospect_level($page, $selected = '', $htmlname = 'prospect_level_id', $empty = 0,$selectOnly=false)
	{
        // phpcs:enable
		global $user, $langs;
	...
	public function select_juridicalstatus($selected = '', $country_codeid = 0, $filter = '', $htmlname = 'forme_juridique_code', $morecss = '')
	{
		...
		$sql .= " FROM ".MAIN_DB_PREFIX."c_cfdimx_regimen_f";
		$sql .= " WHERE active = 1 ";
		if ($filter) {
			$sql .= " ".$filter;
		}
			...
			if ($num) {
				...
				while ($i < $num) {
					...
					if ($obj->code) {		// We exclude empty line, we will add it later
						$labeljs = ($obj->code." -".$obj->label); // $obj->label is already in output charset (converted by database driver)
