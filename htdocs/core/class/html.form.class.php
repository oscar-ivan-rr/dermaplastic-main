...
class Form
{
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				foreach ($countryArray as $row)
				{
					...
					/** Commented to stop showing key on list */
	 ...
	 *  @see select_dolgroups()
	 */
    public function select_dolusers_forevent($action = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = '', $enableonly = '', $force_entity = '0', $maxlength = 0, $showstatus = 0, $morefilter = '', $showproperties = 0, $listofuserid = array(), $listofcontactid = array(), $listofotherid = array(), $selCobranza = 0)
	{
        // phpcs:enable
		global $conf, $user, $langs;
		...
		else
		{
			...
			if($selCobranza != 0){
				$out .= $this->select_dolusers($selCobranza, $htmlname, $show_empty, $exclude, $disabled, $include, $enableonly, $force_entity, $maxlength, $showstatus, $morefilter);
			}else{
    ...
    public function select_produits($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $status = 1, $finished = 2, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = array(), $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '', $selected_combinations = array())
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty($conf->global->PRODUIT_USE_SEARCH_TO_SELECT))
		{
			...
			print '<input type="text" class="minwidth100" placeholder="Búsqueda" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'"'.$placeholder.' '.(!empty($conf->global->PRODUCT_SEARCH_AUTOFOCUS) ? 'autofocus' : '').' />';
			if ($hidelabel == 3) {
				print img_picto($langs->trans("Search"), 'search');
			}
		}
	...
	protected function constructProductListOption(&$objp, &$opt, &$optJson, $price_level, $selected, $hidepriceinlabel = 0, $filterkey = '')
	{
		...
		if (empty($hidepriceinlabel) && !$found)
		{
			if ($objp->price_base_type == 'HT')
			{
				...
				$outval .= ' - '.price(round($objp->price,2), 0, $langs, 0, 0, -1, $conf->currency).' '.$langs->transnoentities("HT");
			}
			else
			{
				$opt .= ' - '.price($objp->price_ttc, 1, $langs, 0, 0, -1, $conf->currency).' '.$langs->trans("TTC");
				$outval .= ' - '.price(round($objp->price_ttc,2), 0, $langs, 0, 0, -1, $conf->currency).' '.$langs->transnoentities("TTC");
			}
    ...
    public function select_produits_fournisseurs($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $ajaxoptions = array(), $hidelabel = 0, $alsoproductwithnosupplierprice = 0, $morecss = '')
	{
		...
		if (!empty($conf->use_javascript_ajax) && !empty($conf->global->PRODUIT_USE_SEARCH_TO_SELECT))
		{
			...
			print ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.'/product/ajax/products.php', $urloption, $conf->global->PRODUIT_USE_SEARCH_TO_SELECT, 0, $ajaxoptions);
			print ($hidelabel ? '' : $langs->trans("RefOrLabel").' : ').'<input type="text" placeholder="Búsqueda" size="20" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="'.$selected_input_value.'">';
		}
		else
		{
    ...
    public function select_produits_fournisseurs_list($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $filterkey = '', $statut = -1, $outputmode = 0, $limit = 100, $alsoproductwithnosupplierprice = 0, $morecss = '')
	{
		...
		if ($result)
		{
			...
			while ($i < $num)
			{
				...
				$opt .= $optlabel;
				if ($objp->cost_price > 0) $outvallabel.= ' - '.price($objp->cost_price, 0, $langs, 0, 0, -1, $conf->currency).'/'.$langs->transnoentities("Unit");
				else $outvallabel .= ' - '.$langs->transnoentities("NoPriceDefinedForThisSupplier");
	 ...
	 *  @param      int		$maxwidth			CSS max-width in pixels
	 *  @return	    string
	 */
    public function select_product_fourn_price($productid, $htmlname = 'productfournpriceid', $selected_supplier = '', $maxwidth = NULL)
	{
        // phpcs:enable
		global $langs, $conf;
		...
		$sql .= " pfp.fk_supplier_price_expression, pfp.fk_product, pfp.tva_tx, s.nom as name,pfp.packing";
		$sql .= " FROM ".MAIN_DB_PREFIX."product as p";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."product_fournisseur_price as pfp ON p.rowid = pfp.fk_product";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."societe as s ON pfp.fk_soc = s.rowid";
		...
		if ($result)
		{
			...
			if ($maxwidth) {
				$form = '<select class="flat" id="select_'.$htmlname.'" name="'.$htmlname.'" style="max-width: '.$maxwidth.'px;">';
			} else {
			...
			else
			{
				...
				while ($i < $num)
				{
					...
					if($objp->packing) $opt .= " - Empaque ".$objp->packing;
					else $opt .= " - Empaque N.A.";
					$opt .= "</option>\n";
	 ...
	 *      @param  string  $morecss        Add more CSS on select tag
	 * 		@param 	int		$add_vf_contado 1=Agregar opcion para ventas fiscal contado
	 *      @param   int    $fieldrequired  1=Hacer campo oblicatorio con 'fieldrequired' CSS
	 * 		@return	void
	 */
    public function select_types_paiements($selected = '', $htmlname = 'paiementtype', $filtertype = '', $format = 0, $empty = 1, $noadmininfo = 0, $maxlength = 0, $active = 1, $morecss = '', $add_vf_contado = 0, $fieldrequired = 0)
	{
        // phpcs:enable
		global $langs, $user;
		...
		print '<select id="select'.$htmlname.'" class="flat selectpaymenttypes'.($morecss ? ' '.$morecss : '').'" name="'.$htmlname.'" ' . ($fieldrequired ? 'required' : '') . '>';
		...
		if($add_vf_contado) print '<option value="200" '.($selected == 200? 'selected' : '').'>Vtas. Fiscal Contado</option>';
		print '</select>';
	 ...
	 */
    public function select_all_categories($type, $selected = '', $htmlname = "parent", $maxlength = 64, $markafterid = 0, $outputmode = 0, $include = 0, $morecss = '',$elemment='')
	{
		// phpcs:enable
		...
		if ($type === Categorie::TYPE_BANK_LINE)
		{
			...
			if($element=='create'){
				if(!($user->rights->societe->cliente_especial)){
					$sql.=" and c.label not like '%Cliente Especial%'";
				} 
			}
		...
		else
		{
            ...
            $cate_arbo = $cat->get_full_arbo($type, $markafterid, $include, $elemment);
		}
		...
		if ($useajax)
		{
			...
			if ($newselectedchoice == 'no')
			{
			    if($question=="¿Desea continuar?"){
                    $formconfirm .= '
						open: function() {
						$(this).parent().find("button.ui-button:eq(1)").blur();
            				$(this).parent().find("button.ui-button:eq(2)").css("border"," 2px solid red");
						},';
                }else{
                    $formconfirm .= '
						open: function() {		    
		...
		if ($d)
		{
			...
			if ($usecalendar != 'combo')
			{
				...
				if ($usecalendar == "eldy")
				{
					...
					$retstring .= '<input id="'.$prefix.'" name="'.$prefix.'" type="text" class="maxwidth100 flat" maxlength="11" value="'.$formated_date.'"';
					$retstring .= ($disabled ? ' disabled' : '');
					$retstring .= ' onChange="dpChangeDay(\''.$prefix.'\',\''.$langs->trans("FormatDateShortJavaInput").'\'); "'; // FormatDateShortInput for dol_print_date / FormatDateShortJavaInput that is same for javascript
					$retstring .= 'style="background-color: #fff; height: 1.5em; "';					
					$retstring .= '>';
	...
	public function select_oc($selected = '', $htmlname = 'select_oc', $htmloption = '', $maxlength = 0, $morecss = 'minwidth300', $usecodeaskey = '', $showempty = 1, $disablefavorites = 0, $addspecialentries = 0)
	{
        // phpcs:enable
		global $conf, $langs, $mysoc;
		...
		$sql .= " WHERE fk_statut = 0 ORDER BY rowid DESC";
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				while ($i < $num)
				{
					...
					$label[$i] = dol_string_unaccent($countryArray[$i]['ref']);
					$i++;
				}

				if (empty($disablefavorites)) array_multisort($favorite, SORT_DESC, $label, SORT_ASC, $countryArray);
				else $countryArray = dol_sort_array($countryArray, 'ref');
				...
				foreach ($countryArray as $row)
				{
					...
					{
						$foundselected = true;
						$out .= '<option value="'.($usecodeaskey ? ($usecodeaskey == 'ref' ? $row['ref'] : $row['ref']) : $row['rowid']).'" selected>';
					}
					else
					{
						$out .= '<option value="'.($usecodeaskey ? ($usecodeaskey == 'ref' ? $row['ref'] : $row['ref']) : $row['rowid']).'">';
					}
					if ($row['ref']) $out .= dol_trunc($row['ref'], $maxlength, 'middle');
					else $out .= '&nbsp;';
					//if ($row['ref']) $out .= ' ('.$row['ref'].')';
					$out .= '</option>';
				}
			}
			$out .= '</select>';
    ...
    public function showLinkToObjectBlock($object, $restrictlinksto = array(), $excludelinksto = array())
	{
		...
		if (is_object($object->thirdparty) && !empty($object->thirdparty->id) && $object->thirdparty->id > 0)
		{
			...
			$possiblelinks = array(
				...
				'supplier_proposal'=>array('enabled'=>$conf->supplier_proposal->enabled, 'perms'=>1, 'label'=>'LinkToSupplierProposal', 'sql'=>"SELECT s.rowid as socid, s.nom as name, s.client, t.rowid, t.ref, '' as ref_supplier, t.total_ht FROM ".MAIN_DB_PREFIX."societe as s, ".MAIN_DB_PREFIX."supplier_proposal as t WHERE t.fk_soc = s.rowid AND t.entity IN (".getEntity('supplier_proposal').')'),
