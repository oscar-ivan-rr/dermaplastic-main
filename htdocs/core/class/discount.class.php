...
class DiscountAbsolute
{
    ...
    /**
     * @var int ID ticket when a discount line is used into an ticket (for credit note)
     */
    public $fk_ticket;
    ...
    public function fetch($rowid, $fk_facture_source = 0, $fk_invoice_supplier_source = 0)
    {
        ...
        $sql.= " sr.fk_user,sr.fk_paiement,sr.fk_paiement_type,";
        ...
        if ($resql)
        {
            if ($this->db->num_rows($resql))
            {
                ...
                $this->fk_paiement_type = $obj->fk_paiement_type;
    ...
    public function create($user)
    {
        ...
        $sql .= " fk_facture_source, fk_invoice_supplier_source,fk_paiement,fk_paiement_type";
        $sql .= ")";
        ...
        $sql .= " ".($this->fk_paiement ? "'".$this->db->escape($this->fk_paiement)."'" : "null").", ";
        $sql .= " ".($this->fk_paiement_type ? "'".$this->db->escape($this->fk_paiement_type)."'" : "null");
    ...
    public function link_to_ticket($rowidline)
    {
        // phpcs:enable
        // Check parameters
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."societe_remise_except";
        $sql .= " SET fk_ticket = ".$rowidline;
        $sql .= " WHERE rowid = ".$this->id;

        dol_syslog(get_class($this)."::link_to_ticket", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql)
        {
			$this->fk_ticket = $rowidline;
			return 1;
        }
        else
        {
    ...
    public function getAvailableDiscounts($company = '', $user = '', $filter = '', $maxvalue = 0, $discount_type = 0)
    {
        ...
        if (!empty($discount_type)) {
        	...
        	$sql .= " AND (rc.fk_facture IS NULL AND rc.fk_facture_line IS NULL AND rc.fk_ticket IS NULL)"; // Available to customer
        }
