...
abstract class CommonObject
{
			...
			if (!empty($this->linkedObjectsIds))
			{
				...
				foreach ($tmparray as $objecttype => $objectids)       // $objecttype is a module name ('facture', 'mymodule', ...) or a module name with a suffix ('project_task', 'mymodule_myobj', ...)
				{
					...
					elseif ($objecttype == 'ticket') {
					    $classpath = 'custom/pos/class'; $module = 'pos';
                    }
	...
	public function formAddObjectLine($dateSelector, $seller, $buyer, $defaulttpldir = '/core/tpl')
	{
		...
		foreach ($dirtpls as $module => $reldir)
		{
			if($this->element != 'facture'){
				$reldir = str_replace("/cfdimx", "", $reldir);
			}
			if (!empty($module))
			{
	...
	public function printObjectLines($action, $seller, $buyer, $selected = 0, $dateSelector = 0, $defaulttpldir = '/core/tpl')
	{
		...
		// Load facture relations lines from orders
		$relationLines = null;
		// if ($object->element == 'invoice_supplier' || $object->element == 'order_supplier') $relationLines = $object->getLinesForRelation();
		// var_dump($relationLines);
		...
		foreach ($this->lines as $line)
		{
			...
			if (is_object($hookmanager))   // Old code is commented on preceding line.
			{
				...
				{
					$parameters = array('line'=>$line, 'num'=>$num, 'i'=>$i, 'dateSelector'=>$dateSelector, 'seller'=>$seller, 'buyer'=>$buyer, 'selected'=>$selected, 'table_element_line'=>$line->table_element, 'relationLines' => $relationLines);
			...
			if (empty($reshook))
			{
				$this->printObjectLine($action, $line, '', $num, $i, $dateSelector, $seller, $buyer, $selected, $extrafields, $defaulttpldir, $relationLines);
			}
        ...
        if($this->element == 'commande' && $this->statut == 0)
        {
            /*print '<tr><td></td>';
            print '<td colspan="9">';
            print '<form name="apply_disc" id="apply_discount" action="' . $_SERVER["PHP_SELF"] . '" method="POST">';
            ...
            print '</tr>';*/
            //print '<script>';
            //print '$("#all_lines").click(function(){$(".product_discount").prop("checked", this.checked);  });';
            //print '$("#apply_discountb").click(function(){$("#apply_discount").submit();  });';
            //print '</script>';
        }
		print "</tbody><!-- end printObjectLines() -->\n";
		$items = (!empty($relationLines)) ? count($relationLines) : 0;
		if ($this->element == 'invoice_supplier' && $items > 0) print $form->textwithpicto($langs->trans("MissingProducts", $items), "Revise los pedidos a proveedor para más información.", -1, 'help', 'margin-bottom: 1rem;', 0, 2);
	}
	 ...
	 *  @param	string				$defaulttpldir		Directory where to find the template
	 *  @param	array|null			$relationLines		Relation lines from orders (Only invoice_supplier)
	 *	@return	void
	 */
	public function printObjectLine($action, $line, $var, $num, $i, $dateSelector, $seller, $buyer, $selected = 0, $extrafields = null, $defaulttpldir = '/core/tpl', &$relationLines = null)
	{
		...
		if ($action != 'editline' || $selected != $line->id)
		{
			...
			if ($line->fk_product > 0)
			{
				...
				$text = $product_static->getNomUrl(1, '', 0, -1, 0, 1,true);
				...
				$label = (!empty($line->label) ? $line->label : $label);
	 ...
	 * @param	int			$notchecked			Don't show checkboxes
	 *  @return	void
	 */
	public function printOriginLinesList($restrictlist = '', $selectedLines = array(), $notchecked = 0)
	{
		...
		if (!$notchecked) print '<td class="center">'.$form->showCheckAddButtons('checkforselect', 1).'</td>';
        print '</tr>';
		$i = 0;
		...
		if (!empty($this->lines))
		{
			foreach ($this->lines as $line)
			{
				if (is_object($hookmanager) && (($line->product_type == 9 && !empty($line->special_code)) || !empty($line->fk_parent_line)))
				{
					if (empty($line->fk_parent_line))
					{
						$parameters = array('line'=>$line, 'i'=>$i, 'notchecked' => $notchecked);
						$action = '';
				...
				{
					$this->printOriginLine($line, '', $restrictlist, '/core/tpl', $selectedLines, $notchecked);
				}
	 ...
	 * 	@return	void
	 */
	public function printOriginLine($line, $var, $restrictlist = '', $defaulttpldir = '/core/tpl', $selectedLines = array(), $notchecked = 0)
	{
		global $langs, $conf;
		...
		if (is_array($extrafields->attributes[$this->table_element]['label']) && count($extrafields->attributes[$this->table_element]['label']) > 0)
		{
			...
			foreach ($extrafields->attributes[$this->table_element]['label'] as $key=>$label)
			{
				...
				else
				{
					...
					$out .= '<tr '.($html_id ? (strcmp($this->element, "facturedet") == 0 ? 'style = "display: none;" ' : ""). ' id="'.$html_id.'" ' : '').$csstyle.' class="'.$class.$this->element.'_extras_'.$key.' trextrafields_collapse'.$extrafields_collapse_num.'" '.$domData.' >';

					// var_dump(strcmp($this->element, "facturedet"));
					if(strcmp($this->element, "facturedet") == 0) {
						$out .= '<td class=""></td>';
					}
					...
					if ($tpl_context == "public") {	// Public page : red dot instead of fieldrequired characters
						$out .= '">';
						if (!empty($extrafields->attributes[$this->table_element]['help'][$key])) $out .= $form->textwithpicto($labeltoshow, $langs->trans($extrafields->attributes[$this->table_element]['help'][$key]));
					...
					if($this->table_element == 'facturedet'){
						$colspan= 11;
					}
	...
	public function show_photos($modulepart, $sdir, $size = 0, $nbmax = 0, $nbbyrow = 5, $showfilename = 0, $showaction = 0, $maxHeight = 120, $maxWidth = 160, $nolink = 0, $notitle = 0, $usesharelink = 0)
	{
        ...
        if($this->element == 'product') {
            $dir .= get_exdir(0, 0, 0, 0, $this, $modulepart) . dol_sanitizeFileName($this->ref) . '/';
            $pdir .= get_exdir(0, 0, 0, 0, $this, $modulepart) . dol_sanitizeFileName($this->ref) . '/';
        }else {
	...
	/**
	 *  Show original public photos of an object (nbmax maximum)
	 *
	 *  @param		string	$modulepart		'product', 'ticket', ...
	 *  @param      string	$sdir        	Directory to scan (full absolute path)
	 *  @param      int		$nbmax       	Nombre maximum de photos (0=pas de max)
	 *  @return     string					Html code to show photo. Number of photos shown is saved in this->nbphoto
	 */
	public function show_cover_public($modulepart, $sdir, $nbmax = 0) {
		global $conf;
		...
		$filearray = dol_dir_list($dir, "files", 0, '', '(\.meta|_preview.*\.png)$', $sortfield, (strtolower($sortorder) == 'desc' ?SORT_DESC:SORT_ASC), 0);
		...
		if (count($filearray))
		{
			...
			foreach ($filearray as $val)
			{
				...
				if (image_format_supported($file) >= 0 && $val['share'])
				{
					$nbphoto++;
					$return .= '<img class="materialboxed" src="'.DOL_URL_ROOT.'/viewimage.php?hashp='.$val['share'].'">';
		...
		if ($nbphoto == 0) {
			if (!empty($conf->global->PRODUCT_NODISPLAYIFNOPHOTO)) {
				...
				$return .= '<img alt="No photo" style="width: 160px; margin: 0 auto;" src="' . DOL_URL_ROOT . $nophoto . '">';
			}
		}
	...
	/**
     * Obtención de la relación de líneas del objeto actual con otro tipo de objeto, la programación de esta función debe adaptarse a las necesidas que se busquen.
     * @param	int		        $only_product	Return only physical products
     * @return	array|null		Array of lines or null
     */
    public function getLinesForRelation($only_product = 0) {
		// Se obtienen los objetos enlazados de este objeto.
		$this->fetchObjectLinked('', '', '', '', 'OR', 0, 'sourcetype',  0);
		
		/**
		 * $elementRelation será el tipo de objeto donde se busca la relación.
		 * $table es la tabla sobre la que se buscarán las relaciones con el objeto actual.
		 * $fk_object es el atributo de $table sobre el cuál se iterará para cada objeto enlazado a este.
		 */
		//  Para pedidos de proveedor, se ocupa sacar la relación con las facturas de proveedor.
		if ($this->element == 'order_supplier') {
			$elementRelation = 'invoice_supplier';
			$table = "facture_fourn_det";
			$fk_object = "facture_fourn";
		}
		//  Para las facturas de proveedor se ocupa scar la relación con los pedidos de proveedor.
		...
		// Obtenemos el número de objetos enlazados de $elementRelation.
        $numOrders = (!empty($this->linkedObjectsIds)) ? count($this->linkedObjectsIds[$elementRelation]) : 0;

        if ($numOrders > 0) {
			$lines = array();

			// Atributos básicos para cada tipo de objeto.
			$sql = "SELECT od.fk_product as prodid, p.ref, p.label";

			// Atributos necesarios cuando el objeto actual es un pedido de proveedor.
			if ($this->element == 'order_supplier') {
				$sql .= ", od.fk_facture_fourn";
			}
			// Atributos necesarios cuando el objeto actual es una factura de proveedor.
			elseif ($this->element == 'invoice_supplier') {
				$sql .= ", od.label, od.description, ifnull(sum(lcfd.qty), 0) as qty";
			}

			// Tabla principal
			$sql .= " FROM " . MAIN_DB_PREFIX . $table . " as od";
			
			/**
			 * En esta sección irán todos los joins que se ncesiten para cada tipo de objeto.
			 */
			// Join necesario cuando el objeto actual es una factura de proveedor.
			if ($this->element == 'invoice_supplier')
				$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "commande_fournisseur_dispatch lcfd ON od.rowid = lcfd.fk_commandefourndet";

			// Join utilizado siempre.
            $sql .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'product p on od.fk_product = p.rowid';
			$sql .= " WHERE";
			
            // Se itera sobre cada objeto enlazado de tipo $elementRelation para buscar registros en el query. $fk_object es la llave foráena de $elementRelation.
            $i = 0;
            foreach($this->linkedObjectsIds[$elementRelation] as $objectid) {
                if ($i++ == 0) {
                    $sql .= " (od.fk_$fk_object = $objectid";
                    continue;
                }

                $sql .= " OR od.fk_$fk_object = $objectid";
            }
            $sql .= ')';

            if ($only_product) $sql .= ' AND od.product_type = 0';
            $sql .= " GROUP BY od.fk_product";

            dol_syslog(get_class($this)."::getLinesForRelation", LOG_DEBUG);
            $result = $this->db->query($sql);
            if ($result) {
                while ($objp = $this->db->fetch_object($result))
                    $lines[$objp->prodid] = $objp;
                
                $this->db->free($result);
            } else  $this->error = $this->db->error()." sql=".$sql;
	...
	public function getDiscount() {
		...
		foreach($this->lines as $line) {
			...
			$discount += number_format(((($line->remise_percent * $line->subprice) / 100)  * $line->qty) , 2);
		}
