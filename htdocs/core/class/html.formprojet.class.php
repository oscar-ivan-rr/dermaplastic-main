...
class FormProjets
{
     ...
     *  @param  string  $filterid       Filtrado id
	 *	@return string           		Return html content
	 */
    public function select_projects($socid = -1, $selected = '', $htmlname = 'projectid', $maxlength = 16, $option_only = 0, $show_empty = 1, $discard_closed = 0, $forcefocus = 0, $disabled = 0, $mode = 0, $filterkey = '', $nooutput = 0, $forceaddid = 0, $morecss = '', $htmlid = '',$filterid='')
    {
        // phpcs:enable
		...
		{
			$out.=$this->select_projects_list($socid, $selected, $htmlname, $maxlength, $option_only, $show_empty, abs($discard_closed), $forcefocus, $disabled, 0, $filterkey, 1, $forceaddid, $htmlid, $morecss,$filterid);
		}
		if ($discard_closed > 0)
		{
	 ...
	 */
    public function select_projects_list($socid = -1, $selected = '', $htmlname = 'projectid', $maxlength = 24, $option_only = 0, $show_empty = 1, $discard_closed = 0, $forcefocus = 0, $disabled = 0, $mode = 0, $filterkey = '', $nooutput = 0, $forceaddid = 0, $htmlid = '', $morecss = 'maxwidth500', $filterid)
    {
        // phpcs:enable
		...
		if (!empty($filterkey)) $sql .= natural_search(array('p.title', 'p.ref','p.fk_soc'), $filterkey);
		if(!empty($filterid) ){
		    $sql.= ' and p.fk_soc = '.$filterid;
        }
	...
	public function select_element($table_element, $socid = 0, $morecss = '', $limitonstatus = -2, $projectkey = "fk_projet")
	{
		...
		switch ($table_element)
		{
			...
			case "payment_various":
				...
				break;
            case "pos_ticket":
                $sql = "SELECT t.rowid, t.ticketnumber as ref";
                break;
			case "chargesociales":
			default:
				$sql = "SELECT t.rowid, t.ref";
        ...
        if($table_element == "pos_ticket")
		    $sql.= " WHERE (".$projectkey." is null or ".$projectkey." = 0)";
        else
