	...
	*/
	...
	public function showInputField($key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = '', $objectid = 0, $extrafieldsobjectkey = '', $mode = 0)
	{
		...
		if (!empty($extrafieldsobjectkey))
		{
			...
			$tmpHelp = $this->attributes[$extrafieldsobjectkey]['help'][$key];
			$help = (!empty($tmpHelp)) ? $langs->trans($tmpHelp) :  $tmpHelp;
                    ...
                    if ($resql) {
                        ...
                        while ($i < $num) {
                            ...
                            if ($value == $obj->rowid) {
                                ...
                                $extraf = 0;
                                if($extrafieldsobjectkey == 'facturedet') {
                                    $sqlextras = "SELECT pe.".$key;
                                    $sqlextras .= " FROM ".MAIN_DB_PREFIX."facturedet as fd";
                                    $sqlextras .= " LEFT JOIN ".MAIN_DB_PREFIX."product_extrafields as pe";
                                    $sqlextras .= " ON fd.fk_product=pe.fk_object";
                                    $sqlextras .= " WHERE fd.rowid=" . $objectid;
                                    $resextras = $this->db->query($sqlextras);
                                    if($resextras){
                                        $extras = $this->db->fetch_object($resextras);
                                        $extraf = $extras->$key;
                                    }
								}

                                $out .= '<option value="'.$obj->rowid.'"';
                                $out .= ($value == $obj->rowid || $obj->rowid === $extraf)? ' selected' : '';
                                $out .= (!empty($parent) ? ' parent="'.$parent.'"' : '');
