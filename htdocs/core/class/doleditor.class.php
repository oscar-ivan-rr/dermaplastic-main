...
class DolEditor
{
	 ...
	 * @param	int		$maxlength						Max length of content
	 */
    public function __construct($htmlname, $content, $width = '', $height = 200, $toolbarname = 'Basic', $toolbarlocation = 'In', $toolbarstartexpanded = false, $uselocalbrowser = true, $okforextendededitor = true, $rows = 0, $cols = 0, $readonly = 0, $maxlength = 1000)
    {
    	global $conf, $langs;
    ...
    public function Create($noprint = 0, $morejs = '', $disallowAnyContent = true, $titlecontent = '', $option = '')
    {
        ...
        if (in_array($this->tool, array('textarea', 'ckeditor')))
        {
            ...
            $out .= '<textarea id="'.$this->htmlname.'" name="'.$this->htmlname.'" rows="'.$this->rows.'"'.(preg_match('/%/', $this->cols) ? ' style="margin-top: 5px; width: '.$this->cols.'"' : ' cols="'.$this->cols.'"').' class="flat" maxlength="'. $this->maxlength . '">';
