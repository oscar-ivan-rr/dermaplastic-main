        ...
        if ($num || $forceshowtitle)
        {
        	...
        	if (is_array($listofactions) && count($listofactions))
        	{
	        	...
	        	foreach ($listofactions as $action)
	        	{
	        		...
	        		print '<td>'.$label.'</td>';
	        		// Date
                    $dateP = new DateTime(date('Y-m-d H:i:s',$action->datep));
                    $dateP->setTimezone(new DateTimeZone('America/Mexico_City'));

                    $dateF = new DateTime(date('Y-m-d H:i:s',$action->datef));
                    $dateF->setTimezone(new DateTimeZone('America/Mexico_City'));

	        		print '<td class="center">'.$dateP->format('h:i A d/m/Y');
	        		if ($action->datef)
	        		{
		        		...
		        		{
		        			if ($tmpa['hours'] != $tmpb['hours'] || $tmpa['minutes'] != $tmpb['minutes'] && $tmpa['seconds'] != $tmpb['seconds']) print '-'.$dateF->format('h:i A d/m/Y');
		        		}
		        		else print '-'.$dateF->format('h:i A d/m/Y');
	        		}
	        		print '</td>';
