...
abstract class CommonInvoice extends CommonObject
{
	 ...
	 *	@return     string        			Label of status
	 */
	public function LibStatut($paye, $status, $mode = 0, $alreadypaid = -1, $type = 0,$facture_fourn=0)
	{
        // phpcs:enable
		global $langs;
		$langs->load('bills');
		...
		if (! $paye){
		    ...
		    elseif (($status == 3 || $status == 2) && $alreadypaid <= 0) {
		        $labelStatus = $langs->trans('BillShortStatusCanceled');
		        $labelStatusShort = $langs->trans('Bill'.$prefix.'StatusClosedUnpaid');
		        $statusType='status5';
		    }
		    elseif (($status == 3 || $status == 2) && $alreadypaid > 0) {
		        $labelStatus = $langs->trans('Bill'.$prefix.'StatusClosedPaidPartially');
		    ...
		    elseif ($alreadypaid <= 0) {
		    	if ($this->type==2)
		    	{
		        	$labelStatus = 'Por Aplicar';
		        	$labelStatusShort = 'Por Aplicar';
		        	$statusType='status1';
		    	}
		    	else
		    	{
		        ...
		        if($facture_fourn)
                {
                    $labelStatus = $langs->trans('BillStatusStartedSupplier');
                    $labelStatusShort = $langs->trans('Bill'.$prefix.'StatusStartedSupplier');
                    $statusType='status3';
                }else{
                    $labelStatus = $langs->trans('Con Abono');
                    $labelStatusShort = $langs->trans('Con Abono');
                    $statusType='status3';
                }
		    }
		}
		else
		{
		    ...
		    if ($type == self::TYPE_CREDIT_NOTE){
		        $labelStatus = 'Aplicada';//$langs->trans('BillStatusPaidBackOrConverted');       // credit note
		        $labelStatusShort = 'Aplicada'; //$langs->trans('Bill'.$prefix.'StatusPaidBackOrConverted');       // credit note
		    }
