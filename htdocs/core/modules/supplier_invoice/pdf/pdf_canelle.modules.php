...
class pdf_canelle extends ModelePDFSuppliersInvoices
{
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// DATOS DE EMISOR
		$sqlrfc = "SELECT lcr.code as rfc FROM llx_entrepot le JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE le.rowid = ".$object->fk_warehouse."";
		...
		if ($showaddress)
		{
			...
			$pdf->SetXY($posx + 2, $posy + 3);
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox, 4, $emisor->razon_social, 0, 'L');
			...
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->SetXY($posx + 2, $posy);
			$pdf->MultiCell($widthrecbox, 4, $emisor->calle.' '.$emisor->numext.', '.$emisor->colonia, 0, 'L');
			$posy = $pdf->getY();
