		...
		*/
		...
		if ($showaddress) {
			...
			if ($object->fk_user_author > 0) {
				...
				$sqlTipo = "SELECT c.label FROM ".MAIN_DB_PREFIX."c_type_fees as c";
				$sqlTipo .= " WHERE c.active = 1 and c.id=".$object->fk_c_type_fees;
				$resqlTipo =$this->db->query($sqlTipo);
				$tipoGasto = $this->db->fetch_object($resqlTipo);
				$pdf->MultiCell(96, 4, $outputlangs->transnoentities("Type")." : ".$tipoGasto->label, 0, 'L');
			}
