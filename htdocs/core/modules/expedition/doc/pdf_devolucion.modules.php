...
/**
 *	\file       htdocs/core/modules/expedition/doc/pdf_devolucion.modules.php
 *	\ingroup    expedition
 *	\brief      Class file used to generate the dispatch slips for the devolucion model
 */
...
/**
 *	Class to build sending documents with model devolucion
 */
class pdf_devolucion extends ModelePdfExpedition
{
    /**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$nblines = sizeof($_SESSION['devolucion_'.$object->id]);
		...
		if ($conf->expedition->dir_output)
		{
			...
			else
			{
				...
				$banPDF = 0;
				while ($banPDF == 0){
					if (file_exists($dir."/".$expref."_Devolucion".$contPDF.".pdf")){
						$contPDF = $contPDF + 1;
					}else{
						$banPDF = 1;
					}
				...
				if ($person != 1) $file = $dir."/".$expref."_Devolucion".$contPDF.".pdf";
				else $file = $dir."/".$expref."_cliente_Devolucion".$contPDF.".pdf";
			}
			...
			if (file_exists($dir))
			{
                ...
                $pdf->SetSubject($outputlangs->transnoentities("Levantamiento"));
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$objProd = new Product($this->db);
					$objProd->fetch($_SESSION['devolucion_'.$object->id][$i][0]);
					$pdf->SetXY($this->posxref - 1, $curY);
					$pdf->MultiCell(($this->posxdesc - $this->posxref), 3, $objProd->ref, '', 'L');

					
					$pdf->SetXY($this->posxdesc - 1, $curY);
					$pdf->MultiCell(($this->posxpicture - $curX), 3, $objProd->label, '', 'J');
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						$pdf->MultiCell(($this->posxdesc - $this->posxref), 3, $object->lines[$i]->ref, '', 'L');
					...
					if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED))
					{
					    ...
					    $pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 3, $_SESSION['devolucion_'.$object->id][$i][2], '', 'C');
					}

					if (empty($conf->global->SHIPPING_PDF_HIDE_QTYTOSHIP))
					{
						$pdf->SetXY($this->posxqtytoship, $curY);
						$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 3, $_SESSION['devolucion_'.$object->id][$i][3], '', 'C');
					}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$nblines = sizeof($_SESSION['devolucion_'.$object->id]);
		for ($i = 0; $i < $nblines; $i++){
			$totalOrdered += $_SESSION['devolucion_'.$object->id][$i][2];

			$totalToShip += $_SESSION['devolucion_'.$object->id][$i][3];
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
        ...
        if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED))
        {
    		...
    		if (empty($hidetop))
    		{
    			...
    			$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 2, $outputlangs->transnoentities("QtySent"), '', 'C');
    		}
        }
        ...
        if (empty($conf->global->SHIPPING_PDF_HIDE_QTYTOSHIP))
        {
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 2, $outputlangs->transnoentities("QtyBack"), '', 'C');
			}
        }
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("SendingSheetReturn");
