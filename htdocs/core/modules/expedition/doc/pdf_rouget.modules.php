...
class pdf_rouget extends ModelePdfExpedition
{
	...
	public function __construct($db = 0)
	{
		...
		$this->posxdesc = $this->page_largeur - $this->marge_droite - 150;
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$lines = $object->lines;
		//* Merge lines with same product
		for($i = 0; $i < $nblines; $i++){
			if($i < $nblines - 1){
				if($lines[$i]->fk_product == $lines[$i+1]->fk_product){
					$lines[$i]->qty_shipped += $lines[$i+1]->qty_shipped;
					$lines[$i]->total_ht += $lines[$i+1]->total_ht;
					$lines[$i]->total_ttc += $lines[$i+1]->total_ttc;
					$lines[$i]->total_tva += $lines[$i+1]->total_tva;
					$lines[$i]->subprice += $lines[$i+1]->subprice;
					unset($lines[$i+1]);

					// Reduce $nblines to account for the removed line
					$nblines--;
					
					// Reindex the array and decrement $i to check the new element at the current index
		...
		if ($conf->expedition->dir_output)
		{
			...
			{
				$person = GETPOST('person', 'int', 2);
				$expref = dol_sanitizeFileName($object->ref);
				$dir = $conf->expedition->dir_output."/sending/".$expref;

				if ($person != 1) $file = $dir."/".$expref.".pdf";
				else $file = $dir."/".$expref."_cliente.pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				if (!empty($object->note_public) || !empty($object->tracking_number))
				{
					...
					$pdf->writeHTMLCell(60, 4, $this->posxref - 1, $tab_top - 1, $outputlangs->transnoentities("TrackingNumber")." : ".$object->tracking_number, 0, 1, false, true, 'L');
					...
					if (!empty($object->tracking_number))
					{
						...
						if (!empty($object->tracking_url))
						{
							if ($object->shipping_method_id > 0)
							{
								...
								$pdf->SetFont('', 'B', $default_font_size - 2);
								$pdf->writeHTMLCell(60, 4, $this->posxref - 1, $tab_top_alt, $label, 0, 1, false, true, 'L');
					...
					if (!empty($object->note_public))
					{
						...
						$pdf->writeHTMLCell(190, 3, $this->posxref - 1, $tab_top_alt, dol_htmlentitiesbr($object->note_public), 0, 1);
					}
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					// Reference of product line
					$pdf->SetXY($this->posxref, $curY - 3);
					$pdf->MultiCell(($this->posxdesc - $this->posxref) + 1, 3, $object->lines[$i]->ref, '', 'L');

					// pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX, 3, $curX, $curY, 1, $hidedesc);

					$pdf->SetXY($this->posxdesc, $curY - 2);
					$pdf->MultiCell(($this->posxweightvol - $curX), 3, $object->lines[$i]->label, '', 'L');
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						$pdf->SetXY($this->posxdesc, $pdf->GetY()-4);
						$pdf->MultiCell(($this->posxweightvol - $curX), 2, $object->lines[$i]->label, '', 'L');
                    ...
                    if (count($childs))
                    {
                        ...
                        foreach($childs as $k => $v)
                        {
                            ...
                            $pdf->MultiCell($this->posxdesc - $this->posxref, 3, $product_child->ref, '', 'L');

                            //DESCRIPCION
                            // $labelproductservice = '> <a href="'.DOL_MAIN_URL_ROOT . '/product/custom/view.php?id='.$k.'">'.$product_child->label.'</a>';
                            // $pdf->writeHTMLCell($this->posxpicture - $curX, 3, $curX,  $curY , $labelproductservice, 0, 1, false, true, 'J', true);
							pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxpicture - $curX, 3, $curX, $curY, $hideref, $hidedesc);

                            if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED))
                            {
                                $pdf->SetXY($this->posxqtyordered, $curY);
                                $pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 3, ($object->lines[$i]->qty_asked)*abs($v[1]), '', 'C');
                            }

                            if (empty($conf->global->SHIPPING_PDF_HIDE_QTYTOSHIP))
                            {
                                $pdf->SetXY($this->posxqtytoship, $curY);
                                $pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 3, ($object->lines[$i]->qty_shipped)*abs($v[1]), '', 'C');
                            }
                            ...
                            if ($pageposafter > $pageposbefore)	// There is a pagebreak
                            {
                                ...
                                {
                                    if ($j == (count($childs) - 1))	// No more lines, and no space left to show total, so we create a new page
                                    {
                                        $pdf->AddPage('', '', true);
                                        if (!empty($tplidx)) $pdf->useTemplate($tplidx);
                                        if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs);
                            ...
                            //var_dump(empty($showpricebeforepagebreak));
                            // We suppose that a too long description or photo were moved completely on next page
                    ...
                    $nexY += 5; // Add space between lines
				}
				...
				// Tableau info
				$posy = $this->_tableau_info($pdf, $object, $posy + 5, $outputlangs, $person);
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->SHIPPING_PDF_HIDE_WEIGHT_AND_VOLUME))
		{
			...
			// Dont show volume, comment this line to show volume
			$totalVolumetoshow = 0;
	 ...
	 *   @param		int			$person			1 = Customer, otherwise Driver
	 *   @return	void
	 */
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs, $person)
	{
        // phpcs:enable
		global $conf,$mysoc;
		$default_font_size = pdf_getPDFFontSize($outputlangs);
		...
		$pdf->MultiCell($larg_sign, 2, $outputlangs->trans($person == 1 ? "ForCustomer" : "ForDriver") .':', '', 'L');

		// Draw signature if this exist in doc 
		$signature = $conf->expedition->dir_output."/sending/" . $object->ref. "/signature.png";
		if (file_exists($signature) && is_readable($signature)) {
			require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';

			$pdf->Image($signature, 2*$larg_sign+$this->marge_gauche - 2, $posy + 8, 0, 28);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxdesc - $this->posxref, 2, $outputlangs->transnoentities("Reference"), '', 'L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqtytoship - $this->posxdesc, 2, $outputlangs->transnoentities("Description"), '', 'L');
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// DATOS DE EMISOR
		$sqlrfc = "SELECT lcr.code as rfc, le.ref as entrepot, le.address as direccion, le.zip as cp, le.town as ciudad, le.description as telefono 
		FROM llx_entrepot le JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE le.rowid = ".$commande->warehouse_id."";
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("Sending");
		...
		else
		{
			...
			$pdf->MultiCell($w, 4, $outputlangs->transnoentities("DeliveryNotValidated"), '', 'R');
			$pdf->SetTextColor(0, 0, 60);
		}
		...
		if($object->origin_id && $object->origin_id > 0){
			$sql_commande_fourn = "SELECT cf.ref as commande_ref";
			$sql_commande_fourn .= " FROM ". MAIN_DB_PREFIX ."commande as c";
			$sql_commande_fourn .= " LEFT JOIN ". MAIN_DB_PREFIX ."commande_fournisseur as cf ON c.fk_commande_fourn = cf.rowid";
			$sql_commande_fourn .= " WHERE c.rowid = " . $object->origin_id;
			$res_commande_fourn = $this->db->query($sql_commande_fourn);
			$obj_commande_fourn = $this->db->fetch_object($res_commande_fourn);

			$pdf->SetFont('', 'B', $default_font_size + 2);
			$pdf->SetXY($posx, $posy + 12);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 20, 'OC Sucursal: ' . $obj_commande_fourn->commande_ref, '', 'R');
		}
		...
		if ($showaddress)
		{
			...
			$pdf->MultiCell($widthrecbox - 2, 4, $rfc->entrepot, 0, 'L');
			$posy = $pdf->getY();

			// Show sender information
			$pdf->SetXY($posx + 2, $posy);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell($widthrecbox - 2, 4, $rfc->direccion.' CP. '.$rfc->cp, 0, 'L');
			$posy = $pdf->getY();
			...
			$pdf->SetXY($posx+2, $posy);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, $rfc->ciudad, 0, 'L');
			$posy=$pdf->getY();

			// RFC
			$pdf->SetXY($posx+2, $posy+1);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, 'RFC: '.$rfc->rfc, 0, 'L');
			$posy=$pdf->getY();

			// Razon Social
			$pdf->SetXY($posx+2, $posy+1);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, 'Razón Social: '.$emisor->razon_social, 0, 'L');
			$posy=$pdf->getY();

			$pdf->SetXY($posx+2, $posy+1);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, 'Teléfono: '.$rfc->telefono, 0, 'L');
			$posy=$pdf->getY();
			...
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("DeliveryAddress").":", 0, 'L');
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		$hidefreetext = 0;
		// DATOS DE EMISOR
		$sqlrfc = "SELECT lcr.code as rfc FROM llx_entrepot le JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE le.rowid = ".$user->fk_warehouse."";
		$resrfc = $this->db->query($sqlrfc);
		$rfc= $this->db->fetch_object($resrfc);
		...
		return pdf_pagefoot($pdf, $outputlangs, 'SHIPPING_FREE_TEXT', '', $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
}
