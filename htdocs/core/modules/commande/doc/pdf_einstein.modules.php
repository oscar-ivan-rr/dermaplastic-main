...
class pdf_einstein extends ModelePDFCommandes
{
		...
		if ($conf->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblines ; $i++)
				{
                    ...
                    $pdf->SetXY($curX, $curY);
                    $pdf->MultiCell($this->posxup - $this->posxqty-0.8, 3, $qty, 0, 'L');  // Enough for 6 chars

                    // Cod.
                    $vat_rate=$object->lines[$i]->ref;
                    $pdf->SetXY($this->posxqty - 2, $curY);
                    $pdf->MultiCell($this->posxup - $this->posxqty+3, 0, $vat_rate, 0, 'L');

					// Description of product line

					pdf_writelinedesc($pdf, $object, $i, $outputlangs, 68, 3, $this->posxup, $curY, $hideref, $hidedesc,0,1);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						// Cod.
						$vat_rate=$object->lines[$i]->ref;
						$pdf->SetXY($this->posxqty - 2, $pdf->GetY()-4);
						$pdf->MultiCell($this->posxup - $this->posxqty+3, 0, $vat_rate, 0, 'L');
	
						/*
                        $pdf->SetXY($curX, $curY);
                        $pdf->MultiCell($this->posxup - $this->posxqty - 0.8,3, $qty, 0, 'L'); // Enough for 6 chars
                        $pdf->SetXY($this->posxqty - 3, $curY);
                        $pdf->MultiCell($this->posxup - $this->posxqty+3, 0, $vat_rate, 0, 'R');
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, 68, 3, $this->posxup, $curY, $hideref, $hidedesc,0,1);
						*/
                    ...
                    $pdf->MultiCell($this->posxunit-$this->posxtva+1.5, 3,'$'.$up_excl_tax, 0, 'R', 0);
					...
					if ($object->lines[$i]->remise_percent)
					{
						...
						$pdf->MultiCell($this->posxupc-$this->posxdiscount, 3, $remise_percent, 0, 'R');
					}
                    //P.U C/DTO
                    if ($object->lines[$i]->remise_percent){
                    ...
                    $pdf->SetXY($this->posxupc-1, $curY);
                    $pdf->MultiCell($this->postotalht - $this->posxupc + 3.5, 3, '$'.$pu, 0, 'R', 0);
                    ...
                    if (count($childs))
                    {
                        ...
                        foreach($childs as $k => $v)
                        {
                            ...
                            //CANTIDAD
                            $pdf->SetXY($curX, $curY);
                            $pdf->MultiCell($this->posxup - $this->posxqty - 0.8, 3, abs($v[1]*$qty), 0, 'L'); // Enough for 6 chars

                            //CODIGO
                            $pdf->SetXY($this->posxqty - 2, $curY);
                            $pdf->MultiCell($this->posxup - $this->posxqty+3, 0, $product_child->ref, 0, 'L');

                            //DESCRIPCION
                            $labelproductservice = '> <a href="'.DOL_MAIN_URL_ROOT . '/product/custom/view.php?id='.$k.'">'.$product_child->label.'</a>';
                            $pdf->writeHTMLCell(68, 3, $this->posxup,  $curY , $labelproductservice, 0, 1, false, true, 'J', true);

                            //TOTAL HIJO
                            $pdf->SetXY($this->posxtva-2,  $curY );
                            //$pdf->MultiCell($this->posxunit - $this->posxtva+1.5, 3, '$'.$up_excl_tax, 0, 'R', 0);
                            //$pdf->SetXY($this->posxunit - 1, $curY);
                            $pdf->MultiCell($this->posxunit - $this->posxtva+1.5, 3, '$'.number_format($product_child->price_ttc*$v[1]*$qty,2,'.',','), 0, 'R');
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
        ...
        //total sin descuento
        $pdf->SetFillColor(248, 255, 255);
        $pdf->SetXY($col1x, $tab2_top + 0);
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Subtotal"), 0, 'L', 1);
        $pdf->SetXY($col2x, $tab2_top + 0);
        $pdf->MultiCell($largcol2,$tab2_hl,price($total_ht+$object->getDiscount()),0,'R',1);
        $index++;
        //Descuento
        $pdf->SetFillColor(248, 248, 248);
        $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Descuentos"), 0, 'L', 1);
        $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
        $pdf->MultiCell($largcol2,$tab2_hl,price($object->getDiscount()),0,'R',1);
        $index++;
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
                ...
                $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
                $pdf->SetFillColor(248, 248, 248);
                $pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("IVA"), $useborder, 'L', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
        ...
        if (empty($hidetop))
        {
            ...
            $pdf->MultiCell(108, 2, $outputlangs->transnoentities("Qty"), '', 'L');
        }
        ...
        if (empty($hidetop))
        {
            ...
            $pdf->MultiCell($this->posxup - $this->posxqty + 3, 2, $outputlangs->transnoentities("Cod."), '', 'L');
        }
        $pdf->line($this->posxup - 1, $tab_top, $this->posxup - 1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxup-1, $tab_top+1);
            $pdf->MultiCell($this->posxtva - $this->posxup - 1, 2, $outputlangs->transnoentities("Designation"), '', 'L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxunit-$this->posxtva, 2, $outputlangs->transnoentities("PriceUHT"), '', 'C');
		}
        ...
        if (empty($hidetop))
        {
            ...
            $pdf->MultiCell($this->posxupc - $this->posxdiscount + 1, 2, $outputlangs->transnoentities("Dto. %"), '', 'C');

        }
        $pdf->line($this->posxupc - 1, $tab_top, $this->posxupc - 1, $tab_top + $tab_height);
        if (empty($hidetop))
        {

            $pdf->SetXY($this->posxupc - 1, $tab_top + 1);
            $pdf->MultiCell($this->postotalht - $this->posxupc + 1, 2, $outputlangs->transnoentities("P.U c/Dto"), '', 'C');

        }

        $pdf->line($this->postotalht+2, $tab_top, $this->postotalht+2, $tab_top + $tab_height);
		if (empty($hidetop))
		{
	...
	{
		// phpcs:enable
		global $conf,$langs,$hookmanager, $user, $mysoc;
		...
		if(strlen($object->ref_client) > 57){
		...
		if(strlen($object->ref_client) > 57){
			$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy + 7, 100, 3, 'R', $default_font_size);
		}else{
		...
		if ($showaddress)
		{
            ...
            if(!empty($object->fk_project)){
                $sql= "SELECT ref,title from llx_projet where rowid =".$object->fk_project;
                $result=$this->db->query($sql);
                $data=$this->db->fetch_object($result);

                $pdf->SetFont('', '', $default_font_size - 1);
                $pdf->SetXY($posx+2, $posy+ 2);
                $pdf->MultiCell($widthrecbox, 4, $outputlangs->transnoentities("Proyecto")." : " . $outputlangs->convToOutputCharset($data->ref." - ".$data->title), '', 'L');
            }
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'ORDER_FREE_TEXT', '', $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
}
