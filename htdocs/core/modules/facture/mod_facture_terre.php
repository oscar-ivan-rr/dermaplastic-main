...
class mod_facture_terre extends ModeleNumRefFactures
{
	 ...
	 * @return  string       			Value
	 */
	public function getNextValue($objsoc, $invoice, $mode = 'next', $forcePFA = false)
	{
        global $db,$user;
        ...
        elseif (!empty($invoice->fk_project) || $forcePFA)
        {
        	$prefix = 'P' . $prefix;
        }
		else{
			// Seleccionar el nombre corto del almacén para concatenarlo al prefijo de la facctura
			$sqlw = "SELECT le.lieu as ref FROM llx_entrepot le WHERE le.rowid = '".$user_warehouse."'";
			$resqlw=$db->query($sqlw);
			if ($resqlw)
			{
				$warehouse = $db->fetch_object($resqlw);
				$ref = str_replace(" ", "-", $warehouse->ref);
				$prefix = $ref.$prefix;
