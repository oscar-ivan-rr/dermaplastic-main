...
class pdf_crabe extends ModelePDFFactures
{
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				// Loop on each lines
				//Para ajustar posiciones se modifico:
					//this->posdiscount = 162
					//this->posxqty = 145
					//this->posxup = 126
					//this->posxunit = 162
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					if(pdf_getlineqty($object, $i, $outputlangs, $hidedetails) == 0){
						continue;
					}
					$curY = $nexY;
					...
					//Sentencia Prod/Unidad
					$sqlClaveUnidad = "SELECT claveprodserv, umed FROM llx_product_extrafields WHERE fk_object =".$object->lines[$i]->fk_product;
					$respClaveUnidad = $this->db->query($sqlClaveUnidad);
					if($respClaveUnidad) $claveUnidad = $this->db->fetch_object($respClaveUnidad);
					
					// ProdServ
					$pdf->SetXY($curX, $curY);
					$pdf->MultiCell(20, 3, (isset($claveUnidad->claveprodserv) && $claveUnidad->claveprodserv != 0)?$claveUnidad->claveprodserv:'', 0, 'L');

					// Unidad					
					$pdf->SetXY($curX + 25, $curY);
					$pdf->MultiCell(20, 3, ($claveUnidad->umed != 0 || !is_null($claveUnidad->umed))?$claveUnidad->umed:'', 0, 'L');

					// Quantity
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($curX + 40, $curY);
					$pdf->MultiCell(20, 3, $qty, 0, 'L');


					pdf_writelinedesc($pdf, $object, $i, $outputlangs, 60, 3, $curX + 55, $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						// ProdServ
						$pdf->SetXY($curX, $pdf->GetY()-4);
						$pdf->MultiCell(20, 3, ($claveUnidad->claveprodserv != 0 || !is_null($claveUnidad->claveprodserv))?$claveUnidad->claveprodserv:'', 0, 'L');

						// Unidad					
						$pdf->SetXY($curX + 25, $pdf->GetY()-4);
						$pdf->MultiCell(20, 3, ($claveUnidad->umed != 0 || !is_null($claveUnidad->umed))?$claveUnidad->umed:'', 0, 'L');

						// Quantity
						$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
						$pdf->SetXY($curX + 40, $pdf->GetY()-4);
						$pdf->MultiCell(20, 3, $qty, 0, 'L');
					...
					$pdf->SetXY(123, $curY);
					$pdf->MultiCell(145 - 126 - 0.8, 3, $up_excl_tax, 0, 'R', 0);

					// Discount on line
					if ($object->lines[$i]->remise_percent)
					{
						$remise_percent = pdf_getlineremisepercent($object, $i, $outputlangs, $hidedetails);
						$pdf->SetXY(145 - 8 , $curY);
						$pdf->MultiCell(162 - 145 - 0.8, 4, $remise_percent, 0, 'R'); // Enough for 6 chars
					}
					...
					if (!empty($conf->global->PRODUCT_USE_UNITS))
					{
						...
						$pdf->SetXY(162, $curY);
						$pdf->MultiCell(162 - 162 - 0.8, 4, $unit, 0, 'L');
					}
					...
					{
						$remise_percent = pdf_getlineremisepercent($object, $i, $outputlangs, $hidedetails);
						$desc = 1-(abs($remise_percent)/100);
					}
					else{
						$desc=1;
					}
					$sin_comas=str_replace(',','',$up_excl_tax);
                    $pu =$sin_comas*$desc;
                    $pu=number_format($pu,2);
					$pdf->SetXY(162 - 8, $curY);
					$pdf->MultiCell($this->posxprogress - 158 + 2, 3, $pu, 0, 'R');
                    ...
                    if (count($childs))
                    {
                    	...
                    	foreach($childs as $k => $v)
                    	{
		                    ...
		                    $pdf->SetXY($curX, $curY);
                    		$labelproductservice = '> '.$product_child->ref.' - '.$product_child->label.'';
                    		$pdf->writeHTMLCell($this->posxpicture - $curX - $progress_width, 3, $curX, $curY, $labelproductservice, 0, 1, false, true, 'J', true);
                   			$posYAfterDescription = $pdf->GetY();
							...
							// Unit price before discount
							$up_excl_tax = '$'.number_format($product_child->price_ttc*$v[1]*$qty,2,'.',',');
							...
							$pdf->MultiCell($this->posxunit - $this->posxqty - 0.8, 4, $v[1]*$qty, 0, 'R'); // Enough for 6 chars
                    	}
                    	$pdf->SetTextColor(0, 0, 0);
                    }
					// Add line
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price(($object->total_ht+$discount), 0, $outputlangs), 0, 'R', 1);

		//Descuentos
        $tab2_top+=3;
        $pdf->SetXY($col1x, $tab2_top);
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Reductions"), 0, 'L', 1);

        $pdf->SetXY($col2x, $tab2_top);
        $pdf->MultiCell($largcol2, $tab2_hl, price($discount, 1, '', 1, - 1, - 1, $conf->currency), 0, 'R', 1);

        //Subtotal con Descuento
        $tab2_top+=3;
        $pdf->SetXY($col1x, $tab2_top);
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Subtotal con Descuento"), 0, 'L', 1);
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS))
		{
			...
			if ($creditnoteamount)
			{
				...
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($col2x - $col1x +15, $tab2_hl, $labeltouse, 0, 'L', 0);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("ClaveProdServ"), '', 'L');
		}

		
		$pdf->line(110 - 77, $tab_top, 110 - 77, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(110 - 85, $tab_top+1);
			$pdf->MultiCell(30,2, $outputlangs->transnoentities("Unidad"),'','C');
		}

		$pdf->line(110 - 60, $tab_top, 110 - 60, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(110 - 68, $tab_top+1);
			$pdf->MultiCell(30,2, $outputlangs->transnoentities("Qty"),'','C');
		}

		$pdf->line(110 - 45, $tab_top, 110 - 45, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(110 - 83, $tab_top+1);
			$pdf->MultiCell(95,2, $outputlangs->transnoentities("Designation"),'','C');
		}
		
		$pdf->line(126 - 1, $tab_top, 126 - 1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(126 - 1, $tab_top + 1);
			$pdf->MultiCell(145 - 126 - 1, 2, $outputlangs->transnoentities("PriceUHT"), '', 'C');
		}

		$pdf->line(145 - 1, $tab_top, 145 - 1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(145 - 4, $tab_top + 1);
			$pdf->MultiCell(162 - 145 - 1, 2, $outputlangs->transnoentities("Dto"), '', 'C');
		}

		if (!empty($conf->global->PRODUCT_USE_UNITS))
		{
			$pdf->line(162 - 1, $tab_top, 162 - 1, $tab_top + $tab_height);
			if (empty($hidetop)) {
				$pdf->SetXY(162 - 1, $tab_top + 1);
				$pdf->MultiCell($this->posxdiscount - 162 - 1, 2, $outputlangs->transnoentities("Unit"), '', 'C');
			}
		}
		
		$pdf->line(162 - 7, $tab_top, 162 - 7, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY(162 - 7, $tab_top + 1);
			$pdf->MultiCell($this->posxprogress - 162 + 1, 2, $outputlangs->transnoentities("C/Desc"), '', 'C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(30, 2, $outputlangs->transnoentities("Importe"), '', 'C');
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox - 2, 4, $emisor->razon_social, 0, 'L');
			$posy = $pdf->getY();

			// Show sender information
			$pdf->SetXY($posx + 2, $posy);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell($widthrecbox - 2, 4, $emisor->calle.' '.$emisor->numext.', '.$emisor->colonia, 0, 'L');
			$posy = $pdf->getY();
