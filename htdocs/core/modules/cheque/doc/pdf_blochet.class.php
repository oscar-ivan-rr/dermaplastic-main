...
class BordereauChequeBlochet extends ModeleChequeReceipts
{
	...
	public function Header(&$pdf, $page, $pages, $outputlangs, $object)
	{
		...
		// Load traductions files required by page
		$outputlangs->loadLangs(array("compta", "banks","main", "propal", "companies", "bills"));
        ...
        $pdf->SetXY($posx, $posy);
        $pdf->SetTextColor(0, 0, 60);
        $pdf->MultiCell(100, 4, $outputlangs->transnoentities("Ref")." : ".$outputlangs->convToOutputCharset($this->ref.($this->ref_ext?" - ".$this->ref_ext:'')), '', 'R');
        ...
        $pdf->SetXY($posx, $posy);
        $pdf->SetTextColor(0, 0, 60);
        $pdf->MultiCell(100, 3, $outputlangs->transnoentities("Date")." : ".dol_print_date($this->date, "day", false, $outputlangs), '', 'R');

        $posy += 4;
        $pdf->SetXY($posx, $posy);
        $pdf->SetTextColor(0, 0, 60);
        $pdf->MultiCell(100, 4, $outputlangs->transnoentities("Owner")." : ".$outputlangs->convToOutputCharset($this->account->proprio), '', 'R');
        ...
        $pdf->MultiCell(75, 35, "", 0, 'R', 1);
        $pdf->SetTextColor(0, 0, 60);

        // Show sender name
        $pdf->SetXY(45 + 2, 5 +1);
        $pdf->SetFont('', 'B', $default_font_size-1.5);
        $pdf->MultiCell(75, 4, $outputlangs->convToOutputCharset($this->emetteur->name), 0, 'L');
        $posy = $pdf->getY();

        // Show sender information
        $pdf->SetXY(45 + 2, $posy+1);
        $pdf->SetFont('', '', $default_font_size - 2);
        $pdf->MultiCell(75, 4, $carac_emetteur, 0, 'L');


        $pdf->SetFont('', '', $default_font_size-2);
        $pdf->SetXY(10, 43);
        $pdf->MultiCell(0, 2, $outputlangs->transnoentities("Account"), 0, 'L');
        pdf_bank($pdf, $outputlangs, 32, 43, $this->account, 1,12);
...
*/
		...
		// Tableau
		$pdf->SetFont('', '', $default_font_size - 2);
		$pdf->SetXY(11, $this->tab_top+7);
		$pdf->MultiCell(40, 2, $outputlangs->transnoentities("Num"), 0, 'L');
		$pdf->line(40, $this->tab_top+5, 40, $this->tab_top + $this->tab_height + 10);

		$pdf->SetXY(41, $this->tab_top+7);
        $pdf->MultiCell(40, 2, $outputlangs->transnoentities("Bank"), 0, 'L');
		$pdf->line(100, $this->tab_top+5, 100, $this->tab_top + $this->tab_height + 10);

        $pdf->SetXY(101, $this->tab_top+7);
        $pdf->MultiCell(40, 2, $outputlangs->transnoentities("CheckTransmitter"), 0, 'L');
		$pdf->line(180, $this->tab_top+5, 180, $this->tab_top + $this->tab_height + 10);

		$pdf->SetXY(180, $this->tab_top+7);
		$pdf->MultiCell(20, 2, $outputlangs->transnoentities("Amount"), 0, 'R');
		$pdf->line(9, $this->tab_top + 12, 201, $this->tab_top + 12);

		$pdf->Rect(9, $this->tab_top+5, 192, $this->tab_height + 5);
	}
