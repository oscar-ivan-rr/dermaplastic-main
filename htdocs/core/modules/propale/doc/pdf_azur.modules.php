...
class pdf_azur extends ModelePDFPropales
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetXY(10, $posy - 5);
				$pdf->MultiCell(100, 9, strtoupper($this->num2letras(str_replace(',','',price($object->total_ttc, 0, $outputlangs)))),'', 'L');
	...
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
        ...
        if ($notetoshow)
        {
            ...
            $pdf->writeHTMLCell(100, 3, $curx, $cury, "Nota: ", 0, 1);
            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->writeHTMLCell(100, 3, $curx+10, $cury, dol_htmlentitiesbr($notetoshow), 0, 1);
        }
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$total=0;
        for ($i = 0; $i < $nblines; $i++){
			$total=$total+($object->lines[$i]->subprice*$object->lines[$i]->qty);
		}
        //total sin descuento
        ...
        $pdf->MultiCell($largcol2,$tab2_hl,price($total+0),0,'R',1);
        $index++;
		//Descuento
		...
		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($largcol2, $tab2_hl, price($total - $object->getDiscount(), 0, $outputlangs), 0, 'R', 1);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Importe Total"), $useborder, 'L', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxunit - $this->posxtva - 1, 2, $outputlangs->transnoentities("PriceUHT"), '', 'C');
		}
		if ($conf->global->PRODUCT_USE_UNITS) {
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'PROPOSAL_FREE_TEXT', '', $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
	...
	*	@result string - Devuelve el n?mero escrito en letra. 
	 */
	protected function num2letras($num, $fem = false, $dec = true) { 
		$matuni[2]  = "dos"; 
		$matuni[3]  = "tres"; 
		$matuni[4]  = "cuatro"; 
		$matuni[5]  = "cinco"; 
