...
class pdf_transfer extends ModeleBankAccountDoc
{
	/**
	 * Issuer
	 * @var Societe
	 ...
	 *
	 *	@param	    AccountLine		$object   		    Object project a generer
	...
	*  @param		int				$hidedetails		Do not show line details (not used for this template)
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (! empty($conf->bank->dir_output))
		{
		    ...
		    else
		    {
		        ...
		        if (! empty($moreparams['force_dir_output'])) $dir = $moreparams['force_dir_output'];
		        else $dir = $conf->bank->dir_output . "/Transfer_" . $objectref;
		        $file = $dir . "/Transferencia$objectref.pdf";
		    }
			...
			if (file_exists($dir))
			{
				...
				$pdf->MultiCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $outputlangs->transnoentitiesnoconv("TransferFrom").' : '. $object->bank_account_label, 0, 'L');

				// Transfer to
				$links = $acct->get_url($object->rowid);
				foreach ($links as $key => $val) {
					if ($links[$key]['type']=='banktransfert') {
						$banklinestatic = new AccountLine($this->db);
						if ($banklinestatic->fetch($links[$key]['url_id']) > 0) {
							$posY=$pdf->GetY();
							$posY+=2;
							$pdf->SetXY($this->marge_gauche, $posY);

							$pdf->MultiCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $outputlangs->transnoentitiesnoconv("TransferTo").' : '. $banklinestatic->bank_account_label, 0, 'L');
						}
					}
				}
				...
				$posY+=1;
				$pdf->SetXY($this->marge_gauche, $posY);
				$pdf->MultiCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $outputlangs->transnoentitiesnoconv("Description").' : '.$object->label, 0, 'L');
				...
				// Payment type
				$payment = $outputlangs->transnoentitiesnoconv('PaymentTypeShort'.$object->fk_type) . (!empty($object->num_chq) ? " ($object->num_chq)" : " ");
				$pdf->MultiCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $outputlangs->transnoentitiesnoconv("Type").' : ' . $payment, 0, 'L');

				// Amount 
				$posY=$pdf->GetY();
				$posY+=1;
				$pdf->SetXY($this->marge_gauche, $posY);
				$pdf->MultiCell($this->page_largeur - $this->marge_gauche - $this->marge_droite, 3, $outputlangs->transnoentitiesnoconv("Amount").' : ' . price($object->amount), 0, 'L');
				...
				$bottomlasttab = $pdf->GetY() + 15;
				// Affiche zone infos
				$posy=$this->_tableau_info($pdf, $object, $bottomlasttab, $outputlangs, $dir);
	 ...
	 *   @param		int			$posy			Y
	 *   @param		Translate	$outputlangs	Langs object
	 *   @param		string		$dir			Outpur directory
	 *   @return	void
	 */
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs, $dir)
	{
        // phpcs:enable
        global $conf, $mysoc;
	    ...
	    $posy+=$this->_signature_area($pdf, $object, $posy, $outputlangs, $dir);
	 ...
	 *
	 *	@param	PDF				$pdf            Object PDF
	 *	@param  AccountLine		$object         Object invoice
	 *	@param	int				$posy			Position depart
	 *	@param	Translate		$outputlangs	Objet langs
	 *	@param		string		$dir			Outpur directory
	 *	@return int								Position pour suite
	 */
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs, $dir)
	{
        // phpcs:enable
	    $default_font_size = pdf_getPDFFontSize($outputlangs);
	    $tab_top = $posy + 4;
		...
		// Digital sign. Draw if this exist in doc 
		...
		if (file_exists($signature) && is_readable($signature)) {
			...
			$posy = $pdf->GetY();
			$pdf->Image($signature, $posx, $posy + 2, 0, $dim['height']);
		...
		$margin = $this->marge_gauche * 7;
		$pdf->line($margin, $posy, $this->page_largeur - $margin, $posy);
	 ...
	 *
	 *  @param	PDF				$pdf     		Object PDF
	 *  @param  AccountLine		$object     	Object to show
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("MenuBankInternalTransfer"), '', 'R');
		$pdf->SetFont('', '', $default_font_size + 2);
