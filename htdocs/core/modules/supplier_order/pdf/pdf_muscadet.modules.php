...
class pdf_muscadet extends ModelePDFSuppliersOrders
{
		...
		if ($conf->fournisseur->commande->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblines ; $i++)
				{
                    ...
                    // $pdf->MultiCell(0, 0, $vat_rate, 0, 'L', false, 1 , $pdf->getX() + 1);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						// Cod.
						// $vat_rate=$object->lines[$i]->ref;
						// $pdf->SetXY($this->posxqty - 1, $curY - 1);
						// $pdf->MultiCell(0, 0, $vat_rate, 0, 'L');
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
        ...
        if (empty($hidetop))
        {
            ...
            $pdf->MultiCell($this->posxup - $this->posxqty + 3, 2, $outputlangs->transnoentities("Designation"), '', 'L');
        }
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$sqlrfc = "SELECT lcr.code as rfc, le.ref as entrepot, le.address as direccion, le.zip as cp, le.town as ciudad, le.description as telefono 
	   	FROM llx_entrepot le JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE le.rowid = ".$object->fk_entrepot."";
		...
		$sql_commande = "SELECT ref FROM llx_commande";
		$sql_commande .= " WHERE entity = 1";
		$sql_commande .= " AND fk_commande_fourn IS NOT NULL";
		$sql_commande .= " AND fk_commande_fourn = ".$object->id;
		
		$res_commande = $this->db->query($sql_commande);
		$obj_commande = $this->db->fetch_object($res_commande);
		if($obj_commande){
			$pdf->SetFont('', 'B', $default_font_size + 3);
			$pdf->SetXY($posx, $posy + 10);
			$pdf->SetTextColor(0, 0, 60);
			$title=$outputlangs->transnoentities("PEDIDO CEDIS").": ".$outputlangs->convToOutputCharset($obj_commande->ref);
			$pdf->MultiCell(100, 3, $title, '', 'R');
			$posy+=1;
		}
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell(80, 4, $rfc->entrepot, 0, 'L');
			$posy=$pdf->getY();

			// Show sender information
			$pdf->SetXY($posx+2, $posy);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, $rfc->direccion.' CP. '. $rfc->cp , 0, 'L');
			$posy=$pdf->getY();
			...
			$pdf->SetXY($posx+2, $posy);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell(80, 4, $emisor->ciudad, 0, 'L');
			$posy=$pdf->getY();
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		// DATOS DE EMISOR
		$sqlrfc = "SELECT lcr.code as rfc FROM llx_entrepot le JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE le.rowid = ".$object->fk_entrepot."";
		$resrfc = $this->db->query($sqlrfc);
		$rfc= $this->db->fetch_object($resrfc);
		...
		return pdf_pagefoot($pdf, $outputlangs, 'SUPPLIER_ORDER_FREE_TEXT', '', $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
}
