 ...
 */
class PDFDraftMovement extends ModelePDFMovement
{
    /**
    ...
    public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        if (file_exists($dir))
        {
            ...
            foreach($object as $objp)
            {
                ...
                //Costo
                $sqlP = 'SELECT cost_price FROM ' . MAIN_DB_PREFIX . 'product WHERE ref = "' . $objp->product_ref . '"';
                $resqlP = $this->db->query($sqlP);
                if (!$resqlP) {
                    error_log("Error en la consulta SQL: " . $this->db->lasterror());
                    $pprice = (object) ['cost_price' => 0]; // Valor predeterminado
                } else {
            ...
            if(strpos($object[0]->label,'Transferencia') === false && strpos($object[0]->label,'Apartado') === false){
                //Totales importe
                //Negativo
                $pdf->SetXY(155, $curY);
                $pdf->MultiCell(25, 3, 'Total negativo:', 0, 'L', 0);
                $pdf->SetXY(185, $curY);
                $pdf->MultiCell(15, 3, price($totalneg), 0, 'L', 0);
                //Positivo
                $pdf->SetXY(155, $curY+10);
                $pdf->MultiCell(25, 3, 'Total positivo:', 0, 'L', 0);
                $pdf->SetXY(185, $curY+10);
                $pdf->MultiCell(15, 3, price($totalposi), 0, 'L', 0);
            }
            ...
            if (file_exists($ssFile) && is_readable($ssFile))
            {
                ...
                $posy = $pdf->GetY();
                $pdf->Image($ssFile, ((70-$width)/2)+10, $posy, 0, $height);	// width=0 (auto)
                //unlink($ssFile);
            }
            ...
            if(strpos($object[0]->label,'Transferencia') === false && strpos($object[0]->label,'Apartado') === false){
                $pdf->line(65, $curY +44, 135, $curY +44);
                $pdf->SetXY(60, $curY+45);
                $pdf->MultiCell(70, 3, 'Nombre y Firma' , 0, 'C');
            } else {
                $sections = ['Almacén Origen', 'Almacén Destino'];

                foreach ($sections as $section) {
                    $pdf->SetXY(5, $curY+20);
                    $pdf->MultiCell(70, 3, $section , 0, 'C');
                    $curY = $pdf->GetY();
                    $pdf->line(10, $curY +10, 70, $curY +10);
                    $curY += 10;
                    $pdf->SetXY(5, $curY);
                    $pdf->MultiCell(70, 3, 'Nombre y Firma' , 0, 'C');
                }
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "")
    {
        ...
        if(strpos($object[0]->label,'Transferencia') === false && strpos($object[0]->label,'Apartado') === false)
            $pdf->MultiCell(70, 3, 'Corrección de Stock', '', 'J');
        else 
            $pdf->MultiCell(70, 3, 'Recibo de transferencia entre almacenes', '', 'J');
        ...
        $sql = "SELECT le.ref as source  FROM llx_stock_mouvement_draft lsm JOIN llx_entrepot le ON lsm.fk_entrepot = le.rowid 
        WHERE lsm.code = " . $object[0]->code;
        $result = $db->query($sql);
        if ($result)
        {
        ...
        $posy = $pdf->GetY()+2;
        $pdf->SetXY(5, $posy);
        $pdf->MultiCell(70, 3, 'Almacén Origen: '.$entrepot_source->source, '', 'L');

        // Obtener el almacén de destino
        $sql = "SELECT le.ref as dest  FROM llx_stock_mouvement_draft lsm JOIN llx_entrepot le ON lsm.fk_entrepot_target = le.rowid 
        WHERE lsm.code = " . $object[0]->code;
        $result = $db->query($sql);
        if ($result)
        {
            $entrepot_dest = $db->fetch_object($result);
        }

        $posy = $pdf->GetY()+2;
        $pdf->SetXY(5, $posy);
        $pdf->MultiCell(70, 3, 'Almacén Destino: '.$entrepot_dest->dest, '', 'L');
