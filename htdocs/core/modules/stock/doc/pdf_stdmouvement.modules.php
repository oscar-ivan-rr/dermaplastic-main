...
class pdf_stdmouvement extends ModelePDFMovement
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->stock->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pagenb++;
				$this->_pagehead($pdf, $object, 1, $outputlangs,"",$search_inventorycode);
				$pdf->SetFont('', '', $default_font_size - 1);
				$pdf->MultiCell(0, 3, '');		// Set interline to 3
				$pdf->SetTextColor(0, 0, 0);

				$tab_top = 27;
				$tab_top_newpage = (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)?27:10);
				$tab_height = 130;
				...
				if ($resql)
				{
					...
					for ($i = 0; $i < $nblines; $i++)
					{
						...
						if ($pageposafter > $pageposbefore)	// There is a pagebreak
						{
							...
							if ($posyafter > ($this->page_hauteur - ($heightforfooter+$heightforfreetext+$heightforinfotot)))	// There is no space left for total+free text
							{
								if ($i == ($nblines-1))	// No more lines, and no space left to show total, so we create a new page
								{
									$pdf->AddPage('', '', true);
									if (! empty($tplidx)) $pdf->useTemplate($tplidx);
									if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs," ",$search_inventorycode);
						...
						$pdf->MultiCell($this->posxdesc - $this->posxidref - 10, 3, $objp->mid, 0, 'L');

						// Date.
						$pdf->SetXY($this->posxdatemouv+12, $curY);
						$pdf->MultiCell($this->posxdesc - $this->posxdatemouv - 0.8, 6, dol_print_date($db->jdate($objp->datem), 'dayhour'), 0, 'C');

						// Ref.
						$pdf->SetXY($this->posxdesc+14, $curY);
						$pdf->MultiCell($this->posxlabel - $this->posxdesc - 1, 3, $productstatic->ref, 0, 'L');

						// Label
						$pdf->SetXY($this->posxlabel+18 + 0.8, $curY);
						$pdf->MultiCell($this->posxup - $this->label - 50, 6, $productstatic->label, 0, 'L');
						...
						// Inv. code
						//$pdf->SetXY($this->posxup+7, $curY);
						//$pdf->MultiCell($this->posxdiscount - $this->posxup - 0.8, 3, $objp->inventorycode, 0, 'L');
						...
						$pricemin = $objp->price;
						$pdf->SetXY($this->posxdiscount+8, $curY);
						if ($origin=="Transferencia interna"){
						    $origin="Transferencia I.";
                        }
						$pdf->MultiCell($this->postotalht - $this->posxdiscount - 6, 3, strip_tags($origin), 0, 'C', 0);
						...
						$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht-3, 3, $objp->qty, 0, 'R', 0);
					...
					// Total
					$pdf->SetXY($this->posxidref, $curY);
					$pdf->MultiCell($this->posxdesc - $this->posxidref, 3, $langs->trans("TotalQty"), 0, 'L');
					...
					// Signature
					$w_sign = 90;
					$area_sign = ($this->page_largeur - $this->marge_gauche) / 2 - $w_sign / 2;

					$pdf->SetXY($area_sign, $curY + 10);
					$pdf->MultiCell($w_sign, 3, 'Almacén' , 0, 'C');
					...
					if (file_exists($signature) && is_readable($signature)) {
						...
						$dim = pdf_getHeightForLogo($signature, false, 28, true);
						$posx = ($this->page_largeur - $dim['width']) / 2; 
						$pdf->Image($signature, $posx, $pdf->GetY(), 0, $dim['height']);
					...
					$curY = $pdf->GetY() + 15;
					$pdf->line($area_sign + 10, $curY +14, $area_sign - 10 + $w_sign, $curY + 14);
					$pdf->SetXY($area_sign, $curY + 15);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
	{
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxdatemouv+12, $tab_top + 1);
	    ...
	    if (empty($hidetop))
	    {
	        $pdf->SetXY($this->posxdesc + 14, $tab_top + 1);
	    ...
	    if (empty($hidetop))
	    {
	        $pdf->SetXY($this->posxlabel + 22, $tab_top + 1);
			$pdf->MultiCell($this->posxup - $this->posxlabel, 2, $outputlangs->transnoentities("Label"), '', 'C');
	    }
	    ...
	    if (empty($hidetop))
	    {
			//$pdf->SetXY($this->posxup + 6, $tab_top + 1);
			//$pdf->MultiCell($this->posxdiscount - $this->posxup, 2, $outputlangs->transnoentities("InventoryCodeShort"), '', 'C');
	    }
	    ...
	    if (empty($hidetop))
	    {
	        $pdf->SetXY($this->posxdiscount + 6, $tab_top + 1);
	    ...
	    if (empty($hidetop))
	    {
	        ...
	        $pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht-2, 2, $outputlangs->transnoentities("Qty"), '', 'R');
	    }
     ...
     *  @param  string      $search_inventorycode Code of movement
	 *  @return	void
	 */
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "",$search_inventorycode)
	{
	    ...
	    $pdf->SetXY($posx, $posy);
	    $pdf->SetTextColor(0, 0, 60);
	    $title=$outputlangs->transnoentities("Tranferencia Interna");
	    ...
	    $pdf->MultiCell(100, 4, $outputlangs->transnoentities("Ref")." : " . $outputlangs->convToOutputCharset($search_inventorycode), '', 'R');

	    $posy+=5;
	    $pdf->SetFont('', '', $default_font_size - 1);
		$pdf->SetXY($posx, $posy);
	    $pdf->SetTextColor(0, 0, 60);
	    $pdf->MultiCell(100, 3, $outputlangs->transnoentities("Movimiento de").' :', '', 'R');
	    $des="";
	    if($object->label=="ALMACEN GPE"){
	        $des="MATRIZ ZAC";
        }
	    if($object->label=="MATRIZ ZAC"){
            $des="ALMACEN GPE";
        }
		$posy+=4;
		$pdf->SetXY($posx-50, $posy);
		$pdf->MultiCell(150, 3, $object->label. " a ".$des, '', 'R');
