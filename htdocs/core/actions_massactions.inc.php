...
if ($massaction == 'confirm_createbills')   // Create bills from orders
{
	...
	$typeObject = get_class($object);

	foreach ($orders as $id_order)
	{
		$cmd = new Commande($db);

		if($typeObject == 'Expedition'){
			$sqlCommande = "SELECT fk_source FROM llx_element_element
				WHERE sourcetype = 'commande' 
				AND targettype = 'shipping'
				AND fk_target = ".$id_order;
			
			if($resCommande = $db->query($sqlCommande)){
				$idCommande = $db->fetch_object($resCommande)->fk_source;
				if ($cmd->fetch($idCommande) <= 0) continue;
			}
		}else{
		...
		if(($typeObject == 'Commande' && $cmd->statut == Commande::STATUS_CLOSED) || ($typeObject == 'Expedition' && ($cmd->statut == Commande::STATUS_VALIDATED || $cmd->statut == Commande::STATUS_SHIPMENTONPROCESS || $cmd->statut == Commande::STATUS_CLOSED))) {
            ...
            if (!empty($createbills_onebythird) && !empty($TFactThird[$cmd->socid])) $objecttmp = $TFactThird[$cmd->socid]; // If option "one bill per third" is set, we use already created order.
				...
				$objecttmp->origin_id = $idCommande;
            ...
            if ($objecttmp->id > 0) {
                ...
                $sql .= ", targettype";
                $sql .= ") VALUES (";
                $sql .= $objecttmp->origin_id;
                $sql .= ", '" . $objecttmp->origin . "'";
                $sql .= ", " . $objecttmp->id;
                $sql .= ", '" . $objecttmp->element . "'";
                $sql .= ")";

				$resElement = $db->query($sql);
                
                if (!$error) {

					$ret = $cmd->classifyBilled($user);
