...
function societe_prepare_head(Societe $object)
{
    ...
    if ($object->client == 1 || $object->client == 2 || $object->client == 3)
    {
        ...
        if (empty($conf->global->SOCIETE_DISABLE_CUSTOMERS) && ($object->client == 1 || $object->client == 3)) $head[$h][1] .= $langs->trans("Saldos Cliente");
        $head[$h][2] = 'customer';
        $h++;
    ...
    if (!empty($conf->global->VIEW_FISCAL_ADDRESSES))
    {
        $head[$h][0] = DOL_URL_ROOT.'/cfdimx/domicilio_fiscal.php?socid='.$object->id;
        $head[$h][1] = $langs->trans("Domicilio Fiscal");
        $head[$h][2] = 'fiscal';
        $h++;
    }
	...
	if (count ($creditfilearray))
	{
		foreach ($head as $k => $v)
		{
			if ($head[$k][2] == 'credit_docs')
			{
				$head[$k][1] .= '<span class="badge marginleftonlyshort">'.count($creditfilearray).'</span>';
				break;
			}
		}
	}	
...
function getFormeJuridiqueLabel($code)
{
	...
	$sql = "SELECT label FROM ".MAIN_DB_PREFIX."c_cfdimx_regimen_f";
	$sql .= " WHERE code='".$code."'";
	$resql = $db->query($sql);
	if ($resql) {
		$num = $db->num_rows($resql);
