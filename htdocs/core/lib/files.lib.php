 ...
 * @param	boolean	$showAlert				Show alert for file transfer complete
 * @return	int                             <=0 if KO, >0 if OK
 */
function dol_add_file_process($upload_dir, $allowoverwrite = 0, $donotupdatesession = 0, $varfiles = 'addedfile', $savingdocmask = '', $link = null, $trackid = '', $generatethumbs = 1, $showAlert = true)
{
		...
		if (dol_mkdir($upload_dir) >= 0)
		{
			...
			for ($i = 0; $i < $nbfile; $i++)
			{
				...
				if (is_numeric($resupload) && $resupload > 0)   // $resupload can be 'ErrorFileAlreadyExists'
				{
					...
					{
						$result = addFileIntoDatabaseIndex($upload_dir, basename($destfile), $TFile['name'][$i], 'uploaded', 1);
						if ($result < 0)
						{
...
function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = '', $refname = '', $mode = 'read')
{
	...
	// Wrapping for POS
	elseif ($modulepart == 'pos' && !empty($conf->pos->dir_output)) {
		$accessallowed = $user->rights->pos->frontend || $user->rights->pos->backend;
		$original_file = $conf->pos->dir_output.'/'.$original_file;
	}
