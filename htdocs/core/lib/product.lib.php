...
function product_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT."/product/batchexp.php?id=".$object->id;
	$head[$h][1] = "Lotes y caducidad";
	$head[$h][2] = 'batch_exp';
	$h++;
    ...
    $h++;
    //Sustitutos
    $head[$h][0] = DOL_URL_ROOT.'/product/substitutes.php?id='.$object->id;
    $head[$h][1] = $langs->trans("Sustitutos");
    $head[$h][2] = 'substitutes';
    $h++;
    //Compuestos
    $head[$h][0] = DOL_URL_ROOT.'/product/complements.php?id='.$object->id;
    $head[$h][1] = $langs->trans("Complementarios");
    $head[$h][2] = 'complements';
    $h++;
...
function show_stats_for_company($product, $socid)
{
	...
	if (!empty($conf->commande->enabled) && $user->rights->commande->lire)
	{
		...
		$langs->load("orders");
		print '<tr><td>';
		print '<a href="commande.php?id='.$product->id.'">'.img_object('', 'order').' '.$langs->trans("ForCustomersOrders").'</a>';
		print '</td><td class="right">';
	...
	{
		$nblines++;
		$ret = $product->load_stats_expedition();
		if ($ret < 0) dol_print_error($db);
		$langs->load("orders");
		print '<tr><td>';
		print '<a href="expedition.php?id='.$product->id.'">'.img_object('', 'order').' '.$langs->trans("Sendings").'</a>';
		print '</td><td class="right">';
	...
	if (!empty($conf->facture->enabled) && $user->rights->facture->lire)
	{
		...
		$langs->load("bills");
		print '<tr><td>';
		print '<a href="facture.php?id='.$product->id.'">'.img_object('', 'bill').' '.$langs->trans("CustomersInvoicesAndCreditNotes").'</a>';
		print '</td><td class="right">';
	...
	if (!empty($conf->fournisseur->enabled) && $user->rights->fournisseur->facture->lire)
	{
		...
		$langs->load("bills");
		print '<tr><td>';
		print '<a href="facture_fournisseur.php?id='.$product->id.'">'.img_object('', 'bill').' '.$langs->trans("SuppliersInvoicesPurchasesAndDevolutions").'</a>';
		print '</td><td class="right">';
	...
	{
		$nblines++;
		$ret = $product->load_stats_pos($socid);
		if ($ret < 0) dol_print_error($db);
		print '<tr><td>';
		print '<a href="pos.php?id='.$product->id.'">'.img_object('', 'bill').''.$langs->trans("POSSalesAndDevolutions").'</a>';
		print '</td><td class="right">';
		print $product->stats_pos['suppliers'];
		print '</td><td class="right">';
		print $product->stats_pos['nb'];
		print '</td><td class="right">';
		print $product->stats_pos['qty'];
		print '</td>';
		print '</tr>';
	}
