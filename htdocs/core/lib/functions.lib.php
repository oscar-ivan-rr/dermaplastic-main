...
function dol_banner_tab($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $onlybanner = 0, $morehtmlright = '')
{
	...
	if ($object->element == 'product' || $object->element == 'bank_account' || $object->element == 'project_task')
	{
		...
		if ($object->element == 'product') {
			...
			if ($object->price_base_type == 'TTC') {
				$morehtmlref.= price($object->price_ttc).' '.$langs->trans($object->price_base_type);
				$morehtmlref.= ' ($'.price($object->price).' '.$langs->trans('HT').')';
			} else {
                $morehtmlref.= price($object->price_ttc).' '.$langs->trans('TTC');
                $morehtmlref.= ' ($'.price($object->price).' '.$langs->trans($object->price_base_type).')';
				//$morehtmlref.= price($object->price).' '.$langs->trans($object->price_base_type);
			}
			$morehtmlref.= '</div>';
		}
	...
	if ($object->element != 'societe' && method_exists($object, 'getBannerAddress') && $object->element != 'product' && $object->element != 'bookmark' && $object->element != 'ecm_directories' && $object->element != 'ecm_files')
	{
...
function dol_print_date($time, $format = '', $tzoutput = 'tzserver', $outputlangs = '', $encodetooutput = false)
{
	...
	elseif ($format == 'pdf_format')		$format = '%d de %B de %Y %R hrs';
...
function img_refresh($titlealt = "default", $other = '')
{
	global $langs;
	if ($titlealt == "default") $titlealt = $langs->trans("Refresh");
	return img_picto($titlealt, 'refresh.png', $other);
}
/**
 *  Show printer logo
 *
...
function img_warning($titlealt = 'default', $moreatt = '', $morecss = 'pictowarning')
{
	...
	return img_picto($titlealt, 'alert-24.png', 'class="'.$morecss.'"'.($moreatt ? ($moreatt == '1' ? ' style="float: right"' : ' '.$moreatt) : ''));
}
