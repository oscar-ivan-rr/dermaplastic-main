 ...
 * @param	bool		$getwidth	Return width and height.
 * @return	number
 */
function pdf_getHeightForLogo($logo, $url = false, $maxheight = 22, $getwidth = false)
{
	global $conf;
	$height = (empty($conf->global->MAIN_DOCUMENTS_LOGO_HEIGHT) ? $maxheight : $conf->global->MAIN_DOCUMENTS_LOGO_HEIGHT);
	$maxwidth = 130;
 ...
 *  @param	int				$addhyperlink		Add hyperlink to custom card of object line.
 * 	@return	string
 */
function pdf_writelinedesc(&$pdf, $object, $i, $outputlangs, $w, $h, $posx, $posy, $hideref = 0, $hidedesc = 0, $issupplierline = 0, $addhyperlink = 0)
{
 ...
 *  @param 	Translate	$outputlangs		Object langs for output
 *  @param	int			$hidedetails		Hide value (0 = no, 1 = yes, 2 = just special lines)
 *  @param	int			$useup				1 = Use price * qty, 0 = Use total_ht with taxes
 *  @return	string							Return total of line incl tax
 */
function pdf_getlinetotalwithtax($object, $i, $outputlangs, $hidedetails = 0, $useup = 0)
{
	global $hookmanager, $conf;
	...
	if (empty($reshook))
	{
		...
		elseif ((empty($hidedetails) || $hidedetails > 1) && !$useup) $result .= price($sign * ($object->lines[$i]->total_ht) + ($object->lines[$i]->total_ht) * ($object->lines[$i]->tva_tx) / 100, 0, $outputlangs);
		
		elseif ((empty($hidedetails) || $hidedetails > 1) && $useup) $result .= price($object->lines[$i]->price * $object->lines[$i]->qty, 0, $outputlangs);
	}
	return $result;
}
