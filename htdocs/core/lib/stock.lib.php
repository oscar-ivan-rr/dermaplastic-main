...
function stock_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/product/custom/in_review.php?id='.$object->id;
	$head[$h][1] = $langs->trans("Productos en Revisión");
	$head[$h][2] = 'in_review';
	$h++;

	$head[$h][0] = DOL_URL_ROOT.'/product/custom/inventory_review.php?id='.$object->id;
	$head[$h][1] = $langs->trans("Revisión de Inventario");
	$head[$h][2] = 'inventory_review';
	$h++;
