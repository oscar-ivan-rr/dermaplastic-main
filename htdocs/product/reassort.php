...
if ($resql)
{
	...
	$moreforfilter .= '</div>';
	$moreforfilter .= '<div class="divsearchfield">';
	$moreforfilter .= $langs->trans('Batch').': ';
	$moreforfilter .= '<input class="flat" type="text" name="search_batch" size="6" value="'.$search_batch.'">';
	$moreforfilter .= '</div>';
...
//	print_liste_field_titre("DesiredStock", $_SERVER["PHP_SELF"], "p.desiredstock", $param, "", '', $sortfield, $sortorder, 'right ');
    // print_liste_field_titre('DesiredStockMatriz', $_SERVER["PHP_SELF"], 'p.desiredstock_principal', $param, '', '', $sortfield, $sortorder, 'right ');
    // print_liste_field_titre('DesiredStockGPE', $_SERVER["PHP_SELF"], 'p.desiredstock_gpe', $param, '', '', $sortfield, $sortorder, 'right ');
	print_liste_field_titre("PhysicalStock", $_SERVER["PHP_SELF"], "stock_physique", $param, "", '', $sortfield, $sortorder, 'right ');
	print_liste_field_titre("StockToBeDelivered", $_SERVER["PHP_SELF"], "", $param, "", '', $sortfield, $sortorder, 'right ');
...
//		print '<td class="right">'.$objp->desiredstock.'</td>';
        ...
        // print '<td class="right">'.$product->desiredstock_principal.'</td>';
        //Desired stock GPE
        // print '<td class="right">'.$product->desiredstock_gpe.'</td>';

		// Real stock
		print '<td class="right">';
		if ($objp->seuil_stock_alerte != '' && ($objp->stock_physique < $objp->seuil_stock_alerte)) print img_warning($langs->trans("StockTooLow")).' ';
		$physical_stock = ($product->stock_warehouse[1]->real > 0)? $product->stock_warehouse[1]->real : 0;
		$physical_stock += $product->stock_warehouse[2]->real;
		print price2num($physical_stock, 'MS');
        // print price2num($objp->stock_physique, 'MS');
		print '</td>';

		// Stock to deliver
		$stock_to_deliver = ($product->stock_warehouse[1]->real < 0)? $product->stock_warehouse[1]->real : 0;
		$result = $product->load_stats_commande(0, '1,2', 1);
		$stock_to_deliver = ($stock_to_deliver - $product->stats_commande['qty']) * -1;		
		$result = $product->load_stats_sending(0, '2', 1, '1,2');
		$stock_to_deliver -= $product->stats_expedition['qty'];
		print '<td class="right">'.$stock_to_deliver.'</td>';
		...
		if ($virtualdiffersfromphysical)
		{
			...
			$stock_to_deliver = ($product->stock_reel < 0) ? $product->stock_reel : 0;
			$result = $product->load_stats_commande(0, '1,2', 1, $entrepotTemp);
			$stock_to_deliver = (($stock_to_deliver - $product->stats_commande['qty']) * -1);
			$stock_to_deliver -= $product->stats_expedition['qty'];

			// Virtual Stock
			$result = $product->load_stats_commande_fournisseur(0, '3,4', 1);
			$virtual_stock = $product->stock_reel - $stock_to_deliver;

			$total_physical_stock += $product->stock_reel;
			$total_to_deliver += $stock_to_deliver;
			$total_virtual_stock += $virtual_stock; 

			$result = $product->load_stats_commande_fournisseur(0, '3,4', 1);
			$pedidosOC = $product->stats_commande_fournisseur['qty'];
			...
			// print $product->load_virtual_stock('nobatch');
