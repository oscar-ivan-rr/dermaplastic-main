...
if(!count($selected))
{
	$selected= explode(',',$conf->global->POS_ALMACEN_MOSTRAR_EN_RESUMEN);
	$selected;
}
...
$sql =	 '(SELECT  t.rowid '."\r\n"
		...
		.'		, s.rowid AS soc_id'."\r\n"
		.'		, \'ticket\' AS obj_type'."\r\n"
		.'		, `td`.`ls_warehouse_status_date` AS `timestamp`'."\r\n"
		.'FROM `'.MAIN_DB_PREFIX.'pos_ticket` AS `t`'."\r\n"
		.'LEFT JOIN `'.MAIN_DB_PREFIX.'pos_ticketdet` AS `td`'."\r\n"
		...
		.'  AND `t`.`fk_statut` <> 3 '."\r\n"
		.'  AND `td`.`ls_warehouse_status` IN ('.implode(',',$selected).')'."\r\n"
		;
...
$sql .=  '( '."\r\n"
		.'SELECT  t.rowid  '."\r\n"
		.'		, t.ref AS ref '."\r\n"
		.'		, t.date_creation AS `date_ticket` '."\r\n"
		.'		, t.fk_soc'."\r\n"
		.'		, t.fk_statut '."\r\n"
		.'		, NULL AS cash_name'."\r\n"
		.'		, t.fk_warehouse AS warehouse'."\r\n"
		.'		, t.fk_user_author'."\r\n"
		.'		,SUM(td.qty) AS total_items'."\r\n"
		.'		, u.firstname AS user_firstname'."\r\n"
		.'		, u.lastname AS user_lastname'."\r\n"
		...
		.'		, s.nom AS soc_name'."\r\n"
		.'		, s.rowid AS soc_id'."\r\n"
		.'		, \'commande\' AS obj_type'."\r\n"
		.'		, t.date_valid AS `timestamp`'."\r\n"
		.'FROM `llx_commande` AS `t`'."\r\n"
		.'LEFT JOIN `llx_commandedet` AS `td`'."\r\n"
		.'  ON `t`.`rowid` = `td`.`fk_commande`'."\r\n"
		.'LEFT JOIN `llx_user` AS `u`'."\r\n"
		.'  ON `u`.`rowid` = `t`.`fk_user_author`'."\r\n"
		.'LEFT JOIN `llx_societe` AS `s`'."\r\n"
		.'  ON `t`.`fk_soc` = `s`.`rowid`'."\r\n"
		.'WHERE #`t`.`fk_warehouse`= 1'."\r\n"
		.'  `t`.`fk_statut` IN (1)'."\r\n"
		.'  AND `t`.`note_public` NOT LIKE \'%ENTREGA A DOMICILIO GENERADA EN EL POS A:%\' '
		;
if (isset($filters['ref']) && strlen($filters['ref']))
{
	$sql .=	'  AND t.ref LIKE \'%'.$db->escape($filters['ref']).'%\''."\r\n";
}
if (isset($filters['cash_name']) && strlen($filters['cash_name']))
{
	$sql .=	'  AND false'."\r\n";
...
{
	$stDate	=	mktime(0,0,0,$filters['month'],1,$filters['year']);
	$sql .=	'  AND t.date_creation >= \''.date('Y-m-d H:i:s',$stDate).'\''."\r\n";
	$enDate	=	mktime(0,0,0,$filters['month']+1,1,$filters['year'])-1;
	$sql .=	'  AND t.date_creation <= \''.date('Y-m-d H:i:s',$enDate).'\''."\r\n";
}
...
$sql =	 'SELECT COUNT(DISTINCT `t`.`rowid`) AS tickets,SUM(ABS(`td`.`qty`)) as prods,`td`.`ls_warehouse_status` '."\r\n"
		.'FROM llx_pos_ticket AS `t` '."\r\n"
		.'LEFT JOIN `llx_pos_ticketdet` AS `td` '."\r\n"
		.'  ON `t`.`rowid`=`td`.`fk_ticket` '."\r\n"
		.'LEFT JOIN `llx_pos_cash` AS `c` '."\r\n"
		.'  ON `c`.`rowid`=`t`.`fk_cash` '."\r\n"
		.'WHERE t.fk_statut IN (0,1,2,3) '."\r\n"
		//.'  AND t.type = 0'."\r\n"
		.'  AND c.fk_warehouse = '.$id."\r\n"
		.'GROUP BY `td`.`ls_warehouse_status`'."\r\n"
		.''."\r\n"
		;
//die($sql);
if (!$rez = $db->query($sql))
...
{
	if (is_null($row->ls_warehouse_status))
	{
		continue;
	}
	$rt[$row->ls_warehouse_status] = $row->tickets;
	$rp[$row->ls_warehouse_status] = $row->prods;
}
...
print load_fiche_titre($langs->trans("PRODUCTOS SOLICITADOS EN ALMACÉN"));
dol_fiche_head(array(array(DOL_URL_ROOT.'/product/custom/by_warehouse.php?id='.$id,'Solicitados','sols'),array(DOL_URL_ROOT.'/product/custom/orphans.php?id='.$id,'Pendientes','pends')), 'sols', 'Almacén', -1, 'stock');

dol_banner_tab($object, 'id', $linkback, '1', 'rowid');

$resumen = $selected;
//die(var_dump($stList,$resumen));
?>
<div id="rc_resumenevdiv">
<table id="rc_resumenev">
	<tbody>
	
		<?php foreach($resumen as $rk): ?>
		<tr>
			<td class="<?php echo $stList[$rk]; ?>"><?php echo $stLabel[$rk]; ?></td>
			<td style="text-align: right;"><?php echo isset($rt[$rk])?$rt[$rk]:0;  ?> Tickets</td>
			<td style="text-align: right;"><?php echo isset($rt[$rk])?$rp[$rk]:0;  ?> Piezas</td>
		</tr>
		<?php endforeach; ?>
	</tbody>
</table>
</div>
<div id="rc_reload_button">
	<a class="btn btn-success" style="width: 100px;height: 68px;" title="Actualizar esta página." onclick="location.reload();"><i class="fa fa-refresh fa-4x" style="font-size: 45px;line-height: 70px;"></i></a>
</div>
...
<div>
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onsubmit="return rc_formsubmit(this);">
		<div style="float: right;">
			<button type="submit" style="height: 45px;border: 1px solid #ddd;"><i class="fa fa-search"></i></button>
		</div>
		<div id="rc_status_filter">
			<input type="text" name="sts_input" id="sts" placeholder="Seleccionar" />
		</div>
...
<table class="noborder centpercent" style="width: 100%;">
	<thead>
		<tr class="liste_titre">
			<?php print_liste_field_titre("<i class='fa fa-clock-o'></i>", "", "timestamp", $params, "", 'style="width:3%;white-space: nowrap;"', $sortfield, $sortorder);?>
			<?php print_liste_field_titre("Ref.", "", "ref", $params, "", 'style="width:10%;"', $sortfield, $sortorder);?>
			<th style="width:3%;"></th>
			<?php print_liste_field_titre("Fecha", "", "date_ticket", $params, "", 'style="width:5%;"', $sortfield, $sortorder);?>
			<?php print_liste_field_titre("Usuario de Terminal", "", "user_firstname,user_lastname,user_login", $params, "", "", $sortfield, $sortorder);?>
			...
			<?php print_liste_field_titre("Estado", "", "fk_statut", $params, "", "", $sortfield, $sortorder);?>
			<?php print_liste_field_titre("Terminal", "", "cash_name", $params, "", "", $sortfield, $sortorder);?>
		</tr>
		<tr class="liste_titre_filter">
			<td colspan="2" class="list_titre" style="vertical-align: top;"><input type="text" name="filters[ref]" value="<?php echo $filters['ref']?>" class="flat" style="background-color: #fff; height: 1.5em; " /></td>
			<td></td>
...
-->				
	...
	<tbody>
		...
		<?php $is_ticket = true; if ($row->obj_type!='ticket'){$is_ticket = false;} ?>
		<?php $static_ticket = $is_ticket ? new Ticket($db): new Commande($db); $static_ticket->fetch($row->rowid); ?>
			<tr>
				<td colspan="2">
					...
					<?php else: ?>
					&nbsp;- (<a href="<?php echo DOL_URL_ROOT; ?>/commande/card.php?id=<?php echo $row->rowid; ?>" target="_blank" style="color:#090;"><?php echo $row->ref;?></a>)
				...
				<a 
						...
						data-source="<?php echo DOL_URL_ROOT; ?>/product/custom/by_warehouse_det.php?id=<?php echo $row->rowid.'&wid='.$row->warehouse; ?>" 
...
<?php foreach($stList AS $k => $v): ?>
    {
      id: <?php echo $k;?>,
      title:'<?php echo html_entity_decode($stLabel[$k]);?>',
