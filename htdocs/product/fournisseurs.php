...
if($id> 0 || $ref) {
    $c = new Categorie($db);
    $categorias=$c->getListForItem($object->id,'product');
...
if ($cancel) $action = '';
if ($action == 'update_price_gain_2'){
    $ref=GETPOST("id_fourn");
    $ref=explode(',',$ref);

    if(GETPOST("id_fourn")!='-1' && GETPOST("gain")>=0){
        ...
        $sql4="SELECT unitprice, real_cost, reason, remise_percent ";
		$sql4.=" FROM ".MAIN_DB_PREFIX."product_fournisseur_price ";
		$sql4.=" WHERE fk_product =".$object->id." and fk_soc = ".$ref[0]." and ref_fourn='".$ref[1]."' and unitprice=".$ref[2];
        $result4=$db->query($sql4);
        $data=$db->fetch_object($result4);
		if($data->real_cost > 0){
			$sql6="UPDATE llx_product set gain = ".GETPOST("gain")." , cost_price = ".$data->real_cost."  where rowid = ".$object->id;
		}else{
			if($data->reason == 'Edición' || $data->reason == 'Cotizacion'){
				$sql6="UPDATE llx_product set gain = ".GETPOST("gain")." , cost_price = ".$data->unitprice*(1-($data->remise_percent/100))."  where rowid = ".$object->id;
			}else{
				$sql6="UPDATE llx_product set gain = ".GETPOST("gain")." , cost_price = ".$data->unitprice."  where rowid = ".$object->id;
			}
		}
        $db->query($sql6);
        //$object->update($object->id,$user);
		if($data->real_cost > 0){
			$newprice=(($data->real_cost)/(1-(GETPOST("gain")/100)))*1.16;
		}else{
			if($data->reason == 'Edición' || $data->reason == 'Cotizacion'){
				$newprice=((($data->unitprice*(1-$data->remise_percent/100)))/(1-(GETPOST("gain")/100)))*1.16;
			}else{
				$newprice=(($data->unitprice)/(1-(GETPOST("gain")/100)))*1.16;
			}
		}
        ...
        if($db->num_rows($max)>0){
            while($products_ids= $db->fetch_object($max)){
				...
				if($line->real_cost > 0){
					$object_aux->updatePrice($newprice, 'TTC', $user, 16, $data->real_cost*1.05, '', '', '', '', '');
				}else{
					$object_aux->updatePrice($newprice, 'TTC', $user, 16, $data->unitprice*1.05, '', '', '', '', '');
				}
			...
			if($line->real_cost > 0){
				$object->updatePrice($newprice, 'TTC', $user, 16, $data->real_cost*1.05, '', '', '', '', '');
			}else{
				$object->updatePrice($newprice, 'TTC', $user, 16, $data->unitprice*1.05, '', '', '', '', '');
			}

        }
        //$up=$object->updatePriceGain(GETPOST("id_fourn"),GETPOST("gain"),$user);
        $object->fetch($id, $ref);
    }
	if( GETPOST("id_fourn") == '-1' ){
		setEventMessages($langs->trans("EmptyFourn"),null, 'errors');
		$action = 'update_price_gain';
	}if( GETPOST("gain") == '' ){
		setEventMessages($langs->trans("EmptyGain"), null, 'errors');
		$action = 'update_price_gain';
	}
}
...
if (empty($reshook))
{
	if ($action == 'setcost_price')
	{
		if ($id)
		{
			...
			$newprice=(($cost_price)/(1-($object->gain/100)))*1.16;
			$result = $object->update($object->id, $user);
            if($db->num_rows($max)>0){
                while($products_ids= $db->fetch_object($max)){
                        $object_aux= new Product($db);
                        $object_aux->fetch($products_ids->fk_product);
                        $object_aux->updatePrice($newprice, 'TTC', $user, 16, $cost_price*1.05, 0, 0, 0, 0, '');
                        unset($object_aux);
                }
            }else{
                $object->cost_price = price2num($cost_price);
                $newprice=(($cost_price)/(1-($object->gain/100)))*1.16;
                $result = $object->update($object->id, $user);
                $object->updatePrice($newprice, 'TTC', $user, 16, $cost_price*1.05, 0, 0, 0, 0, '');
            }
			if ($result > 0)
			{
	...
	if ($action == 'confirm_remove_pf')
	{
		if ($rowid)	// id of product supplier price to remove
		{
			$action = '';
            /*if($db->num_rows($max)>0) {
                $sql4="Select * from llx_product_fournisseur_price where rowid=".$rowid;
                $result2=$db->query($sql4);
                $data=$db->fetch_object($result2);
                $sql5="select rowid from llx_product_fournisseur_price where fk_soc = ".$data->fk_soc." and ref_fourn = '".$data->ref_fourn."' and price = ". $data->price." and quantity = ".$data->quantity." and remise_percent = ".$data->remise_percent." and tva_tx = ".$data->tva_tx." and fk_multicurrency = ".$data->fk_multicurrency." and multicurrency_code = '".$data->multicurrency_code."'";
                $result3=$db->query($sql5);
                while ($rowsid = $db->fetch_object($result3)) {
                    if($rowsid->rowid != $rowid){
                        $object->remove_product_fournisseur_price($rowsid->rowid);
			...
			if ($result > 0) {
				$db->query("DELETE FROM ".MAIN_DB_PREFIX."product_fournisseur_price_extrafields WHERE fk_object = ".$rowid);
				$sql1="SELECT COUNT(*) as total FROM llx_product_fournisseur_price where fk_product= ".$object->id;
				$ressql1=$db->query($sql1);
                $obj1 = $db->fetch_object($ressql1);
				if($obj1->total>0){
                    $object->updatePriceGain(1,$object->gain,$user);
                }
	...
	if ($action == 'save_price')
	{
        ...
        $packing = GETPOST('packing', 'alpha', 2);
        ...
        if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY) {
            ...
            if (price2num($_POST["multicurrency_tx"]) <= 0 || $_POST["multicurrency_tx"] == '') {
                $error++;
                $langs->load("errors");
                setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentities("TypeRate")), null, 'errors');
            }
		...
		if (!$error)
		{
			...
			if (!$error)
			{
                ...
                if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY)
                {
                	$multicurrency_tx = price2num(1 / GETPOST("multicurrency_tx", 'alpha'));
                	$multicurrency_price = price2num(GETPOST("multicurrency_price", 'alpha'));
                	$multicurrency_code = GETPOST("multicurrency_code", 'alpha');

					//ACTUALIZAR PRECIO MANUAL
					if(is_null($object->cost_price)){
						$tipo_cambio = GETPOST("multicurrency_tx", 'alpha');
						if($remise_percent == ''){
							$price_remise = $newprice * $tipo_cambio;
							$price_remise = $price_remise * $tipo_cambio;
						}else{
							$newprice = $newprice * $tipo_cambio;
							$price_remise = $newprice * (1 - ($remise_percent / 100));
							$price_remise = $price_remise * $tipo_cambio;
						}						
						$sql7="UPDATE llx_product set cost_price = ".$price_remise."  where rowid = ".$object->id;
        				$db->query($sql7);
					}
					if(GETPOST('rowid')){
						$ret = $object->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', $multicurrency_price, $_POST["multicurrency_price_base_type"], $multicurrency_tx, $multicurrency_code, $supplier_description, $barcode, $fk_barcode_type, $packing,'Edición');
					}else{	
						$ret = $object->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', $multicurrency_price, $_POST["multicurrency_price_base_type"], $multicurrency_tx, $multicurrency_code, $supplier_description, $barcode, $fk_barcode_type, $packing,'Cotizacion');
					}
                } else {

					if(GETPOST('rowid')){
						$ret = $object->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', 0, 'HT', 1, '', $supplier_description, $barcode, $fk_barcode_type, $packing,'Edición');
					}else{	
						$ret = $object->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', $multicurrency_price, $_POST["multicurrency_price_base_type"], $multicurrency_tx, $multicurrency_code, $supplier_description, $barcode, $fk_barcode_type, $packing,'Cotizacion');
						$ret = $object->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', 0, 'HT', 1, '', $supplier_description, $barcode, $fk_barcode_type, $packing,'Cotizacion');
					}
                }
				if ($ret < 0)
				{
			...
			if (!$error)
			{
				$db->commit();
                //$object->updatePriceGain(1,$object->gain,$user);
                $newprice=((price2num(GETPOST("price", "alpha")))/(1-($object->gain/100)))*1.16;

                if($db->num_rows($max)>0){
                    while($products_ids= $db->fetch_object($max)){
                        $object_aux= new Product($db);
                        $object_aux->fetch($products_ids->fk_product);
                        //$object_aux->updatePrice($newprice, 'TTC', $user, 16, price2num(GETPOST("price", "alpha"))*1.05, 0, 0, 0, 0, '');
                        unset($object_aux);
                    }
                }
				$action = '';
                $object->fetch($id, $ref);
                //$object->cost_price = price2num(GETPOST("price", "alpha"));
        ...
        if($db->num_rows($max)>0) {
            while ($como = $db->fetch_object($max)) {
                if($como->fk_product != $id){
                    $object_aux = new ProductFournisseur($db);
                    $object_aux->fetch($como->fk_product);
                    $object_aux->add_fournisseur($user, $id_fourn, $ref_fourn_old, $quantity);
                    if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY)
                    {
                        $multicurrency_tx = price2num(1 / GETPOST("multicurrency_tx", 'alpha'));
                        $multicurrency_price = price2num(GETPOST("multicurrency_price", 'alpha'));
                        ...
                        $ret1 = $object_aux->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', $multicurrency_price, $_POST["multicurrency_price_base_type"], $multicurrency_tx, $multicurrency_code, $supplier_description, $barcode, $fk_barcode_type, $packing,'Cotizacion');
                    } else {
                        $ret1 = $object_aux->update_buyprice($quantity, $newprice, $user, $_POST["price_base_type"], $supplier, $_POST["oselDispo"], $ref_fourn, $tva_tx, $_POST["charges"], $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, array(), '', 0, 'HT', 1, '', $supplier_description, $barcode, $fk_barcode_type, $packing,'Cotizacion');
...
if ($id > 0 || $ref)
{
	if ($result)
	{
		...
		if ($action != 'edit' && $action != 're-edit')
		{
			...
			//$text = $form->textwithpicto($langs->trans("CostPriceSellBase"), $textdesc, 1, 'help', '');
			//print $form->editfieldkey($text, 'cost_price',round($object->cost_price,2), $object, $usercancreate, 'amount:6');
			print $langs->trans("Precio de compra CEDIS");
            print '</td><td colspan="2">';
            //print $form->editfieldval($text, 'cost_price',round($object->cost_price,2), $object, $usercancreate, 'amount:6');
			print "$".price($object->cost_price);
			if($object->cost_price != null){
				$sqlDateReason = "SELECT pf.tms, pf.reason FROM llx_product_fournisseur_price AS pf WHERE pf.fk_product = ".$object->id." AND (pf.price = ".$object->cost_price. " OR pf.real_cost = ".$object->cost_price." OR pf.price*(1-(pf.remise_percent/100)) = ".$object->cost_price.") ORDER BY rowid ASC LIMIT 1 ";
				$ressqlDateReason=$db->query($sqlDateReason);
				$DateReason = $db->fetch_array($ressqlDateReason);
				if($DateReason[0]){
				print '<span>&nbsp;&nbsp;&nbsp;('.dol_print_date($DateReason[0], 'day').')</span>';
				}
				if($DateReason[1]){
					if($DateReason[1] == 'Edición'){
						print '<span>&nbsp;&nbsp; Motivo:Compra</span>';
					}else{
						print '<span>&nbsp;&nbsp; Motivo:'.$DateReason[1].'</span>';
					}
				}
			}
			...
			print '<tr><td>';
			print $langs->trans("Precio de compra Sucursal");
			print '</td><td colspan="2">';
			print "$".price($object->cost_price_sucursal);
			print '</td></tr>';

            $rc_tmp_cost = floatval($object->cost_price);
            $rc_product_fourn_list = $product_fourn->list_product_fournisseur_price($object->id, 'pfp.tms', 'desc', 1,'0');
            if (count($rc_product_fourn_list) && !empty($rc_product_fourn_list[0]->fourn_unitprice))
            {
                $rc_last_purchase_price = floatval($rc_product_fourn_list[0]->fourn_unitprice);
                if ($rc_tmp_cost != $rc_last_purchase_price)
                {
                    print '<tr><td>';
                    print $langs->trans("Ultimo Precio de compra añadido").'</td><td>';
					
					if($rc_product_fourn_list[0]->fourn_remise_percent != 0){
						$last_price = $rc_product_fourn_list[0]->fourn_unitprice * (1 - ($rc_product_fourn_list[0]->fourn_remise_percent / 100));
						print "$".price($last_price);
					}else{
						print "$".price($rc_product_fourn_list[0]->fourn_unitprice);
					}
            ...
            //Ganancia
            print '<tr><td>';
            print $langs->trans("Gain").'%</td><td>';
			print number_format($object->gain,2);
            print '</td></tr>';
			...
			if (($action == 'add_price' || $action == 'update_price') && $usercancreate)
			{
                ...
                if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY) {
                    ...
                    // Currency tx
                    print '<tr><td class="fieldrequired">'.$langs->trans("TypeRate").'</td>';
                    print '<td><input class="flat" name="multicurrency_tx" size="4" value="'.vatrate(GETPOST('multicurrency_tx') ?GETPOST('multicurrency_tx') : (isset($object->fourn_multicurrency_tx) ? price(1 / $object->fourn_multicurrency_tx) : '')).'">';
                    print '</td>';
                    print '</tr>';
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            $('select[name="multicurrency_code"]').change(function () {
                $('input[name="multicurrency_tx"]').val(Math.round((1 / currencies_array[$(this).val()]) * 100) / 100);
            });
        });
    </script>
SCRIPT;
				...
				// Packing
				print '<tr>';
				print '<td>'.$langs->trans('Packing').'</td>';
				print '<td><input class="flat" name="packing" value="'.($rowid ? $object->packing : '').'"></td>';
				print '</tr>';
            ...
            // Form to update a price acording to a margin of gain
            if ($action == 'update_price_gain') {
                $langs->load("suppliers");
                //obtenemos los datos de los precios de proveedores para este producto
				$sql = " SELECT 
							s.nom,
							s.rowid,
							pfp.ref_fourn,
						...
						JOIN ".MAIN_DB_PREFIX."product_fournisseur_price AS pfp ON pfp.fk_soc = s.rowid
						WHERE
							pfp.fk_product = ".GETPOST('id');
                $res = $db->query($sql);
                print '<form action="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'" method="POST">';
                print '<input type="hidden" name="action" value="update_price_gain_2">';
                ...
                print '<select name="id_fourn">';//Proveedores
                print '<option value="-1"></option>';
                if($res){
                    while ($ob = $db->fetch_object($res)) {
                        print '<option value="'.$ob->rowid.','.$ob->ref_fourn.','.$ob->unitprice.'">'.$ob->nom.' ('.$ob->ref_fourn.')($'.price($ob->unitprice * (1 - ($ob->remise_percent / 100))).')</option>';
                    }
                }
                print '</select>';
                //Margen de ganancia
                print '</td></tr>';
                print '<tr><td class="fieldrequired" width="25%">Margen de ganacia</td><td>';
                print '<input type="text" name="gain" size="5">%';
                print '</td></tr>';
                print '</table>';
                dol_fiche_end();
			...
			if ($action != 'add_price' && $action != 'update_price')
			{
				...
				if (empty($reshook))
				{
					if ($usercancreate)
					{
                        print '<a class="butAction" href="'.DOL_URL_ROOT.'/product/fournisseurs.php?id='.$object->id.'&amp;action=update_price_gain">';
                        print $langs->trans("UpdateSupplierPrice").'</a>';
			...
			if ($user->rights->fournisseur->lire) // Duplicate ? this check is already in the head of this file
			{
				...
				print '<tr style="height: 90px;background: #e9eaed; border-top-color: #FEFEFE;">';
				...
				print_liste_field_titre("Precio Base con Descuento", $_SERVER["PHP_SELF"], '', '', $param, '', $sortfield, $sortorder, 'right ');
				print_liste_field_titre("RealCost",'', '', '', $param, '', $sortfield, $sortorder, 'right ');
                if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY) {
					...
					print_liste_field_titre("TypeRate", $_SERVER["PHP_SELF"], "", "", $param, '', $sortfield, $sortorder, 'right ');
                }
                ...
                print_liste_field_titre("Motivo", $_SERVER["PHP_SELF"], "", "", $param, '', $sortfield, $sortorder, 'right ');
				print_liste_field_titre("Packing", $_SERVER["PHP_SELF"], "pfp.packing", "", $param, '', $sortfield, $sortorder, 'center ');
				...
				if (is_array($product_fourn_list))
				{
					foreach ($product_fourn_list as $productfourn)
					{
						...
						{
							print '<td class="left"><div style="width: 80px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">'.$productfourn->getNomUrl().'</div></td>';
						}
						else
						{
							print '<td class="left">style="width: 80px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"'.$productfourn->fourn_ref.'</div></td>';
						}
                        ...
                        if (false && ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY)) {
						...
						// Unit price
						print '<td class="right">';
						if ($productfourn->reason == "Compra"){
							$sql8 = "SELECT a.pu_ht ";
							$sql8 .= " FROM ".MAIN_DB_PREFIX."facture_fourn_det AS a, ".MAIN_DB_PREFIX."facture_fourn AS b, ".MAIN_DB_PREFIX."product_fournisseur_price AS c";
							$sql8 .= " WHERE ";
							$sql8 .= " c.fk_soc = b.fk_soc AND b.fk_statut >= 1";
							//$sql8 .= " AND c.remise_percent = a.remise_percent";
							$sql8 .= " AND ".$productfourn->fourn_remise_percent." = a.remise_percent";
							$sql8 .= " AND c.quantity = a.qty";
							//$sql8 .= " AND a.total_ht/a.qty = c.unitprice";
							$sql8 .= " AND a.total_ht/a.qty = ".$productfourn->fourn_unitprice."";
							$sql8 .= " AND c.fk_product = ".$productfourn->id."";
							$sql8 .= " AND c.fk_product = a.fk_product";
							//$sql8 .= " AND b.tms = c.tms";
							$fecha_valid = explode(" ",$productfourn->fourn_date_creation1);
							$sql8 .= " AND b.date_valid = '".$fecha_valid[0]."'";
							$sql8 .= " AND c.reason = 'Compra'";
							$ressql8=$db->query($sql8);
							$obj8 = $db->fetch_object($ressql8);
							print price($obj8->pu_ht);	
						}else{
						...
						print '<td class="right">';
						if($productfourn->fourn_remise_percent == 0 || $productfourn->reason == "Compra"){
							print price($productfourn->fourn_unitprice);
						}else{
							print price($productfourn->fourn_unitprice * (1 - ($productfourn->fourn_remise_percent / 100)));
						}
						print '</td>';

						print '<td class="right">';
							print price($productfourn->real_cost);
						print '</td>';

                        if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY) {
                            // Unit price in currency
                            print '<td class="right">';
							if($productfourn->fourn_multicurrency_unitprice != null)
							...
							// CurrencyRate
                            print '<td class="right">';
                            print price(1 / $productfourn->fourn_multicurrency_tx);
                            print '</td>';
                        }
						...
						if ($productfourn->reason == "Cotización"){
							print '<td class="right" style="color:green;">';
						}else{
							print '<td class="right">';
						}
						if($productfourn->reason != "Edición"){
							print $productfourn->reason;
						
							$sql5="	SELECT d.ref";
							$sql5 .= " FROM " . MAIN_DB_PREFIX . "supplier_proposaldet a, " . MAIN_DB_PREFIX . "supplier_proposal b, llx_societe s, ";
							$sql5 .= "" . MAIN_DB_PREFIX . "element_element c, " . MAIN_DB_PREFIX . "propal d, " . MAIN_DB_PREFIX . "propaldet e ";
							$sql5 .= " WHERE a.fk_product = ".$productfourn->id." AND a.fk_supplier_proposal = b.rowid AND b.fk_statut = 2 AND c.fk_target = d.rowid";
							$sql5 .= " AND c.fk_source = b.rowid AND c.targettype = 'propal' AND c.sourcetype = 'supplier_proposal' AND e.fk_product = ".$productfourn->id." ";
							$sql5 .= " AND '".$productfourn->fourn_name."' = s.nom AND s.rowid=b.fk_soc AND e.fk_propal = c.fk_target ";
							$sql5 .= " AND a.subprice = ".$productfourn->fourn_unitprice." AND b.date_cloture='".$productfourn->fourn_date_creation1."'";

							$ressql5=$db->query($sql5);

							if($ressql5){
								while($obj5 = $db->fetch_object($ressql5)){
										print '<br>'.$obj5->ref;
								}
							}
						}else{
						...
						// Date
						print '<td align="right">';
						print dol_print_date(($productfourn->fourn_date_creation ? $productfourn->fourn_date_creation : $productfourn->date_creation), 'dayhour').'</td>';
						print '</td>';
						...
						if ($usercancreate)
						{
							...
							if ($user->rights->produit->delete_price_buy) 
							{
								print ' &nbsp; ';
