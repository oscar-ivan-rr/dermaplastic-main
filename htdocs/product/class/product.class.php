...
class Product extends CommonObject
{
    ...
    */
    ...
    public $gain;
    /**
     * Product locacion matriz_1
     *
     * @var string
     */
    public $location_matriz;
    /**
     * Product locacion gpe_1
     *
     * @var string
     */
    public $location_gpe;
    /**
     * Product locacion matriz_2
     *
     * @var string
     */
    public $location_matriz2;
    /**
     * Product locacion gpe_2
     *
     * @var string
     */
    public $location_gpe2;
    ...
    */
    ...
    /*
    * @var integer temp_discount
    */
    public $temp_discount = 0;
    ...
    public $revicedstock = 0;
    public $desiredstock_principal = 0;
    public $desiredstock_gpe = 0;
    ...
    */
    ...
    /**
     * @var string The object's old reference
     */
    public $old_ref;
    ...
    /*
    * @var integer|string date_lim_discount
    */
    public $date_lim_discount;
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        $this->gain = $this->gain? price2num($this->gain):0;
        ...
        if ($result >= 0) {
            ...
            if ($result) {
                ...
                if ($obj->nb == 0) {
                    ...
                    $sql .= ", date_compra";
                    $sql .= ", date_venta";
                    $sql .= ", location_matriz2";
                    $sql .= ", location_gpe2";
                    $sql .= ", rotation";
                    $sql .= ", desc_max";
                    $sql .= ", cant_dentro_empaque";
                    $sql .= ", empaque";
                    $sql .= ", exentoiva";
                    $sql .= ", date_lim_discount";
                    $sql .= ", temp_discount";
                    ...
                    $sql .= ", ".(!empty($this->old_ref) ? "'".$this->db->escape($this->old_ref)."'" : "null");
                    ...
                    $sql .= ", ".($this->gain > 0 ? "0" : $this->gain);
                    $sql .= ", ".(!empty($this->location_matriz) ? "'".$this->db->escape($this->location_matriz)."'" : "null");
                    $sql .= ", ".(!empty($this->location_gpe) ? "'".$this->db->escape($this->location_gpe)."'" : "null");
                    $sql.= ", ".($this->date_compra?"'".$this->db->idate($this->date_compra)."'":"null");
                    $sql.= ", ".($this->date_venta?"'".$this->db->idate($this->date_venta)."'":"null");
                    $sql .= ", ".(!empty($this->location_matriz2) ? "'".$this->db->escape($this->location_matriz2)."'" : "null");
                    $sql .= ", ".(!empty($this->location_gpe2) ? "'".$this->db->escape($this->location_gpe2)."'" : "null");
                    $sql .= ", ".(!empty($this->rotation) ? "'".$this->db->escape($this->rotation)."'" : "null");
                    $sql .= ", ".(!empty($this->desc_max) ? "'".($this->desc_max)."'" : "null");
                    $sql .= ", ".(!empty($this->cant_dentro_empaque) ? "'".($this->cant_dentro_empaque)."'" : "null");
                    $sql .= ", ".(!empty($this->empaque) ? "'".($this->empaque)."'" : "null");
                    $sql .= ", ".(!empty($this->exentoiva) ? "'".($this->exentoiva)."'" : "null");
                    $sql .= ", ".($this->date_lim_discount?"'".$this->db->idate($this->date_lim_discount)."'":"null");
                    $sql .= ", ".(!empty($this->temp_discount)? "'".($this->temp_discount)."'" : "null");
                    //* Si no se ha indicado almacén por defecto, se asigna el almacén 16 (CEDIS)
                    if (empty($this->fk_default_warehouse) || $this->fk_default_warehouse == -1) $this->fk_default_warehouse = 16;
                    ...
                    if ($result) {
                        ...
                        if ($id > 0) {
                            ...
                            // Bases
                            $this->base_price            = $price_ht;
                            $this->base_price_ttc        = $price_ttc;
                            $this->base_price_min        = $price_min_ht;
                            $this->base_price_min_ttc    = $price_min_ttc;
                            ...
                            // Automatic barcode redirect to custom/view.php?id
                            // $barcodeValue = DOL_MAIN_URL_ROOT . "/product/custom/view.php?id=$id";
                            
                            // $result = $this->check_barcode($barcodeValue, 'QRCODE');

                            // if ($result >= 0)
                            //     $result = $this->setValueFrom('barcode', $barcodeValue, '', null, 'text', '', $user);
                            // else
                            // {
                            //     $langs->load("errors");
    ...
    public function update($id, $user, $notrigger = false, $action = 'update', $updatetype = false)
    {
        ...
        $this->unidad_entrada = trim($this->unidad_entrada);
        $this->unidad_salida =trim($this->unidad_salida);
        $this->desc_max =trim($this->desc_max);
        $this->cant_dentro_empaque =trim($this->cant_dentro_empaque);
        $this->empaque =trim($this->empaque);
        ...
        if ($result >= 0) {
            ...
            $sql .= ", old_ref = ".(!empty($this->old_ref) ? "'".$this->db->escape($this->old_ref)."'" : "null");
            ...
            $sql .= ", revicedstock = ".((isset($this->revicedstock) && $this->revicedstock != '') ? (int) $this->revicedstock : "null");
            $sql .= ", desiredstock_principal = ".((isset($this->desiredstock_principal) && $this->desiredstock_principal != '') ? (int) $this->desiredstock_principal : "0");
            $sql .= ", desiredstock_gpe = ".((isset($this->desiredstock_gpe) && $this->desiredstock_gpe != '') ? (int) $this->desiredstock_gpe : "0");
            $sql .= ", cost_price = ".($this->cost_price != '' ? $this->db->escape($this->cost_price) : 'null');
            $sql .= ", fk_unit= ".(!$this->fk_unit ? 'NULL' : (int) $this->fk_unit);
            $sql .= ", price_autogen = ".(!$this->price_autogen ? 0 : 1);
            $sql .= ", fk_price_expression = ".($this->fk_price_expression != 0 ? (int) $this->fk_price_expression : 'NULL');
            $sql .= ", fk_user_modif = ".($user->id > 0 ? $user->id : 'NULL');
            $sql .= ", gain = ".($this->gain > 0 ? $this->gain : '0');
            $sql .= ", location_matriz = '".$this->db->escape($this->location_matriz)."'";
            $sql .= ", location_gpe = '".$this->db->escape($this->location_gpe)."'";
            $sql .= ", unit_entrada = ".($this->unidad_entrada != '' ? "'".$this->db->escape($this->unidad_entrada)."'" : 'NULL');
            $sql .= ", unit_salida = ".($this->unidad_salida != '' ? "'".$this->db->escape($this->unidad_salida)."'" : 'NULL');
            $sql .= ", date_compra = ".($this->date_compra?"'".$this->db->idate($this->date_compra)."'":"null");
            $sql .= ", date_venta = ".($this->date_venta?"'".$this->db->idate($this->date_venta)."'":"null");
            $sql .= ", location_matriz2 = '".$this->db->escape($this->location_matriz2)."'";
            $sql .= ", location_gpe2 = '".$this->db->escape($this->location_gpe2)."'";
            $sql .= ", rotation = '".$this->db->escape($this->rotation)."'";
            $sql .= ", desc_max = '".($this->desc_max)."'";
            $sql .= ", cant_dentro_empaque = '".($this->cant_dentro_empaque)."'";
            $sql .= ", empaque = '".($this->empaque)."'";
            $sql .= ", exentoiva = '".($this->exentoiva)."'";
            $sql .= ", date_lim_discount = ".($this->date_lim_discount?"'".$this->db->idate($this->date_lim_discount)."'":"null");
    ...
    */
    ...
    public function getSellPrice($thirdparty_seller, $thirdparty_buyer, $pqp = 0)
    {
        ...
        //Comentar funcion ya que el IVA se setea a 0
     ...
     * @param  int    $copyBases         Copy prices to bases 
     * @return int                            <0 if KO, >0 if OK
     */
    public function updatePrice($newprice, $newpricebase, $user, $newvat = '', $newminprice = 0, $level = 0, $newnpr = 0, $newpbq = 0, $ignore_autogen = 0, $localtaxes_array = array(), $newdefaultvatcode = '', $copyBases = 1)
    {
        global $conf, $langs;
        ...
        if (!empty($newminprice) && ($newminprice > $newprice)) {
            $this->error = ($copyBases) ? 'ErrorPriceCantBeLowerThanMinPrice' : 'ErrorPriceCantBeLowerThanMinPriceInMassive';
            return -1;
        }
        $gain =$this->gain/100;
        if ($newprice !== '' || $newprice === 0) {
            ...
            if ($copyBases) {
                $sql .= ", base_price=".$price;
                $sql .= ", base_price_ttc=".$price_ttc;
                $sql .= ", base_price_min=".$price_min;
                $sql .= ", base_price_min_ttc=".$price_min_ttc;
            }
    ...
    public function fetch($id = '', $ref = '', $ref_ext = '', $barcode = '', $ignore_expression = 0, $ignore_price_load = 0, $ignore_lang_load = 0)
    {
        ...
        $sql .= " price_min, price_min_ttc, price_base_type, cost_price,cost_price_sucursal, default_vat_code, tva_tx, recuperableonly as tva_npr, localtax1_tx, localtax2_tx, localtax1_type, localtax2_type, tosell,";
        ...
        $sql .= " datec, tms, import_key, entity, desiredstock, tobatch, fk_unit,revicedstock,desiredstock_principal,desiredstock_gpe,";
        $sql .= " fk_price_expression, price_autogen, gain, fk_oc,";
        $sql .= " base_price, base_price_ttc, base_price_min, base_price_min_ttc,";
        ...
        if ($resql) {
            ...
            if ($this->db->num_rows($resql) > 0) {
                ...
                $this->revicedstock                    = $obj->revicedstock;
                $this->desiredstock_principal           = $obj->desiredstock_principal;
                $this->desiredstock_gpe                  = $obj->desiredstock_gpe;
                ...
                $this->base_price_ttc                    = $obj->base_price_ttc;
                $this->base_price_min                    = $obj->base_price_min;
                $this->base_price_min_ttc                = $obj->base_price_min_ttc;

                // Stock percentages
                $this->wh1percent = $obj->wh1percent;
                $this->wh2percent = $obj->wh2percent;

                // Stock limits
                $this->wh1_limit = $obj->wh1_limit;
                $this->wh2_limit = $obj->wh2_limit;

                //Location
                $this->location_matriz= $obj->location_matriz;
                $this->location_gpe= $obj->location_gpe;
                $this->location_matriz2 = $obj->location_matriz2;
                $this->location_gpe2 = $obj->location_gpe2;
                //Entradas y Salidas
                $this->unidad_entrada=$obj->unit_entrada;
                $this->unidad_salida=$obj->unit_salida;
                //dates
                $this->date_compra = $obj->date_compra;
                $this->date_venta = $obj->date_venta;

                // Rotation

                $this->rotation = $obj->rotation;

                $this->desc_max = $obj->desc_max;
                $this->cant_dentro_empaque = $obj->cant_dentro_empaque;
                $this->empaque = $obj->empaque;
                $this->exentoiva = $obj->exentoiva;
                $this->date_lim_discount = $obj->date_lim_discount;
    ...
    /**
     *  Charge tableau des stats expedition pour le produit/service
     *
     * @return array               Tableau des stats
     */
    public function load_stats_expedition()
    {
        // phpcs:enable
        global $conf;
        global $user;

        $sql = "SELECT COUNT(DISTINCT exp.fk_soc) as nb_customers, COUNT(DISTINCT exp.rowid) as nb, SUM(expdet.qty) as qty";
        ...
        $sql .= " AND expdet.fk_origin_line = commdet.rowid AND commdet.fk_product = ".$this->id;
     ...
     */
    public function load_stats_commande($socid = 0, $filtrestatut = '', $forVirtualStock = 0, $ref_entrepot = 0)
    {
        // phpcs:enable
        global $conf, $user;

        $sql = "SELECT COUNT(DISTINCT c.fk_soc) as nb_customers, COUNT(DISTINCT c.rowid) as nb,";
        $sql .= " COUNT(cd.rowid) as nb_rows, IFNULL(SUM(cd.qty),0) as qty";
        $sql .= " FROM " . MAIN_DB_PREFIX . "commandedet as cd";
        $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "commande as c on cd.fk_commande = c.rowid";
        $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "societe as s on c.fk_soc = s.rowid";
        $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "product lp on cd.fk_product = lp.rowid";
        ...
        if ($ref_entrepot != 0) {
            $sql .= " AND c.fk_warehouse = " . $ref_entrepot;
        }
     ...
     */
    public function load_stats_sending($socid = 0, $filtrestatut = '', $forVirtualStock = 0, $filterShipmentStatus = '', $ref_entrepot = 0)
    {
        // phpcs:enable
        global $conf, $user;

        $sql = "SELECT COUNT(DISTINCT e.fk_soc) as nb_customers, COUNT(DISTINCT e.rowid) as nb,";
        $sql .= " COUNT(ed.rowid) as nb_rows,  IFNULL(SUM(ed.qty), 0) AS qty";
    ...
    public function getQtyToStock(){

        $sql = "SELECT SUM(ptd.qty) AS qty 
                    FROM llx_pos_ticket AS pt, llx_pos_ticketdet AS ptd
                        WHERE ptd.fk_product = ".$this->id."
                                AND ptd.ls_warehouse_status = 4
                                AND ptd.fk_ticket = pt.rowid
                                AND pt.fk_statut = 2";
    ...
    public function load_stats_pos($socid = 0)
    {
        // phpcs:enable
        global $conf;
        global $user;
        ...
        $sql = "SELECT COUNT(DISTINCT p.fk_soc) as nb_suppliers, COUNT(DISTINCT p.rowid) as nb,";
        $sql .= " COUNT(pd.rowid) as nb_rows, SUM(pd.qty) as qty";
        $sql .= " FROM ".MAIN_DB_PREFIX."pos_ticketdet as pd";
        $sql .= ", ".MAIN_DB_PREFIX."pos_ticket as p";
        $sql .= ", ".MAIN_DB_PREFIX."societe as s";
        if (!$user->rights->societe->client->voir && !$socid) {
            $sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
        }
        $sql .= " WHERE p.rowid = pd.fk_ticket";
        $sql .= " AND p.fk_soc = s.rowid";
        ...
        if ($result) {
            $obj = $this->db->fetch_object($result);
            $this->stats_pos['suppliers'] = $obj->nb_suppliers;
            $this->stats_pos['nb'] = $obj->nb;
            $this->stats_pos['rows'] = $obj->nb_rows;
            $this->stats_pos['qty'] = $obj->qty ? $obj->qty : 0;
            return 1;
        }
        else
        {
        ...
        else
        {
            ...
            if ($result) {
                ...
                else
                {
                    ...
                    {
                        $sql= "SELECT fk_product_fils,qty from llx_product_association where fk_product_pere = ".$id_pere;
                        $result =$this->db->query($sql);
                        $total_price=0;
                        $totalmin=0;
                        $totalmaxttc=0;
                        $totalminttc=0;
                        $price=0;
                        while ($data = $this->db->fetch_object($result)){
                            $aux_product = new Product($this->db);
                            $aux_product->fetch($data->fk_product_fils);
                            $total_price += $aux_product->price*$data->qty;
                            $totalmaxttc += $aux_product->price_ttc*$data->qty;
                            $totalmin += $aux_product->price_min*$data->qty;
                            $totalminttc += $aux_product->price_min_ttc*$data->qty;
                            $price += $aux_product->cost_price *$data->qty;
                        }

                        $sql1 =	 'UPDATE '.MAIN_DB_PREFIX.'product '
                            .'SET '
                            .'price = '.$total_price.', '
                            .'price_ttc = '.$totalmaxttc.', '
                            .'price_min = '.$totalmin.', '
                            .'price_min_ttc = '.$totalminttc.', '
                            .'cost_price = '.$price.' '
                            .'WHERE rowid = '.$id_pere;
                        $this->db->query($sql1);
                         return 1;
     ...
     * @return int                < 0 if KO, > 0 if OK
     */
    public function add_complement($parent, $child, $qty)
    {
        // phpcs:enable
        // Clean parameters
        ...
        // Check not already father of id_pere (to avoid father -> child -> father links)
        $sql = 'SELECT id from complementary_products';
        $sql .= ' WHERE fk_product_parent  = '.$parent.' AND fk_product_child = '.$child;
        if (!$this->db->query($sql)) {
            dol_print_error($this->db);
            return -1;
        }
        else
        {
            ...
            if ($result) {
                $num = $this->db->num_rows($result);
                if ($num > 0) {
                    $this->error = "isComplementOfThis";
                    return -1;
                }
                else
                {
                    $sql = 'INSERT INTO complementary_products(fk_product_parent,fk_product_child,qty)';
                    $sql .= ' VALUES ('.$parent.', '.$child.', '.$qty.')';
                    if (!$this->db->query($sql)) {
                        dol_print_error($this->db);
                        return -1;
                    }
     ...
     *  Link a product/service to a parent product/service
     *
     * @param  int $id_ori Id of parent product/service
     * @param  int $id_sus Id of child product/service
     * @return int                < 0 if KO, > 0 if OK
     */
    public function add_substitute($id_ori, $id_sus)
    {
        // phpcs:enable
        // Clean parameters
        ...
        if (!is_numeric($id_sus)) {
            $id_sus = 0;
        }
        // Check not already father of id_pere (to avoid father -> child -> father links)
        $sql = 'SELECT fk_product_ori from product_subtitute';
        $sql .= ' WHERE fk_product_ori  = ' . $id_ori . ' AND fk_product_sus = ' . $id_sus;
        if (!$this->db->query($sql)) {
            dol_print_error($this->db);
            return -1;
        } else {
            ...
            if ($result) {
                $num = $this->db->num_rows($result);
                if ($num > 0) {
                    $this->error = "isSubstituteOfThis";
                    return -1;
                }
                else {
                    $sql = 'INSERT INTO product_subtitute(fk_product_ori,fk_product_sus)';
                    $sql .= ' VALUES (' . $id_ori . ', ' . $id_sus . ')';
                    if (!$this->db->query($sql)) {
                        dol_print_error($this->db);
                        return -1;
                    } else {
    ...
    public function del_sousproduit($fk_parent, $fk_child)
    {
        ...
        $sql= "SELECT fk_product_fils,qty from llx_product_association where fk_product_pere = ".$fk_parent;
        $result =$this->db->query($sql);
        $total_price=0;
        $totalmin=0;
        $totalmaxttc=0;
     ...
     * @return int                    < 0 if KO, > 0 if OK
     */
    public function del_complement($fk_parent, $fk_child)
    {
        // phpcs:enable
        if (!is_numeric($fk_parent)) {
            $fk_parent = 0;
        ...
        $sql = "DELETE FROM complementary_products";
        $sql .= " WHERE fk_product_parent  = ".$fk_parent;
        $sql .= " AND fk_product_child = ".$fk_child;

        dol_syslog(get_class($this).'::del_com', LOG_DEBUG);
        if (!$this->db->query($sql)) {
            dol_print_error($this->db);
            return -1;
        }
     ...
     * @return int                    < 0 if KO, > 0 if OK
     */
    public function update_complement($fk_parent, $fk_child,$qty)
    {
        // phpcs:enable
        if (!is_numeric($fk_parent)) {
            $fk_parent = 0;
        ...
        $sql = "UPDATE complementary_products SET qty=".$qty;
        $sql .= " WHERE fk_product_parent  = ".$fk_parent;
        $sql .= " AND fk_product_child = ".$fk_child;
     ...
     * @return int                    < 0 if KO, > 0 if OK
     */
    public function del_substitute($fk_parent, $fk_child)
    {
        // phpcs:enable
        if (!is_numeric($fk_parent)) {
            $fk_parent = 0;
        ...
        $sql = "DELETE FROM product_subtitute";
        $sql .= " WHERE fk_product_ori  = ".$fk_parent;
        $sql .= " AND fk_product_sus = ".$fk_child;

        dol_syslog(get_class($this).'::del_sub', LOG_DEBUG);
        if (!$this->db->query($sql)) {
            dol_print_error($this->db);
            return -1;
        }
    ...
    // phpcs:disable PEAR.NamingConventions.ValidFunctionName.ScopeNotCamelCaps
    /**
     *  Verifie si es un complemento
     *
     * @param  int $fk_parent Id du produit auquel le produit est lie
     * @param  int $fk_child  Id du produit lie
     * @return int                    < 0 si erreur, > 0 si ok
     */
    public function is_complement($fk_parent, $fk_child)
    {
        // phpcs:enable
        $sql = "SELECT fk_product_child, qty";
        $sql .= " FROM complementary_products";
        $sql .= " WHERE fk_product_parent  = '".$fk_parent."'";
        $sql .= " AND fk_product_child = '".$fk_child."'";
    ...
    /**
     *  Verifie si es un sustituto
     *
     * @param  int $fk_parent Id du produit auquel le produit est lie
     * @param  int $fk_child  Id du produit lie
     * @return int                    < 0 si erreur, > 0 si ok
     */
    public function is_sustitute($fk_parent, $fk_child)
    {
        // phpcs:enable
        $sql = "SELECT id";
        $sql .= " FROM product_subtitute";
        $sql .= " WHERE fk_product_ori  = ".$fk_parent;
        $sql .= " AND fk_product_sus = ".$fk_child;
     ...
     */
    public function add_fournisseur($user, $id_fourn, $ref_fourn, $quantity,$real_cost = 0)
    {
        // phpcs:enable
        global $conf;
    ...
    /**
     *  Return all complement products for current product (first level only)
     *
     * @return array         Array of product
     */
    public function getComplements()
    {
        $sql = "SELECT p.rowid, p.label as label, p.ref as ref, cp.fk_product_child as id, p.fk_product_type, cp.qty, p.entity, p.tosell,p.tobuy";
        $sql .= " FROM complementary_products as cp,";
        $sql .= " ".MAIN_DB_PREFIX."product as p";
        $sql .= " WHERE p.rowid = cp.fk_product_child";
        $sql .= " AND cp.fk_product_parent = ".$this->id;
    ...
    /**
     *  Return all sustitute products for current product (first level only)
     *
     * @return array         Array of product
     */
    public function getSubstitute()
    {
        $sql = "SELECT p.rowid, p.label as label, p.ref as ref, ps.fk_product_sus as id, p.fk_product_type, p.entity";
        $sql .= " FROM product_subtitute as ps,";
        $sql .= " ".MAIN_DB_PREFIX."product as p";
        $sql .= " WHERE p.rowid = ps.fk_product_sus";
        $sql .= " AND ps.fk_product_ori = ".$this->id;
    ...
    public function getChildsArbo($id, $firstlevelonly = 0, $level = 1)
    {
        ...
        if($id != null)
        {
            ...
            $sql .= " AND pa.fk_product_pere = ''";
            $sql .= " AND pa.fk_product_fils != ''"; // This should not happens, it is to avoid infinite loop if it happens
        }     
     ...
     * @param  int    $showSellPrice		 Show selling price
     * @return string                                String with URL
     */
    public function getNomUrl($withpicto = 0, $option = '', $maxlength = 0, $save_lastsearch_value = -1, $notooltip = 0, $showSellPrice = 0, $red_ref = false)
    {
        ...
        if ($showSellPrice) {
            $label.= '<br><b>' . $langs->trans("SellingPrice").":</b> $";
            if ($this->price_base_type == 'TTC')
                $label.= price($this->price_ttc).' '.$langs->trans($this->price_base_type);
            else
                $label.= price($this->price).' '.$langs->trans($this->price_base_type);
        }
        ...
        if($red_ref && $this->status_buy == 0 && $this->status == 0) $linkstart.= ' style="color:red;" ';
        $linkstart .= $linkclose.'>';
        $linkend = '</a>';
    ...
    public function getLibFinished()
    {
        ...
        if ($this->finished == '0') { return $langs->trans("OnLine");
        }
        if ($this->finished == '1') { return $langs->trans("OutLine");
        }
        return '';
    }
     ...
     * @return int                     <0 if KO, >0 if OK
     */
    public function correct_stock($user, $id_entrepot, $nbpiece, $movement, $label = '', $price = 0, $inventorycode = '', $origin_element = '', $origin_id = null,$draft=0,$id_entrepot_target=null,$return_id=false)
    {
        // phpcs:enable
        if ($id_entrepot) {
            $this->db->begin();
            ...
            $result = $movementstock->_create($user, $this->id, $id_entrepot, $nbpiece, $movement, $price, $label, $inventorycode);
    ...
    public function correct_stock_batch($user, $id_entrepot, $nbpiece, $movement, $label = '', $price = 0, $dlc = '', $dluo = '', $lot = '', $inventorycode = '', $origin_element = '', $origin_id = null)
    {
        ...
        $movementstock = new MouvementStock($this->db);
        $movementstock->setOrigin($origin_element, $origin_id);
        if($origin_element == 'commande_fournisseurdet'){
            $movementstock->fk_origin = $origin_id ?? 0;
            $movementstock->origintype = $origin_element ?? '';
        }
        $result = $movementstock->_create($user, $this->id, $id_entrepot, $nbpiece, $movement, $price, $label, $inventorycode, '', $dlc, $dluo, $lot);
    ...
    public function create_draft_movement($user, $id_entrepot, $id_entrepot_target, $nbpiece, $movement, $label = '', $dlc = '', $dluo = '', $lot = '', $inventorycode = ''){
        // phpcs:enable
        $this->db->begin();

        $sql = "INSERT INTO " . MAIN_DB_PREFIX . "stock_mouvement_draft (fk_entrepot, fk_entrepot_target, fk_product, batch, eatby, sellby, qty, fk_user, datem, code, label) VALUES (";
        $sql .= $id_entrepot . ",";
        $sql .= $id_entrepot_target . ",";
        $sql .= $this->id . ",";
        $sql .= "'" . $this->db->escape($lot) . "',";
        $sql .= "'" . dol_print_date($dlc, "%Y-%m-%d") . "',";
        $sql .= "'" . dol_print_date($dluo, "%Y-%m-%d") . "',";
        $sql .= $nbpiece . ",";
        $sql .= $user->id . ",";
	...
	public function load_virtual_stock()
	{
        ...
        /** Custom calculation for virtual stock */
        $this->stock_theorique = ($this->stock_reel < 0)? $this->stock_reel : 0;
        $result = $this->load_stats_commande(0, '1,2', 1);
        $this->stock_theorique = ($this->stock_theorique - $this->stats_commande['qty']) * -1;
        $result = $this->load_stats_commande_fournisseur(0, '3,4', 1);
        $result = $this->load_stats_sending(0, '2', 1, '1,2');
        $this->stock_theorique -= $this->stats_expedition['qty'];
        $this->stock_theorique = $this->stats_commande_fournisseur['qty'] - $this->stock_theorique;
		return 1;
	}
    ...
    {
        return ($this->status_batch == 1 ? true : true);
    }
     ...
     * @return void
     */
    public function setCategories($categories,$ban = 0)
    {
        ...
        if($ban == 1){
            $sql = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'categorie WHERE rowid=166 OR fk_parent=166 ORDER BY rowid ASC';
            $resultado = $this->db->query($sql);
            if($resultado){
                while($obc = $this->db->fetch_object($resultado)){
                    if (($key = array_search($obc->rowid, $to_del)) !== false) {
                        unset($to_del[$key]);
                    }
                }
            }
	 ...
	 */
    public function AddProductCategory($categorie_id)
	{
        // phpcs:enable
    ...
    /**
     * Set discount.
     * @param   float   $decimal                 Discount value in decimal
     * @param   int     $date_lim_discountday    Day limit of discount
     * @param   int     $date_lim_discountmonth  Month limit of discount
     * @param   int     $date_lim_discountyear   Year limit of discount
     * @return  int     -1 KO | 0 Product don't have a price | 1 OK
     */
    public function setDiscount($decimal, $date_lim_discountday = '', $date_lim_discountmonth = '', $date_lim_discountyear = '') {
        global $conf, $mysoc, $langs, $user;
        // Obtener el precio adecuado al producto
        if ($this->price_base_type == 'TTC') {
            $price = $this->base_price_ttc;
            $price_min = $this->base_price_min_ttc;
        } else {
            $price = $this->base_price;
            $price_min = $this->base_price_min;
        }

        if ($price > 0) {
            $pricestoupdate = array();
            $maxpricesupplier = $this->min_recommended_price();

            $tva_tx_txt = $this->tva_txt;
            ...
            $pricestoupdate[0] = array(
                'price' => $price * $decimal,
                'price_min' => ($price * $decimal) * ($price_min / $price),
                'price_base_type' => $this->price_base_type,
            ...
            foreach ($pricestoupdate as $key => $val) {
                ...
                if ($val['price'] < $val['price_min'] && !empty($this->fk_price_expression)) {
                ...
                if (!empty($conf->global->PRODUCT_MINIMUM_RECOMMENDED_PRICE) && $newprice_min < $maxpricesupplier) {
                    setEventMessages($langs->trans("MinimumPriceLimitInMassive", price($maxpricesupplier, 0, '', 1, -1, -1, 'auto'), "($this->ref)"), null, 'errors');
                    $error++;
                    break;
                }

                if ($date_lim_discountday && $date_lim_discountmonth && $date_lim_discountyear)
                {
                    // Actualizar fecha limite de descuento temporal
                    $date_lim_discount = $date_lim_discountyear . "-" . $date_lim_discountmonth . "-" . $date_lim_discountday;
                    $discount = (1 - $decimal) * 100;
                    $sql = "UPDATE ".MAIN_DB_PREFIX."product SET";
                    $sql .= " date_lim_discount = '" . $date_lim_discount . "'";
                    $sql .= " , temp_discount = '" . $discount . "'";
                    $sql .= " WHERE rowid = ".$this->id;
                    $resql = $this->db->query($sql);

                    if ($resql < 0) {
                        $error++;
                        setEventMessages($langs->trans($this->error, "($this->ref)"), $this->errors, 'errors');
                        break;
                    }
                }
                else 
                {
                    if ($this->multiprices[$key] != $newprice || $this->multiprices_min[$key] != $newprice_min || $this->multiprices_base_type[$key] != $val['price_base_type'])
                    {
                        //Guardar registros en temporal para: PC, PV y Ganancia
                        $sql = "UPDATE ".MAIN_DB_PREFIX."product SET";
                        $sql .= " gainTemp = gain , costPriceTemp = cost_price, precioTemp = price "; 
                        ...
                        $res = $this->updatePrice($newprice, $val['price_base_type'], $user, $val['vat_tx'], $newprice_min, $key, $val['npr'], 0, 0, $val['localtaxes_array'], $val['default_vat_code'], 0);
                    }
                    else $res = 0;
                }

                if ($decimal == 1) {
                    // Eliminar fecha de caducidad
                    $sqlDate = "UPDATE " . MAIN_DB_PREFIX . "product SET date_lim_discount = NULL, temp_discount = 0 WHERE rowid = " . $this->id;
                    $resqlDate = $this->db->query($sqlDate);

                    if ($resqlDate < 0) {
                        $this->error = 'Error al eliminar fecha de caducidad de producto caducado: ' . $this->db->lasterror();
                        return -2;
                    }
                }
                else $res = 0;
    ...
    public function load_board($user, $mode)
    {
        ...
        if ($mode == 'stock'){
            $sql = "SELECT ps.fk_product ";
            $sql .= " FROM ".MAIN_DB_PREFIX."product_stock AS ps ";
            $sql.= " GROUP BY fk_product HAVING SUM(ps.reel) <= 0 ";
        }elseif ($mode == 'mas_vendidos'){
            $sql = "SELECT null limit 0"; // don't show number in panel
        }elseif ($mode == 'menos_vendidos'){
            $sql = "SELECT null limit 0";
        }elseif ($mode == 'por_transferir'){
            // los productos a transferir sólo existen en session
            $sql = "SELECT null limit 0";
        }elseif ($mode == 'alertas_stock'){
            $sql = "SELECT p.rowid, p.ref, p.desiredstock, p.seuil_stock_alerte, SUM(s.reel)
                    FROM  ".MAIN_DB_PREFIX."product AS p  LEFT JOIN ".MAIN_DB_PREFIX."product_stock AS s ON p.rowid = s.fk_product
                    WHERE -- s.fk_product = p.rowid AND <- esto agrega validacion on  s.fk_product = null 
	                    p.tobuy = 1 AND p.desiredstock > 0 
                    GROUP BY p.rowid
                    HAVING p.seuil_stock_alerte > SUM( IF ( s.reel IS NULL, 0, s.reel ) )";
        }
        ...
        if ($resql)
        {
            ...
            if ($mode == 'stock') {
                $response->warning_delay = null;
                $response->label = $langs->trans("ProductsWithoutStock");
                $response->labelShort = $langs->trans("WithoutStock");
                $response->url = DOL_URL_ROOT . '/product/list.php?leftmenu=product&type=0&action=sin_stock';
                $response->img = img_object('', "projectpub");
            }if ($mode == 'mas_vendidos') {
                $response->warning_delay = null;
                $response->label = $langs->trans("MostSelledProducts");
                $response->labelShort = $langs->trans("MostSelled");
                $response->url = DOL_URL_ROOT . '/product/list.php?leftmenu=product&type=0&action=mas_vendidos&limit=25';
                $response->img = img_object('', "projectpub");
            }if ($mode == 'menos_vendidos') {
                $response->warning_delay = null;
                $response->label = $langs->trans("LessSelledProducts");
                $response->labelShort = $langs->trans("LessSelled");
                $response->url = DOL_URL_ROOT . '/product/list.php?leftmenu=product&type=0&action=menos_vendidos&limit=25';
                $response->img = img_object('', "projectpub");
            }if ($mode == 'por_transferir') {
                $response->warning_delay = null;
                $response->label = $langs->trans("ProductsToTransfer");
                $response->labelShort = $langs->trans("ToTransfer");
                $response->url = DOL_URL_ROOT . '/product/stock/massstockmove.php?mainmenu=products';
                $response->img = img_object('', "projectpub");
            }if ($mode == 'alertas_stock') {
                $response->warning_delay = null;
                $response->label = $langs->trans("ProductsStockAlert");
                $response->labelShort = $langs->trans("StockAlert");
                $response->url = DOL_URL_ROOT . '/product/stock/replenish.php?mainmenu=products';
                $response->img = img_object('', "projectpub");
            }
            ...
            while ($obj = $this->db->fetch_object($resql)) {
                $response->nbtodo++;
                $product_static->statut = $obj->status;
                $product_static->opp_status = $obj->opp_status;
                $product_static->datee = $this->db->jdate($obj->datee);
            }
            if ($mode == 'por_transferir') { // en éste caso los datos vienen de Session, no de BD
                $listofdata = json_decode($_SESSION['massstockmove'], true);
                $response->nbtodo = sizeof($listofdata);
            }
    ...
    /**
     * Stock movements calculated by cronjob
     * @see cronjob "MassStock" in dolibarr system.
     */
    public function massStockAutomatic() {
        global $langs;
        ...
        // $sql = "SELECT p.rowid, p.ref, p.stock, ps.fk_entrepot, ps.reel, e.tms_percent FROM llx_product p, llx_product_stock ps, llx_entrepot e WHERE stock > 0 AND p.rowid = ps.fk_product AND ps.fk_entrepot = e.rowid ORDER BY p.rowid";

        /*
        $sql = "SELECT p.rowid, p.ref, p.stock, ps.fk_entrepot, ps.reel, p.wh1percent, p.wh2percent";
        $sql .= " FROM ".MAIN_DB_PREFIX."product p, ".MAIN_DB_PREFIX."product_stock ps";
        $sql .= " WHERE p.stock > 0 AND (p.wh1percent != 0 OR p.wh2percent != 0) AND p.rowid = ps.fk_product ORDER BY p.rowid";
        */

        $sql = "SELECT p.rowid, p.ref, ps.fk_entrepot, ps.reel, p.wh2_limit, p.desiredstock_gpe
                    FROM llx_product p, llx_product_stock ps 
                        WHERE p.stock > 0 AND p.wh2_limit != 0 AND p.rowid = ps.fk_product and ps.fk_entrepot = 2 ORDER BY p.rowid";

        $result = 1;
        $res = $this->db->query($sql);
        if ($res) {
            while ($objG = $this->db->fetch_object($res)) {
                //SI STOCK FISICO GPE ES MENOR A LIMITE ALERTA STOCK GPE
                if($objG->reel < $objG->wh2_limit){
                     // FORMULA: STOCK DESEADO GPE - STOCK FISICO GPE
                    $qty = $objG->desiredstock_gpe - $objG->reel;
                    if($qty > 0){
                        // SI STOCK FISICO ZAC ES MAYOR A LA FORMULA
                        $sqlZ = "SELECT p.rowid, p.ref, ps.fk_entrepot, ps.reel
                                    FROM llx_product p, llx_product_stock ps 
                                        WHERE p.rowid = ps.fk_product and ps.fk_entrepot = 1 and p.rowid = ".$objG->rowid."  ORDER BY p.rowid";
                        $resZ = $this->db->query($sqlZ);
                        if ($resZ) {
                            $objZ = $this->db->fetch_object($resZ);
                            if($objZ->reel > $qty){
                                // SE PASA QTY EN FORMULA DE ZAC A GPE
                                $transfers [] = "($objG->rowid, $qty, 1, 2)";
                            }
                        }
                    } 
                }
...
/*
            // $transfers [] = "($rowid, $qty, 1, 2)";
            


                // Prevent duplicate key in $registers if prodduct has more than one register.
                if (!key_exists($obj->rowid, $registers)) {
                    $registers[$obj->rowid]['ref'] = $obj->ref;
                    $registers[$obj->rowid]['stock'] = $obj->stock;
                }
                // Get exptected warehouse stock
                $tms_percent = ($obj->fk_entrepot == 1) ? $obj->wh1percent : $obj->wh2percent;
                $expected = round($obj->stock * ($tms_percent / 100), 0, PHP_ROUND_HALF_EVEN);
                // $expected = round($obj->stock * ($obj->tms_percent / 100), 0, PHP_ROUND_HALF_EVEN);
                
                // 0: Nothing 1: Transfer, 2: Receive
                // if ($expected < $obj->reel) $action = 1;
                // elseif ($expected > $obj->reel) $action = 2;
                // else $action = 0;

                if ($expected < $obj->reel) {
                    $registers[$obj->rowid]['entrepot'] = $obj->fk_entrepot;
                    $registers[$obj->rowid]['reel'] = $obj->reel;
                    $registers[$obj->rowid]['expected'] = $expected;
                }
            }
            // Get the sql part to insert
            foreach($registers as $rowid => $register) {
                // Only registers with warehouse defined
                if (!empty($register['entrepot'])) {
                    $qty = $register['reel'] - $register['expected'];
                    $entrepot_sc = $register['entrepot'];
                    $entrepot_tg = ($register['entrepot'] == 1) ? 2 : 1;
                    if($entrepot_tg==1){
                        $sql1="SELECT desiredstock_principal as limi, desiredstock_gpe as limig, (SELECT  reel from llx_product_stock where fk_product=".$rowid." and  fk_entrepot=1) as reel from llx_product where rowid=".$rowid;
                    }
                    if($entrepot_tg==2){
                        $sql1="SELECT desiredstock_gpe as limi, (SELECT  reel from llx_product_stock where fk_product=".$rowid." and  fk_entrepot=2) as reel from llx_product where rowid=".$rowid;
                    }
                    $result1=$this->db->query($sql1);
                    $obj2 = $this->db->fetch_object($result1);
                    $cantidad=$obj2->reel?$obj2->reel:0;
                    if($obj2->limig==0 && $obj2->limig!=null){
                        $sql3="SELECT  reel from llx_product_stock where fk_product=".$rowid." and  fk_entrepot=2";
                        $result2=$this->db->query($sql3);
                        $obj3=$this->db->fetch_object($result2);
                        $qty=$obj3->reel;
                    }else{
                        if ($obj2->limi!=0){
                            if($qty>$obj2->limi){
                            ...
                            $total_qty=$qty+$cantidad;
                            if($total_qty>$obj2->limi){
                                $qty=$obj2->limi-$cantidad;
                            }
                        }else{
                            $qty=0;
                        }
                        if($qty<0){
                            $qty=abs($qty);
                            $aux=$entrepot_sc;
                            $entrepot_sc =$entrepot_tg;
                            $entrepot_tg = $aux;
                        }
                    }
                    if ($entrepot_sc==2 && $qty>0){
                        $sql4="SELECT desiredstock_gpe as limi, (SELECT  reel from llx_product_stock where fk_product=".$rowid." and  fk_entrepot=2) as reel ,wh2percent, (SELECT sum(reel)  from llx_product_stock where fk_product=".$rowid.") as total from llx_product where rowid=".$rowid;
                        $result2=$this->db->query($sql4);
                        $data=$this->db->fetch_object($result2);
                        if($data->reel>$data->limi){
                            $qty=round($data->total*($data->wh2percent/100),0,PHP_ROUND_HALF_EVEN);
                            if($qty<$data->limi){
                                $qty=$data->reel-$qty;
                            }else{
                                $qty=$data->reel-$data->limi;
                            }

                        }
                    }
                    if($entrepot_sc==1 && $qty>0){
                        $sql4="SELECT desiredstock_gpe as limi, (SELECT  reel from llx_product_stock where fk_product=".$rowid." and  fk_entrepot=2) as reel,wh2percent, (SELECT sum(reel)  from llx_product_stock where fk_product=".$rowid.") as total from llx_product where rowid=".$rowid;
                        $result2=$this->db->query($sql4);
                        $data=$this->db->fetch_object($result2);
                        if($data->reel>$data->limi){
                    ...
                    if($qty!=0 && $qty!=$cantidad){
                        $consulta="SELECT rowid from llx_stock_mouvement_draft where fk_product = ".$rowid." AND origin='MassStockAuto'";
                        $result3=$this->db->query($consulta);
                        if($this->db->num_rows($result3)==0){
                            $transfers [] = "($rowid, $qty, $entrepot_sc, $entrepot_tg)";
                        }
                    }
                }
            }
        ...
        if (sizeof($transfers) > 0) {
            ...
            $sql_t = "TRUNCATE stock_mouvement_auto";
            $res_t = $this->db->query($sql_t);
            if ($res_t) {

                $sql = "INSERT INTO stock_mouvement_auto(fk_product, qty, fk_entrepot_source, fk_entrepot_target) VALUES " . implode(", ", $transfers);

                $sql .= " ON DUPLICATE KEY UPDATE qty = VALUES (qty), fk_entrepot_source = VALUES(fk_entrepot_source), fk_entrepot_target = VALUES(fk_entrepot_target)";

                $res = $this->db->query($sql);
                if ($res) {
                    dol_syslog(__METHOD__." sql=".$sql, LOG_DEBUG);
                    $this->output = $langs->trans("TransfersCalcultated", $this->db->affected_rows($res));
                    $result = 0;
                }
                else {
            ...
            $this->output = $langs->trans("TransfersCalcultated", 0);
            $result = 0;
        }
        return $result;
    }
    ...
    /**
     * Get number of intern transfers
     * @return  int   Number of records
     */
    public static function numInternTransfers() {
        global $db;

        $sql = "SELECT count(rowid) as num FROM stock_mouvement_auto";

        $res = $db->query($sql);
        if ($res) return $db->fetch_object($res)->num;
        else return 0;
    }

    /**
     * Get the service id of 'FLETE Y ACARREO'
     * @return  int    Service id.
     */
    public static function getFreightServiceID() {
        global $db;

        dol_syslog("Product::".__METHOD__);

        $id = -1;
        $sql = "SELECT rowid FROM " . MAIN_DB_PREFIX . "product WHERE ref = 'FLETE' AND fk_product_type = 1";
    ...
    /**
     * Update warehouse percentages of product.
     * @param   double  $tms_w1 Matriz percentage
     * @param   double  $tms_w2 Gdpe percentage
     * @return  int     1 OK | < 0 KO
     */
    public function checkPercentages($tms_w1, $tms_w2) {
        // Check parameters
        if (is_numeric($tms_w1) && is_numeric($tms_w2)) {
            // Check total
            if (($tms_w1 + $tms_w2) != 100) {
                $this->error = 'ErrorTotalTmsPercent';
                return -1;
            }
            
            // Update
            $sql = "UPDATE " . MAIN_DB_PREFIX . $this->table_element;
            $sql .= " SET wh1percent = $tms_w1";
            $sql .= ", wh2percent = $tms_w2";
            $sql .= " WHERE rowid=$this->id";
            ...
            $this->error = 'ErrorFieldMustBeANumeric';
            return -3;
        }
    }

    /**
     * Update warehouse limits of product.
     * @param   double  $tms_w1 Matriz percentage
     * @param   double  $tms_w2 Gdpe percentage
     * @return  int     1 OK | < 0 KO
     */
    public function checkLimits($tms_w1, $tms_w2) {
        // Check parameters
        if (is_numeric($tms_w1) && is_numeric($tms_w2)) {
                        
            // Update
            $sql = "UPDATE " . MAIN_DB_PREFIX . $this->table_element;
            $sql .= " SET wh1_limit = ".$tms_w1;
            $sql .= ", wh2_limit = ".$tms_w2;
            $sql .= ", seuil_stock_alerte = ".($tms_w1+$tms_w2);
            $sql .= " WHERE rowid=$this->id";
    ...
    public function rc_updateParentPrices($parent)
    {
        $sql = 'SELECT fk_product_fils from '.MAIN_DB_PREFIX.'product_association';
        $sql .= ' WHERE fk_product_pere  = '.$parent;
        ...
        {
        	$child[] = $row->fk_product_fils;
        }
        
        if (is_array($child) && count($child))
        {
			$sql =   'SELECT `price`,`price_ttc`,`price_min`,`price_min_ttc`,`cost_price` '
					.'FROM '.MAIN_DB_PREFIX.'product '
					.'WHERE rowid IN ('.implode(',',$child).') 	'
					;
	        if (!$res = $this->db->query($sql))
   		    {
   		    	dol_print_error ($this->db);
			...
			$ch_price_min	= 0;
			$ch_price_min_ttc = 0;
			$ch_price_cost_price = 0; 
        	while ($row = $this->db->fetch_object($res))
        	{
	        	$ch_price		+= $row->price;
				$ch_price_ttc	+= $row->price_ttc;
				$ch_price_min	+= $row->price_min;
				$ch_price_min_ttc += $row->price_min_ttc;
				$ch_price_cost_price += $row->cost_price; 
        	}
        	
        	$sql =	 'UPDATE '.MAIN_DB_PREFIX.'product '
        			.'SET '
        			.'price = '.$ch_price.', '
        			.'price_ttc = '.$ch_price_ttc.', '
        			.'price_min = '.$ch_price_min.', '
        			.'price_min_ttc = '.$ch_price_min_ttc.', '
        			.'cost_price = '.$ch_price_cost_price.' '
        			.'WHERE rowid = '.$parent
        			;
			if (!$this->db->query($sql))
			{
    ...
    /**
     * Stock movements calculated by cronjob
     * @see cronjob "Actualizar stocks de reaprovisionamiento" in dolibarr system.
     */
	public function updateStockReplenish()
	{
		global $db;

		// Para cada producto en llx_product
		// Actualizar productos en product_replenish
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."product_replenish (fk_product) SELECT rowid FROM ".MAIN_DB_PREFIX."product
			ON DUPLICATE KEY UPDATE fk_product = rowid";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al actualizar productos';
			return -1;
		}


		// // Calcular Commandes cli
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET commandes_cli = (
				SELECT
					IF( SUM(cd1.qty) IS NULL, 0, SUM(cd1.qty))
				FROM
					".MAIN_DB_PREFIX."commandedet AS cd1,
					".MAIN_DB_PREFIX."commande AS c1
				WHERE
					c1.rowid = cd1.fk_commande AND c1.entity IN(1) AND cd1.fk_product = pr.fk_product AND c1.fk_statut IN(1, 2)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular commandes_cli';
			return -2;
		}


		// // Calcular Expeditions cli
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET expeditions_cli =(
				SELECT
					IF( SUM(ed2.qty) IS NULL, 0, SUM(ed2.qty))
				FROM
					".MAIN_DB_PREFIX."expedition AS e2,
					".MAIN_DB_PREFIX."expeditiondet AS ed2,
					".MAIN_DB_PREFIX."commandedet AS cd2
				WHERE
					ed2.fk_expedition = e2.rowid AND cd2.rowid = ed2.fk_origin_line AND e2.entity IN(1) AND cd2.fk_product = pr.fk_product AND e2.fk_statut IN(1, 2)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular expeditions_cli';
			return -3;
		}
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET commandes_fourn =(
				SELECT 
					IF( SUM(cd3.qty) IS NULL, 0, SUM(cd3.qty))
				FROM
					".MAIN_DB_PREFIX."commande_fournisseurdet AS cd3,
					".MAIN_DB_PREFIX."commande_fournisseur AS c3
				WHERE
					c3.rowid = cd3.fk_commande AND c3.entity IN(1) AND cd3.fk_product = pr.fk_product AND c3.fk_statut IN(3, 4)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular commandes_fourn';
			return -4;
		}
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET reception_fourn =(
				SELECT IF( SUM(fd4.qty) IS NULL, 0, SUM(fd4.qty))
				FROM
					".MAIN_DB_PREFIX."commande_fournisseur AS cf4,
					".MAIN_DB_PREFIX."commande_fournisseur_dispatch AS fd4
				WHERE
					fd4.fk_commande = cf4.rowid AND cf4.entity IN(1) AND fd4.fk_product = pr.fk_product AND cf4.fk_statut IN(3, 4)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular reception_fourn';
			return -5;
		}

		// // Production to produce
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET production_to_produce =(
				SELECT GREATEST(
					0,
					IF(
						SUM(
							IF( mp5.role = 'toproduce', mp5.qty, - mp5.qty)
						) IS NULL,
						0,
						SUM(
							IF( mp5.role = 'toconsume', mp5.qty, - mp5.qty)
						)
					)
				)
				FROM
					".MAIN_DB_PREFIX."mrp_mo AS mm5,
					".MAIN_DB_PREFIX."mrp_production AS mp5
				WHERE
					mm5.rowid = mp5.fk_mo AND mm5.entity IN(1) AND mp5.fk_product = pr.fk_product AND mp5.role IN('toproduce', 'produced') AND mm5.status IN(1, 2)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular production_to_produce';
			return -6;
		}

		// // Production to consume
		$sql = "UPDATE ".MAIN_DB_PREFIX."product_replenish AS pr
			SET production_to_consume =(
				SELECT
				GREATEST( 0,
					IF(
						SUM(
				...
				FROM
					...
					".MAIN_DB_PREFIX."mrp_production AS mp5
				WHERE
					mm5.rowid = mp5.fk_mo AND mm5.entity IN(1) AND mp5.fk_product = pr.fk_product AND mp5.role IN('toconsume', 'consummed') AND mm5.status IN(1, 2)
		)";
		$resql = $db->query($sql);
		if (empty($resql))
		{
			$this->error = 'Error al calcular production_to_consume';
			return -7;
		}
    ...
    function calculateStockOnSales(){
        
        try {
            global $db;
            global $object;

            $intervalo = $object->datelimit;
            $sql = "select
            IF(SUM(lsm.value) < 0,
                SUM(lsm.value * -1),
                SUM(lsm.value)) as value,
            IF(SUM(lsm.value) < 0,
                SUM(lsm.value * -1) * 1.30,
                SUM(lsm.value) * 1.30) as stocks,
            IF(SUM(lsm.value) < 0,
                SUM(lsm.value * -1) - (SUM(lsm.value * -1) * 0.30),
                SUM(lsm.value) - (SUM(lsm.value) * 0.30)) as alerts,
            lsm.fk_product ,
            lsm.fk_entrepot
            from llx_stock_mouvement lsm 
            where lsm.type_mouvement = 2
            and lsm.datem <= now() and lsm.datem >= DATE_SUB(now(), INTERVAL " . $intervalo . " DAY)
            group by lsm.fk_product , lsm.fk_entrepot";
            $resql = $db->query($sql);
            if (empty($resql))
            {
                $this->error = 'Error al obtener los productos con movimientos de venta en la fecha especificada';
                return -8;
            }
            // recorremos el resultado y vamos actualizando el stock en la tabla de llx_product_warehouse_properties en al columna desiredstock con el valor de value, haciendo un conteo de las actualizaciones para ver si se ha actualizado todo correctamente y devolver lo que se ha actualizado
            $count = 0;
            $fk_data = [];
            while ($obj = $db->fetch_object($resql))
            {
                // en caso de que no exista el registro en la tabla de llx_product_warehouse_properties lo creamos
                $sql = "SELECT rowid, calculateStock FROM ".MAIN_DB_PREFIX."product_warehouse_properties
                WHERE fk_product = ".$obj->fk_product." AND fk_entrepot = ".$obj->fk_entrepot;
                $resql2 = $db->query($sql);
                $obj2 = $db->fetch_object($resql2);
                if($db->num_rows($resql2) <= 0){
                    $sql = "INSERT INTO ".MAIN_DB_PREFIX."product_warehouse_properties
                    (fk_product, fk_entrepot, seuil_stock_alerte, desiredstock, import_key)
                    VALUES(".$obj->fk_product.", ".$obj->fk_entrepot.", ".$obj->alerts.", ".$obj->stocks.", NULL)";
                    $resql3 = $db->query($sql);
                    if (empty($resql3))
                    { 
                        $this->error = 'Error insertando el stock en ventas del producto '.$obj->fk_product.' en el almacén '.$obj->fk_entrepot;
                        return -8;
                    }
                    // Se guarda en el array el fk_product del registro que ha sido insertado puesto que calculateStock es = 1 por default
                    $fk_product[] = $obj->fk_product;
                    $count++;
                }

                if($obj2->calculateStock == 1){
                    $sql = "UPDATE ".MAIN_DB_PREFIX."product_warehouse_properties
                    SET desiredstock = ".$obj->stocks.",
                        seuil_stock_alerte = ".$obj->alerts."
                    WHERE fk_product = ".$obj->fk_product." AND fk_entrepot = ".$obj->fk_entrepot;
                    $resql3 = $db->query($sql);
                    if (empty($resql3))
                    { 
                        $this->error = 'Error actualizando el stock en ventas del producto '.$obj->fk_product.' en el almacén '.$obj->fk_entrepot;
                        return -8;
                    }
                    // vamos guardando los productos y almacenes que se han actualizado en un array de claves para realizar operaciones posteriores
                    $fk_product[] = $obj->fk_product; 
            ...
            // recorremos el array para realizar una sumatoria de los stocks en ventas de los productos y almacenes que se han actualizado y actualizar el stock y alertas en la tabla de llx_product
            for ($i=0; $i < count($fk_product); $i++) { 
                // hacemos la sumatoria de la tabla de llx_product_warehouse_properties para los stock y alertas de los productos que se han actualizado y los actualizamos en la tabla de llx_product 
                $sql = "SELECT SUM(desiredstock) as stocks, SUM(seuil_stock_alerte) as alerts FROM ".MAIN_DB_PREFIX."product_warehouse_properties WHERE fk_product = ".$fk_product[$i];
                $resql4 = $db->query($sql);
                if (empty($resql4))
                { 
                    $this->error = 'Error sumando los stocks en ventas del producto '.$fk_product[$i];
                    return -8;
                }
                ...
                SET desiredstock = ".$obj->stocks.",
                    seuil_stock_alerte = ".$obj->alerts."
                WHERE rowid = ".$fk_product[$i];
                $resql5 = $db->query($sql);
                if (empty($resql5))
                { 
                    $this->error = 'Error actualizando el stock en ventas del producto '.$fk_product[$i];
                    return -8;
                }
            }


            $this->output = "Se actualizaron " . $count . " productos";
            return 0;
        } catch (\Throwable $th) {
            $this->error = 'Error al calcular el stock en ventas ' . $th->getMessage();
            return -9;
        }
    }

	/**
	 * Actualizar precios de productos con descuento temporal
	 * @return int 0 if OK, <0 if KO
	 **/
	function reassignTempDiscounts() {
		try {
			require_once DOL_DOCUMENT_ROOT . '/product/class/product.class.php';

			global $db;

			// Obtener id de productos con descuento caducado
			$sql = "SELECT rowid FROM " . MAIN_DB_PREFIX . "product WHERE date_lim_discount < DATE_FORMAT(CURDATE(), '%Y-%m-%d')";
			$resql = $db->query($sql);

			if ($resql < 0) {
				$this->error = 'Error al determinar productos caducados: ' . $this->db->lasterror();
				return -1;
			}

			$count = 0;

			// Desasignar descuento para cada producto
			while ($obj = $db->fetch_object($resql)) {
				...
				// Eliminar fecha de caducidad
				$sqlDate = "UPDATE " . MAIN_DB_PREFIX . "product SET date_lim_discount = NULL, temp_discount = 0 WHERE rowid = " . $obj->rowid;
			...
			$this->output = "Caducó el descuento temporal de " . $count . " producto(s).";
			return 0;

		} catch (\Throwable $th) {
			$this->error = 'Error al reasignar descuentos temporales: ' . $th->getMessage();
			return -9;
		}
	}

    /**
	 * Regresar los productos en el almacén de apartados a su almacén de origen
	 * @return int 0 if OK, <0 if KO
	 **/
	function reassignProducts() {
		try {
			...
			$refdate = strtotime('-10 days', strtotime($currentDate));
			$refdate = date('Y-m-d' , $refdate);
            
            // Obtener almacén definido para apartados en la configuración
			...
			if (empty($resql))
            {
                $this->error = 'Error al obtener productos apartados';
                return -1;
            }
            ...
            while ($obj = $db->fetch_object($resql))
            {
                ...
                $this->fetch($obj->fk_product);
                $result1 = $this->correct_stock_batch(
                    $user,
                    $wrhsapartado->id,
                    $obj->value,
                    1,
                ...
                $result2 = $this->correct_stock_batch(
                    $user,
                    $obj2->origin,
                    $obj->value,
                    0,
            ...
            $this->output = "Productos liberados: " . $count . "";
            return 0;

		} catch (\Throwable $th) {
			$this->error = 'Error al reasignar productos apartados: ' . $th->getMessage();
			return -9;
		}
	}

    /**
	 * Establecer en 0 el stock deseado y el límite si el cálculo automático está desactivado
	 * @return int 0 if OK, <0 if KO
	 **/
	function reassignProductsStock() {
		try {
            global $db, $user, $conf;

            $sql = "SELECT rowid, fk_product, fk_entrepot FROM ".MAIN_DB_PREFIX."product_warehouse_properties
            WHERE calculateStock = 0";
            $resql = $db->query($sql);

            if (empty($resql))
            {
                $this->error = 'Error al obtener registros';
                return -8;
            }
            ...
            $this->output = "Productos actualizados: " . $count . "";
            return 0;

		} catch (\Throwable $th) {
			$this->error = 'Error al actualizar stock de productos: ' . $th->getMessage();
			return -9;
		}
	}
	 ...
	 * @return int 0 if OK, <0 if KO
	 **/
	function addMissingBatches() {
		try {
            global $db;
            
            $count = 0;

            // Get product lots that don't exist in the llx_product_lot table
            $sql = "SELECT DISTINCT ps.fk_product, pb.batch";
            $sql .= " FROM ".MAIN_DB_PREFIX."product_stock ps";
            $sql .= " LEFT JOIN ".MAIN_DB_PREFIX."product_batch pb ON pb.fk_product_stock = ps.rowid";
            $sql .= " LEFT JOIN ".MAIN_DB_PREFIX."product_lot pl ON pl.batch = pb.batch  AND pl.fk_product = ps.fk_product";            
            $sql .= " WHERE pl.batch IS NULL";
            $resql = $db->query($sql);
            ...
            if ($resql)
            {
                while ($obj = $db->fetch_object($resql))
                {
                    // Get the expiration date according to the closest date and with the greatest quantity
                    $sql2 = "SELECT pb.qty, DATE(pb.eatby) as eatby";
                    $sql2 .= " FROM ".MAIN_DB_PREFIX."product_stock ps";
                    $sql2 .= " LEFT JOIN ".MAIN_DB_PREFIX."product_batch pb ON pb.fk_product_stock = ps.rowid";                   
                    $sql2 .= " WHERE ps.fk_product = ".$obj->fk_product." AND pb.batch = '".$obj->batch."'";
                    $sql2 .= " ORDER BY pb.qty DESC, eatby LIMIT 1";
                    ...
                    if ($resql2){
                        ...
                        if(!empty($obj2->eatby)){
                            // Add lot
                            $sql_lot = "INSERT INTO llx_product_lot";
                            $sql_lot .= " (entity, fk_product, batch, eatby, datec)";
                            $sql_lot .= " VALUES(1, ".$obj->fk_product.", '".$obj->batch."', '".$obj2->eatby."', NOW())";
                        ...
                        $this->error = 'Error al obtener fecha de caducidad';
                        return -7;
                    }
                }
            } else {
                $this->error = 'Error al obtener lotes faltantes';
                return -8;
            }

            $this->output = "Lotes agregados: " . $count . "";
            return 0;

		} catch (\Throwable $th) {
			$this->error = 'Error al insertar lotes: ' . $th->getMessage();
			return -9;
		}
	}
}
