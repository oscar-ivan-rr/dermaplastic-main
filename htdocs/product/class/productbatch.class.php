...
class Productbatch extends CommonObject
{
     ...
     */
    public function fetch($id, $lotid = 0, $warehouseid = 0)
    {
		global $langs;
		$sql = "SELECT";
		$sql .= " t.rowid,";
		...
		$sql .= " w.fk_entrepot,";
		$sql .= " w.fk_product,";
		$sql .= " pl.rowid AS lotid,";
		$sql .= " pl.eatby,";
		$sql .= " pl.sellby";

        $sql .= " FROM ".MAIN_DB_PREFIX."product_batch AS t INNER JOIN ".MAIN_DB_PREFIX."product_stock w on t.fk_product_stock = w.rowid";
        $sql .= " LEFT JOIN ".MAIN_DB_PREFIX."product_lot AS pl on pl.fk_product = w.fk_product AND pl.batch = t.batch AND pl.sellby = DATE(t.sellby) AND pl.eatby = DATE(t.eatby)";
        if ($id > 0) {
			$sql .= " WHERE t.rowid=".$id;
		} elseif ($lotid > 0 && $warehouseid > 0) {
			$sql .= " WHERE pl.rowid=".$lotid;
			$sql .= " AND w.fk_entrepot=".$warehouseid;
		} else {
			$this->error = "Error: no id or lotid given";
			return -1;
		}
     ...
     */
    public static function findAll($db, $fk_product_stock, $with_qty = 0, $fk_product = 0, $sortfield = 't.batch', $sortorder = 'ASC', $search_batch = '', $search_eatby = '', $search_sellby = '')
    {
    	global $langs;
		$ret = array();
		...
		if ($fk_product > 0) {
			...
			$sql .= " AND pl.sellby = DATE(t.sellby) AND pl.eatby = DATE(t.eatby)";
		}
		$sql .= " WHERE fk_product_stock= $fk_product_stock";
		if ($with_qty) {
			$sql .= " AND t.qty <> 0";
        ...
        if ($resql) {
            ...
            while ($i < $num)
            {
				...
				$ret[] = $tmp; // $ret is for a $fk_product_stock and unique key is on $fk_product_stock+batch
				$i++;
            }
            $db->free($resql);
