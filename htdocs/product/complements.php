...
/**
 *  \file       htdocs/product/complements.php
 *  \ingroup    product
...
if($action == 'delete' && $user->rights->produit->supprimer){
    if($object->del_complement($id, $com))
    {
        $action = 'view';
    }
}
if($action == 'update')
{
    if($object->update_complement($id,$com,$qtycom))
        $action='view';
}
...
if ($action == 'add_prod' && ($user->rights->produit->creer || $user->rights->service->creer))
{
    ...
    for ($i=0; $i<$_POST["max_prod"]; $i++)
    {
        ...
        {
            if ($object->add_complement($id, $_POST["prod_id_".$i], $_POST["prod_qty_".$i]) < 0)
            {
                $error++;
                $action = 're-edit';
                if ($object->error == "isComplementOfThis") {
                    setEventMessages($langs->trans("Ya es un producto complementario"), null, 'errors');
                }
                else
                {
...
if ($action == 'search')
{
    ...
    $sql.= ' p.fk_product_type, p.tms as datem, p.tosell, p.tobuy';
...
dol_fiche_head($head, 'complements', $titre, -1, $picto);
...
if ($id > 0 || ! empty($ref))
{
    ...
    if ($user->rights->produit->lire || $user->rights->service->lire)
    {
        ...
        $prodsfather = $object->getComplements(); 		// Parent Products
        ...
        if($action == 'edit')
        {
            print '<form action="'.DOL_URL_ROOT.'/product/complements.php?id='.$id.'" method="POST" id="editForm">';
            print '<input type="hidden" name="action" value="update">';
            print '<input type="hidden" name="id" value="'.$id.'">';
            print '<input type="hidden" name="comp" value="'.$com.'">';
        }
        ...
        if (count($prodsfather) > 0)
        {
            foreach($prodsfather as $value)
            {
                ...
                $productstatic->status=$value['tosell'];
                $productstatic->status_buy=$value['tobuy'];
                if($action == 'edit' && $com==$idprod)
                {
                    print '<tr class="oddeven">';
                    print '<td>' . $productstatic->getNomUrl(1, 'composition') . '</td>';
                    print '<td>' . $productstatic->label . '</td>';
                    print '<td><input type="text" name="qtycom" value="'.$value['qty'].'"></td>';
                    print '<td>';
                    print '<input type="submit" class="button" value="Editar" onclick="$(\'editForm\').submit();">';
                    print '</td>';
                    print '</tr>';
                }
                else {
                    ...
                    print '<td>';
                    print '<a href="' . DOL_URL_ROOT . '/product/complements.php?id=' . $id . '&action=delete&comp=' . $productstatic->id . '">' . img_picto($langs->trans("Remove"), 'delete') . '</a>';
                    print '<a href="' . DOL_URL_ROOT . '/product/complements.php?id=' . $id . '&action=edit&comp=' . $productstatic->id . '">' . img_picto($langs->trans("Edit"), 'edit') . '</a>';
                    print '</td>';
                    print '</tr>';
                }
            }
        ...
        if ((empty($action) || $action != 'edit') && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            print '<br>';
            ...
            print '<form action="'.DOL_URL_ROOT.'/product/complements.php?id='.$id.'" method="POST">';
            ...
            if ($resql)
            {
                ...
                while ($i < $num)
                {
                    ...
                    if($objp->rowid != $id)
                    {
                        ...
                        if($object->is_complement($id, $objp->rowid))
                        {
                            //$addchecked = ' checked';
                        ...
                        // Qty
                        print '<td class="right"><input type="hidden" class="chck" name="prod_id_'.$i.'" value="'.$objp->rowid.'"><input type="text" size="2" name="prod_qty_'.$i.'" value="'.($qty?$qty:'').'"></td>';

                        // Inc Dec
                        print '<td class="center">';
                        if ($qty) print '<input type="checkbox" class="chck" name="prod_incdec_'.$i.'" value="1" '.($incdec?'checked':'').'>';
                        else
                        {
                            // TODO Hide field and show it when setting a qty
                            print '<input type="checkbox"  class="chck" name="prod_incdec_'.$i.'" value="1" checked>';
