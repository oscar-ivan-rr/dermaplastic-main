...
if ($id > 0 || !empty($ref))
{
	...
	if ($result > 0)
	{
		...
		if ($user->rights->propale->lire)
		{
			...
			$sql .= " p.ref_client,";
			$sql .= "p.datep, p.fk_statut as statut, d.rowid, d.qty, (d.total_ht/d.qty) as pu";
			...
			if ($result)
			{
				...
				print '</div>';
				//print '<div> Ref:<input type="text" name="search_ref" class="flat maxwidth100" value="'.$search_ref.'"> </div>';
				print '</div>';
				print '</div>';
				...
				if ($num > 0)
				{
					while ($i < min($num, $limit))
					{
						...
						print dol_print_date($db->jdate($objp->datep), 'day')."</td>";
