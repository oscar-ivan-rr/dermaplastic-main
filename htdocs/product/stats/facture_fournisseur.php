...
if ($id > 0 || !empty($ref))
{
	...
	if ($result > 0)
	{
		...
		if ($user->rights->fournisseur->facture->lire)
		{
			...
			$sql .= " f.rowid as facid, f.ref, f.type, f.ref_supplier, f.datef, f.libelle as label, f.total_ht, f.total_ttc, f.total_tva, f.paye, f.fk_statut as statut, d.qty, (d.total_ht/d.qty) as pu";
			...
			if ($result)
			{
				...
				if ($num > 0)
				{
					while ($i < min($num, $limit))
					{
						...
						print '<td class="center">'.($objp->type == 2 ? ($objp->qty*(-1)) : $objp->qty)."</td>\n";
