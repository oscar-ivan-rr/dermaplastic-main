 ...
 * @param 	boolean     $getQty		    Get the products qtys.
 * @return	array|integer[]
 */
function getProducts($order_id, $getQty = false)
{
    global $db;
    ...
    if($f) {
        foreach($order->lines as $line) {
            if (!$getQty && !in_array($line->fk_product, $products))
                $products[] = $line->fk_product;
            elseif ($getQty) {
                $count = (is_numeric($products[$line->fk_product])) ? (int)$products[$line->fk_product] : 0;
                $products[$line->fk_product] = $line->qty + $count;
            }
        }
    }
    return $products;
