...
if(!$user->rights->stock->show_all_warehouses){
    $sql .= ' AND cf.fk_entrepot = '.$user->fk_warehouse;
}
...
if ($resql)
{
    ...
    if (!empty($sproduct)) $param .= '&sproduct='.urlencode($sproduct);
    ...
    print '<td class="liste_titre">'.
         ...
         '</td>'.
         ($sproduct ? '<td class="liste_titre"></td>' : '').
         '<td class="liste_titre">'.
    ...
    if ($sproduct)
        print_liste_field_titre('Quantity', $_SERVER['PHP_SELF'], '', '', $param, '', $sortfield, $sortorder, 'right ');
	...
	while ($i < min($num, $sproduct ? $num : $conf->liste_limit))
    {
        ...
        $prods = getProducts($obj->rowid, true);

        $showline = dolDispatchToDo($obj->rowid) && (!$sproduct || key_exists($sproduct, $prods));
        ...
        if ($showline)
        {
            ...
            // Qty
            if ($sproduct) {
                print '<td class="right">'.$prods[$sproduct].'</td>';
                if (in_array($obj->fk_statut, [1,2,3,4])) $totalqty += $prods[$sproduct];
            }
            ...
            $totalamount += $obj->total_ttc;
        }
        $i++;
    }

    if ($sproduct) {
        // Total qty for product
        print '<tr class="liste_total"><td class="right liste_total">'. $langs->trans("Total").':</td>';
        print '<td class="liste_total right" colspan="3">'.$form->textwithtooltip(price2num($totalqty, 'MS'), $langs->trans('TotalQtyOrdered')).'</td>';
        // Amount total of orders
        print '<td class="liste_total right">';
        print price(price2num($totalamount, 'MT'), 1);
        print '</td>';
        print '<td class="right" colspan="2">&nbsp;</td>';
        print "</tr>";
    }
