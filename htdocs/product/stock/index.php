...
llxHeader("", $langs->trans("Stocks"), $help_url,'',0,0,array('https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.js'),array('https://cdnjs.cloudflare.com/ajax/libs/jquery-modal/0.9.1/jquery.modal.min.css'));

echo '<style>.modal{max-width: 95% !important;
    width: 98%;
    max-height: 93%;
    overflow-y: auto;
...
print '<div class="fichecenter"><div class="fichethirdleft">'; // fichethirdleft
// Last movements
$max=10;
...
if ($result)
{
    ...
    print '<div class="div-table-responsive-no-min">';
    print '<table class="noborder centpercent">';
    print '<tr class="liste_titre"><th colspan="3">'.$langs->trans("Warehouses").'</th></tr>';
    ...
    if ($num)
    {
        ...
        while ($i < $num)
        {
            ...
            print '<td style="width:60px;" class="right">'.$entrepot->LibStatut($objp->statut, 5).'</td>';
            print "</tr>\n";
            $i++;
        }
        $db->free($result);
...
print '</div></div><!-- fichetwothirdright --></div>';
