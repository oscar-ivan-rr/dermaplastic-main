...
class Entrepot extends CommonObject
{
	...
	/**
	 * @var int	Percent of automated transfers stock
	 */
	public $tms_percent;
	...
	public function update($id, $user)
	{
		...
		$this->fk_rfc=($this->fk_rfc > 0 ? $this->fk_rfc : 0);
	...
	public function fetch($id, $ref = '')
	{
		...
		$sql  = "SELECT rowid, fk_parent, ref as label, description, statut, lieu, address, zip, town, fk_pays as country_id, tms_percent, fk_rfc";
		...
		if ($result)
		{
			if ($this->db->num_rows($result) > 0)
			{
				...
				$this->tms_percent    = $obj->tms_percent;
				$this->fk_rfc    = $obj->fk_rfc;
	...
	/**
	 * Update automated transfers percent.
	 * @param		int	$tms	Percent
	 * @return		int	1 OK | 0 KO
	 * @deprecated 	Global percentage per warehouse is not used now.
	 */
	public function updateTransferPercent($tms) {
		$sql = "UPDATE ".MAIN_DB_PREFIX."entrepot SET tms_percent = $tms WHERE rowid = $this->id";
