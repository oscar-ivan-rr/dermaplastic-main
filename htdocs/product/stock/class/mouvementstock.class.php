...
class MouvementStock extends CommonObject
{
	 ...
	 */
    public function _create($user, $fk_product, $entrepot_id, $qty, $type, $price = 0, $label = '', $inventorycode = '', $datem = '', $eatby = '', $sellby = '', $batch = '', $skip_batch = false, $id_product_batch = 0, $id_lote = 0)
	{
	    // phpcs:disable
	    global $conf, $langs;
		...
		$now=(! empty($datem) ? $datem : dol_now('tzuser'));
		...
		if($product->cost_price != NULL) $price = $product->cost_price;
		else $price = 0;
		$this->db->begin();
		...
		if ($product->hasbatch() && !$skip_batch) {
			if (empty($batch)) {
			...
			if(is_numeric($eatby)) $eatby = date("Y-m-d", $eatby);
			if(is_numeric($sellby)) $sellby = date("Y-m-d", $sellby);
			...
			if($eatby) $sql .= " AND pb.eatby = '" . $eatby . "'";
			if($sellby) $sql .= " AND pb.sellby = '" . $sellby . "'";
		...
		if ($movestock && $qty < 0 && empty($conf->global->STOCK_ALLOW_NEGATIVE_TRANSFER)) {
			if ($product->hasbatch() && !$skip_batch) {
				...
				foreach ($product->stock_warehouse[$entrepot_id]->detail_batch as $batchcursor => $prodbatch) {
					$auxarray[] =
						[
							...
							$prodbatch->batch,
							$eatby,
							dol_print_date($prodbatch->eatby, 'dayrfc'),
							$sellby,
							dol_print_date($prodbatch->sellby, 'dayrfc'),
							$prodbatch->qty,
							$qty
						];
					if ((trim($batch) != trim($prodbatch->batch))){
						if($label != 'Liberación de apartado' && (($eatby != dol_print_date($prodbatch->eatby, 'dayrfc'))|| ($sellby != dol_print_date($prodbatch->sellby, 'dayrfc')))){
							continue;
						}
					}
					$foundforbatch = 1;
					if ($prodbatch->qty < abs($qty)){
						...
						if(count($product->stock_warehouse[$entrepot_id]->detail_batch) == 1){
							break;
						}else{
							$qtyisnotenough = 0;
						}
				...
				if($label == 'Liberación de apartado') $qtyisnotenough = 0;
		...
		{
			$fk_project = 0;
			if (!empty($this->origin) && is_object($this->origin)) {			// This is set by caller for tracking reason
			...
			if(!empty($this->fk_origin) && !empty($this->origintype) && $this->origintype == 'commande_fournisseurdet'){
				$origintype = $this->origintype;
				$fk_origin = $this->fk_origin;
			}
			...
			if (! $error /* && ! empty($conf->productbatch->enabled) && $product->hasbatch() && ! $skip_batch */)
			{
				if ($id_product_batch > 0)
				{
					...
					if ($result == -2 && $fk_product_stock > 0) {	// The entry for this product batch does not exists anymore, bu we already have a llx_product_stock, so we recreate the batch entry in product_batch
						$param_batch = array('fk_product_stock' =>$fk_product_stock, 'batchnumber'=>$batch, 'eatby'=>$eatby, 'sellby'=>$sellby);
	 ...
	 *	@return		int						     <0 if KO, >0 if OK
	 */
	public function reception($user, $fk_product, $entrepot_id, $qty, $price = 0, $label = '', $eatby = '', $sellby = '', $batch = '', $datem = '', $id_product_batch = 0, $inventorycode='', $id_lote = 0)
	{
	    global $conf;

	    $skip_batch = empty($conf->productbatch->enabled);

	    return $this->_create($user, $fk_product, $entrepot_id, $qty, 3, $price, $label, $inventorycode, $datem, $eatby, $sellby, $batch, $skip_batch, $id_product_batch, $id_lote);
	}
	...
	private function createBatch($dluo, $qty)
	{
		...
		elseif (is_array($dluo))
		{
			if (isset($dluo['fk_product_stock']))
			{
				...
				$result = $pdluo->find($vfk_product_stock, $eatby, $sellby, $vbatchnumber);  // Search on batch number only (eatby and sellby are deprecated here)
			}
			else
			{
