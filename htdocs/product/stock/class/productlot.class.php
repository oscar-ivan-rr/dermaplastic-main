...
class Productlot extends CommonObject
{
	 ...
	 */
	public function fetch($id = 0, $product_id = 0, $batch = '', $eatby = '', $sellby = '')
	{
		global $conf;
		dol_syslog(__METHOD__, LOG_DEBUG);
		...
		if ($product_id > 0 && $batch != '' && $eatby != '' && $sellby != '') {
			$sql .= " WHERE t.batch = '".$this->db->escape($batch)."' AND t.fk_product = ".$product_id;
			$sql .= " AND t.eatby = '".$this->db->idate($eatby)."' AND t.sellby = '".$this->db->idate($sellby)."'";
		} elseif($id > 0){
			$sql .= ' WHERE t.rowid = '.$id;
		}else{
			$sql .= ' WHERE t.batch = "'.$batch.'"';
		}
		$resql = $this->db->query($sql);
		if ($resql) {
