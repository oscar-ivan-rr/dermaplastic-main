...
if (empty($reshook))
{
	...
	if (($action == 'update_price') && !$cancel && $object->getRights()->creer)
    {
		...
		$psq = GETPOST('psqflag');
        $verificar_min=GETPOST("verifica_preciomin");
        $permiso_cambiomin_free=false;
		...
		if($verificar_min){
		    if ($user->rights->produit->change_min){
		        $permiso_cambiomin_free=true;
            }else{
                $permiso_cambiomin_free=false;
                setEventMessages("No cuenta con el permiso para la entrada de precio minimo libre", null, 'warnings');
            }
        }
		...
		if (!$error) {
			...
			foreach ($pricestoupdate as $key => $val) {
				...
				if($verificar_min=='on' && $permiso_cambiomin_free){
                        $newprice_min;
                }else{
					// Precio mínimo de venta de acuerdo al descuento máximo del producto
					if($object -> desc_max > 0){
						$newprice_min= $newprice - ($newprice* ($object -> desc_max)/100);
					}else{
						$newprice_min= $newprice;
					}
                }
			    ...
			    while($products_ids= $db->fetch_object($max)){
			        if($products_ids->fk_product != $id){
						...
						$resql = $db->query($sql);
                      	...
                      	$object_aux->updatePrice($newprice, $val['price_base_type'], $user, $val['vat_tx'], $newprice_min, $key, $val['npr'], $psq, 0, $val['localtaxes_array'], $val['default_vat_code']);
                    }
			        unset($object_aux);
                }
            }
	...
	if (($action == 'update_price_discount') && !$cancel && ($user->rights->produit->creer || $user->rights->service->creer)) {
		$percent = GETPOST('percent', 'int', '2');
		if ($percent != '') {
			// Multiplicado de precio
			$decimal = (1 - $percent / 100);

			// Aplicar descuento
			$res = $object->setDiscount($decimal);

			// Respuestas
			if ($res == 0) setEventMessage($langs->trans("ProductWithoutPrice"), 'warnings');
			elseif ($res > 0) setEventMessage($langs->trans("DiscountApplied", $percent));
			$action = '';
		}
		// Valor vacío
		else {
			setEventMessage($langs->trans("EmptyPercent"), 'errors');
					...
					foreach ($object->prices_by_qty_list[$i] as $ii => $prices)
					{
						if ($action == 'edit_price_by_qty' && $rowid == $prices['rowid'] && ($user->rights->produit->creer || $user->rights->service->creer)) {
							...
							if (($user->rights->produit->creer || $user->rights->service->creer)) {
								...
								print img_edit().'</a>';
								if ($user->rights->produit->delete_price_sale)
								{
...
else
{
	...
	if ($object->price_ttc == 0 || $object->base_price_ttc == 0) $percent = 0;
	else $percent = 100 - ($object->price_ttc * 100 / $object->base_price_ttc);

	$helpondiff = '<strong>'.$langs->trans("PriceOriginal").'</strong><br>';
	$helpondiff .= $langs->trans("ProductPriceOriginalLabel", $percent);
	...
	print '</td><td>';
	print ($percent > 0) ? $form->textwithpicto(price($object->base_price_ttc).' '.$langs->trans($object->price_base_type), $helpondiff) : '&nbsp;';
	print '</td></tr>';
	$price_min_formula=price2num($object->base_price_ttc*.75);
	$price_min =price2num($object->cost_price*1.05);
	$contador=0;
	$text ="";
	if ($price_min_formula!=$object->price_min_ttc){
        $contador++;
    }
    if ($price_min!=$object->price_min_ttc){
        $contador++;
    }
    if ($contador==2){
        $text="Precio minimo modificado";
    }
	// Price minimum gum
	print '<tr><td>'.$langs->trans("MinPrice").'</td><td>';
	if ($object->price_base_type == 'TTC') {
		print price($object->price_min_ttc).' '.$langs->trans($object->price_base_type).' '.'<label style="color: #6C7C8B">'.$text.'</label>';
	} else {
		print price($object->price_min).' '.$langs->trans($object->price_base_type).' '.'<label style="color: #6C7C8B">'.$text.'</label>';
	}
	print '</td><td>';
	print ($percent > 0) ? $form->textwithpicto(price($object->base_price_min_ttc).' '.$langs->trans($object->price_base_type), $helpondiff) : '&nbsp;';
	print '</td></tr>';

    //Ganancia
    print '<tr><td>'.$langs->trans("Gain").'</td><td>';
    print $object->gain.'%';
    print '</td><td></td></tr>';
	...
	|| $action == 'activate_price_by_qty' || $action == 'disable_price_by_qty')
{
	...
	// Discount
	print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=apply_discount&amp;id='.$object->id.'">'.$langs->trans("ApplyDiscount").'</a></div>';
    ...
    if ($object->isVariant()) {
		...
		if (empty($conf->global->PRODUIT_MULTIPRICES) && empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES)) {
			if ($user->rights->produit->creer || $user->rights->service->creer) {
				print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=edit_price&amp;id='.$object->id.'">'.$langs->trans("Cambiar Precio").'</a></div>';
			}
		}
...
print '<style>
.botonclaro{
background-color: #777777;color: black;
...
.botonamarillo{
background-color: #EFB810;color: white;
transition: 0.5s ease;
}
.botonamarillo:hover{
background-color: white;color: #EFB810;
}
.botonobscuro{
background-color: #454545;color: white;transition: 0.5s ease;
}
.botonobscuro:hover{
...
print '<table style="background-color: black;color:white;width: 25%;height: 50%;padding: 3%;border-radius: 9%;">';
print '<tr><td colspan="4"><input type="text" id="total" class="total" maxlength="11"></td></tr>';
print '<tr>';
print '<td><span class="boton botonclaro" id="clear">AC</span></td><td><span id="cambiarsig" class="boton botonclaro">+/-</span></td><td><span class="boton botonclaro" id="porcentaje">%</span></td><td><span class="boton botonamarillo" id="dividir">/</span></td>';
print '</tr>';
print '<tr><td><span class="boton botonobscuro" id="siete">7</span></td><td><span class="boton botonobscuro" id="ocho">8</span></td><td><span class="boton botonobscuro" id="nueve">9</span></td><td><span class="boton botonamarillo" id="por">x</span></td></tr>';
print '<tr><td><span class="boton botonobscuro" id="cuatro">4</span></td><td><span class="boton botonobscuro" id="cinco">5</span></td><td><span class="boton botonobscuro" id="seis">6</span></td><td><span class="boton botonamarillo" id="menos">-</span></td></tr>';
print '<tr><td><span class="boton botonobscuro" id="uno">1</span></td><td><span class="boton botonobscuro" id="dos">2</span></td><td><span class="boton botonobscuro" id="tres">3</span></td><td><span class="boton botonamarillo" id="mas">+</span></td></tr>';
print '<tr><td colspan="2"><span class="botonlargo botonobscuro" id="cero">0</span></td><td><span class="boton botonobscuro" id="punto">.</span></td><td><span class="boton botonamarillo" id="igual">=</span></td></tr>';
print '</table>';
print '<script>';
...
function calcular(){
    var result = 0;
    switch(operacion){
    case "+" : result =primernumero + segundonumero;break;
    case "-" : result =primernumero - segundonumero;break;
    case "x" : result =primernumero * segundonumero;break;
    case "d" : result =primernumero / segundonumero;break;
    default : result = 0;break;
    }
    return result;
}
$("#cambiarsig").click(function(){
...
$("#mas").click(function(){
if((primernumero == undefined || primernumero == null) || bnd == 1){
    primernumero = parseFloat($("#total").val());
    $("#total").val("");
    bnd = 0;
    operacion="+";
}else{
    segundonumero = parseFloat($("#total").val());
    result = calcular();
    primernumero = result;
    $("#total").val(primernumero);
    segundonumero = null;
    bandera = 1;
    operacion="+";
}
...
$("#porcentaje").click(function(){
    var calculos = parseFloat($("#total").val())/100;
    $("#total").val(calculos);
});
$("#total").keydown(function(e){//console.log(e.which);
    if(((e.which >= 96 && e.which <= 105) || (e.which >= 48 && e.which <= 57) || (e.which == 107 || e.which == 187)
    || (e.which == 109 || e.which == 189) || (e.which == 106 || e.which == 186) || (e.which == 111 || e.which == 191)
    || (e.which == 110 || e.which == 190) || (e.which == 13) || (e.which == 37 || e.which == 39)  || (e.which == 8 || e.which == 46)) && $("#total").val().length < 10){
        
        if(bnd == 1){
        bnd=0;
        $(this).val("");
        }
        if(bandera == 1){$("#total").val("");bandera=0;}
        switch(e.which){
            case 107:case 187: e.preventDefault();$("#mas").click();break;
            case 109:case 189: e.preventDefault();$("#menos").click();break;
            case 106:case 186: e.preventDefault();$("#por").click();break;
            case 111:case 191: e.preventDefault();$("#dividir").click();break;
            case 13: e.preventDefault();$("#igual").click();break;
        }
    }else{
    e.preventDefault();
    }
...
if(bandera == 1){$("#total").val("0");bandera=0;}
}
});
$("#uno").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "1");
if(bandera == 1){$("#total").val("1");bandera=0;}
});
$("#dos").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "2");
if(bandera == 1){$("#total").val("2");bandera=0;}
});
$("#tres").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "3");
if(bandera == 1){$("#total").val("3");bandera=0;}
});
$("#cuatro").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "4");
if(bandera == 1){$("#total").val("4");bandera=0;}
});
$("#cinco").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "5");
if(bandera == 1){$("#total").val("5");bandera=0;}
});
$("#seis").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "6");
if(bandera == 1){$("#total").val("6");bandera=0;}
});
$("#siete").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "7");
if(bandera == 1){$("#total").val("7");bandera=0;}
});
$("#ocho").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "8");
if(bandera == 1){$("#total").val("8");bandera=0;}
});
$("#nueve").click(function(){
if($("#total").val().length < 10)
    $("#total").val($("#total").val() + "9");
if(bandera == 1){$("#total").val("9");bandera=0;}
});
$("#punto").click(function(){
if($("#total").val().length < 10){
    if($("#total").val().length==0 || bandera == 1){
        $("#total").val("0.");}
    else{
        $("#total").val($("#total").val() + ".");}
}
...
if ($action == 'edit_price' && $object->getRights()->creer)
{
	...
	if (empty($conf->global->PRODUIT_MULTIPRICES) && empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES))
	{
		...
		// VAT
		print '<tr><td class="titlefield fieldrequired">'.$langs->trans("DefaultTaxRate").'</td><td>';
		print $form->load_tva("tva_tx", $object->default_vat_code ? $object->tva_tx.' ('.$object->default_vat_code.')' : $object->tva_tx, $mysoc, '', $object->id, $object->tva_npr, $object->type, false, 1);
		print '</td></tr>';
		...
		if ($object->price_base_type == 'TTC') {
			...
			if ($user->rights->produit->change_min){
                ...
                print '<input type="checkbox" id="verifica_preciomin" name="verifica_preciomin"><label> (Omitir formula para precio minimo)</label>';
            }else{
                print '<input name="price_min" size="10" value="'.price($object->price_min_ttc).'" disabled> &nbsp;&nbsp;';
                print '<input type="checkbox" id="verifica_preciomin" name="verifica_preciomin" disabled > <label style="color: #5f5e5e"><del>(Omitir formula para precio minimo)</del></label>';
            }
		} else {
            ...
            if ($user->rights->produit->change_min){
                ...
                print '<input name="price_min" size="10" value="'.price($object->price_min).'" disabled> &nbsp;&nbsp;';
                print '<input type="checkbox" id="verifica_preciomin" name="verifica_preciomin" disabled > <label style="color: #5f5e5e"><del>(Omitir formula para precio minimo)</del></label>';
            }
		}
...
if ($action == 'apply_discount' && $object->getRights()->creer) {
	print load_fiche_titre($langs->trans("NewDiscount"), '');

	print '<!-- Apply price discount -->'."\n";
	print '<form action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="POST">';
	print '<input type="hidden" name="token" value="'.$_SESSION ['newtoken'].'">';
	print '<input type="hidden" name="action" value="update_price_discount">';
	print '<input type="hidden" name="id" value="'.$object->id.'">';
	...
	print '</td><td>';
	print '<input name="percent" size="10" value="">';
	print '</td></tr>';
...
if ((empty($conf->global->PRODUIT_CUSTOMER_PRICES) || $action == 'showlog_default_price') && !in_array($action, array('edit_price', 'edit_vat', 'apply_discount')))
{
    	...
    	if ($num > 0)
    	{
    		...
    		while ($i < $num)
    		{
    			...
    			if ($user->rights->produit->supprimer)
    			{
    				...
    				print '<td class="right">';
    				if ($candelete && $user->rights->produit->delete_price_sale)
    				{
