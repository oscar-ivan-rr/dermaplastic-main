...
$search_rfc = GETPOST('search_rfc', 'alpha');
...
//Variable checkbox para filtro de incidencias
$search_note_private = GETPOST('note_private');
...
$arrayfields = array(
	...
	's.nom'=>array('label'=>$langs->trans("ThirdPartyListSupplier"), 'checked'=>1),
	...
	'dynamount_payed'=>array('label'=>$langs->trans("PayedList"), 'checked'=>0),
	'rtp'=>array('label'=>$langs->trans("Saldo"), 'checked'=>0),
	'f.datec'=>array('label'=>$langs->trans("DateCreation"), 'checked'=>0, 'position'=>500),
	'f.tms'=>array('label'=>$langs->trans("DateModificationShort"), 'checked'=>0, 'position'=>500),
	'f.fk_statut'=>array('label'=>$langs->trans("Status"), 'checked'=>1, 'position'=>1000),
	'warehouse'=>array('label'=>$langs->trans("Warehouse"), 'checked'=>1, 'position'=>1000),
	'rfc'=>array('label'=>$langs->trans("RFC"), 'checked'=>1, 'position'=>1000),
);
// Extra fields
...
if ($massaction == 'clasify_paid') {
	$max = (empty($conf->global->MAIN_LIMIT_FOR_MASS_ACTIONS) ? 500 : $conf->global->MAIN_LIMIT_FOR_MASS_ACTIONS);
	if (count($toselect) <= $max) {
		foreach ($toselect as $item) {
			$temporal_invoice = new FactureFournisseur($db);
			$temporal_invoice->fetch($item);
			$temporal_invoice->set_paid($user);
		}
		header('Location: '.$_SERVER['PHP_SELF'].'?leftmenu=suppliers_bills_notpaid&search_status=1');
		setEventMessage('Facturas clasificadas como pagadas');
	} else {
		header('Location: '.$_SERVER['PHP_SELF'].'?leftmenu=suppliers_bills_notpaid&search_status=1');
		setEventMessage('No se pueden clasificar más de '.$max.' facturas a la vez', 'errors');
	}
}
...
$sql .= " s.rowid as socid, s.nom as name, s.email, s.town, s.zip, s.fk_pays, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta as code_compta_client, s.code_compta_fournisseur, s.earlypayment_discount, s.earlypayment_validity, ";
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."entrepot as le ON f.fk_warehouse = le.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."c_rfc as lcr ON lcr.rowid = le.fk_rfc";
...
if(!$user->rights->stock->show_all_warehouses) $sql.= " AND f.fk_warehouse = ".$user->fk_warehouse."";
$sql .= ' AND f.entity IN ('.getEntity('facture_fourn').')';
if ($search_note_private) $sql .= " AND f.note_private != ''";
...
if ($search_warehouse != '') $sql .= natural_search("f.fk_warehouse", $search_warehouse);
if ($search_rfc != '') $sql .= natural_search("lcr.rowid", $search_rfc);
...
if ($option == 'late_discount') $sql .= " AND '".$db->idate(dol_now())."' > ADDDATE(f.datef, INTERVAL s.earlypayment_validity  DAY) AND s.earlypayment_validity <> 0";
...
if ($resql)
{
    ...
    if ( $massaction == 'generate_payment' ) {
        // Edit for the discount applied for the provider in case is edited

        $temporal_discount = GETPOST('discount_applied');
        if($temporal_discount) {
            $temporal_supplier = new Societe($db);
            $temporal_invoice = new FactureFournisseur($db);
            $temporal_invoice->fetch($arrayofselected[0]);
            $temporal_supplier->fetch($temporal_invoice->socid);
            $temporal_supplier->earlypayment_discount = $temporal_discount;
            $temporal_supplier->update($temporal_supplier->id);
        }
        ...
        header('Location: '.DOL_MAIN_URL_ROOT.'/fourn/facture/paiement.php'.$strout);
    }
	...
	if ($search_note_private == 1) $param .= "&expiration_alert=".urlencode($search_note_private);
	...
	$arrayofmassactions = array(
		...
		'generate_payment'=>'Pagar Facturas',
		'clasify_paid'=>$langs->trans('ClassifyPaid')
	...
	print_barre_liste($langs->trans("BillsSuppliers").($socid ? ' '.$soc->name : ''), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'invoicing', 0, $newcardbutton, '', $limit);
	...
	$moreforfilter .= $langs->trans('Incidencias').'  <input type="checkbox" class="valignmiddle" name="note_private" value="1"'.($search_note_private== 1 ? ' checked="checked"' : '').'/> ';
	$moreforfilter .= '</div>';
	...
	*/
	...
	{
		print '<td class="liste_titre left" colspan="2">';
	...
	// Almacén
	if (!empty($arrayfields['warehouse']['checked']))
	{
		print '<td class="liste_titre">';
		if($user->rights->stock->show_all_warehouses){
			$query = "SELECT rowid, ref FROM ".MAIN_DB_PREFIX."entrepot";
			$requery = $db->query($query);
			print '<select class="flat" name="search_warehouse">'; 
			print '<option value=""></option>';
			print '<option value="0">SIN ALMACÉN</option>';
			while ($warehouse = $db->fetch_object($requery))
			{
				print '<option value="'.$warehouse->rowid.'">'.$warehouse->ref.'</option>';
				$i++;
			}
			print '</select>';
		}
	...
	// RFC
	if (!empty($arrayfields['rfc']['checked']))
	{
		print '<td class="liste_titre">';
		if($user->rights->stock->show_all_warehouses){
			$query2 = "SELECT rowid, code FROM ".MAIN_DB_PREFIX."c_rfc WHERE active = '1'";
			$requery2 = $db->query($query2);
			print '<select class="flat" name="search_rfc">'; 
			print '<option value=""></option>';
			while ($rfc = $db->fetch_object($requery2))
			{
				print '<option value="'.$rfc->rowid.'">'.$rfc->code.'</option>';
				$i++;
			}
			print '</select>';
		}	
    ...
    if (!empty($arrayfields['s.earlypayment_validity']['checked']))
    {
		...
		print '<input class="flat width25 valignmiddle" type="text" maxlength="2" name="day_lim" value="'.dol_escape_htmltag($date_start_limitday).'">';
        print '<br><input type="checkbox" name="option" value="late_discount"'.($option == 'late_discount' ? ' checked' : '').'> '.$langs->trans("LateList");
        print '</td>';
    }
	// Project
	...
	if (!empty($arrayfields['f.fk_statut']['checked']))
	{
		...
		$liststatus = array('0'=>$langs->trans("Draft"), '1'=>$langs->trans("Unpaid"), '2'=>$langs->trans("Paid"), '3'=>$langs->trans("Canceled"));
	...
	if (!empty($arrayfields['f.ref']['checked']))                print_liste_field_titre($arrayfields['f.ref']['label'], $_SERVER['PHP_SELF'], 'f.ref,f.rowid', '', $param, 'colspan="2"', $sortfield, $sortorder);
    ...
    if (!empty($arrayfields['warehouse']['checked']))         print_liste_field_titre($arrayfields['warehouse']['label'], $_SERVER["PHP_SELF"], "warehouse", "", $param, '', $sortfield, $sortorder);
	if (!empty($arrayfields['rfc']['checked']))         print_liste_field_titre($arrayfields['rfc']['label'], $_SERVER["PHP_SELF"], "rfc", "", $param, '', $sortfield, $sortorder);
    ...
    if (!empty($arrayfields['s.earlypayment_validity']['checked'])) print_liste_field_titre($arrayfields['s.earlypayment_validity']['label'], $_SERVER['PHP_SELF'], "s.earlypayment_validity", '', $param, '', $sortfield, $sortorder, 'center ');
		...
		while ($i < min($num, $limit))
		{
            ...
            if (!empty($arrayfields['s.earlypayment_validity']['checked']))
            {
                ...
                if($thirdparty->earlypayment_discount > 0){
					...
					$aux = date("Y-m-d",strtotime($date_invoiced.$agregated_days));

                    print '<td class="center nowrap">'.$date_valid;
//                    print img_info($thirdparty->earlypayment_discount.'%');
                    print '<span style="font-size: 12px;"> '.$thirdparty->earlypayment_discount.'%</span>';
                    if (date(strtotime($aux)) < date("Y-m-d"))
                    {
			...
			//Obtener total para calculadora
			if(!empty($remaintopay)){
				$total_remaintopay += $obj->total_ttc;
			}
			...
			if (!empty($arrayfields['f.fk_statut']['checked']))
			{
				...
				print $facturestatic->LibStatut($obj->paye, $obj->fk_statut, 5, $paiement, $obj->type,1);
				print "</td>";
				if (!$i) $totalarray['nbfield']++;
			}

			// Action column
            //Action col to check for
	...
	print '
		...
		let facturasnormales = 0;
	        ...
	        $(\'input[type="checkbox"]\').click(function(){
                ...
                let url = "'.DOL_MAIN_URL_ROOT.'/fourn/facture/amount_calculator.php";
	...
	//                            	console.log(total_amount+" - "+amount+" = "+total);
								...
								total = total.toFixed(2);console.log(total_amount+" + "+amount+" = "+total);
	        ...
	        function applyDiscount(){
				...
				let amountDiscounted = 0.00;
				if(total_amount > 0.00){
	            	amountDiscounted = total_amount * discountApplied / 100;
	            	total_discount = total_amount - amountDiscounted;
				}
	...
	print '<div style="font-size:16px;">
            ...
            <strong style="">Descuento: % <input type="number" value="0" id="discount_input" name="discount_input" step="0.01"><button onclick="applyDiscount()">Aplicar</button></strong><br>
	...
	<script>
		$(document).ready(function(){
			$("input[name=\'confirmmassaction\']").click(function(e) {
				console.log($("select[name=\'massaction\']").val() == "generate_payment");
