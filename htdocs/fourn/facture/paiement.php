...
if (empty($reshook))
{
	...
	if ($action == 'confirm_paiement' && $confirm == 'yes')
	{
        ...
        foreach ($amounts as $key => $value)	// How payment is dispatched
        {
            ...
            if ($tmpsociete->earlypayment_discount) {
    ...
    if ($resql)
    {
        ...
        if ($num)
        {
            ...
            if (!empty($conf->banque->enabled))
            {
                ...
                $form->select_comptes($conf->global->FACTURE_BANK_CXP, 'accountid', 0, ($conf->global->FACTURE_BANK_CXP ? 'rowid = '.$conf->global->FACTURE_BANK_CXP : ''), 2);
                print '</td></tr>';
            }
            else
            {
        ...
        print '<script>
                $(document).ready(function () {
                    if($("#selectpaiementid").val() == 4 && '.$temp.' != "0"){
                    ...
                    $("#selectpaiementid").on("change",function(){
                        if($("#selectpaiementid").val() == 4 && '.$temp.' != "0"){
                            $("#selectaccountid option[value='.$temp.']").attr("selected", true);
                            $("#selectaccountid > option").each(function() {
                                if( this.value != '.$temp.' ) {
			...
			if (empty($reshook))
			{
	            ...
	            {
	                $sql .= ' AND f.type IN (0,1,2,3,5)'; // Standard invoice, replacement, deposit, situation
	            }
	            else
	            {
	                $sql .= ' AND f.type IN (0,1,2,3,5)'; // If paying back a credit note, we show all credit notes
	            }
	            ...
	            if ($resql)
	            {
	                ...
	                {
	                    $i = 0;
                        $supplierstatic->fetch($supplierstatic->id);
	                    print '<br>';
                        ...
                        if($massive_payment){//
                            ...
                            print '<input type="text" value="'.$supplierstatic->earlypayment_discount.'%" style="width: 50px;" disabled>';
                            print'</td>';
                        }
	                    ...
	                    while ($i < $num)
	                    {
	                        ...
	                        if(!$invoice->discount_applied && $massive_payment && $supplierstatic->earlypayment_discount && $invoice->type != 2) {
	                            $ep_discount = $objp->total_ttc * $supplierstatic->earlypayment_discount / 100;
	                            $after_discount = $objp->total_ttc - $ep_discount;
                            }
	                        else {
                                $ep_discount = 0;
                                $after_discount = $invoice->total_ttc;
	                            //$after_discount = $objp->total_ttc;
                            }
	                        	...
	                        	if (!empty($conf->use_javascript_ajax))
	                        		...
	                        		if($massive_payment){
	                        		    if(in_array($objp->facid, $massive_ids)){
                                            ...
                                            print '<input type="text" size="8" class="amount" name="'.$namef.'" value="'.$remaintopay.'">';
                                        }
                                        else{
	                    ...
	                    if ($i > 1)
	                    {
							...
							if (!empty($conf->multicurrency->enabled)) print '<td>&nbsp;</td>';
							if (!empty($conf->multicurrency->enabled)) print '<td>&nbsp;</td>';
							print '<td class="right"><b>'.price($sign * $total).'</b></td>';
...
if (empty($action) || $action == 'list')
{
    ...
    if(GETPOST('yearyear')){
        $sql.= ' AND DATE(p.datep) = "'.GETPOST('yearyear').'-'.GETPOST('yearmonth').'-'.GETPOST('yearday').'"';
    }
    ...
    if ($resql)
    {
        ...
        if(GETPOST('yearyear'))    $param.= "&yearyear=".urlencode(GETPOST('yearyear'));
        if(GETPOST('yearmonth'))    $param.= "&yearmonth=".urlencode(GETPOST('yearmonth'));
        if(GETPOST('yearday'))    $param.= "&yearday=".urlencode(GETPOST('yearday'));
        ...
        if(GETPOST('yearyear'))$date_search=dol_mktime(0,0,0,GETPOST('yearmonth'),GETPOST('yearday'),GETPOST('yearyear'));
        else $date_search=null;
        print $form->select_date($date_search,'year',0,0,0,'',1,0,1);
        print '</td>';
        ...
        print_liste_field_titre("Proveedor", $_SERVER["PHP_SELF"], 's.nom', '', $param, '', $sortfield, $sortorder);
