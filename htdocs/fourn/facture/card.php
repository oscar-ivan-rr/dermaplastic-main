...
if (empty($reshook))
{
    ...
    else if($action =='apply_line_comptadiscount' && $object->statut==FactureFournisseur::STATUS_DRAFT && $usercancreate ){
        //Revisamos si se mando algo descuento financiero
        if(GETPOST('comptadiscount_percent') != null){
            //Sacamos un array de los productos seleccionados
            $productos = explode(',',GETPOST('products_tocomptadiscount'));
            //Recuperamos el descuento financiero y lo calculamos como decimal
            $compta_discount = 1-(GETPOST('comptadiscount_percent')/100);
            //Guardamos el entero del descuento
            $compta_discount_n = GETPOST('comptadiscount_percent');
            foreach($object->lines as $line){
                //Revisamos si es un producto seleccionado
                if(in_array($line->id,$productos)) {
                    //Calculamos el costo real
                    $desc = 1-($line->remise_percent/100);
                    $pu= $line->subprice;
					if($compta_discount_n == 0){
						$real_cost = 0;
					}else{
						$real_cost = ($pu*$desc)*$compta_discount;
					}
                    $object->updateline($line->id, $line->description, $line->subprice, $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, $line->qty, $line->fk_product, 'HT', $line->info_bits, $line->product_type, $line->remise_percent, 0, $line->date_start, $line->date_end, $line->array_options, $line->fk_unit, $line->pu_ht, $line->ref_supplier, $line->notfound,$real_cost,$compta_discount_n);
                }
            }
        }
        //Si no se mando uno mandamos Error
        else{
            setEventMessage(("No se aceptan valores vacios para el descuento financiero"),'errors');
        }
    }
    else if($action =='apply_line_discount' && $object->statut==FactureFournisseur::STATUS_DRAFT && $usercancreate ){
        if(GETPOST('discount_percent')!=null){
            foreach($object->lines as $line){
                $object->updateline($line->id, $line->description, $line->subprice,$line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, $line->qty,$line->fk_product,'HT',$line->info_bits,$line->product_type, GETPOST('discount_percent'),0,$line->date_start, $line->date_end, $line->array_options,$line->fk_unit,$line->pu_ht,$line->ref_supplier,$line->notfound);
            }
        }else{
            setEventMessage(("No se aceptan valores vacios"),'errors');
        }
    }
	...
	{
		$idwarehouse = GETPOST('idwarehouse') ? GETPOST('idwarehouse') : $object->fk_warehouse;
		...
		if (!$error)
		{
			...
			if ($result < 0)
			{
				...
				if($object->type != FactureFournisseur::TYPE_CREDIT_NOTE){
					//Precio de compra en productos
					foreach ($object->lines as $line)
					{
						$fournProduct = new ProductFournisseur($db);
						$fournProduct->fetch($line->id);
						$fournProduct->cost_price = price2num($line->pu_ttc);

						//EDITS
						$db->query($sql6);
						$result = $fournProduct->update($fournProduct->id, $user);
						$product =new Product($db);
						$product->fetch($line->fk_product);
						if($line->fk_product > 0) {
							$c = new Categorie($db);
							$categorias=$c->getListForItem($line->fk_product,'product');
	...
	elseif ($action == 'confirm_cancel' && $confirm == 'yes'){
		...
		if ($result > 0){
			...
			setEventMessages(($object->type == 0 ? $langs->trans("CancelErrorFacture") : $langs->trans("CancelErrorNote")), null, 'errors');
		}
	}
	...
	// Almacén
	if ($action == 'set_entrepot' && $user->rights->fournisseur->commande->creer)
	{
		$result = $object->set_id_warehouse($user, GETPOST('fk_entrepot', 'int'));
		if ($result < 0) setEventMessages($object->error, $object->errors, 'errors');
	}

	// Delete Massive Products
	elseif ($action == 'confirmDelete'){
		$linesURL = GETPOST('lines');
		if(!empty($linesURL)){
			$lines = explode(",", $linesURL);
		
			//Formato al Array de Lines (Se quedan solo los IDs)
			foreach($lines as $k => $line){
				$lines[$k] = explode("_", $line)[1];
			}
	
			//Se elimina el último valor que siempre es null
			unset($lines[count($lines)-1]);
	
			foreach($lines as $line){
				$res = $object->deleteline($line);
			}
	
			if($res < 0){
				setEventMessages('Error al Eliminar Productos', '', 'errors');
				$error++;
			}else{
				// reorder lines
			...
			setEventMessages('Seleccione al menos un Producto para Eliminar', '', 'errors');
			$error++;
		}
	}
	...
	elseif ($action == "setabsolutediscount" && $usercancreate)
	{
		...
		if (!empty($_POST["remise_id_for_payment"]))
		{
			...
			if (price2num($discount->amount_ttc) > price2num($object->getRemainToPay(0)))
			{
				...
				$aux = price2num($discount->amount_ttc - $object->total_ttc);
				...
				{
					$db->commit();
					$result = $newdiscount1->link_to_invoice(0, $id);
					$result = $object->set_paid($user);
					if ($result < 0) setEventMessages($object->error, $object->errors, 'errors');
				}
				else
	...
	elseif ($action == 'confirm_delete_paiement' && $confirm == 'yes' && $user->rights->fournisseur->facture->creer)
	{
		...
		if ($object->statut == FactureFournisseur::STATUS_VALIDATED && $object->paye == 0)
		{
			...
			if ($result > 0) {
                ...
                //Quitar el descuento de pronto pago
                //Solo si se elimina y no hay pagos
                $sqlS = "SELECT count(fk_paiementfourn) as payments from ".MAIN_DB_PREFIX."paiementfourn_facturefourn where fk_facturefourn=".$id;
                $resqlS = $db->query($sqlS);
                if($resqlS){
                    $countp = $db->fetch_object($resqlS);
                    if($countp->payments == 0){
                        $sqlU = "UPDATE ".MAIN_DB_PREFIX."facture_fourn SET discount_applied = null, total_ttc=total_ttc_before_discount where rowid =".$id;
                        $resU = $db->query($sqlU);
                    }
                }
	...
	elseif ($action == 'add' && $usercancreate)
	{
		...
		if ($_POST['type'] == FactureFournisseur::TYPE_STANDARD || $_POST['type'] == FactureFournisseur::TYPE_DEPOSIT)
		{
			...
			if (!$error)
			{
				...
				$object->multicurrency_code = GETPOST('multicurrency_code', 'alpha');
				$object->multicurrency_tx = GETPOST('originmulticurrency_tx', 'int');
				if(empty(GETPOST('originid', 'int'))){
					$object->fk_facture_source = 'NULL';
					if (!GETPOST('fk_entrepot'))
					{
						setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentities('Almacén')), null, 'errors');
						$action = 'create';
						$_GET['socid'] = $_POST['socid'];
						$error++;
					}
				} else{
					$object->fk_facture_source = GETPOST('originid', 'int');
				}
				$object->fk_warehouse = GETPOST('fk_entrepot', 'int');
				...
				if (!$error && $_POST['origin'] && $_POST['originid'])
				{
					...
					if ($id > 0)
					{
						...
						if ($result > 0)
						{
							...
							// Create invoice for warehouse if the supplier is CEDIS
							//! TODO: Commented for now because it's not necessary to create an invoice for CEDIS
							// if($object->origin == 'order_supplier' && $srcobject->socid == $conf->global->{'CEDIS_SUPPLIER'}){
							// 	// Select warehouse customer
							// 	$sql = "SELECT ls.rowid, ls.nom FROM llx_societe ls JOIN llx_entrepot le ON ls.nom = le.`ref` WHERE le.rowid =".$object->fk_warehouse;
							// 	$resql=$db->query($sql);
							// 	if($db->num_rows($resql) > 0){
							// 		$customer = $db->fetch_object($resql);
							// 		$invoice = new Facture($db);

							// 		$invoice->socid = $customer->rowid;
							// 		$invoice->type               = Facture::TYPE_STANDARD;
							// 		$invoice->date               = $object->date;
							// 		$invoice->note_public 	     = $object->note_public ;
							// 		$invoice->note_private       = $object->note_private;
							// 		$invoice->ref_client         = $customer->nom;
							// 		$invoice->fk_project 	     = $object->fk_project;
							// 		$invoice->cond_reglement_id	 = $object->cond_reglement_id;
							// 		$invoice->mode_reglement_id	 = $object->mode_reglement_id;
							// 		$invoice->fk_incoterms 		 = $object->fk_incoterms;
							// 		$invoice->location_incoterms = $object->location_incoterms;
							// 		$invoice->multicurrency_code = $object->multicurrency_code;
							// 		$invoice->multicurrency_tx   = $object->multicurrency_tx;
							// 		$invoice->fk_facture_fourn   = $object->id;
							...
							// 				$label = (!empty($lines[$i]->product_label) ? $lines[$i]->product_label : '');
							// 				$desc = (!empty($lines[$i]->product_desc) ? $lines[$i]->product_desc : '');
							...
							// 					$lines[$i]->remise_percent, $date_start, $date_end, 0, $lines[$i]->info_bits, 0,
							// 					'HT', 0, $product_type, $lines[$i]->rang, $lines[$i]->special_code, $object->origin, $lines[$i]->id,
							// 					$fk_parent_line, null, $lines[$i]->subprice, $label, $array_options,
							...
							// 			if(!$error){
							// 				$invoice->validate($user, '', $conf->global->{'CEDIS_WAREHOUSE'});
							// 			}

							// 		} else {
							// 			$error++;
							// 			setEventMessages('Error al crear factura de CEDIS', null, 'errors');
							// 		}
							// 	} else {
							// 		$error++;
							// 		setEventMessages('Error al encontrar cliente de sucursal', null, 'errors');
							// 	}
							// }
						}
						else
	...
	elseif ($action == 'updateline' && $user->rights->fournisseur->facture->creer)
	{
		...
		if (GETPOST('productid') > 0)
		{
			...
			// Ignorar IVA si el producto es exento de IVA
			$tva_tx = $prod->exentoiva ? 0 : $tva_tx;
		...
		//Costo real
		//Recuperamos el descuento financiero y lo calculamos como decimal
		$compta_discount = 1-(GETPOST('compta_discount')/100);
		//Guardamos el entero del descuento
		$compta_discount_n = GETPOST('compta_discount');
		$desc = 1-($remise_percent/100);
		$pu= $up;
		if($compta_discount_n == 0){
			$real_cost = 0;
		}else{
        ...
        $result = $object->updateline(GETPOST('lineid'), $label, $up, $tva_tx, $localtax1_tx, $localtax2_tx, GETPOST('qty'), GETPOST('productid'), $price_base_type, $info_bits, $type, $remise_percent, 0, $date_start, $date_end, $array_options, $_POST['units'], $pu_ht_devise, null, GETPOSTISSET('notfound') ? GETPOST('notfound', 'int', 2) : 0, $real_cost,$compta_discount_n);
        if ($result >= 0)
        {
            unset($_POST['label']);
	...
	// Add last product created to facture line
	elseif ($action == 'addprod_toline') {
		$lineid = GETPOST('lineid', 'int');
		$prodid = GETPOST('prodid', 'int');

		// Check parameters
		if (!empty($lineid) && !empty($prodid)) {
			// Join product id with facture line.
			$sql = "UPDATE ".MAIN_DB_PREFIX."facture_fourn_det SET fk_product = $prodid, label = null, notfound = 0 WHERE rowid = $lineid";
			$res = $db->query($sql);
	...
	elseif ($action == 'addline' && $user->rights->fournisseur->facture->creer)
	{
		...
		if ($prod_entry_mode != 'free' && empty($error))	// With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
		{
			...
			if ($idprod > 0)
			{
				...
				// Ignorar IVA de proveedor si el producto es Exento de IVA
				$product = new Product($db);
				$product->fetch($idprod);
				if($product->exentoiva == 1){
					$tva_tx = 0;
				}
	...
	elseif ($action == 'confirm_edit' && $confirm == 'yes' && $user->rights->fournisseur->facture->creer)
	{
		...
		if (!$ventilExportCompta)
		{
			...
			if ($resteapayer == price2num($object->total_ttc, 'MT', 1) && ($object->statut == FactureFournisseur::STATUS_VALIDATED || $object->statut == FactureFournisseur::STATUS_ABANDONED))
		    {
	            $idwarehouse = (is_null($object->fk_warehouse) ? GETPOST('idwarehouse') : $object->fk_warehouse);
	            ...
	            {
	                $langs->load("stocks");
	                if ((!$idwarehouse || $idwarehouse == -1) && $object->statut != FactureFournisseur::STATUS_ABANDONED)
	                {
	                    $error++;
	                    setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentitiesnoconv("Warehouse")), null, 'errors');
	                    $action = '';
...
if ($action == 'create')
{
	...
	*/
	...
	if($origin=='order_supplier'){
		print '<tr><td class="fieldrequired">Fecha de recepción</td><td>';
	}else{
		print '<tr><td class="fieldrequired">'.$langs->trans('DateInvoice').'</td><td>';
	}
	print $form->selectDate(($dateinvoice?$db->idate(dol_now()):$dateinvoice), '', '', '', '', "add", 1, 1);
	print '</td></tr>';
	...
	print '<script>
	$(document).ready(function() {
		// Obtener la fecha en el formato que utiliza la función Date de JS (mm/dd/yyyy)
		...
		// Agregar un "0" a la izquierda si el día o el mes solo tienen un dígito
		...
		// Asignar el valor de la fecha límite de pago al refrescar la página
		var date_re = getDateTxt($("#re").val());
		var date_ech = new Date(date_re);
		var value = $("#cond_reglement_id").val();
		// Ajax que devuelve el valor de días según el id de la opción seleccionada
		$.ajax("'.DOL_URL_ROOT.'/fourn/ajax/ajaxcond_reglement.php", {
			type: "post",   // Usually post or get
			async: true,
			data: {value: value},
			success: function (result) {
				result = JSON.parse(result);
				var dias = parseInt(result.days); // Número de días a agregar 
				date_ech.setDate(date_ech.getDate() + dias); 
				var d = format(date_ech.getDate());
				var m = format((date_ech.getMonth()+1));             
				$("#ech").val(d + "/" + m + "/" + date_ech.getFullYear())
			}
		});

		// Cambiar fecha límite de pago si cambia la fecha de facturación
		var $select = $("#re").change(function() {
			var date = getDateTxt($(this).val());
			...
			$.ajax("'.DOL_URL_ROOT.'/fourn/ajax/ajaxcond_reglement.php", {
				...
				success: function (result) {
					result = JSON.parse(result);
					var dias = parseInt(result.days); // Número de días a agregar 
					datenew.setDate(datenew.getDate() + dias); 
					var d = format(datenew.getDate());
					var m = format((datenew.getMonth()+1));                
					$("#ech").val(d + "/" + m + "/" + datenew.getFullYear());
				}
			});
		});

		// Cambiar fecha límite de pago si se presiona el botón de "Ahora" en fecha de facturación
		var $select = $("#reButtonNow").click(function() {
			var date = getDateTxt($("#re").val());
			var datenew = new Date(date);
			var value = $("#cond_reglement_id").val();
			$.ajax("'.DOL_URL_ROOT.'/fourn/ajax/ajaxcond_reglement.php", {
				type: "post",   // Usually post or get
		...
		// Cambiar fecha límite de pago si cambian los días de crédito
		var $select = $("#cond_reglement_id").change(function() {
			var value = $(this).val();
			var date = getDateTxt($("#re").val());
			var datenew = new Date(date);
			$.ajax("'.DOL_URL_ROOT.'/fourn/ajax/ajaxcond_reglement.php", {
	...
	$form->select_types_paiements(GETPOSTISSET('mode_reglement_id') ?GETPOST('mode_reglement_id', 'int') : 2, 'mode_reglement_id', 'DBIT');
	print '</td></tr>';

	// Bank Account
	print '<tr><td>'.$langs->trans('BankAccount').'</td><td>';
	$form->select_comptes((GETPOSTISSET('fk_account') ?GETPOST('fk_account', 'alpha') : 1), 'fk_account', 0, '', 1);
	print '</td></tr>';
	...
	if (!is_object($objectsrc))
	{
		...
		if($user->rights->stock->show_all_warehouses){
			print '<tr><td class="fieldrequired">' . $langs->trans("Almacén") . '</td><td colspan="3">';
			print $formproduct->selectWarehouses($user->fk_warehouse, 'fk_entrepot', 'warehouseopen', 0);
			print '</td></tr>';
	
			// Script para obtener el RFC asociado al almacén seleccionado (en evento change)
			print '<script type="text/javascript">
				$(document).ready(function() {
					$("#fk_entrepot").change(function() {
						var fk_entrepot = $(this).val();
						$.ajax("' . DOL_URL_ROOT . '/fourn/ajax/ajaxentrepotrfc.php", {
							type: "post",   // Usually post or get
							async: true,
							data: {fk_entrepot: fk_entrepot},
							success: function (result) {
								result = JSON.parse(result);                   
								$("#lbl").val(result.rfc);
							}
		...
		print '<input readOnly= "true" name="lbl" id="lbl"></input>';
		print '</td></tr>';

		// Script para obtener el RFC asociado al almacén seleccionado
		print '<script type="text/javascript">
		$(document).ready(function() {
			var fk_entrepot = $("#fk_entrepot").val();
				$.ajax("' . DOL_URL_ROOT . '/fourn/ajax/ajaxentrepotrfc.php", {
					type: "post",   // Usually post or get
					async: true,
					data: {fk_entrepot: fk_entrepot},
	...
	if (is_object($objectsrc))
	{
		...
		// Obtener el id del almacen que tiene asociado la orden de origen
		$queryw = "SELECT lcf.fk_entrepot as id FROM llx_commande_fournisseur lcf WHERE lcf.rowid = ".$originid."";
		$requeryw = $db->query($queryw);
		$objw = $db->fetch_object($requeryw);
		print '<input type="hidden" name="fk_entrepot" value="'.$objw->id.'">';

		// Obtener el nombre y RFC del almacen
		$query2 = "SELECT le.`ref` as entrepot, lcr.code as rfc
		FROM llx_commande_fournisseur lcf JOIN llx_entrepot le ON lcf.fk_entrepot = le.rowid 
		JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE lcf.rowid = ".$originid."";
		$requery2 = $db->query($query2);
		$obj2 = $db->fetch_object($requery2);
		...
		print '<td>'.$obj2->entrepot.'</td></tr>';
		print '<br>';
		print '<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$langs->trans('RFC').'</td>';
		print '<td>'.$obj2->rfc.'</td></tr>';
		echo '</td></tr>';
...
else
{
	if ($id > 0 || !empty($ref))
	{
        ...
        if ($action == 'edit')
        {
			...
			if(is_null($object->fk_warehouse)){
				if (!empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_SUPPLIER_BILL) && $qualified_for_stock_change)
				{
					$langs->load("stocks");
					require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
		...
		if ($action == 'cancel')
		{
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, ($object->type == 0 ? $langs->trans('CancelBillCXP') : $langs->trans('CancelNcCXP')), ($object->type == 0 ? $langs->trans('ConfirmCancelBillCXP') : $langs->trans('ConfirmCancelNcCXP')), 'confirm_cancel', '', 0, 1);
		}
		
		if($action == 'reopenCanceled'){
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ReOpen'), ($object->type == 0 ? $langs->trans('ConfirmReOpenCanceled') : $langs->trans('ConfirmReOpenCanceled')), 'confirm_edit', '', 0, 1);
		}
    	...
    	// Thirdparty
    	$morehtmlref .= '<br>'.$langs->trans('Supplier').' : '.$object->thirdparty->getNomUrl(1);
        ...
        print '<tr><td class="titlefield">'.$langs->trans('Type');
        ?>
		<div style="float:right;margin-right: 15px;">
			<i id="rc_expand" class="fa fa-plus-circle" style="color: #009;" onclick="rc_expand();"></i>
			<i id="rc_colapse" class="fa fa-minus-circle" style="color: #900;display: none;" onclick="rc_colapse();"></i>
		</div>
		<script>
			$(document).ready(function(){
				$('#rc_colapse').hide();
        ...
        if ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE || $object->type == FactureFournisseur::TYPE_DEPOSIT) {
            ...
            if ($result > 0) {
                print '. '.html_entity_decode($langs->trans("CreditNoteConvertedIntoDiscount", $object->getLibType(1), $discount->getNomUrl(1, 'discount'))).'<br>';
            }
        }
        print '</td></tr>';
		...
		$langs->load('bills');
		print '<tr class="rc_colapsable"><td class="nowrap">';
	...
	print $langs->trans('Almacén');
	print '<br>';
	print $langs->trans('RFC');
	print '</td>';
	if ($action != 'edit_entrepot') print '<td class="right"><a class="editfielda" href="'.DOL_URL_ROOT.'/fourn/facture/card.php?facid='.$object->id.'&action=edit_entrepot">'.img_edit($langs->trans('Almacén'), 1).'</a></td>';
	print '</tr></table>';
	print '</td><td>';
	if ($action == 'edit_entrepot')
	{
		print '<form name="set_entrepot" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
		print '<input type="hidden" name="token" value="'.newToken().'">';
		print '<input type="hidden" name="action" value="set_entrepot">';
		require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
		$formproduct = new FormProduct($db);
		if($user->rights->stock->show_all_warehouses){
			print $formproduct->selectWarehouses($object->fk_warehouse, 'fk_entrepot', 'warehouseopen', 0);
		} else {
			print $formproduct->selectWarehouses($user->fk_warehouse, 'fk_entrepot', 'warehouseopen', 0, 1);
		}
		print '<input type="submit" class="button" value="'.$langs->trans('Modify').'">';
		print '</form>';
		print '<input value="" readOnly= "true" name="lbl" id="lbl"></input>';
		// Script para obtener el RFC asociado al almacén seleccionado
		print '<script type="text/javascript">
			$(document).ready(function() {
				var fk_entrepot = $("#fk_entrepot").val();
	...
	{
		$query = "SELECT le.`ref` as entrepot, lcr.code as rfc
		FROM llx_facture_fourn ff JOIN llx_entrepot le ON ff.fk_warehouse = le.rowid 
		JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE ff.rowid = ".$object->id."";
	...
	print '</td></tr>';
		...
		if ($result)
		{
			...
			if ($num > 0)
			{
				while ($i < $num)
				{
					...
					{
						$bankaccountstatic = new Account($db);
						$bankaccountstatic->fetch($objp->fk_bank);
		...
		if ($object->type != FactureFournisseur::TYPE_CREDIT_NOTE)
		{
			...
			if ($resteapayeraffiche >= 0)
				...
				print "<span class='amountremaintopay'>". $langs->trans('ExcessPaid')."</span>";
			print ' :</td>';
		...
		if (!empty($object->lines) && ($object->statut == FactureFournisseur::STATUS_DRAFT)) {
		//Eliminar Productos
		print '<div class="tabsAction">';
		print '<a id="deleteLinesButton" class="butAction" onclick="return addLinesURL();" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;">'.$langs->trans('DeleteProducts').'</a>';
		print '</div>';

		print '<div hidden id="delete_line_dialog" name="delete_line_dialog" title="Eliminar Productos">
		...
		print '<script>';
		print "
			function addLinesURL(){
				$( \"#delete_line_dialog\" ).dialog({
					autoOpen:false,
					width:400,
					height:200,
					buttons: [
						{
							text: \"Si\",
							click: function() {
									var l = '".DOL_URL_ROOT."/fourn/facture/card.php?id=".$id."&action=confirmDelete&lines=';
									var q = [];
									$('.product_discount').each(function(i,e){
										if($(e).is(':checked'))
										{
											l += $(e).attr('name')+',';
										}
				...
				$( \"#delete_line_dialog\" ).dialog(\"open\");
				return false;
			}
			";
		print '</script>';
		...
		if (!empty($object->lines)) {
            ...
            echo '</table><table class="noborder noshadow" width="100%">';
            ?>
            <script>
                $(document).ready(function() {
                    $('#all_lines').click(function(e){
                        switch_all_checks();
                    });
                    $('.ptp_checkbox').click(function(e){
                        switch_checks();
                    });
                    function switch_all_checks()
                    {
                        var prod = '';
                        var all = $('#all_lines');
                        if (all.is(':checked'))
                        {
                            $('.ptp_checkbox').each(function(i,e){
                                prod += $(e).val() + ',';
                                $(e).prop( "checked", true );
                            });
                        ...
                        $("#products_tocomptadiscount").val(prod);
                    }
                    function switch_checks()
                    {
                        var prod = '';
                        var all = $('#all_lines');
                        var ct = 0;
                        ...
                        if (sm == ct)
                        {
                            $('#all_lines').prop( "checked", true );
                        }
                        else
                        {
                            $('#all_lines').prop( "checked", false );
                        }
                    }
                });
            </script>
        ...
        print '<table class="border total centpercent">';
        print '<tr>';
        print '<td>';
        print '<form name="apply_disc" id="apply_discount" action="' . $_SERVER["PHP_SELF"] .'?id='.$object->id.'" method="POST">';
        print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
        print '<input type="hidden" name="action" value="apply_line_discount">';
        print 'Descuento General ';
        print '<input type="text" placeholder="Porcentaje" name="discount_percent">';
        print '<label>%</label>';
        print '<input type="submit" class="button" value="Guardar" name="apply_discountb">';
        print '</form>';
        print '</td>';
        print '</tr>';
        //Descuento Financiero
        print '<tr>';
        print '<td>';
        print '<form name="apply_disc" id="apply_comptadiscount" action="' . $_SERVER["PHP_SELF"] .'?id='.$object->id.'" method="POST">';
        print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
        print '<input type="hidden" name="action" value="apply_line_comptadiscount">';
        print '<input type="hidden" name="products_tocomptadiscount" id="products_tocomptadiscount" value="">';
        print 'Descuento Financiero ';
        print '<input type="text" placeholder="Porcentaje" name="comptadiscount_percent">';
        print '<label>%</label>';
        print '<input type="submit" class="button" value="Guardar" name="apply_comptadiscountb">';
        print '</form>';
        print '</td>';
        print '</tr>';
        print '</table>';
		...
		if (!empty($conf->multicurrency->enabled) && ($object->multicurrency_code != $conf->currency))
		{
			...
			print '<tr><td class="titlefieldmiddle center">'.$form->editfieldkey('MulticurrencyAmountHT', 'multicurrency_total_ht', '', $object, 0).'</td>';
			print '<td class="nowrap center">'.price($object->multicurrency_total_ht, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
			print '</tr>';

			// Multicurrency Amount VAT
			print '<tr><td class="center">'.$form->editfieldkey('MulticurrencyAmountVAT', 'multicurrency_total_tva', '', $object, 0).'</td>';
			print '<td class="center">'.price($object->multicurrency_total_tva, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
			print '</tr>';

			// Multicurrency Amount TTC
			print '<tr><td class="center" height="10">'.$form->editfieldkey('MulticurrencyAmountTTC', 'multicurrency_total_ttc', '', $object, 0).'</td>';
			print '<td class="center">'.price($object->multicurrency_total_ttc, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
			print '</tr>';
		}
		...
		print '<tr><td class="titlefield center">'.$langs->trans('AmountHT').'</td><td class="center">'.price($object->total_ht, 1, $langs, 0, -1, -1, $conf->currency).'</td></tr>';

		// Discount
		$discount = $object->getDiscount();
		print '<tr><td class="titlefieldmiddle center">'.$langs->trans('Reductions').'</td>';
		print '<td class="center">'.price($discount, 1, '', 1, - 1, - 1, $conf->currency).$alert.'</td>';

		// Amount VAT
		print '<tr><td class="center">'.$langs->trans('AmountVAT').'</td><td class="center">'.price($object->total_tva, 1, $langs, 0, -1, -1, $conf->currency).'<div class="block">';
		...
		{
			print '<tr><td class="center">'.$langs->transcountry("AmountLT1", $societe->country_code).'</td>';
			print '<td class="center">'.price($object->total_localtax1, 1, $langs, 0, -1, -1, $conf->currency).'</td>';
			print '</tr>';
		}
		if ($societe->localtax2_assuj == "1") //Localtax2
		{
			print '<tr><td class="center">'.$langs->transcountry("AmountLT2", $societe->country_code).'</td>';
			print '<td class="center">'.price($object->total_localtax2, 1, $langs, 0, -1, -1, $conf->currency).'</td>';
			print '</tr>';
		}
		// Case of early payment discount
        if ($object->discount_applied) {
            print '<tr><td class="titlefield center">Importe previo al descuento</td><td class="center">'.price($object->total_ttc_before_discount,1,$langs,0,-1, -1, $conf->currency).'</td></tr>';
            print '<tr><td class="titlefield center">';
            print 'Descuento por prontopago';
            ...
            print '<td class="center">'.price($ep_discount, 1, $langs, 0, -1, -1, $conf->currency).'</td></tr>';
        }
		print '<tr><td class="center">'.$langs->trans('AmountTTC').'</td><td class="center" colspan="3">'.price($object->total_ttc, 1, $langs, 0, -1, -1, $conf->currency).'</td></tr>';
		...
		if ($action != 'presend')
		{
			...
			if (empty($reshook))
			{
					...
					&& ($object->statut == FactureFournisseur::STATUS_CLOSED || $object->statut == FactureFournisseur::STATUS_ABANDONED))				// A paid invoice (partially or completely)
				{
					...
					{
						if($object->statut == FactureFournisseur::STATUS_ABANDONED){
							print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=reopenCanceled">'.$langs->trans('ReOpen').'</a></div>';
						}else{
				...
				//Cancelar Factura/Nota de Credito
	            if ($object->statut == FactureFournisseur::STATUS_VALIDATED){
					print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=cancel&amp;socid='.$object->socid.'">'.$langs->trans('Cancel').'</a></div>';
				}
