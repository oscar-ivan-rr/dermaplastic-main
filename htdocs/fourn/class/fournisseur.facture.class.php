...
class FactureFournisseur extends CommonInvoice
{
    ...
    /**
     * @var double Total before discount
     */
    ...
    /**
     * @var int Warehouse Related to Facture
     */
    public $fk_warehouse;
    ...
    public function create($user)
    {
        ...
        $lim_reg = !empty($this->date_echeance)?" '".$this->db->idate($this->date_echeance)."'":"NUll";
        ...
        $sql.= ", ".(isset($this->fk_warehouse)?$this->fk_warehouse:"NULL");
        $sql.= ")";
        ...
        if ($resql)
        {
			...
			if (count($this->lines) && is_object($this->lines[0]))	// If this->lines is array of InvoiceLines (preferred mode)
			{
                ...
                {
                	if (empty($this->lines[$i]->buyprice_updated))
                	{
                		$this->lines[$i]->buyprice_updated = 0;
                	}
                	if ($this->note_private=="Pedido de proveedor generado desde XML"){
                        $this->lines[$i]->buyprice_updated = 1;
                        $this->lines[$i]->multicurrency_code = "MXN";
                        $this->lines[$i]->fk_multicurrency=0;
                    }
                    $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'facture_fourn_det (fk_facture_fourn, special_code, fk_multicurrency, multicurrency_code, buyprice_updated,claveprodservdet,umeddet,uentrada,usalida)';
                    $sql .= ' VALUES ('
									.$this->id
									.',"'.intval($this->lines[$i]->special_code).'"'
                                    .',"'.$this->lines[$i]->fk_multicurrency.'"'
                                    .',"'.$this->lines[$i]->multicurrency_code.'"'
									.',"'.$this->lines[$i]->buyprice_updated.'"'
									.',"'.$this->lines[$i]->claveprodserv.'"'
									.',"'.$this->lines[$i]->umed.'"'
									.',"'.$this->lines[$i]->uentrada.'"'
									.',"'.$this->lines[$i]->usalida.'")';
    ...
    public function fetch($id = '', $ref = '')
    {
        ...
        $sql.= " t.import_key,";
        $sql.= " t.extraparams,";
        $sql.= " t.total_ttc_before_discount,"; // Earlypayment discount fields
        $sql.= " t.discount_applied,";
        $sql.= " t.fk_warehouse,";
    ...
    public function fetch_lines()
    {
        ...
        $sql .= ', p.rowid as product_id, p.ref as product_ref, p.label as label, p.description as product_desc, p.tosell, p.tobuy';
        ...
        $sql .= ', f.notfound, buyprice_updated,claveprodservdet,umeddet,uentrada,usalida,f.real_cost,f.compta_discount';
        ...
        if ($resql_rows)
        {
            ...
            if ($num_rows)
            {
                ...
                while ($i < $num_rows)
                {
                    ...
                    $line->tosell	        = $obj->tosell;
                    $line->tobuy	        = $obj->tobuy;
                    ...
                    $line->real_cost		= $obj->real_cost;
                    $line->compta_discount	= $obj->compta_discount;
                    ...
                    $line->claveprodserv          = $obj->claveprodservdet;
                    $line->umed          = $obj->umeddet;
                    $line->uentrada          = $obj->uentrada;
                    $line->usalida          = $obj->usalida;
                    ...
                    // XML
                    $line->notfound         = $obj->notfound;
                    $line->buyprice_updated = $obj->buyprice_updated;
    ...
    public function createDiscountCreditNote(User $user, $invertdetail = 0,$discount = 0)
    {
        ...
        $facture->ref_supplier    = $this->ref_supplier.'-1';
        ...
        $lres = $facture->addline('Bonificación por Descuento',$total_lines,16,0,0,1,0,0,'','',0,'','TTC',0);
        ...
        $paiement->paiementid   = GETPOST('paiementid');
        ...
        /*if (!$error)
        {
            ...
            if (GETPOST('type') == FactureFournisseur::TYPE_CREDIT_NOTE) $label = '(CustomerInvoicePaymentBack)'; // Refund of a credit note
    ...
    public function cancel($user){
		...
		if ($resql){
            ...
            //Quitar precio de compra de producto
            $object_product = new Product($this->db);
            $num = count($this->lines);
            $idwarehouse = $this->fk_warehouse;
            for ($i = 0; $i < $num; $i++){
                if ($this->lines[$i]->fk_product > 0){
                    ...
                    if($this->type == self::TYPE_STANDARD){
                        //* Descomentar para habilitar el decremento de stock al cancelar una factura
                        // $result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceFournCancelled", $this->ref));
                        $result = 1;
                        if ($result > 0) {
                            //Verificar que tenga el mismo PC el producto que el detallado

                            $sqlCheck = " SELECT pf.rowid ";
                            $sqlCheck .= " FROM llx_product_fournisseur_price as pf  ";
                            $sqlCheck .= " LEFT JOIN llx_product as p ON (p.cost_price = pf.price AND p.rowid = pf.fk_product) ";
                            $sqlCheck .= " WHERE pf.fk_soc = ".$this->fk_soc." AND pf.reason = 'Compra' ";
                            $$sqlCheck .= " AND pf.price = ".$this->lines[$i]->total_ht." AND pf.fk_product = ".$this->lines[$i]->fk_product;
                            $resqlCheck  = $this->db->query($sqlCheck);
                            if( $resqlCheck ){
                                if( $this->db->num_rows($resqlCheck) > 0 ){
                                    //Regresar PC  y Gain anterior a cada producto
                                    $sqlCost = "UPDATE ".MAIN_DB_PREFIX."product SET";
                                    $sqlCost .= " gain = gainTemp , cost_price = costPriceTemp"; 
                                    $sqlCost .= " WHERE rowid = ".$this->lines[$i]->fk_product;
                                    $resqlCost = $this->db->query($sqlCost);
                                    if ($resqlCost){
                                        //Actualizar PV con su anterior registro
                                        $sqlProd = "SELECT precioTemp FROM ".MAIN_DB_PREFIX."product WHERE rowid = ".$this->lines[$i]->fk_product;
                                        $resqlProd  = $this->db->query($sqlProd);
                                        if ($resqlProd) {
                                            $objProd = $this->db->fetch_object($resqlProd);
                                            $precioTemp = $objProd->precioTemp;

                                            $object_product->fetch($this->lines[$i]->fk_product);
                                            $object_product->updatePrice($precioTemp, 'HT', $user, 16);
                            ...
                            $sqlDelete = "DELETE FROM ".MAIN_DB_PREFIX."product_fournisseur_price";
                            $sqlDelete .= " WHERE fk_product = ".$this->lines[$i]->fk_product;
                            $sqlDelete .= " AND reason = 'Compra'";
                            $sqlDelete .= " AND fk_soc = ".$this->fk_soc;
                            $sqlDelete .= " AND price = ".$this->lines[$i]->total_ht;
                    ...
                    //Nota de Credito
                        ...
                        $result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("CreditNoteFournCancelled", $this->ref));
                        if ($result < 0) {
                            $error++;
                        }
                    }
    ...
    // Función que actualiza el almacén 
    public function set_id_warehouse($user, $fk_entrepot)
    {
        // phpcs:enable
        if ($user->rights->fournisseur->commande->creer)
        {
            ...
            $sql = "UPDATE ".MAIN_DB_PREFIX."facture_fourn";
            $sql .= " SET fk_warehouse = ".($fk_entrepot > 0 ? (int) $fk_entrepot  : 'null');
            $sql .= " WHERE rowid = ".$this->id;
    ...
    public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0)
    {
        ...
        // Check parameters
        if ($this->statut > self::STATUS_DRAFT && $this->statut != self::STATUS_ABANDONED)	// This is to avoid to validate twice (avoid errors on logs and stock management)
        {
        ...
        foreach($this->lines as $line){
            ...
            $sql .= " WHERE rowid = ".$line->fk_product;
            $resql = $this->db->query($sql);
        }
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."facture_fourn";
        $sql .= " SET ref='".$num."', fk_statut = 1, fk_user_valid = ".$user->id.", date_valid = '".$this->db->idate($now)."', fk_warehouse = ".$idwarehouse;
        $sql .= " WHERE rowid = ".$this->id;
        ...
        if ($resql)
        {
            ...
            // Update buy prices of facture lines products
            if (!$error && $this->type != FactureFournisseur::TYPE_CREDIT_NOTE) {
                ...
                foreach ($this->lines as $index => $line) {
                    ...
                    $line->buyprice_updated = 1;

                    if ($line->buyprice_updated && !$line->notfound && $line->fk_product > 0 && $line->subprice>0.10) {
                        $prodid = $line->fk_product;

                        $productFourn = new ProductFournisseur($this->db);
                        $productNormal = new Product($this->db);
                        // Add buy price to product defined
                        if ($productFourn->fetch($prodid)) {

                            //Add ClaveProdServ y Ummed al producto
                            if(!empty($line->claveprodserv) && !empty($line->umed)) {
                                $productFourn->array_options['options_claveprodserv'] = $line->claveprodserv;
                                $productFourn->array_options['options_umed'] = $line->umed;
                                $resproduct = $productFourn->insertExtraFields();
                                if ($resproduct < 0) {
                                    dol_syslog("error " . $this->error, LOG_DEBUG);
                                    $error++;
                                    $this->error = $productFourn->error;
                                    break;
                                }
                                $productNormal->fetch($prodid);
                                $productNormal->description = $line->description;
                                $productNormal->update($prodid, $user);
                            }
                            //Unidad de entrada y de salida
                            if (!empty($line->uentrada) || !empty($line->usalida))
                            {
                                $productNormal->fetch($prodid);
                                $productNormal->unidad_entrada = $line->uentrada;
                                $productNormal->unidad_salida = $line->usalida;
                                dol_syslog("Antes de actualizar ".$line->uentrada." ".$line->usalida, LOG_DEBUG);
                                $productNormal->update($prodid, $user);
                            }
                            ...
                            $quantity = $line->qty;
                            if($line->real_cost == null){
                                $line->real_cost = 0;
                            }
                            $real_cost = $line->real_cost;

                            $npr = preg_match('/\*/', 0) ? 1 : 0;
                            $tva_tx = 16;
    
                            //$ret = $productFourn->add_fournisseur($user, $id_fourn, $ref_fourn, $quantity); // This insert record with no value for price. Values are update later with update_buyprice
                            //echo "<br><br><br><br><br>Agregando precio {$line->ref_supplier}<br>\r\n";
                            $ret = $productFourn->add_fournisseur($user, $id_fourn, $line->ref_supplier, $quantity, $real_cost); // This insert record with no value for price. Values are update later with update_buyprice                            
                            ...
                            if ($ret > 0) {
                                ...
                                // Se actualiza el precio de compra de proveedor
                                $ret = $productFourn->update_buyprice($quantity, $total, $user, 'HT', $supplier, null, $line->ref_supplier, $tva_tx, 0, $line->remise_percent, 0, $npr, '', '', array(), '', $total, 'HT', 1, $conf->currency,'','','','','Compra',$real_cost);
                                if($ret < 0)
                                {
                                    $error++;
                                    $this->error = $productFourn->error;
                                ...
                                //$oldprices = ProductFournisseur::getOldPrices($id_fourn, $ref_fourn, $productFourn->product_fourn_price_id);
        
                                // Remove prices with same supplier and ref_supplier
                                //foreach ($oldprices as $priceid)
                                    //$productFourn->remove_product_fournisseur_price($priceid);
                            }
                            elseif ($ret < 0) {
                                $error++;
                                $this->error = $productFourn->error;
                            ...
                            $error++;
                            $this->error = 'ProductNotDefined';
                            break;
                        }
                        
                        // Se actualiza el precio de compra (Base de venta)
                        $product = new Product($this->db);
                        if ($product->fetch($prodid)) {
                            if($this->socid != $conf->global->CEDIS_SUPPLIER){
                                $product->cost_price = $line->subprice;
                                $product->update($prodid, $user);
                            }
                        }else {
                            $error++;
                            $this->error = 'ProductNotDefined';
                        ...
                        //Guardar registros en temporal para: PC y PV
                        $sql = "UPDATE ".MAIN_DB_PREFIX."product SET";
                        $sql .= " costPriceTemp = cost_price, precioTemp = price "; 
                        $sql .= " WHERE rowid = ".$line->fk_product;
                        $resql = $this->db->query($sql);
                    }
                }

                // Update prices of products with category same price
                foreach($c_samePrice as $catid => $prices) {
                    $c = new Categorie($this->db);
                    if ($c->fetch($catid) > 0) {
                        // Update products prices.
                        $done = $c->setPriceToProducts($prices);
                        if ($done < 0)
                            setEventMessage($langs->trans("ErrorUpdateCat", $c->label), 'errors');
                        elseif ($done > 0)
                            setEventMessage($langs->trans("UpdatedCat", $c->label));
                    }
                }
            }
    ...
    public function setDraft($user, $idwarehouse = -1)
    {
        ...
        if ($result)
        {
            ...
            if($this->statut != self::STATUS_ABANDONED){
	 ...
	 * @param		double		$pu_ht_devise		Amount in currency
	 * @param		string		$ref_supplier		Supplier ref
	 * @param		int 		$notfound   		Not founded in system (Product free)
     * @return    	int           					<0 if KO, >0 if OK
     */
    public function updateline($id, $desc, $pu, $vatrate, $txlocaltax1 = 0, $txlocaltax2 = 0, $qty = 1, $idproduct = 0, $price_base_type = 'HT', $info_bits = 0, $type = 0, $remise_percent = 0, $notrigger = false, $date_start = '', $date_end = '', $array_options = 0, $fk_unit = null, $pu_ht_devise = 0, $ref_supplier = '', $notfound = 0, $real_cost = null,$compta_discount = null)
    {
    	global $mysoc, $langs;
	    ...
	    //Costo Real
        if($compta_discount == null) {
            $desc = 1-($line->remise_percent/100);
            //Recuperamos el descuento financiero y lo calculamos como decimal
            $compta_discount = (!empty($line->compta_discount)?1-($line->compta_discount/100):0);
            $real_cost = ($line->subprice*$desc)*$compta_discount;
            $line->real_cost = $real_cost;
            $line->compta_discount = (!empty($line->compta_discount)?$line->compta_discount:0);
        }else{
            $line->real_cost = $real_cost;
            $line->compta_discount = $compta_discount;
        }
	...
	public function deleteLinesMassive($linesIDs){
		$sqlDelLines = "DELETE FROM ".MAIN_DB_PREFIX."facture_fourn_det WHERE rowid IN (".implode(",", $linesIDs).")";
...
class SupplierInvoiceLine extends CommonObjectLine
{
    ...
    public $compta_discount;

    /**
     * @var int Product not found in XML
     */
    public $notfound;

    /**
     * @var int Buy price updated for product line
     */
    public $buyprice_updated;
	...
	public function fetch($rowid)
	{
        ...
        $sql.= ', f.notfound, f.buyprice_updated, f.real_cost,f.compta_discount';
		$sql.= ', p.rowid as product_id, p.ref as product_ref, p.label as product_label, p.description as product_desc';
		$sql.= ', f.multicurrency_subprice, f.multicurrency_total_ht, f.multicurrency_total_tva, multicurrency_total_ttc,f.claveprodservdet,f.umeddet,f.uentrada,f.usalida';
		$sql.= ' FROM '.MAIN_DB_PREFIX.'facture_fourn_det as f';
		$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'product as p ON f.fk_product = p.rowid';
		...
		$this->real_cost			= $obj->real_cost;
		$this->compta_discount			= $obj->compta_discount;
        ...
        $this->notfound         = $obj->notfound;
        $this->buyprice_updated = $obj->buyprice_updated;
        $this->claveprodserv = $obj->claveprodservdet;
        $this->umed = $obj->umeddet;
        $this->uentrada = $obj->uentrada;
        $this->usalida = $obj->usalida;
	...
	public function update($notrigger = 0)
	{
        ...
        if (empty($this->notfound))       $this->notfound = 0;
        ...
        // XML
        $sql .= " , notfound = $this->notfound";
        $sql .= " , real_cost = $this->real_cost";
        $sql .= " , compta_discount = $this->compta_discount";
    ...
    public function insert($notrigger = 0)
    {
        ...
        $sql .= ", ".$this->notfound;
        $sql .= ", 1"; //Always just like notfound
        $sql .= ')';
    ...
    /**
     * Set buyprice_updated to 0 in DB.
     * @return  int  1 Ok | 0 KO
     */
    public function buyPriceUpdated() {
        // Query
        $sql = "UPDATE ".MAIN_DB_PREFIX."facture_fourn_det SET buyprice_updated = 0 WHERE rowid = $this->id";
