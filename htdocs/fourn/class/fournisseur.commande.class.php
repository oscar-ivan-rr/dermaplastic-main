...
class CommandeFournisseur extends CommonOrder
{
    ...
    public function fetch($id, $ref = '')
    {
        ...
        $sql .= " WHERE c.entity = ".$conf->entity . 'LIMIT 1';
    ...
    public function fetch_lines($only_product = 0)
    {
    	...
    	$sql .= " p.rowid as product_id, p.ref as product_ref, p.label as product_label, p.description as product_desc, p.tosell, p.tobuy,";
    	$sql .= " l.fk_unit,";
    ...
    public function valid($user, $idwarehouse = 0, $notrigger = 0)
    {
       	...
       	|| (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->supplier_order_advance->validate)))
        {
            ...
            $sql .= " date_valid='".$this->db->idate(dol_now('tzuser'))."',";
    ...
    public function LibStatut($status, $mode = 0, $billed = 0)
    {
    	...
    	if (empty($this->statuts) || empty($this->statutshort)) {
	        ...
	        $this->statuts[5] = 'Recibido completo';
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        $sql .= ", ".($this->fk_entrepot > 0 ? $this->fk_entrepot : 'NULL');
        $sql .= ")";
     ...
     * @return 	int						<0 if KO, >0 if OK
     */
    public function dispatchProduct($user, $product, $qty, $entrepot, $price = 0, $comment = '', $eatby = '', $sellby = '', $batch = '', $fk_commandefourndet = 0, $notrigger = 0, $isGift = 0)
    {
        global $conf, $langs;
        ...
        // Comprobamos que no exista ya un dispatch igual con una búsqueda más completa
        $sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."commande_fournisseur_dispatch WHERE fk_commande = ".$this->id;
        $sql .= " AND fk_product = ".$product." AND fk_entrepot = ".$entrepot;
        if ($fk_commandefourndet > 0) $sql .= " AND fk_commandefourndet = ".$fk_commandefourndet;
        
        // Add batch specific conditions if batch is used
        if (!empty($batch)) {
            $sql .= " AND batch = '".$this->db->escape($batch)."'";
            if (!empty($eatby)) $sql .= " AND eatby = '".$this->db->idate($eatby)."'";
            if (!empty($sellby)) $sql .= " AND sellby = '".$this->db->idate($sellby)."'";
        }
        
        // Add exact quantity match to prevent duplicate entries with same quantity
        $sql .= " AND qty = ".$qty;
        
        // Add time-based check to prevent duplicate submissions
        $timeWindow = 60; // 60 seconds window
        $datec = dol_now() - $timeWindow;
        $sql .= " AND datec > '".$this->db->idate($datec)."'";
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                $this->error = $langs->trans('DispatchAlreadyExists');
                dol_syslog(get_class($this)."::dispatchProduct error: ".$this->error, LOG_WARNING);
                return -1;
            }
        }
        ...
        if (($this->statut == self::STATUS_ORDERSENT || $this->statut == self::STATUS_RECEIVED_PARTIALLY || $this->statut == self::STATUS_RECEIVED_COMPLETELY))
        {
            ...
            $sql .= " LEFT JOIN ".MAIN_DB_PREFIX."commande_fournisseurdet as cfd2 ON cfd2.rowid = cfd.fk_commandefourndet";
            $sql .= " WHERE cfd.fk_commande = ".$this->id;
            $sql .= " AND cfd.fk_product = ".$product;
            $sql .= " AND cfd2.rowid = ".$fk_commandefourndet;
            if (!empty($batch)) $sql .= " AND cfd.batch = '". $batch ."'";
            if (!empty($eatby)) $sql .= " AND cfd.eatby = '". date('Y-m-d', $eatby) ."'";
            if (!empty($sellby)) $sql .= " AND cfd.sellby = '". date('Y-m-d', $sellby) ."'";
            // Validar que el producto no sea gift y que el precio sea 0
            $sql .= " AND (cfd2.subprice > 0.1 OR cfd2.total_ttc = 0)";
            $resql = $this->db->query($sql);
            if ($resql)
            {
                $num = $this->db->num_rows($resql);
            ...
            if ($num == 0 || $isGift){
                ...
                $id = $row[0];
                $sql = "UPDATE ".MAIN_DB_PREFIX."commande_fournisseur_dispatch SET qty = qty + ".$qty." WHERE rowid = ".$id;
                $resql = $this->db->query($sql);
                if ($resql)
                {
                    $this->db->commit();
            ...
            //Comentar funcionalidad para incrementar en almacen
            /*
    ...
    public function set_id_entrepot($user, $fk_entrepot)
    {
        // phpcs:enable
        if ($user->rights->fournisseur->commande->creer)
        {
            ...
            $sql = "UPDATE ".MAIN_DB_PREFIX."commande_fournisseur";
            $sql .= " SET fk_entrepot = ".($fk_entrepot > 0 ? (int) $fk_entrepot  : 'null');
            $sql .= " WHERE rowid = ".$this->id;
    ...
    /**
     * Indicates that an email has already been sent.
     * @return  int -1 KO, 0 Nothing happen, ! OK
     */
    public function setEmailSent() {
        if (!$this->emailsent) {
            $sql = "UPDATE ". MAIN_DB_PREFIX ."commande_fournisseur SET emailsent = 1 WHERE rowid = $this->id";

            dol_syslog(get_class($this)."::setEmailSent", LOG_DEBUG);
            $this->db->begin();
     ...
     * @return  int -1 KO, 0 Nothing happen, ! OK
     */
    public function getCommandeQty() {
        $sql = "SELECT COUNT(c.rowid) as total FROM ". MAIN_DB_PREFIX ."commande as c WHERE c.fk_commande_fourn = ".$this->id;
