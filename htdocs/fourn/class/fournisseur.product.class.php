...
class ProductFournisseur extends Product
{
    ...
    public $reason; //razon por la cual se añade precio de compra
     ...
     *
     */
    public function updatePriceGain($idFourn,$gain,$user)
    {
    	require_once(DOL_DOCUMENT_ROOT.'/categories/class/categorie.class.php');
    	$main_cat = new Categorie($this->db);
    	$main_cat->fetch(166);
    	$cats = $main_cat->get_full_arbo('product',166,1);
    	//die(var_dump($cats));
        date_default_timezone_set('America/Mexico_City');
        if( $idFourn > 0 ){
            $idprod = $this->id;
            //sentencia para obtener el ultimo precio
            $sql = "SELECT rowid, unitprice as price, tva_tx FROM ".MAIN_DB_PREFIX."product_fournisseur_price WHERE fk_product =".$idprod." ORDER BY tms DESC LIMIT 1";//." AND fk_soc = ".GETPOST("id_fourn");
            $res = $this->db->query($sql);
            $obj =$this->db->fetch_object($res);

            //Margen de ganacia en precio de venta minimo
            $con = utf8_encode(file_get_contents(DOL_DOCUMENT_ROOT."/product/sell_gain.json"));
            $min_sell = json_decode($con, true);
            $min_sell = intval($min_sell['gain']);

#            $x = ($obj->price ) * (1 + ($gain/100));
#            $y = ($obj->price) * (1 + ($min_sell/100));
            $x = ($obj->price ) / (1 - ($gain/100));
            $y = ($obj->price)  / (1 - ($min_sell/100));
            $x1 = (($obj->price ) / (1 - ($gain/100)))*1.16;
            $y2 = $obj->price*1.05;

            $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'product_price (fk_product, date_price, price_level, price, price_ttc, price_min, price_min_ttc, price_base_type, fk_user_author, tva_tx)';
            $sql.= ' VALUES ('.$idprod.' , "'.date("Y-m-d H:i:s").'" ,1 ,'.$x.','.$x1.', '.$y.','.$y2.', "HT" , '.$user->id.', 16)';
            $res2 = $this->db->query($sql);
            if($res2){
                $price_ttc_temp = $x1;
                $price_ttc_min_temp = $y2;
                $sql = 'UPDATE '.MAIN_DB_PREFIX.'product SET gain='.$gain.',price='.$x.', base_price='.$x.',price_ttc='.$price_ttc_temp.',base_price_ttc='.$price_ttc_temp.',price_min='.$y.',base_price_min='.$y.',price_min_ttc='.$price_ttc_min_temp.', base_price_min_ttc='.$price_ttc_min_temp.' WHERE rowid='.$idprod;
                #$sql = 'UPDATE '.MAIN_DB_PREFIX.'product SET gain='.$gain.',price='.$x.',price_ttc='.($x * (1 + $obj->tva_tx/100) ).',price_min='.$y.',price_min_ttc='.($y * (1 + $obj->tva_tx/100) ).' WHERE rowid='.$idprod;
                $res3 = $this->db->query($sql);
                if($res3)
                {
                    $this->price = $x;
                    $this->price_min = $y;
                    $this->price_ttc = $price_ttc_temp;
                    $this->price_min_ttc = $price_ttc_min_temp;
                    $this->db->commit();
                    $action='';
                }
                else{
     ...
     *
     */
    public function updatePriceGain_original($idFourn,$gain,$user)
    {
        date_default_timezone_set('America/Mexico_City');
        if( $idFourn > 0 ){
            $idprod = $this->id;
            //sentencia para obtener el ultimo precio
            $sql = "SELECT rowid, MAX(unitprice) as price, tva_tx FROM ".MAIN_DB_PREFIX."product_fournisseur_price WHERE fk_product =".$idprod." ORDER BY tms DESC LIMIT 1";;//." AND fk_soc = ".GETPOST("id_fourn");
            $res = $this->db->query($sql);
            $obj =$this->db->fetch_object($res);
...
#            $y = ($obj->price) * (1 + ($min_sell/100));
            ...
            $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'product_price (fk_product, date_price, price_level, price, price_ttc, price_min, price_min_ttc, price_base_type, fk_user_author, tva_tx)';
            $sql.= ' VALUES ('.$idprod.' , "'.date("Y-m-d H:i:s").'" ,1 ,'.$x.','.($x * (1 + $obj->tva_tx/100) ).', '.$y.','.($y * (1 + $obj->tva_tx/100) ).', "HT" , '.$user->id.', 16)';
            $res2 = $this->db->query($sql);
            if($res2){
                $price_ttc_temp = $x * (1 + $obj->tva_tx/100);
                $price_ttc_min_temp = $y * (1 + $obj->tva_tx/100);
                $sql = 'UPDATE '.MAIN_DB_PREFIX.'product SET gain='.$gain.',price='.$x.', base_price='.$x.',price_ttc='.$price_ttc_temp.',base_price_ttc='.$price_ttc_temp.',price_min='.$y.',base_price_min='.$y.',price_min_ttc='.$price_ttc_min_temp.', base_price_min_ttc='.$price_ttc_min_temp.' WHERE rowid='.$idprod;
                #$sql = 'UPDATE '.MAIN_DB_PREFIX.'product SET gain='.$gain.',price='.$x.',price_ttc='.($x * (1 + $obj->tva_tx/100) ).',price_min='.$y.',price_min_ttc='.($y * (1 + $obj->tva_tx/100) ).' WHERE rowid='.$idprod;
                $res3 = $this->db->query($sql);
                if($res3)
     ...
     *    @param  	string|null $packing     	                Product price packing. If is null it is not updated
     *    @param    string      $reason                         Cotizacion o Compra
     *    @return	int											<0 if KO, >=0 if OK
     */
    public function update_buyprice($qty, $buyprice, $user, $price_base_type, $fourn, $availability, $ref_fourn, $tva_tx, $charges = 0, $remise_percent = 0, $remise = 0, $newnpr = 0, $delivery_time_days = 0, $supplier_reputation = '', $localtaxes_array = array(), $newdefaultvatcode = '', $multicurrency_buyprice = 0, $multicurrency_price_base_type = 'HT', $multicurrency_tx = 1, $multicurrency_code = '', $desc_fourn = '', $barcode = '', $fk_barcode_type = '', $packing = '', $reason = '', $real_cost= 0)
    {
        // phpcs:enable
        global $conf, $langs;
        //global $mysoc;
        ...
        if ($this->product_fourn_price_id > 0)
        {
            ...
            if ($packing !== null)
                $sql .= " packing = ".(empty($packing) ? 'NULL' : "'".$this->db->escape($packing)."'").",";
            $sql .= " reason = ".(empty($reason) ? 'NULL' : "'".$this->db->escape($reason)."'");
        ...
        else
        {
            ...
            if ($resql) {
                ...
                $sql .= "datec, fk_product, fk_soc, ref_fourn, desc_fourn, fk_user, price, quantity, remise_percent, remise, unitprice, tva_tx, charges, fk_availability, default_vat_code, info_bits, entity, delivery_time_days, supplier_reputation, barcode, fk_barcode_type, real_cost)";
                $sql .= " values(";
    ...
    public function fetch_product_fournisseur_price($rowid, $ignore_expression = 0)
    {
        ...
        $sql .= "  pfp.barcode, pfp.fk_barcode_type, pfp.packing, pfp.reason";
    ...
    public function list_product_fournisseur_price($prodid, $sortfield = '', $sortorder = '', $limit = 0, $offset = 0)
    {
        ...
        $sql .= " pfp.barcode, pfp.fk_barcode_type, pfp.packing, pfp.reason, pfp.real_cost";
        ...
        if ($resql)
        {
            ...
            while ($record = $this->db->fetch_array($resql))
            {
                ...
                $prodfourn->fourn_date_creation1          = $record['datec'];
                ...
                $prodfourn->packing                         = $record["packing"];
                $prodfourn->reason                         = $record["reason"];
                $prodfourn->real_cost                         = $record["real_cost"];
    ...
    public function display_price_product_fournisseur($showunitprice = 1, $showsuptitle = 1, $maxlen = 0, $notooltip = 0, $productFournList = array())
    {
        ...
        if (count($productFournList) > 0) {
            ...
            foreach ($productFournList as $productFourn) {
                if($this->reason == "Compra"){
                    $out .= '<tr><td class="right">'.($showunitprice ?price($productFourn->fourn_unitprice - $productFourn->fourn_remise) : '').'</td>';
                }else{
            ...
            if($this->reason == "Compra"){
                $out = ($showunitprice ?price($this->fourn_unitprice + $this->fourn_remise).' '.$langs->trans("HT").' &nbsp; (' : '').($showsuptitle ? $langs->trans("Supplier").': ' : '').$this->getSocNomUrl(1, 'supplier', $maxlen, $notooltip).' / '.$langs->trans("SupplierRef").': '.$this->fourn_ref.($showunitprice ? ')' : '');
            }else{
     ...
     */
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1, $showSellPrice = 0)
    {
    ...
    /**
     * Function to get product with given supplier product ref.
     * @param	string			$ref	Product ref
     * @return	object|null		Product obejct if found, otherwise null
     */
    public static function getProduct($ref) {
        global $db;
        
        $sql = "SELECT pfp.fk_product as id, p.fk_product_type as type, p.ref, p.label, p.description, pfp.fk_soc as fourn FROM ".MAIN_DB_PREFIX."product_fournisseur_price pfp, ".MAIN_DB_PREFIX."product p WHERE ref_fourn = '$ref' AND pfp.fk_product = p.rowid";
    ...
    /**
     * Function to get product with given the label of the product
     * @param	string			$label	Product ref
     * @return	object|null		Product obejct if found, otherwise null
     */
    public static function getProductByLabel($label) {
        global $db;

        $sql = "SELECT pfp.fk_product as id, p.fk_product_type as type, p.ref, p.label, p.description, pfp.fk_soc as fourn FROM ".MAIN_DB_PREFIX."product_fournisseur_price pfp, ".MAIN_DB_PREFIX."product p WHERE (p.label = '$label' or p.description like '%$label%') AND pfp.fk_product = p.rowid";
    ...
    /**
     * Get the old prices for product with fourn and ref_supplier.
     * @param	int			$fourn			Societe id
     * @param	string		$ref_supplier	Supplier product ref
     * @param	int			$newPriceID		Curren product price id 
     * @return	array|null	Array with price ids if exists, otherwise null
     */
    public static function getOldPrices($fourn, $ref_supplier, $newPriceID) {
        global $db;

        $sql = "SELECT rowid as id FROM ".MAIN_DB_PREFIX."product_fournisseur_price pfp WHERE rowid != $newPriceID AND ref_fourn = '$ref_supplier' AND fk_soc = $fourn";
    ...
    /**
     * Update prices with multicurrency (for now only with USD). TODO: Add all currencies.
     * @param   float   $rate   New rate to convert.
     * @return  int     -2 Multicurrenci not enabled | -1 Query error | 0 Prices not found with given currency | > 0 OK
     */
    public static function setRateForProducts($rate) {
        global $db, $user, $conf;
        
        // Prevent multicurrency not enabled.
        if ($conf->multicurrency->enabled || $conf->global->PRODUIT_FOURN_MULTICURRENCY) {
            // Get prices with currency = 'USD'
            $sql = "SELECT rowid, fk_product, fk_soc as socid, ref_fourn, quantity, multicurrency_tx, multicurrency_price, multicurrency_code, packing FROM llx_product_fournisseur_price WHERE multicurrency_code = 'USD'";
            ...
            if ($res) {
                ...
                while ($obj = $db->fetch_object($res)) {
                    ...
                    // Fetch product
                    if ($productFourn->fetch($obj->fk_product)) {
                        $error = 0;
                        $id_fourn = $obj->socid;
                        ...
                        if (!$error) {
                            ...
                            $supplier->fetch($id_fourn);
                            
                            // Price with new rate.
                            $newprice = price2num($obj->multicurrency_price / $rate);
                            $ret = $productFourn->update_buyprice($quantity, $newprice, $user, 'HT', $supplier, null, $ref_fourn, $tva_tx, 0, 0, 0, $npr, '', '', array(), '', $obj->multicurrency_price, 'HT', $rate, $obj->multicurrency_code, '', '', '', $obj->packing);
                            
                            if($ret > 0) {
                                // Successful process
                                if (!$productFourn->updatePriceGain($id_fourn,$productFourn->gain,$user))
                                    setEventMessage("No se pudo agregar la ganancia al producto $productFourn->ref", 'warning');
                            }
                            else {
                                $error++;
                                setEventMessage($db->lasterror(), 'error');
