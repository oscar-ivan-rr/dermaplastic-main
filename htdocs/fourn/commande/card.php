...
if (empty($reshook))
{
	...
	{
		$result = $object->set_id_entrepot($user, GETPOST('fk_entrepot', 'int'));
		if ($result < 0) setEventMessages($object->error, $object->errors, 'errors');
	}
				...
				if ($newstatus == 0)
				{
					...
					if($resql > 0){
						while ($obj = $db->fetch_object($resql)) {
							...
							$res_commande = $commande->fetch($obj->rowid);
	...
	if($action =='apply_line_discount' && $object->statut == CommandeFournisseur::STATUS_DRAFT && $user->rights->fournisseur->commande->creer){
	    if(GETPOST('discount_percent')!=null){
            foreach($object->lines as $line){
                $result = $object->updateline($line->id, $line->desc, $line->subprice, $line->qty, GETPOST('discount_percent'), $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, 'HT', $line->info_bits,$line->product_type,0, $line->date_start, $line->date_end,  0,$line->fk_unit,$line->pu_ht,$line->ref_supplier);
            }
	    }else{
	...
	if ($action == 'updateline' && $user->rights->fournisseur->commande->creer && !GETPOST('cancel', 'alpha'))
	{
		...
		$productsql = "SELECT exentoiva FROM " . MAIN_DB_PREFIX . "product WHERE rowid = " . $line->fk_product;
		$reproductsql = $db->query($productsql);
		if ($reproductsql) {
			$productline = $db->fetch_object($productsql);
		}
		$vat_rate = $productline->exentoiva ? 0 : $vat_rate;
	...
	{
		//Se obtiene el ID del Producto Relacionado al Line
		$sqlProdID = "SELECT fk_product FROM ".MAIN_DB_PREFIX."commande_fournisseurdet WHERE rowid = ".$lineid;
		if($resProdID = $db->query($sqlProdID)){
			$prodID = $db->fetch_object($resProdID)->fk_product;
		}

		$result = $object->deleteline($lineid);
		if ($result > 0)
		{
			//Delete FK_OC from Product
			$sqlOC = "UPDATE ".MAIN_DB_PREFIX."product SET fk_oc = null WHERE rowid = ".$prodID;
			if(!$resOC = $db->query($sqlOC)){
				setEventMessages('Error al Eliminar OC de Producto', $object->errors, 'errors');
				$action = '';
			}
			// Define output language
			$outputlangs = $langs;
	...
	{
		$object->date_commande = dol_now('tzuser');
	...
	if ($action == 'confirm_order_to_cedis' && $confirm == 'yes')
	{
		$sql = "SELECT ls.rowid, ls.nom FROM llx_societe ls JOIN llx_entrepot le ON ls.nom = le.`ref` WHERE le.rowid =".$object->fk_entrepot;
		$resql=$db->query($sql);
			...
			if($commande_id > 0){
				...
				for ($i = 0; $i < $num; $i++) {
					...
					$result = $commande->addline(
						...
						$lines[$i]->remise_percent, $lines[$i]->info_bits, 0, 'HT', 0, $date_start, $date_end, $product_type,
						$lines[$i]->rang, $lines[$i]->special_code, $fk_parent_line, null, $lines[$i]->pu_ht, $label, $array_options,
						$lines[$i]->fk_unit, $object->origin, $lines[$i]->id
					);
					...
					if ($result < 0) {
						...
						sendUpdateStockNotification($db, $lines[$i]->fk_product, $commande->warehouse_id);
					}
				...
				if(!$error){
					$commande->valid($user, $commande->warehouse_id);

					$result = $object->commande($user, dol_now('tzuser'), 0, '');
					if ($result > 0)
					{
						if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
						{
				...
				setEventMessages('Error al crear pedido a CEDIS', null, 'errors');
			}
	...
	if ($action == 'add' && $user->rights->fournisseur->commande->creer)
	{
		...
		if (!$error)
		{
			...
			// Validación de campo de Almacén obligatorio
	...
	if ($action == 'webservice' && GETPOST('mode', 'alpha') == "send" && !GETPOST('cancel', 'alpha'))
	{
		...
		if (empty($conf->syncsupplierwebservices->enabled)) {
			...
			$order = array(
				...
				'date'          => dol_print_date(dol_now('tzuser'), 'dayrfc'),
...
if ($action == 'create')
{
        ...
        print '<script type="text/javascript">
			$(document).ready(function() {
				$("#socid").change(function() {
					var socid = $(this).val();
        			$.ajax("'.DOL_URL_ROOT.'/fourn/ajax/ajaxsociete.php", {
	...
	// Ref supplier
	print '<tr><td>'.$langs->trans('RefSupplier').'</td><td><input name="refsupplier" id="refsupplier" type="text"></td>';
	print '</tr>';
	...
	$form->select_types_paiements(isset($_POST['mode_reglement_id']) ? $_POST['mode_reglement_id'] : 2, 'mode_reglement_id');
	print '</td></tr>';

	// Planned delivery date
	print '<tr><td>';
	print $langs->trans('DateDeliveryPlannedFourn');
	print '</td>';
	print '<td>';
	$usehourmin = 0;
	...
	print '<tr><td><label data-toggle="collapse" data-target="#note_public" onclick="$(\'#note_public\').toggle(); ">'.$langs->trans('NotePublic').'</label></td>';
	print '<td>';
	...
	print '<tr><td><label data-toggle="collapse" data-target="#note_private" onclick="$(\'#note_private\').toggle(); ">'.$langs->trans('NotePrivate').'<label></td>';
	print '<td>';
	...
	print $doleditor->Create(1);
	print '<script>$(document).ready(function(){$("#note_public").css("display","none");$("#note_private").css("display","none");});</script>';
	print '</td>';
...
elseif (!empty($object->id))
{
	...
	// Confirmation de l'envoi de la commande
	if ($action == 'order_to_cedis')
	{
		$formconfirm = $form->formconfirm($_SERVER['PHP_SELF']."?id=".$object->id, $langs->trans("MakeOrderToCedis"), $langs->trans("ConfirmMakeOrderToCedis"), "confirm_order_to_cedis", '', 0, 2);
	}
	...
	// Thirdparty
	$morehtmlref .= '<br>'.$langs->trans('Supplier');
	...
	print '</td>';
	if ($action != 'edit_entrepot' && $user->rights->stock->show_all_warehouses) print '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=edit_entrepot&amp;id='.$object->id.'">'.img_edit($langs->trans('Almacén'), 1).'</a></td>';
	print '</tr></table>';
	print '</td><td>';
	if ($action == 'edit_entrepot')
	{
		...
		require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
		$formproduct = new FormProduct($db);
		print $formproduct->selectWarehouses($object->fk_entrepot, 'fk_entrepot', 'warehouseopen', 0);
		print '<input type="submit" class="button" value="'.$langs->trans('Modify').'">';
		print '</form>';
	...
	else
	{
		...
		JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE lcf.rowid = ".$object->id."";
					...
					buttons: [
						{
							text: \"Si\",
							click: function() {
									var l = '".DOL_URL_ROOT."/fourn/commande/card.php?id=".$id."&action=confirmDelete&lines=';
									var q = [];
    ...
    //print '<input type="hidden" name="totalines" value="'.$totalines.'">';
    print '<label>%</label>';
	...
	print '<td class="center">'.price($subtotal, '', $langs, 0, - 1, - 1, $conf->currency).$alert.'</td>';
	print '</tr>';
	...
	// Total VAT
	print '<tr><td>'.$langs->trans("VAT").'</td><td class="center">'.price($object->total_tva, '', $langs, 0, -1, -1, $conf->currency).'</td>';
	print '</tr>';
	...
	if ($mysoc->localtax1_assuj == "1" || $object->total_localtax1 != 0) //Localtax1
	{
		...
		print '<td class="center">'.price($object->total_localtax1, '', $langs, 1, -1, -1, $conf->currency).'</td>';
		print '</tr>';
	}
	if ($mysoc->localtax2_assuj == "1" || $object->total_localtax2 != 0) //Localtax2
	{
		print '<tr><td class="center">'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td>';
		print '<td class="center">'.price($object->total_localtax2, '', $langs, 1, -1, -1, $conf->currency).'</td>';
		print '</tr>';
	}

	// Total TTC
	print '<tr><td>'.$langs->trans("AmountTTC").'</td><td class="center">'.price($object->total_ttc, '', $langs, 1, -1, -1, $conf->currency).'</td>';
	print '</tr>';
	...
	if ($user->socid == 0 && $action != 'editline' && $action != 'delete')
	{
		...
		if (empty($reshook))
		{
			...
			if (in_array($object->statut, array(CommandeFournisseur::STATUS_ACCEPTED, 3, 4, 5)) || !empty($conf->global->SUPPLIER_ORDER_SENDBYEMAIL_FOR_ALL_STATUS))
			{
				if ($user->rights->fournisseur->commande->commander)
				{
					$btnCss = ($object->emailsent) ? 'style="background-color: #009900; color: whitesmoke;"' : '';
					print '<a class="butAction" '.$btnCss.' href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=presend&mode=init#formmailbeforetitle">'.$langs->trans('SendMail').'</a>';
				}
			}
			...
			if (!empty($conf->fournisseur->enabled) && ($object->statut >= 2 && $object->statut != 7 && $object->billed != 1))  // statut 2 means approved, 7 means canceled
			{
				...
				{
					if($object->statut == CommandeFournisseur::STATUS_RECEIVED_COMPLETELY)
						// Normal bill
						print '<a class="butAction" href="'.DOL_URL_ROOT.'/fourn/facture/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("Crear Compra").'</a>';
						
					// Xml bill
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/fourn/facture/loadXml.php?origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("Crear Compra con XML").'</a>';
				}
			}
			//}
		...
		if ($user->rights->fournisseur->commande->commander && $object->statut == CommandeFournisseur::STATUS_ACCEPTED && $action == 'makeorder')
		{
			...
			print $form->selectDate(dol_now('tzuser'), '', 1, 1, '', "commande", 1, 1);
			print '</td></tr>';
		...
		if ($action != 'makeorder')
		{
			...
			if ($action == 'classifyreception')
			{
				if ($user->rights->fournisseur->commande->receptionner && ($object->statut == CommandeFournisseur::STATUS_ORDERSENT || $object->statut == CommandeFournisseur::STATUS_RECEIVED_PARTIALLY))
				{
					...
					$datepreselected = dol_now('tzuser');
