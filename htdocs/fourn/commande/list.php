...
$search_order_start      = GETPOST("search_order_start", "alpha");
$search_order_startyear  = GETPOST("search_order_startyear", "int");
$search_order_startmonth = GETPOST("search_order_startmonth", "int");
$search_order_startday   = GETPOST("search_order_startday", "int");
$search_order_end        = GETPOST("search_order_end", "alpha");
$search_order_endyear    = GETPOST("search_order_endyear", "int");
...
if (empty($reshook))
{
	...
	if (GETPOST('button_removefilter_x', 'alpha') || GETPOST('button_removefilter.x', 'alpha') || GETPOST('button_removefilter', 'alpha')) // All tests are required to be compatible with all browsers
	{
		...
		$search_project_ref = '';
		$search_status = -1;
		$search_order_startyear = '';
		$search_order_startmonth = '';
		$search_order_startday = '';
...
$sql .= " JOIN ".MAIN_DB_PREFIX."entrepot as le ON cf.fk_entrepot = le.rowid";
$sql .= " JOIN ".MAIN_DB_PREFIX."c_rfc as lcr ON lcr.rowid = le.fk_rfc";
...
if(!$user->rights->stock->show_all_warehouses) $sql.= " AND le.rowid = ".$user->fk_warehouse."";
$sql .= ' AND cf.entity IN ('.getEntity('supplier_order').')';
if ($search_note_private) $sql .= " AND cf.note_private != ''";
...
// Date Order
if (date('',$search_order_start) > date('',$search_order_end)) {
	setEventMessage('Verificar rango entre fechas de pedido: (' . $search_order_start . ' es mayor que ' . $search_order_end . ')', 'warnings');
	$search_order_start = '';
	$search_order_end = '';
} elseif ($search_order_startday && $search_order_endday) {
...
if ($search_warehouse != '') $sql .= " AND cf.fk_entrepot = '".$search_warehouse."' ";
if ($search_rfc != '') $sql .= natural_search("lcr.code", $search_rfc);
...
if ($resql)
{
	...
	if ($search_order_start)   $param .= '&search_order_start='.urlencode($search_order_start);
	if ($search_order_end)   $param .= '&search_order_end='.urlencode($search_order_end);
	if ($search_order_startday)   $param .= '&search_order_startday='.urlencode($search_order_startday);
	if ($search_order_startmonth) $param .= '&search_order_startmonth='.urlencode($search_order_startmonth);
	if ($search_order_startyear)  $param .= '&search_order_startyear='.urlencode($search_order_startyear);
	...
	if ($search_rfc != '')   $param .= "&search_rfc=".$search_rfc;
	...
	{
		print '<td class="liste_titre" colspan="2"><input type="text" class="flat" name="search_ref" value="'.$search_ref.'"></td>';
	}
	// Ref customer
	...
	{
		print '<td class="liste_titre"><input type="text" class="flat" name="search_project_ref" value="'.$search_project_ref.'"></td>';
	}
	// Request author
	if (!empty($arrayfields['u.login']['checked']))
	{
		print '<td class="liste_titre">';
		print '<input type="text" class="flat" name="search_request_author" value="'.$search_request_author.'">';
		print '</td>';
	}
	// Thirpdarty
	...
	// Town
	if (!empty($arrayfields['s.town']['checked'])) print '<td class="liste_titre"><input class="flat " type="text" name="search_town" value="'.$search_town.'"></td>';
	// Zip
	...
	if (!empty($arrayfields['warehouse']['checked']))
	{
		...
		if($user->rights->stock->show_all_warehouses){
			...
			{
				print '<option value="'.$warehouse->rowid.'" '.($search_warehouse == $warehouse->rowid ? 'selected':'' ).'>'.$warehouse->ref.'</option>';
				$i++;
			}
			print '</select>';
		}
	...
	if (!empty($arrayfields['rfc']['checked']))
	{
		...
		if($user->rights->stock->show_all_warehouses){
			...
			{
				print '<option value="'.$rfc->rowid.'" '.($search_rfc == $rfc->rowid ? 'selected':'' ).'>'.$rfc->code.'</option>';
				$i++;
			}
			print '</select>';
		}	
	...
	if (!empty($arrayfields['cf.date_commande']['checked']))
	{
		...
		print "Desde:<br>".$form->select_date($search_order_start ? $search_order_startyear."-".$search_order_startmonth."-".$search_order_startday : '-1','search_order_start',0,0,0,'',1,1,1);
		print "<br>Hasta:<br>".$form->select_date($search_order_end ? $search_order_endyear."-".$search_order_endmonth."-".$search_order_endday : '','search_order_end',0,0,0,'',1,1,1);
		print '</td>';
	}
	// Date delivery
	...
	if (!empty($arrayfields['cf.total_ht']['checked']))
	{
		...
		print '<input class="flat" size="8" type="text" name="search_total_ht" value="'.$search_total_ht.'">';
		print '</td>';
	}
	if (!empty($arrayfields['cf.total_vat']['checked']))
	{
		// Amount
		print '<td class="liste_titre right">';
		print '<input class="flat" size="8" type="text" name="search_total_vat" value="'.$search_total_vat.'">';
		print '</td>';
	}
	if (!empty($arrayfields['cf.total_ttc']['checked']))
	{
		// Amount
		print '<td class="liste_titre right">';
		print '<input class="flat" size="8" type="text" name="search_total_ttc" value="'.$search_total_ttc.'">';
		print '</td>';
	}
	// Extra fields
	...
	if (!empty($arrayfields['cf.ref']['checked']))            print_liste_field_titre($arrayfields['cf.ref']['label'], $_SERVER["PHP_SELF"], "cf.ref", "", $param, 'colspan="2"', $sortfield, $sortorder);
