...
if ($object->id > 0)
{
    ...
    if ($conf->fournisseur->enabled)
    {
		...
		$link = DOL_URL_ROOT.'/fourn/facture/list.php?socid='.$object->id.'&search_status=1';
        $icon = 'bill';
	...
	*/
	if ($conf->fournisseur->enabled)
	{
	    ...
	    $outstandingTotalIncTax = $tmp['total_ttc'];
		*/
		$sql = "SELECT SUM(total_ttc) as total FROM llx_commande_fournisseur WHERE fk_soc = ".$object->id." AND fk_statut = 3";
		$resql = $db->query($sql);
	    if ($resql){
			$objOC = $db->fetch_object($query);
			$text = "O. DE COMPRA";
			$link = DOL_URL_ROOT.'/fourn/commande/list.php?socid='.$object->id.'&search_status=3';
			$icon = 'bill';
			if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
			$boxstat .= '<div class="boxstats" title="'.dol_escape_htmltag($text).'">';
			$boxstat .= '<span class="boxstatstext">'.img_object("", $icon).' '.$text.'</span><br>';
			$boxstat .= '<span class="boxstatsindicator">'.price($objOC->total, 1, $langs, 1, -1, -1, $conf->currency).'</span>';
			$boxstat .= '</div>';
			if ($link) $boxstat .= '</a>';
	    }else{
	        dol_print_error($db);
	...
	if ($conf->fournisseur->enabled)
	{
	    ...
	    $link = DOL_URL_ROOT.'/fourn/facture/list.php?socid='.$object->id.'&search_status=99';
	    $icon = 'bill';
	    if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
	    $boxstat .= '<div class="boxstats" title="'.dol_escape_htmltag($text).'">';
	    $boxstat .= '<span class="boxstatstext">'.img_object("", $icon).' '.$text.'</span><br>';
	    $boxstat .= '<span class="boxstatsindicator">'.price($outstandingTotalIncTax, 1, $langs, 1, -1, -1, $conf->currency).'</span>';
	    $boxstat .= '</div>';
	    if ($link) $boxstat .= '</a>';
	}
