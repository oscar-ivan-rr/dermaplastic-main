 ...
 */
function ErrorControl($value, $function="",$msg='')
{
	global $langs;
	$langs->load("pos");
	...
	if (! is_array($value))
	{
		return ControlNotArray($value,$function,$msg);
	}
	else 
	{
...
function ControlNotArray($value, $function,$msg='')
{
	global $langs;
	$langs->load("pos");
	...
	switch ($value) 
	{
    	...
    	default:
			switch ($function)
    		{
    			...
    			case "SetTicket":
					$ret['error']['value'] = 0;
    				$ret['error']['desc'] = $langs->trans("TicketSaved").$msg;
    				$ret['data'] = $value;
        			break;
        		case "closeCash":
    				$ret['error']['value'] = 0;
