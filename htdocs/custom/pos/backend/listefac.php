...
if (!$user->rights->pos->backend)
{
	...
	accessforbidden(null,0,0);
}	
else {
	if ($page == -1) $page = 0 ;
	
	 $sql = 'SELECT';
        $sql.= ' f.rowid as facid, f.ref, f.type, f.increment, f.total, f.total_ttc,';
        ...
        if ($_GET['search_user'])
        {
        	$sql.= ' AND u.lastname LIKE \'%'.$db->escape(trim($_GET['search_user'])).'%\'';
        }
        ...
        $sql.= ' GROUP BY f.rowid, f.ref, f.type, f.increment, f.total, f.total_ttc,';
...
//                    print '</td>';
			...
			if ($closeid)
			{
				...
				print '<a class="butAction" href="'.dol_buildpath('/pos/backend/listefac.php',1).'?closeid='.$closeid.'&viewstatut=2&action=mail">'.$langs->trans('MailCopy').'</a>';
