...
if(isset($_GET['mesg'])) { setEventMessage($langs->trans('Email enviado exitosamente')); }
...
if (!$user->rights->pos->backend)
{
	...
	$sql = "SELECT t.rowid, t.ticketnumber as ref, f.rowid as frowid, f.ref as facnumber, f.type as ftype, pt.amount, t.type, p.datep,p.fk_paiement,";
    ...
    $sql .=" WHERE DATE(p.datep) = DATE('".$date_end."')";
    $sql .= " AND p.rowid = pt.fk_paiement AND t.rowid = pt.fk_ticket AND s.rowid = t.fk_soc AND p.fk_paiement = c.id";
    $sql.= " AND t.fk_cash in (SELECT rowid FROM ".MAIN_DB_PREFIX."pos_cash WHERE fk_warehouse = ".$cash->fk_warehouse.")";
	if ($search_ref)
    {
    ...
    if ($resql)
    {
        ...
        if ($num > 0)
        {
            ...
            while ($i < min($num,$limit))
            {
                ...
                unset($facturestatic);
                $facturestatic=new Facture($db);
                $facturestatic->id=$objp->frowid;
                $facturestatic->ref=$objp->facnumber;
                $facturestatic->type=$objp->ftype;
                print '<td>';
                print $objp->facnumber ? $facturestatic->getNomUrl(1):"";
                print '</td>';
		...
		if( GETPOST('action','string') == 'mail')
		{
			...
			$formmail->param['returnurl']=$_SERVER["PHP_SELF"].'?closeid='.$closeid.'&action=send';
			$formmail->show_form();
