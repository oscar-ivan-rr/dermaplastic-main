...
$masspaiement = GETPOST('masspaiement');
...
if ($masspaiement == 'true'){
    $massive_ids = array();
    foreach($_REQUEST as $clave=>$valor){
        $substring = substr($clave,0,8);
        if( $substring == 'ticketid'){
...
if ($action == 'confirm_paiement' && $confirm == 'yes')
{
    ...
    if (! $error)
    {
        ...
        if ($invoiceid > 0) 
        {
			...
			$sql ="UPDATE ".MAIN_DB_PREFIX."pos_ticket set customer_pay=".$totalpaye.",`difpayment`=(`total_ttc`-".$totalpaye.") WHERE rowid=".$invoiceid;
			$result = $db->query($sql);
    ...
    if ($result >= 0)
    {
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                print '<span class="fieldrequired" style="float:left; line-height:22px;">Recepción de pago</span>';
                print '<input type="number" step="any" id="general" val="" style="float:left; margin-left:5px;">';
                print '<div class="button" id="generar" style="float:left; cursor:pointer;">Generar pago</div>';
                print '</div>';
                print '<br>';
                ...
                while ($i < $num)
                {
					...
					//{
                        ...
                        // Checkpoint: guardar los montos en una cadena;
                        $montos .= $remaintopay.',';
	                    ...
	                    if ($action != 'add_paiement')
	                    {
                            ...
                            if ($masspaiement == 'true' and in_array($objp->ticketid, $massive_ids))
	                            print '<input type="text" size="8" class="monto" name="'.$namef.'" rel="'.$rel.'" value="'.$remaintopay.'">';
                            else
                                print '<input type="text" size="8" class="monto" name="'.$namef.'" rel="'.$rel.'" value="'.$_POST[$namef].'">';
	                    }
	                    else
	                    {
                ...
                if ($i > 1)
                {
                    ...
                    print '<td align="right"><b><span id="totP">0.0</span></b></td>';
                    print "</tr>\n";
                }
                print "</table>";
                //print "</td></tr>\n";
            ...
            print "
                    <script> 
                        $('#general').attr({'max':".($total_ttc - $totalrecu)."});
                        var cadena_montos = '$montos';
                        var montos = cadena_montos.split(',');
                        var i = 0;
                        ...
                        $('#generar').on('click', function(){
                            ...
                            m = parseFloat($('#general').val());
                            //alert(montos[i]);
                            while(m > 0) {
                                if(m <= parseFloat(montos[i])) {
                                    $('.monto').each(function(){
                                        if($(this).attr('rel') == i + 1) {
                                            $(this).val(m);
                                        }
                                    });
                                    m -= m;
                                    ...
                                    $('.monto').each(function(){
                                        if($(this).attr('rel') == i + 1) {
                                            $(this).val(parseFloat(montos[i]));
                                        }
                                    });
                                    m -= parseFloat(montos[i]).toFixed(2);
                                    m = m.toFixed(2);
                                    i++;
                                }
                            }
