...
class Payment extends CommonObject
{
	...
	function create($user,$soc=null)
	{
		...
		$payment->datepaye = $this->datepaye;
		$ref = $payment->getNextNumRef(is_object($soc)?$soc:'');
				...
				if (is_numeric($amount) && $amount <> 0)
				{
					...
					if ($resql)
					{
						$sql =	 'UPDATE llx_pos_ticket '
								.'SET	 difpayment = difpayment - '.$amount.' '
								.'		,customer_pay = customer_pay +'.$amount.' '
						...
						$sql =	 'UPDATE llx_pos_ticket '
								.'SET	 paye = IF (ABS(difpayment) <= 0.109, 1, 0) '
								.'WHERE rowid = '.$ticketid
								;
						$resql=$this->db->query($sql);
						if (!$resql)
