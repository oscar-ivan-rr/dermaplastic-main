...
if ($pending_print[0][0]=="F" || $pending_print[0][0]=="J")
{
	...
	$json_data['header_lines'] = dol_string_unaccent(str_pad(substr($langs->transnoentities("Label"),0,23),25,' ', STR_PAD_RIGHT).str_pad(substr($langs->transnoentities("Qty"),0,5),6,' ', STR_PAD_LEFT).str_pad(substr($langs->transnoentities("Total"),0,5),12,' ', STR_PAD_LEFT));
	$rows = array();
	...
	$json_data['total_ttc1'] = str_pad($langs->transnoentities("TotalTTC").":".price($facture->total_ttc)." ".$langs->trans(currency_name($conf->currency)),36,' ', STR_PAD_LEFT);
	$json_data['total_ttc2'] = str_pad($langs->transnoentities("TotalTTC").":".price($facture->total_ttc)." ".$langs->trans(currency_name($conf->currency)),36,' ', STR_PAD_RIGHT);
	$json_data['header_desg'] = dol_string_unaccent(str_pad(substr($langs->trans("TotalHT"),0,11),12,' ', STR_PAD_RIGHT).str_pad(substr($langs->trans("VAT"),0,11),12,' ', STR_PAD_RIGHT).str_pad(substr($langs->trans("TotalVAT"),0,11),12,' ', STR_PAD_LEFT));
	
	$desgs = array();
	if(! empty($subtotal)){
		foreach($subtotal as $totkey => $totval){
			if($totkey > 0){
				$desg = str_pad(price($subtotal[$totkey],"","","","",2),12,' ', STR_PAD_RIGHT).str_pad(price($totkey,"","","","",2)."% ",12,' ', STR_PAD_RIGHT).str_pad(price($subtotaltva[$totkey],"","","","",2),12,' ', STR_PAD_LEFT);
				$desgs[] = $desg;
			}
		}
	}
	...
	if($facture->total_localtax1!=0){
		$json_data['localtax1'] = $langs->transcountrynoentities("TotalLT1",$mysoc->country_code)." ".price($localtax1,"","","","",2)."% ".price($facture->total_localtax1,"","","","",2)." ".$langs->trans(currency_name($conf->currency));
	}
	else 
		$json_data['localtax1'] = "";
	...
	if (! empty($conf->rewards->enabled)){
		if ($moneypoints>0){
			$json_data['pays_point'] = dol_string_unaccent($langs->transnoentities("Bonification").': '.$usepoints." ".$langs->trans("Points")." ".price($moneypoints,"","","","",2)." ".$langs->trans(currency_name($conf->currency)));
		}
	}
	
	$json_data['customer_ret'] = dol_string_unaccent(($diff_payment<0?$langs->trans("CustomerRet"):$langs->trans("CustomerDeb"))." ".price(abs($diff_payment),"","","","",2)." ".$langs->trans(currency_name($conf->currency)));
	
	if ($points != 0 && ! empty($conf->rewards->enabled))
	{
		$json_data['total_points']= dol_string_unaccent($langs->trans("TotalPointsInvoice")." ".price($points,"","","","",2)." ".$langs->trans('Points'));
		$total_points = $rewards->getCustomerPoints($facture->socid);
		$json_data['dispo_points'] = dol_string_unaccent($langs->trans("DispoPoints")." ".price($total_points,"","","","",2)." ".$langs->trans('Points'));
	}
...
if ($pending_print[0][0]=="T" or $pending_print[0][0]=="G")
{
	...
	$json_data['total_ttc1'] = str_pad($langs->transnoentities("TotalTTC").":".price($ticket->total_ttc)." ".$langs->trans(currency_name($conf->currency)),36,' ', STR_PAD_LEFT);
	$json_data['total_ttc2'] = str_pad($langs->transnoentities("TotalTTC").":".price($ticket->total_ttc)." ".$langs->trans(currency_name($conf->currency)),36,' ', STR_PAD_RIGHT);
	$json_data['header_desg'] = dol_string_unaccent(str_pad(substr($langs->trans("TotalHT"),0,11),12,' ', STR_PAD_RIGHT).str_pad(substr($langs->trans("VAT"),0,11),12,' ', STR_PAD_RIGHT).str_pad(substr($langs->trans("TotalVAT"),0,11),12,' ', STR_PAD_LEFT));
	...
	if(! empty($subtotal)){
		foreach($subtotal as $totkey => $totval){
			if($totkey > 0){
				$desg = dol_string_unaccent(str_pad(price($subtotal[$totkey],"","","","",2),12,' ', STR_PAD_RIGHT).str_pad(price($totkey,"","","","",2)."% ",12,' ', STR_PAD_RIGHT).str_pad(price($subtotaltva[$totkey],"","","","",2),12,' ', STR_PAD_LEFT));
				$desgs[] = $desg;
			}
		}
	}
...
if ($pending_print[0][0]=="C") { 
	...
	$json_data['header_lines'] = str_pad($langs->transnoentities("Ticket"),18,' ', STR_PAD_RIGHT).str_pad($langs->transnoentities("Total"),18,' ', STR_PAD_LEFT);
	...
	if ($result)
	{
		...
		if($num>0)
		{
			...
			while ($i < $num)
			{
				...
				$row['label'] = str_pad($objp->ticketnumber,18,' ', STR_PAD_RIGHT).str_pad(price($objp->amount),18,' ', STR_PAD_LEFT);
				$i++;
	...
	$sql = "SELECT t.ticketnumber, p.amount, t.type";
	$sql .=" FROM ".MAIN_DB_PREFIX."pos_ticket as t, ".MAIN_DB_PREFIX."pos_paiement_ticket as pt, ".MAIN_DB_PREFIX."paiement as p";
	$sql .=" WHERE t.fk_control = ".$id." AND t.fk_cash=".$terminal." AND (p.fk_paiement=".$cash->fk_modepaybank." OR p.fk_paiement=".$cash->fk_modepaybank_extra." OR p.fk_paiement=".$cash->fk_modepaybank_extra_2.")AND t.fk_statut > 0";
	$sql .= " AND p.rowid = pt.fk_paiement AND t.rowid = pt.fk_ticket ";
	 
	$sql .= " UNION SELECT f.facnumber, p.amount, f.type";
	$sql .= " FROM ".MAIN_DB_PREFIX."pos_facture as pf,".MAIN_DB_PREFIX."facture as f, ".MAIN_DB_PREFIX."paiement_facture as pfac, ".MAIN_DB_PREFIX."paiement as p ";
	$sql .= " WHERE pf.fk_control_cash = ".$id." AND pf.fk_cash=".$terminal." AND (p.fk_paiement=".$cash->fk_modepaybank." OR p.fk_paiement=".$cash->fk_modepaybank_extra." OR p.fk_paiement=".$cash->fk_modepaybank_extra_2.") AND pf.fk_facture = f.rowid and f.fk_statut > 0";
	$sql .= " AND p.rowid = pfac.fk_paiement AND f.rowid = pfac.fk_facture";
	...
	if(!empty($conf->rewards->enabled)){
		...
		if ($result)
		{
			...
			if($num>0)
			{
				...
				{
					$objp = $db->fetch_object($result);
					$row['label'] = str_pad($objp->facnumber,18,' ', STR_PAD_RIGHT).str_pad(price($objp->amount),18,' ', STR_PAD_LEFT);
					$i++;
