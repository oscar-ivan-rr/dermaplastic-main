...
$sql .= " FROM ".MAIN_DB_PREFIX.$object->table_element." as t JOIN llx_const lc ON t.posnumber = REPLACE (lc.name , 'CASHDESK_ID_WAREHOUSE', '') ";
if (is_array($extrafields->attributes[$object->table_element]['label']) && count($extrafields->attributes[$object->table_element]['label'])) $sql .= " LEFT JOIN ".MAIN_DB_PREFIX.$object->table_element."_extrafields as ef on (t.rowid = ef.fk_object)";
if ($object->ismultientitymanaged == 1) $sql .= " WHERE t.entity IN (".getEntity($object->element).")";
else $sql .= " WHERE 1 = 1";
if(!$user->rights->stock->show_all_warehouses) $sql.= " AND lc.value = ".$user->fk_warehouse."";
...
if ($sortfield == 't.ref') {
	$sql .= ' ORDER BY CAST(' . $sortfield . ' AS UNSIGNED) ' . $sortorder;
} else {
...
while ($i < min($num, $limit))
{
	...
	foreach ($object->fields as $key => $val)
	{
		...
		if (!empty($arrayfields['t.'.$key]['checked']))
		{
			...
			elseif ($key == 'posnumber') print $conf->global->{"CASHDESK_NAME".$obj->$key};
