...
require_once DOL_DOCUMENT_ROOT.'/cfdimx/lib/numero_a_letra.php';
...
if (empty($reshook))
{
	...
	/** Datos de configuracion de CFDI */
	...
	// Action Generar CFDI
	if (isset($action) && $action == "generaCFDI") {
		foreach ($_REQUEST as $key => $value) {
			//echo $key .'=>'. $value.'<br>';
			$$key = $value;
		}
		include( DOL_DOCUMENT_ROOT ."/cfdimx/generaDesdeFactureCardCFDI.php");
	}
	/** Mensaje final cfdi */
    ...
    else if($action =='apply_line_discount' && $object->statut==Facture::STATUS_DRAFT && $usercancreate ){
        if(GETPOST('discount_percent')!=null){
            if($user->posdesc < GETPOST('discount_percent')){
                setEventMessages('El descuento no puede ser mayor al descuento que se puede asignar por el usuario ('.$user->posdesc.'%)', null, 'errors');
            }else {
                foreach ($object->lines as $line) {
                    $object->updateline($line->id, $line->desc, $line->subprice, $line->qty, GETPOST('discount_percent'), $line->date_start, $line->date_end, $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, 'HT', $line->info_bits, $line->product_type, $line->fk_parent_line, 0, $line->fk_fournprice, $line->pa_ht, $line->label, $line->special_code, $line->array_options, $line->situation_percent, $line->fk_unit, $line->pu_ht);
                }
            }
        }else{
	...
	elseif ($action == 'setcommission_porcent' && $usercancreate)
	{
		$object->fetch($id);
		$object->setcommission_porcent(GETPOST('commission_porcent'));
	}

	//Delete Massive Products
	elseif ($action == 'confirmDelete' && $usercancreate)
	{
	...
	elseif ($action == 'confirm_valid' && $confirm == 'yes' && $usercanvalidate)
	{
		...
		if(GETPOST('ref_type') == 'PFA') $forcePFA = true;
		else $forcePFA = false;
		...
		if (!$error)
		{
			$result = $object->validate($user, '', $idwarehouse, 0, $forcePFA);
			if ($result >= 0)
			{
				// Define output language
				if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
		...
		if(GETPOST("uuid") == ""){
			...
			if($resultado["return"] != ""){
				if($resultado["return"]["httpStatusCode"] == 200){
					$cancela_fact = "UPDATE " . MAIN_DB_PREFIX . "facture SET close_code = 'abandon', close_note = 'CFDI Cancelado', fk_statut = 3 WHERE rowid = " . $_REQUEST["facid"];
					...
					/** Genera XML de cancelacion **/
					$xml_response = $resultado["return"];
					$dir_path = DOL_DATA_ROOT."/facture/".$object->ref;
					if(!file_exists($dir_path)) 
					{
						mkdir($dir_path, 0777, true);                                
					}
					file_put_contents($dir_path."/acuse_cancelacion.xml", $xml_response);

					/** Genera PDF de cancelacion **/
					// include_once("../../cfdimx/generaPDFCancelacion.php");
					...
					setEventMessages($msg_cfdi_final, null, 'errors');
	...
	elseif ($action == 'add' && $usercancreate)
	{
		...
		if ($id > 0 && !$error)
		{
			...
			$devolucion = GETPOST('devolucion');

			if($devolucion == '1'){
				header('Location: '.$_SERVER["PHP_SELF"].'?contextpage=poslist&facid='.$id.'&action=valid');
			} else header('Location: '.$_SERVER["PHP_SELF"].'?facid='.$id);
			exit();
		}
		else
		{
	...
	elseif ($action == 'addline' && $usercancreate)
	{
		...
		if (!$error && ($qty >= 0) && (!empty($product_desc) || !empty($idprod))) {
			...
			if (!empty($idprod))
			{
				...
				$tva_tx = ($datapriceofproduct['tva_tx'] ? $datapriceofproduct['tva_tx'] : $tva_tx);
	...
	elseif ($action == 'updateline' && $usercancreate && !GETPOST('cancel', 'alpha'))
	{
		...
		if (!empty($productid))
		{
			...
			// Ignorar IVA si el producto es exento de IVA
			$vat_rate = $product->exentoiva ? 0 : $vat_rate;
        ...
        if($user->posdesc < GETPOST('remise_percent')){
            setEventMessages('El descuento no puede ser mayor al descuento que se puede asignar por el usuario ('.$user->posdesc.'%)', null, 'errors');
            $error++;
        }
	...
	elseif ($action=='create_commande')
	{
		if ($object->origin == 'commande' || isset($objects->linked_objects['commande']) )
		{
		...
		$rc_commande = new Commande($db);
		$rc_commande->socid = $object->socid;
		$rc_commande->ref_client = $object->ref_client;
		$rc_commande->ref_int = $object->ref_int;
		$rc_commande->contactid = null;
		$rc_commande->billed = 1;
		$rc_commande->brouillon = null;
		$rc_commande->cond_reglement_code = $object->cond_reglement_code;
		$rc_commande->fk_account = $object->fk_bank;
		$rc_commande->mode_reglement = null;
		$rc_commande->mode_reglement_code = $object->mode_reglement_code;
		$rc_commande->mode_reglement_id = null;
		$rc_commande->availability_id = null;
		$rc_commande->availability_code = null;
		$rc_commande->availability = null;
		$rc_commande->date = $object->date;
		$rc_commande->date_commande = $object->date;
		$rc_commande->date_livraison = null; // $object->date_lim_reglement;
		$rc_commande->cond_reglement_id = $object->cond_reglement_id;
		$rc_commande->linked_objects['facture'] = $object->id;
		
		$cmid = $rc_commande->create($user,0);

		foreach ($object->lines as $iLine)
		{
			$rc_commande->addline(
							$iLine->desc
							,$iLine->subprice
							...
							,0
							,$iLine->fk_product
							,$iLine->remise_percent
							,0
							,$iLine->fk_remise_except
							,'HT'
							,$iLine->total_ttc
							,''
							,''
		...
		$object->origin_id = $cmid;
		$objects->linked_objects['commande'] = $cmid;
		header("Location: ".DOL_URL_ROOT.'/commande/card.php?mainmenu=commercial&leftmenu=orders&id='.$cmid);
       	exit();
...
if ($action == 'create')
{
    ...
    print '<style>.select2-selection{min-width: 250px;}</style>';
	...
	print '<input type="hidden" name="devolucion" value="'.GETPOST('return', 'int').'">';
	...
	if ($soc->id > 0 && (!GETPOST('fac_rec', 'int') || !empty($invoice_predefined->frequency)))
	{
		...
		$sql_extrafields ="SELECT e.formpagcfdi,e.usocfdi FROM ".MAIN_DB_PREFIX."societe_extrafields as e WHERE e.fk_object =".$soc->id;
		$result_extrafields = $db->query($sql_extrafields);
		if ($result_extrafields){
			$data_extrafields = $db->fetch_object($result_extrafields);
			$pagoCFDI = $data_extrafields->formpagcfdi;
			$usoCFDI = $data_extrafields->usocfdi;
		}
		print '<script type="text/javascript">
			$(document).ready(function() {
				$("#options_formpagcfdi").val("'.$pagoCFDI.'");
				$("#options_usocfdi").val("'.$usoCFDI.'");
			});
			</script>';
	}
	else
		...
		{*/
			...
			$(document).ready(function() {
				$("#socid").change(function() {
        			...
        			$.ajax("'.DOL_URL_ROOT.'/core/ajax/ajaxsocfacture.php", {
                        type: "post",   // Usually post or get
                        async: true,
                        data: {soc: socid},
                        success: function (result) {
                            result = JSON.parse(result);
                            $("#cond_reglement_id").val(result.cond);
                            $("#selectmode_reglement_id").val(result.mode);
                            $("#selectfk_account").val(result.bank);
                            $("#options_formpagcfdi").val(result.pag);
                            $("#options_usocfdi").val(result.uso);
                        }
				    });
			    });
			});
	...
	print '<tr><td class="tdtop fieldrequired dropdown-toggle" id="invoice-type-toggle" style="cursor: pointer;">'.$langs->trans('Type').'</td><td colspan="2">';

	print '<div class="tagtable" id="invoice-type-area" style="display: none;">'."\n";
	...
	print '
	<!-- Toggle invoices type area -->
	<script>
	$(document).ready(function() {
		$("#invoice-type-toggle").on("click", function(event) {
			$("#invoice-type-area").fadeToggle("slow");
	...
	$form->select_types_paiements(
		// Pago en cheque si ninguna opción está definida
		isset($_POST['mode_reglement_id']) ? $_POST['mode_reglement_id'] : ( $mode_reglement_id ? $mode_reglement_id : 7 ),
		'mode_reglement_id',
		'CRDT',
		0, 1, 0, 0, 1, '', 0, 0
	);
	...
	$doleditor = new DolEditor('note_public', $note_public, '', 80, 'dolibarr_notes', 'In', 0, false, true, ROWS_3, '90%', 0 , 800);
	...
	if (empty($user->socid))
	{
		...
		print '</td>';
		print '<td valign="top" colspan="2">';
		$doleditor = new DolEditor('note_private', $note_private, '', 80, 'dolibarr_notes', 'In', 0, false, true, ROWS_3, '90%', 0 , 800);
		print $doleditor->Create(1);
...
elseif ($id > 0 || !empty($ref))
{
	...
	//* TODO: Agregar funcionalidad para verificar si se tiene forma de pago y uso de cfdi para poder facturar
	...
	if ($action == 'valid')
	{
		...
		if ($object->type != Facture::TYPE_DEPOSIT /*&& !empty($conf->global->STOCK_CALCULATE_ON_BILL)*/)
		{
			...
			$from_ticket = false;
			foreach($object->linked_objects as $lob)
			{
				if ($lob->element == 'ticket')
				{
					$from_ticket = true;
					break;
				}
			...
			if ($qualified_for_stock_change && !$from_ticket && $object->type == Facture::TYPE_CREDIT_NOTE)
			{
				$langs->load("stocks");
				require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
				...
				if (count($warehouse_array) == 1) {
					...
					$value = $formproduct->selectWarehouses(GETPOST('idwarehouse') ?GETPOST('idwarehouse') : $user->fk_warehouse, 'idwarehouse', '', 1);
				}
				$formquestion = array(
									// 'text' => $langs->trans("ConfirmClone"),
		...
		if (GETPOST('contextpage')=='poslist'){
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?contextpage=poslist&facid='.$object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), 2);
		}else $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?facid='.$object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), 2);
	}

	// Confirmation of Assign Reference to Facture
	if ($action == 'asign_ref')
	{
		$formquestion = array(
			array('type' => 'radio', 'name' => 'ref_type', 'values' => array('PFA' => 'Proyecto (PFA)', 'Normal' => 'Normal (MFA/GFA)'))
		);
		
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?facid='.$object->id, $langs->trans('SelectRef'), $text, 'confirm_valid', $formquestion, 'yes', 2, 170);
	}
	...
	if ($action == 'modif') {
		...
		$select = "SELECT count(*) as count FROM `".MAIN_DB_PREFIX."element_element` WHERE ";
		$select .= "`fk_source` = $object->id AND `sourcetype` = \"facture\" AND `targettype` = \"ticket\"";
		$reselect = $db->query($select);
		$count = $db->fetch_object($reselect)->count;
		if ($object->type == Facture::TYPE_CREDIT_NOTE && $count == 0)
	...
	if ($action == 'canceled') {
		...
		if ($objectidnext && $statusreplacement == 0) {
			...
			// Hacemos una consulta para saber si la factura está timbrada o no. Esto se define por si existe en la tabla cfdimx
			$sql = "SELECT * FROM  " . MAIN_DB_PREFIX . "cfdimx WHERE fk_facture = " . $_REQUEST["facid"];
			$resql = $db->query($sql);
			$result_cfdi = $db->fetch_object($resql);
			...
			if(!$result_cfdi){
				$formquestion = array('text' => $langs->trans("ConfirmCancelBillQuestion"),
				...
				$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'].'?facid='.$object->id. '&id=' . $object->id.'&rfc_emisor='.$conf->global->MAIN_INFO_SIREN.'&uuid='.$result_cfdi->uuid, $langs->trans('CancelBill'),'<br>'. $langs->trans('ConfirmCancelBill', $object->ref), 'confirm_canceled', $formquestion, 0 ,1, 260, 550);
			}else{
				// Cree un tableau formulaire
				$formquestion = array('text' => $langs->trans("ConfirmCancelBillQuestion"),
									array('type' => 'radio', 'name' => 'close_code', 'label' => $langs->trans("Reason"), 'values' => $arrayreasons), 
									...
									// array('type' => 'onecolumn', 'value' => '<br><strong>'.$pregunta.'</strong>'),
									array('type' => 'other', 'label' =>'<br><strong>'.$pregunta.'</strong>'),
				...
				$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'].'?facid='.$object->id. '&id=' . 	$object->id.'&rfc_emisor='.$conf->global->MAIN_INFO_SIREN.'&uuid='.$result_cfdi->uuid, 	$langs->trans('CancelBill'),'<br>'. $langs->trans('ConfirmCancelBill', $object->ref), 'confirm_canceled', $formquestion, 0 ,1, 500, 750);
			}
		}
	}
	...
	// Thirdparty
	$morehtmlref .= '<br>'.$langs->trans('Customer').' : '.$object->thirdparty->getNomUrl(1, 'customer');
	...
	if ($user->rights->facture->pay_commission) {
		$morehtmlref .= $form->editfieldkey("Comisión", 'commission_porcent', price($object->commission_porcent), $object, $usercancreate, 'numeric', '', 0, 1);
		$morehtmlref .= $form->editfieldval("Comisión", 'commission_porcent', price($object->commission_porcent), $object, $usercancreate, 'string', '', null, null, '', 1)."%";
		$morehtmlref .= "&nbsp;&nbsp;&nbsp;( $". price($object->commission)." )";
	}else{
		$morehtmlref .= "Comisión: ";
		$morehtmlref .= price($object->commission_porcent)."%";
		$morehtmlref .= "&nbsp;&nbsp;&nbsp;( $". price($object->commission)." )";
	}
	...
	print '</td>';
	if (($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && !empty($object->brouillon) && $usercancreate && empty($conf->global->FAC_FORCE_DATE_VALIDATION)) || ($object->type == Facture::TYPE_CREDIT_NOTE && $user->rights->facture->creditnote_changedate))
	...
	if ($object->type != Facture::TYPE_CREDIT_NOTE) {
        ...
        if ($action == 'editinvoicedate' && $user->rights->facture->creditnote_changedate) {
            $form->form_date($_SERVER['PHP_SELF'].'?facid='.$object->id, $object->date, 'invoicedate');
        } else {
            print dol_print_date($object->date, 'day');
        }
	...
	// Fecha de Timbrado (sin posibilidad de edición)
	$sql = "SELECT * FROM  " . MAIN_DB_PREFIX . "cfdimx WHERE fk_facture = " . $object->id;
	$resql = $db->query($sql);
	$obj = $db->fetch_object($resql);
	print '<tr><td>';
	print '<table class="nobordernopadding" width="100%"><tr><td>';
	print $langs->trans('Fecha Timbrado');
	print '</td>';
	print '</tr></table>';
	print '</td><td>';
	print $obj->fecha_timbrado . ' ' . $obj->hora_timbrado;
	print '</td>';
	print '</tr>';
	...
	if ($object->type != Facture::TYPE_CREDIT_NOTE) {
		...
		// Remainder to pay
		print '<tr><td colspan="'.$nbcols.'" class="right">';
		print '<b>'.$langs->trans('RemainderToPay').'</b>';
		if ($resteapayeraffiche < 0)
		    print '<span class="amountremaintopay"> ('.$langs->trans('ExcessReceived').')</span>';
		print ' :</td>';
	...
	if (!empty($object->lines) && ($object->statut == 0)) {
		//Eliminar Productos
					...
					buttons: [
						{
							text: \"Si\",
							click: function() {
									var l = '".DOL_URL_ROOT."/compta/facture/card.php?id=".$id."&action=confirmDelete&lines=';
									var q = [];
	...
	if (!empty($conf->multicurrency->enabled) && ($object->multicurrency_code != $conf->currency))
	{
		...
		print '<td class="nowrap amountcard center">'.price($object->multicurrency_total_ht, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';

		// Multicurrency Amount VAT
		print '<tr><td class="center">'.$form->editfieldkey('MulticurrencyAmountVAT', 'multicurrency_total_tva', '', $object, 0).'</td>';
		print '<td class="nowrap amountcard center">'.price($object->multicurrency_total_tva, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';

		// Multicurrency Amount TTC
		print '<tr><td class="center">'.$form->editfieldkey('MulticurrencyAmountTTC', 'multicurrency_total_ttc', '', $object, 0).'</td>';
		print '<td class="nowrap amountcard center">'.price($object->multicurrency_total_ttc, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';
	}
	...
	foreach ($object->lines as $line) {
		$subtotal += $line->subprice * $line->qty;
		$discount += ($line->remise_percent * ($line->subprice * $line->qty)) / 100;
	}
	// Amount
	print '<tr><td class="titlefieldmiddle center">'.$langs->trans('AmountHT').'</td>';
	print '<td class="nowrap amountcard center">'.price($subtotal, 1, '', 0, - 1, - 1, $conf->currency).'</td></tr>';

	// Discount
	print '<tr><td class="titlefieldmiddle center">'.$langs->trans('Reductions').'</td>';
	print '<td class="center">'.price($discount, 1, '', 0, - 1, - 1, $conf->currency).$alert.'</td>';

	//Subtotal con Descuento
    print '<tr><td class="titlefieldmiddle center">Subtotal con Descuento</td><td class="center">'.price($subtotal - $discount, 1, '', 0, - 1, - 1, $conf->currency).'</td></tr>';

	// Vat
	print '<tr><td class="center">'.$langs->trans('AmountVAT').'</td><td colspan="3" class="nowrap amountcard center">'.price($object->total_tva, 1, '', 0, - 1, - 1, $conf->currency).'</td></tr>';
	print '</tr>';

	// Amount Local Taxes
	if (($mysoc->localtax1_assuj == "1" && $mysoc->useLocalTax(1)) || $object->total_localtax1 != 0) 	// Localtax1
	{
		print '<tr><td class="center">'.$langs->transcountry("AmountLT1", $mysoc->country_code).'</td>';
		print '<td class="nowrap amountcard center">'.price($object->total_localtax1, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
	}
	if (($mysoc->localtax2_assuj == "1" && $mysoc->useLocalTax(2)) || $object->total_localtax2 != 0) 	// Localtax2
	{
		print '<tr><td class="center">'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td>';
		print '<td class=nowrap amountcard center">'.price($object->total_localtax2, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
	}
	...
	// Total with tax
	print '<tr><td class="center">'.$langs->trans('AmountTTC').'</td><td class="nowrap amountcard center">'.price($object->total_ttc, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline')
	{
		...
		if (empty($reshook)) {
			...
			if ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT)
				...
				&& $user->rights->facture->invoice_advance->reopen)				// A paid invoice (partially or completely)
			{
			...
			// Crear pedido
			//die(var_dump($object->paye == 1));
			if ($object->type == Facture::TYPE_STANDARD
				//&& ($object->statut == Facture::STATUS_CLOSED  || ($object->statut == 1 && $object->paye == 1))   
				&& ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $usercancreate) || $usercanreopen))
			{
					print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=create_commande">'.$langs->trans('Crear Pedido').'</a>';
			}
			...
			if ($object->statut == Facture::STATUS_DRAFT && count($object->lines) > 0 && ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA || $object->type == Facture::TYPE_SITUATION) && (!empty($conf->global->FACTURE_ENABLE_NEGATIVE) || $object->total_ttc >= 0)) || ($object->type == Facture::TYPE_CREDIT_NOTE && $object->total_ttc <= 0))) {
				if ($usercanvalidate)
				{
					/** Skiping confirmation for every facture unless is a credit note */
					if($object->type == Facture::TYPE_CREDIT_NOTE){
						...
						if(empty($object->fk_project) && $user->rights->facture->edit_ref){
							print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&amp;action=asign_ref">'.$langs->trans('Validate').'</a>';
						}else{
							print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&amp;action=confirm_valid&amp;confirm=yes">'.$langs->trans('Validate').'</a>';
						}					
					}
			...
			/** Enabling "Cancelation" for Credit notes */
			if ($object->statut == Facture::STATUS_VALIDATED && $object->type == Facture::TYPE_CREDIT_NOTE && $usercanissuepayment)
				print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?facid='.$object->id.'&amp;action=canceled">'.$langs->trans('Cancel').'</a>';
...
print '<script>';
print '
	$("#all_lines").change(function (){
		if ($(this).is(":checked")) {
			$(".product_discount").prop("checked",true);
		} else {
			$(".product_discount").prop("checked",false);
		}
	})';
print '</script>';
