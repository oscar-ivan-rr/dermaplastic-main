...
//$data2 = $stats->getTicketsByMonthWithPrevYear($endyear, $startyear);
// $data = array(array('Lib',val1,val2,val3),...)
...
if (! $mesg)
{
	...
	$px2->mode='depth';
	$px2->SetTitle($langs->trans("Importe total de las facturas por mes (Sin IVA)"));
...
if (! $mesg)
{
    ...
    $px3->mode='depth';
    $px3->SetTitle($langs->trans("Importe Promedio (Con IVA) "));

    $px3->draw($filename_avg, $fileurl_avg);
}

// Build graphic amount of object
$data = $stats->getTicketsByMonthWithPrevYear($endyear, $startyear);
// $data = array(array('Lib',val1,val2,val3),...)

$filenameticket = $dir."/ticketamountinyear-".$year.".png";
if ($mode == 'customer') $fileurlticket = DOL_URL_ROOT.'/viewimage.php?modulepart=billstats&amp;file=ticketamountinyear-'.$year.'.png';
if ($mode == 'supplier') $fileurlticket = DOL_URL_ROOT.'/viewimage.php?modulepart=billstatssupplier&amp;file=ticketamountinyear-'.$year.'.png';
...
if (! $mesg)
{
	...
	$px4->SetMaxValue($px2->GetCeilMaxValue());
	$px4->SetMinValue(min(0, $px2->GetFloorMinValue()));
	$px4->SetWidth($WIDTH);
	$px4->SetHeight($HEIGHT);
	$px4->SetYLabel($langs->trans('Importe de Tickets'));
	$px4->SetShading(3);
	$px4->SetHorizTickIncrement(1);
	$px4->mode='depth';
	$px4->SetTitle($langs->trans("Importe total de tickets no facturados por mes"));
	$px4->draw($filenameticket, $fileurlticket);
}
