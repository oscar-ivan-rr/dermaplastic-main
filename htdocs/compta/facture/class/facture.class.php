...
class Facture extends CommonInvoice
{
     ...
     */
    public $total_ttc_before_discount;
	...
	/**
	 * @var int $commission_porcent
	 */
	public $commission_porcent;
	...
	/**
	 * @var int $medical_condition
	 */
	public $medical_condition;

	/**
	 * @var Date $date_timbre
	 */
	public $date_timbre;
	...
	public $fields = array(
		...
		'commission_porcent' =>array('type'=>'double(24,8)', 'label'=>'commission_porcent', 'enabled'=>1, 'visible'=>-2, 'position'=>220),
		'commission' =>array('type'=>'double(24,8)', 'label'=>'commission', 'enabled'=>1, 'visible'=>-2, 'position'=>220),
	);
	...
	public function estaTimbrada()
	{
		$resql = $this->db->query("SELECT cancelado FROM  " . MAIN_DB_PREFIX . "cfdimx_cancelado WHERE fk_facture = " . $this->id);
		if ($resql) {
			$obj = $this->db->fetch_object($resql);
			if (empty($obj)) {
		    	return 1; // sin timbrar
			}
		    if ($obj->cancelado == 1) {
		    	return 3; // cancelada
		    }
	 ...
	 * 
	 */
	public function regeneraTimbradoconXML()
	{
		require DOL_DOCUMENT_ROOT . '/cfdimx/regenFromXMLFactureList.php';
	}
    ...
    public function create(User $user, $notrigger = 0, $forceduedate = 0)
	{
		...
		$sql .= ", ".($this->medical_condition ? $this->medical_condition : 0);
		$sql .= ", ".($this->fk_facture_fourn ? "'".$this->db->escape($this->fk_facture_fourn)."'" : "null");
		$sql .= ", ".($this->date_timbre ? "'".$this->db->idate($this->date_timbre)."'" : "null");
		$sql .= ")";
		...
		if ($resql)
		{
			...
			if (!$error)
			{
				...
				if ($result > 0)
				{
					...
					{
						$this->db->commit();
						// Aqui se confirma la creación y todos los valores calculados
                        // Enviaremos una notificacion con los datos TODO: hacerlo desde hooks
                        include_once DOL_DOCUMENT_ROOT . "/compta/facture/admin/notifications/notification.class.php";
                        $notif = new Notification();

                        $url = DOL_MAIN_URL_ROOT . "/compta/facture/card.php?facid=$this->id";
                        $tipo = $this->type;

                        switch ($tipo){
                            case self::TYPE_STANDARD:
                                $tipo = $langs->trans("InvoiceStandard");
                            break;
                            case self::TYPE_REPLACEMENT:
                                $tipo = $langs->trans("InvoiceReplacement");
                            break;
                            case self::TYPE_CREDIT_NOTE:
                                $tipo = $langs->trans("InvoiceAvoir");
                            break;
                            case self::TYPE_DEPOSIT:
                                $tipo = $langs->trans("InvoiceDeposit");
                            break;
                        ...
                        $fecha = dol_print_date(dol_now('tzuser'));
                        $soc = new Societe($this->db);
                        $soc->fetch($this->socid);
                        $cliente = $soc->name;
                        $monto = $this->total_ttc;

                        $notif->send_slack_notification($this->ref, $url, $tipo, $fecha, $cliente, $this->total_ttc);
						return $this->id;
					}
					else
					{
	 ...
	 *	@return		int						<0 if KO, >0 if OK
	 */
    public function createDiscountCreditNote(User $user, $invertdetail = 0,$discount = 0)
	{
		global $conf;
		...
		$facture->note_private      = $this->note_private.' Nota de Crédito generada automáticamente para la factura '.$this->ref;
		...
		foreach ($facture->lines as $i => $tmpline)
		{
			...
			if ($invertdetail)
			{
				$facture->lines[$i]->subprice  = -$facture->lines[$i]->subprice*$discount;
				$facture->lines[$i]->total_ht  = -$facture->lines[$i]->total_ht*$discount;
				$facture->lines[$i]->total_tva = -$facture->lines[$i]->total_tva*$discount;
				$facture->lines[$i]->total_localtax1 = -$facture->lines[$i]->total_localtax1*$discount;
				$facture->lines[$i]->total_localtax2 = -$facture->lines[$i]->total_localtax2*$discount;
				$facture->lines[$i]->total_ttc = -$facture->lines[$i]->total_ttc*$discount;
				$total_lines += $facture->lines[$i]->total_ttc;
			}
		}
		$facture->lines = array();
		$facid = $facture->create($user);
		$lres = $facture->addline('Bonificación por Descuento',$total_lines,1,16,0,0,0,0,'','',0,0,'','TTC',$total_lines,1);
		...
		$facture->add_object_linked('facture', $this->id);
		//$this->add_object_linked('facture', $facture->id);
        ...
        $amounts = array($this->id => abs($total_lines) , $facture->id=>-1*abs($total_lines) );
        $multicurrency_amounts = array();
        
	    $paiement = new Paiement($this->db);
	    $paiement->datepaye     = dol_mktime(12, 0, 0, GETPOST('remonth', 'int'), GETPOST('reday', 'int'), GETPOST('reyear', 'int'));;
	    $paiement->amounts      = $amounts; // Array with all payments dispatching with invoice id
	    $paiement->multicurrency_amounts = $multicurrency_amounts; // Array with all payments dispatching
	    $paiement->paiementid   = dol_getIdFromCode($this->db, GETPOST('paiementcode'), 'c_paiement', 'code', 'id', 1);
	    ...
	    if (!$error)
	    {
	        ...
	        //setEventMessages('Creando NC para '.$this->ref.' de '.$thirdparty->nom.' pago '.$paiement->ref,'msg');
	    }
	...
	public function createFromClone(User $user, $fromid = 0)
	{
		...
		// Clear fields
		$object->date               = (empty($this->date) ? dol_now('tzuser') : $this->date);
    ...
    public function createFromOrder($object, User $user)
	{
		...
		for ($i = 0; $i < $num; $i++)
		{
			...
			$line->tva_tx = (empty($object->lines[$i]->tva_tx))?16:$object->lines[$i]->tva_tx;
    ...
    public function getNomUrl($withpicto = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $addlinktonotes = 0, $save_lastsearch_value = -1, $target = '')
	{
		...
		// Iconos para estado de timbrado
		$icons = array(
			'fa-circle',
			'fa-circle-o',
			'fa-dot-circle-o',
		...
		$key = $this->estaTimbrada();
		$result .= ' <i style="opacity:70%;'
			.$stles[$key]
			.'" class="classfortooltip fa '
			.$icons[$key].'" title="'
			.$labls[$key]
			.'"></i> ';
    ...
    public function fetch($rowid, $ref = '', $ref_ext = '', $ref_int = '', $fetch_situation = false)
	{
		...
		$sql .= ", f.total_ttc_before_discount, discount_applied";
		...
		$sql .= ", f.commission_porcent, f.commission";
		$sql .= ", f.medical_condition, f.fk_facture_fourn";
		$sql .= ", f.date_timbre";
		...
		if ($result)
		{
			if ($this->db->num_rows($result))
			{
				...
				$this->commission_porcent  = $obj->commission_porcent;
				$this->commission  = $obj->commission;
				$this->medical_condition  = $obj->medical_condition;
				...
				$this->date_timbre = $obj->date_timbre;
	...
	public function fetch_lines($only_product = 0, $loadalsotranslation = 0)
	{
		...
		$sql .= ' l.note,';
		$sql .= ' p.ref as product_ref, p.fk_product_type as fk_product_type, p.label as product_label, p.description as product_desc,p.tosell,p.tobuy';
    ...
    public function update(User $user, $notrigger = 0)
	{
		...
		// Discounted
        if (empty($this->total_ttc_before_discount)) $this->total_ttc_before_discount = 0;
        if (empty($this->discount_applied)) $this->discount_applied = 0;
        ...
        $sql .= " total_ttc_before_discount=".(isset($this->total_ttc_before_discount)? $this->db->escape($this->total_ttc_before_discount):0).",";
        $sql .= " discount_applied=".(isset($this->discount_applied)?$this->db->escape($this->discount_applied):0).",";
		if(isset($this->commission)) $sql .= " commission=".$this->commission.",";
		if(isset($this->commission_porcent)) $sql .= " commission_porcent=".$this->commission_porcent;
		if(isset($this->medical_condition)) $sql .= " , medical_condition=".$this->medical_condition;

		if(isset($this->date_timbre)) $sql .= " , date_timbre=".$this->db->idate($this->date_timbre);
	...
	/***
	 * Calculate commission of the facture
	 */
	public function calculate_commission($notrigger = 0){
		// phpcs:enable
	    global $user;
		...
		$this->db->begin();
		if(!empty($this->commission_porcent)){
			$sql = 'UPDATE '.MAIN_DB_PREFIX.'facture';
			$sql .= ' SET commission = \''.($this->commission_porcent*$this->multicurrency_total_ttc / 100).'\'';
			$sql .= ' WHERE rowid = '.$this->id;

			dol_syslog(__METHOD__.' this->id='.$this->id.', commission_porcent='.$this->commission_porcent, LOG_DEBUG);
			$resql = $this->db->query($sql);
			if (!$resql)
			{
			...
			if (!$error)
			{
				$this->commission = $this->commission_porcent * $this->multicurrency_total_ttc;
			}
	 ...
	 *	Set customer ref
	 *
	 *	@param     	string	$commission_porcent		Commission porcent
	 *  @param     	int		$notrigger		1=Does not execute triggers, 0= execute triggers
	 *	@return		int						<0 if KO, >0 if OK
	 */
    public function setcommission_porcent($commission_porcent, $notrigger = 0)
	{
        // phpcs:enable
	    global $user;
		...
		$this->db->begin();
		if(!empty($commission_porcent)){
			$sql = 'UPDATE '.MAIN_DB_PREFIX.'facture';
			$sql .= ' SET commission_porcent = \''.$commission_porcent.'\'';
			$sql .= ' WHERE rowid = '.$this->id;

			dol_syslog(__METHOD__.' this->id='.$this->id.', commission_porcent='.$commission_porcent, LOG_DEBUG);
			$resql = $this->db->query($sql);
			if (!$resql)
			{
				$this->errors[] = $this->db->error();
			...
			if (!$error)
			{
				$this->commission_porcent = $commission_porcent;
			}
    ...
    public function delete($user, $notrigger = 0, $idwarehouse = -1)
	{
		...
		if($this->statut > 0){
			//Restar Saldo por Pagar en Pedidos
			if (!$error)
			{
				//Cambiar estado de Pedido a Facturado
				$commArray = array();
				$sqlCommD = "SELECT ee.rowid, IF(ee.sourcetype = 'commande', ee.fk_source, ee.fk_target) AS pedido
				FROM ".MAIN_DB_PREFIX."element_element AS ee
					WHERE 
						(ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_source = ".$this->id.") 
						OR 
						(ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_target = ".$this->id.")";
				$resCommD = $this->db->query($sqlCommD);
				if ($resCommD){
					$numCommD = $this->db->num_rows($resCommD);
					if ($numCommD > 0){
						$iCommD = 0;
						while ($iCommD < $numCommD)
						{
							$objCommD = $this->db->fetch_object($resCommD);
							$slqUpCommD = "UPDATE ".MAIN_DB_PREFIX."commande SET billedRest = (billedRest + ".$this->total_ht.") WHERE rowid = ".$objCommD->pedido;
							$resuPCommD = $this->db->query($slqUpCommD);
							if(!$resuPCommD){
								$error++;
							}
							array_push($commArray,$objCommD->pedido);
							$iCommD++;
						}
					}
				}else{
		...
		//QUITAR ESTADO FACTURADO DE PEDIDOS RELACIONADOS SI YA NO SE TIENEN FACTURAS
		if (!$error)
		{
			foreach ($commArray as $rowidComm) {

				$sqlCommDC = "SELECT ee.rowid
						FROM ".MAIN_DB_PREFIX."element_element AS ee, ".MAIN_DB_PREFIX."facture AS f
							WHERE 
								(ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_source = f.rowid AND f.fk_statut > 0 AND ee.fk_target = ".$rowidComm.") 
								OR 
								(ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_target = f.rowid AND f.fk_statut > 0 AND ee.fk_source = ".$rowidComm.")";
				$resCommDC = $this->db->query($sqlCommDC);
				if ($resCommDC){
					$numCommDC = $this->db->num_rows($resCommDC);
					if ($numCommDC == 0){
						$slqUpCommFacC = "UPDATE ".MAIN_DB_PREFIX."commande SET facture = 0 WHERE rowid = ".$rowidComm;
						$resuPCommFacC = $this->db->query($slqUpCommFacC);
						if(!$resuPCommFacC){
							$error++;
						}
					}
		...
		if (!$error)
		{
			...
			$select .= "`fk_source` = $this->id AND `sourcetype` = \"facture\" AND `targettype` = \"ticket\"";
			$reselect = $this->db->query($select);
			$count = (int) $this->db->fetch_object($reselect)->count;
			$select = "SELECT count(*) as count FROM `".MAIN_DB_PREFIX."element_element` WHERE ";
			$select .= "`fk_target` = $this->id AND `sourcetype` = \"ticket\" AND `targettype` = \"facture\"";
			$reselect = $this->db->query($select);
			$count += (int) $this->db->fetch_object($reselect)->count;
			if ($this->type == self::TYPE_CREDIT_NOTE && $count == 0)
    ...
    public function set_paid($user, $close_code = '', $close_note = '')
	{
		...
		// Si la referencia de la factura contiene el prefijo PROV, hacemos nuevamente la validacion 
		if (preg_match('/^PROV/', $this->ref)){
			$num = $this->getNextNumRef($this->thirdparty, 'next');
			$this->ref = $num;
			$now = dol_now('tzuser');
			$sql_update_ref = 'UPDATE '.MAIN_DB_PREFIX.'facture';
			$sql_update_ref .= " SET ref='".$num."', fk_statut = ".self::STATUS_VALIDATED.", fk_user_valid = ".($user->id > 0 ? $user->id : "null").", date_valid = '".$this->db->idate($now)."'";
			if (!empty($conf->global->FAC_FORCE_DATE_VALIDATION))	// If option enabled, we force invoice date
			{
				$sql_update_ref .= ", datef='".$this->db->idate($this->date)."'";
				$sql_update_ref .= ", date_lim_reglement='".$this->db->idate($this->date_lim_reglement)."'";
			}
			$sql_update_ref .= ' WHERE rowid = '.$this->id;
			$this->db->query($sql_update_ref);
		}
    ...
    public function set_unpaid($user)
	{
		...
		if (!$error)
		{
			...
			if ($this->type == self::TYPE_CREDIT_NOTE && $count == 0)
			{
				/** Setting up warehouse id for stock */
				$idwarehouse = $user->fk_warehouse;
				...
				for ($i = 0; $i < $num; $i++)
				{
					if ($this->lines[$i]->fk_product > 0)
					{
						$mouvP = new MouvementStock($this->db);
						$mouvP->origin = &$this;
						$result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceReOpenedInDolibarr", $this->ref));
					}
				}
			}
			$this->db->commit();
    ...
    public function set_canceled($user, $close_code = '', $close_note = '')
	{
		...
		if ($resql)
		{
			...
			if ($resql)
			{
                $pos = 'UPDATE `llx_pos_ticket` SET `fk_facture` = NULL WHERE `fk_facture` = '.$this->id;
                $posRes = $this->db->query($pos);
				...
				if ($resCommD){
					...
					if ($numCommD > 0){
						...
						while ($iCommD < $numCommD)
						{
							...
							$resuPCommD = $this->db->query($slqUpCommD);
							if(!$resuPCommD){
								$this->error = $this->db->error()." sql=".$slqUpCommD;
								$this->db->rollback();
								return -1;
							}
							array_push($commArray,$objCommD->pedido);
						...
						foreach ($commArray as $rowidComm) {
							//QUITAR ESTADO FACTURADO DE PEDIDOS RELACIONADOS SI YA NO SE TIENEN FACTURAS
							$sqlCommDC = "SELECT ee.rowid, f.fk_statut
									FROM ".MAIN_DB_PREFIX."element_element AS ee, ".MAIN_DB_PREFIX."facture AS f
										WHERE 
											(ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_source = f.rowid AND f.fk_statut > 0 AND ee.fk_target = ".$rowidComm.") 
											OR 
							...
							if ($resCommDC){
								...
								if ($numCommDC == 0){
									...
									$resuPCommFacC = $this->db->query($slqUpCommFacC);
									if(!$resuPCommFacC){
										$this->error = $this->db->error()." sql=".$slqUpCommFacC;
										$this->db->rollback();
										return -1;
									}
								}
								//QUITAR ESTADO FACTURADO DE PEDIDOS RELACIONADOS SI TODAS SUS FACTURAS ESTAN CANCELADAS
								$iComFac = 0;
								$banComFac = true;
								while ($iComFac < $numCommDC){
									$objComFac = $this->db->fetch_object($resCommDC);
									if($objComFac->fk_statut != 3){
										$banComFac = false;
										break;
									}
									$iComFac++;
								...
								if( $banComFac ){
									$slqUpComFac = "UPDATE ".MAIN_DB_PREFIX."commande SET facture = 0 WHERE rowid = ".$rowidComm;
									$resuPComFac = $this->db->query($slqUpComFac);
									if(!$resuPComFac){
										$this->error = $this->db->error()." sql=".$slqUpComFac;
										$this->db->rollback();
										return -1;
									}
								}
					...
					$this->error = $this->db->error()." sql=".$sqlCommD;
					$this->db->rollback();
					return -1;
				}
				...
				if ($this->type == self::TYPE_CREDIT_NOTE && $count == 0)
				{
					...
					for ($i = 0; $i < $num; $i++)
					{
						if ($this->lines[$i]->fk_product > 0)
						{
							$mouvP = new MouvementStock($this->db);
							$mouvP->origin = &$this;
							$result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceCancelledInDolibarr", $this->ref));
						}
					}
				}
				$this->db->commit();
	 ...
	 */
    public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0, $forcePFA = false)
	{
		global $conf, $langs;
		require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
		...
		elseif (preg_match('/^[\(]?PROV/i', $this->ref) || empty($this->ref)) // empty should not happened, but when it occurs, the test save life
		{
			...
			$num = $this->getNextNumRef($this->thirdparty, 'next', $forcePFA);
		}
		else
		{
			$num = $this->ref;
		...
		if ($num)
		{
			...
			{
				//Cambiar estado de Pedido a Facturado
				$sqlComm = "SELECT ee.rowid, IF(ee.sourcetype = 'commande', ee.fk_source, ee.fk_target) AS pedido
								FROM ".MAIN_DB_PREFIX."element_element AS ee
									WHERE 
										(ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_source = ".$this->id.") 
										OR 
										(ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_target = ".$this->id.")";
				$resComm = $this->db->query($sqlComm);
				if ($resComm){
					$numComm = $this->db->num_rows($resComm);
					if ($numComm > 0){
						$iComm = 0;
						while ($iComm < $numComm)
						{
							$objComm = $this->db->fetch_object($resComm);
												...
												SELECT c.total_ht AS TOTAL_PEDIDO,  SUM(f.total) AS TOTAL_FACTURA
													FROM llx_element_element AS ee, llx_facture AS f, llx_commande AS c
														WHERE 
															...
															OR 
															(ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_source = c.rowid AND c.fk_statut > 0 AND f.fk_statut > 0 AND ee.fk_target = f.rowid AND c.rowid = ".$objComm->pedido.")
											)saldo
									) AS resta
										SET cm.billedRest = resta.devolucion
										WHERE cm.rowid = ".$objComm->pedido;
					...
					if ($this->type != self::TYPE_DEPOSIT && $result >= 0 && !empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_BILL) && $idwarehouse > 0)
					{
						...
						for ($i = 0; $i < $cpt; $i++)
						{
							if ($this->lines[$i]->fk_product > 0)
							{
								...
								$sql = "SELECT batch, expdate, sellby FROM ".MAIN_DB_PREFIX."facturedet_extrafields WHERE fk_object = ".$this->lines[$i]->rowid;
								$resql = $this->db->query($sql);
								if ($resql){
									$obj = $this->db->fetch_object($resql);
									$batch = $obj->batch;
								...
								if ($this->type == self::TYPE_CREDIT_NOTE && $count == 0) $result = $mouvP->reception($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, 0, $langs->trans("InvoiceValidatedInDolibarr", $num), $eatby, '',$batch);
								else $result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceValidatedInDolibarr", $num), '', $eatby, $sellby, $batch);
								if ($result < 0) {
									$error++;
			...
			if ($this->type != self::TYPE_DEPOSIT && $result >= 0 && !empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_BILL))
			{
				...
				for ($i = 0; $i < $num; $i++)
				{
					if ($this->lines[$i]->fk_product > 0)
					{
						...
						$mouvP->origin = &$this;
						// We decrease stock for product
						if ($this->type == self::TYPE_CREDIT_NOTE && $count == 0) $result = $mouvP->livraison($user, $this->lines[$i]->fk_product, $idwarehouse, $this->lines[$i]->qty, $this->lines[$i]->subprice, $langs->trans("InvoiceBackToDraftInDolibarr", $this->ref));
	 ...
	 *  @param 	    string      $batch              Numero de lote
    ...
    public function addline(
		...
		dol_syslog(get_class($this)."::addline id=$this->id,desc=$desc,pu_ht=$pu_ht,qty=$qty,txtva=$txtva, txlocaltax1=$txlocaltax1, txlocaltax2=$txlocaltax2, fk_product=$fk_product,remise_percent=$remise_percent,date_start=$date_start,date_end=$date_end,ventil=$ventil,info_bits=$info_bits,fk_remise_except=$fk_remise_except,price_base_type=$price_base_type,pu_ttc=$pu_ttc,type=$type, fk_unit=$fk_unit, batch=$batch", LOG_DEBUG);
		...
		if ($this->statut == self::STATUS_DRAFT)
		{
			...
			if (!empty($fk_product))
			{
				...
				$pa_ht = price2num(!$this->pos_source ? $product->cost_price : $product->cost_price_sucursal);
			...
			if ($batch) {
				...
				$response = $lot->fetch(0,0,$batch);

				if ($response > 0) {
					$this->line->array_options['options_batch'] = $lot->batch;
					$this->line->array_options['options_expdate'] = $lot->eatby;
					$this->line->array_options['options_sellby'] = $lot->sellby;
				}
			}

            $extras = "SELECT claveprodserv,umed FROM ".MAIN_DB_PREFIX."product_extrafields WHERE fk_object= ".$fk_product;
            $resextras = $this->db->query($extras);
            if($resextras){
                $showextrs = $this->db->fetch_object($resextras);
                $this->line->array_options['options_claveprodserv']=$showextrs->claveprodserv;
                $this->line->array_options['options_umed']=$showextrs->umed;
            }
	 ...
	 */
    public function getNextNumRef($soc, $mode = 'next', $forcePFA = false)
	{
		global $conf, $langs;
		$langs->load("bills");
		...
		if (!empty($conf->global->FACTURE_ADDON))
		{
			...
			$obj = new $classname();
			$numref = "";
			$numref = $obj->getNextValue($soc, $this, $mode, $forcePFA);
    ...
    /**
     *  Cancel the draft invoices with canceled CFDI
     *  @return		int				Number of invoices canceled
     */
    function setCanceledInvoicesWithCfdiCanceled(){
        $invoices = 0;
        $posInv = 0;
        $sql = "SELECT f.rowid, f.ref";
        $sql.= " FROM ".MAIN_DB_PREFIX."facture as f";
        $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."cfdimx as cf ON cf.fk_facture=f.rowid";
        $sql.= " WHERE f.fk_statut = 0 AND cf.cancelado=1";
        $resql = $this->db->query($sql);
        if($resql){
            $num = $this->db->num_rows($resql);
            while ($obj = $this->db->fetch_object($resql)){
                $cancel = "UPDATE ".MAIN_DB_PREFIX."facture SET fk_statut=3, close_note='CFDI Cancelado',close_code='abandon'";
                $cancel.= " WHERE rowid=".$obj->rowid;
                $res = $this->db->query($cancel);
                if($res){
                    $invoices ++;
                }
                $pos = 'UPDATE `llx_pos_ticket` SET `fk_facture` = NULL WHERE `fk_facture` = '.$obj->rowid;
                $posRes = $this->db->query($pos);
                $posRows = $posRes->affected_rows($posRes);
                if($posRows){
                    $posInv += $posRows;
                }
            }
            $this->output = 'Se cancelaron '.$invoices.' facturas en borrador con CFDI cancelado';
            $this->output.= 'De '.$num.' totales';
        }else{
            $this->output = 'Error en la sentencia';
        }
        return $invoices;
    }
	
	//Se eliminan todos los Lines que se manden por argumento
	public function deleteLinesMassive($linesIDs){
		$sqlDelLines = "DELETE FROM ".MAIN_DB_PREFIX."facturedet WHERE rowid IN (".implode(",", $linesIDs).")";
		if($this->db->query($sqlDelLines)){
			return 1;
		}else{
			return -1;
		}
	...
	/**
	 * Eliminar los borradores de factura generados en TPV
	 * @return int 0 if OK, <0 if KO
	 **/
	function deleteProvTpv() {
		try {
            global $db, $user, $conf;

			// Se seleccionan los borradores
            $sql = "SELECT rowid, ref FROM ".MAIN_DB_PREFIX."facture
            WHERE ref LIKE '(PROV-POS%'";
            $resql = $db->query($sql);
            if (empty($resql))
            {
                $this->error = 'Error al obtener PROV';
                return -8;
            }
            ...
            {
				// Se eliminan líneas de factura si es que tiene, para poder eliminar el borrador
                $sql2 = "DELETE FROM ".MAIN_DB_PREFIX."facturedet WHERE fk_facture = ".$obj->rowid;
				$resql2 = $db->query($sql2);
                if (empty($resql2))
                { 
                    $this->error = 'Error eliminando producto en factura '.$obj->ref;
                    return -8;
                }

                // Se elimina borrador
                $sql3 = "DELETE FROM ".MAIN_DB_PREFIX."facture WHERE rowid = ".$obj->rowid;
				$resql3 = $db->query($sql3);
				if (empty($resql3))
                { 
                    $this->error = 'Error eliminando factura '.$obj->ref;
                    return -8;
                }
                $count++;
            }

            $this->output = "Borradores eliminados: " . $count . "";
            return 0;

		} catch (\Throwable $th) {
			$this->error = 'Error al eliminar borradores: ' . $th->getMessage();
			return -9;
		}
	}
}
...
class FactureLigne extends CommonInvoiceLine
{
    ...
    public function insert($notrigger = 0, $noerrorifdiscountalreadylinked = 0)
	{
		...
		$sql .= ", ".(empty($this->fk_unit) || !$this->fk_unit ? "null" : $this->fk_unit);
		$sql .= ", ".$user->id;
		$sql .= ", ".$user->id;
		$sql .= ", ".(int) $this->fk_multicurrency;
		$sql .= ", '".$this->db->escape($this->multicurrency_code)."'";
