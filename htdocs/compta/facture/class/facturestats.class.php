...
class FactureStats extends Stats
{
	...
	/**
	 * 	Return the un-invoiced amount by month for a year
	 *
	 *	@param	int		$year		Year to scan
     *	@param	int		$format		0=Label of abscissa is a translated text, 1=Label of abscissa is month number, 2=Label of abscissa is first letter of month
	 *	@return	array				Array with amount by month
	 */
	public function getTicketsByMonth($year, $format = 0)
	{
		global $user;

		$where = '';
		$where = " f.fk_statut IN (1,2)";
		$where.= " AND f.entity IN (1)";
		$where.= " AND fk_facture IS NULL";
		//if (!$user->rights->societe->client->voir && !$this->socid) $this->where .= " AND f.fk_soc = sc.fk_soc AND sc.fk_user = " .$user->id;
		if (false && $mode == 'customer') $where.=" AND (f.fk_statut <> 3 OR f.close_code <> 'replaced')";	// Exclude replaced invoices as they are duplicated (we count closed invoices for other reasons)
		if ($this->socid)
		{
			$where.=" AND f.fk_soc = ".$this->socid;
		}
        if ($this->userid > 0) $where.=' AND f.fk_user_author = '.$this->userid;
		if (false && ! empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) $where.= " AND f.type IN (0)";
		else $where.= " AND f.type IN (0)";
		

		$sql = "SELECT date_format(date_creation,'%m') as dm, SUM(f.total_ttc)";
		$sql.= " FROM llx_pos_ticket as f";
		$sql.= " WHERE f.date_creation BETWEEN '".$this->db->idate(dol_get_first_day($year))."' AND '".$this->db->idate(dol_get_last_day($year))."'";
		$sql.= " AND ".$where;
        $sql.= " GROUP BY dm";
        $sql.= $this->db->order('dm', 'DESC');
