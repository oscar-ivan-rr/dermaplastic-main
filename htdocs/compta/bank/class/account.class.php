...
class Account extends CommonObject
{
    ...
    public function create(User $user, $notrigger = 0)
	{
		...
		if (!is_numeric($this->fk_warehouse) || $this->fk_warehouse <= 0) $this->fk_warehouse = 'NULL';
    ...
    public function update(User $user, $notrigger = 0)
	{
		...
		$sql .= ",fk_warehouse = ".$this->fk_warehouse;
    ...
    public function solde($option = 0)
	{
		...
		$sql .= " FROM ".MAIN_DB_PREFIX."bank";
		$sql .= " WHERE fk_account = ".$this->id;
		if ($option == 1) $sql .= " AND dateo <= '".$this->db->idate(dol_now('tzuser'))."'";
	...
	/**
	 * 	Get bank id of given warehouse id.
	 * 	@param	int	$fk_warehouse	Warehouse id.
	 * 	@return	int					Bank id if exists, otherwise ''.
	 */
	public static function getBankWithWarehouse($fk_warehouse) {
		global $db;

		if (is_numeric($fk_warehouse) && $fk_warehouse > 0) {
			$sql = "SELECT rowid FROM llx_bank_account WHERE fk_warehouse = $fk_warehouse LIMIT 1";
...
class AccountLine extends CommonObject
{
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele))
		{
			...
			} elseif (!empty($conf->global->BANK_ADDON_PDF)) {
				$modele = $conf->global->BANK_ADDON_PDF;
			}
		}
