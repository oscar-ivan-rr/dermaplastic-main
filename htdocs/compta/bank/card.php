...
if ($action == 'add')
{
	...
	$object->fk_warehouse    = GETPOST("fk_warehouse", 'int', 2);
...
if ($action == 'create')
{
	...
	print $formproduct->selectWarehouses(isset($_POST["fk_warehouse"]) ? GETPOST("fk_warehouse") : '', 'fk_warehouse', '', 1);
	print '<td>' . $warehouse . '</td></tr>';
...
else
{
	if (($_GET["id"] || $_GET["ref"]) && $action != 'edit')
	{
		...
		// Warehouse
		print '<tr class="liste_titre_add"><td class="titlefield">'.$langs->trans("Warehouse").'</td>';
		$warehouse = '&nbsp';
		if (!empty($object->fk_warehouse)) {
			$entrepot = new Entrepot($db);
			if ($entrepot->fetch($object->fk_warehouse) > 0)
				$warehouse = $entrepot->getNomUrl(1);
	...
	if (GETPOST('id', 'int') && $action == 'edit' && $user->rights->banque->configurer)
	{
		...
		// Warehouse
		print '<tr><td>'.$langs->trans("Warehouse").'</td><td>';
		print $formproduct->selectWarehouses(isset($_POST["fk_warehouse"]) ? GETPOST("fk_warehouse") : $object->fk_warehouse, 'fk_warehouse', '', 1);
		print '<td>' . $warehouse . '</td></tr>';
