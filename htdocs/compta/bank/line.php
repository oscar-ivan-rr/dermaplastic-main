...
if ($action == 'digital_sign' && $conf->global->MAIN_SIGNATURE_MODULE) {
    $id = $rowid;
	$ref = $langs->trans('Transfer') . " $id";
    $element = "AccountLine";
    $objectfile = "/compta/bank/class/account.class.php";
    $dir = $conf->bank->dir_output."/Transfer_" . $id;

    $model_pdf = 'transfer';
    $moreinputs = array('force_redirect' => 1, 'url' => DOL_URL_ROOT . '/compta/bank/line.php?rowid=' . $id);
...
$permissiontoadd = $user->rights->banque->transfer;
$id = $rowid;
...
if ($result)
{
    ...
    if ($db->num_rows($result))
    {
        ...
        if (count($links))
        {
            ...
            foreach($links as $key=>$val)
            {
                ...
                elseif ($links[$key]['type']=='banktransfert') {
                    ...
                    print '</a>';
                    $origin = $bankline->rowid < $links[$key]['url_id'];
                }
        ...
        // Regen signature and doc
        if ($origin && $conf->global->MAIN_SIGNATURE_MODULE) {
            print '<div class="tabsAction">';
            print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?rowid='.$rowid.'&amp;action=digital_sign">'.$langs->trans("ReGenerateTransferPDF").'</a>';
            print '</div>';
        }
        ...
        // Docs
        $filename = dol_sanitizeFileName("Transfer_$rowid");
        $filedir = $conf->bank->multidir_output[1].'/'.$filename;
        $urlsource = $_SERVER['PHP_SELF'].'?rowid='.$rowid;

        // Perms
        print $formfile->showdocuments('bank', $filename, $filedir, $urlsource, false, $user->rights->banque->transfer, 'transfer', 1, 0, 0, 28, 0, '', '', '', $soc->default_lang, '', $bankline);
