...
$arrayfields = array(
    ...
    'b.debit'=>array('label'=>$langs->trans("Egreso"), 'checked'=>1, 'position'=>600),
    'b.credit'=>array('label'=>$langs->trans("Ingreso"), 'checked'=>1, 'position'=>605),
...
$sql .= " bb.balance, bb.balance_antes,";
$sql .= " bu.url_id,";
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."bank_balance as bb ON bb.fk_bank = b.rowid";
...
if ($resql)
{
	...
	if ($action != 'addline' && $action != 'reconcile')
	{
		if (empty($conf->global->BANK_DISABLE_DIRECT_INPUT))
		{
			...
			{
                $newcardbutton = dolGetButtonTitle($langs->trans('AddBankRecord'), '', 'fa fa-plus-circle', $_SERVER["PHP_SELF"].'?action=addline&page='.$page.$param, '', $user->rights->banque->create);
			}
		}
		else
		{
    ...
    while ($i < min($num, $limit))
    {
    	...
    	if (!empty($arrayfields['balancebefore']['checked']))
    	{
			if ($balancebefore >= 0)
			{
				print '<td class="nowrap right">&nbsp;'.price($objp->balance-$objp->amount).'</td>';
			}
			else
			{
				print '<td class="error nowrap right">&nbsp;'.price($objp->balance-$objp->amount).'</td>';
			}
    		if (!$i) $totalarray['nbfield']++;
    	}
    	// Balance
    	if (!empty($arrayfields['balance']['checked']))
    	{
			if ($balance >= 0)
			{
				print '<td class="nowrap right">&nbsp;'.price($objp->balance).'</td>';
			}
			else
			{
				print '<td class="error nowrap right">&nbsp;'.price($objp->balance).'</td>';
			}
    		if (!$i) $totalarray['nbfield']++;
    	}
