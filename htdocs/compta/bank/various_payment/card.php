...
$type_cash_move =       GETPOST("type", "alpha");
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'variouspaymentcard';

if ($contextpage == 'takepos') {
	$_GET['optioncss'] = 'print';
	$paymenttype = 4;
	$accountid =  $conf->global->{'CASHDESK_ID_BANKACCOUNT_CASH' . $_SESSION["takeposterminal"]};
	...
	$label = $type_cash_move == 'salida' ? 'Salida de efectivo' : 'Entrada de efectivo';
}
...
if (empty($reshook))
{
	...
	{
		$error = 0;
		if($user->rights->banque->positive_amount && GETPOST("amount", 'alpha') < 0){
			setEventMessages("Solo puedes realizar registros mayores a cero", null, 'errors');
			$error++;
		}

		$datep = dol_mktime(0, 0, 0, GETPOST("datepmonth", 'int'), GETPOST("datepday", 'int'), GETPOST("datepyear", 'int'));
		$datev = dol_mktime(0, 0, 0, GETPOST("datevmonth", 'int'), GETPOST("datevday", 'int'), GETPOST("datevyear", 'int'));
		...
		$object->datev = $datev;
		$object->datep = $datep;
		$type_cash_move == "salida" ? $object->amount = price2num(-GETPOST("amount", 'alpha')) : $object->amount = price2num(GETPOST("amount", 'alpha'));
...
if ($action == 'create')
{
	...
	print '<input type="hidden" name="type" value="' . $type_cash_move . '">';
	if ($contextpage == 'takepos') {
		print '<input type="hidden" name="contextpage" value="takepos">';
	}

	print '<script>console.log('.$type_cash_move.')</script>';
	...
	print $form->selectDate($datep ?: dol_now('tzuser'), "datep", '', '', '', 'add', 1, 1);
	print '</td></tr>';
	...
	if ($contextpage != 'takepos'){
		...
		print $form->selectDate($datev ?: dol_now('tzuser'), "datev", '', '', '', 'add', 1, 1);
		print '</td></tr>';
	}

	// Label
	print '<tr><td>';
	print $form->editfieldkey('Concepto', 'label', '', $object, 0, 'string', '', 1).'</td><td>';
