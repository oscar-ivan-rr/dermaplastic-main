...
if ($action == 'add') {
    ...
    if (!$error) {
        if (dol_add_file_process($conf->mycompany->dir_temp, 1, -1, 'file_xsl', '', null, '', 0, false) > 0) {
            ...
            $import = new ImportExcel($db,5, $filepath, $delimiter, $user, '', '');
            $info = $import->importData();

            // TODO: Show inserts, updates and errors

            if ($info['inserts'] > 0) setEventMessage($langs->trans("Cuentas agregadas ".$info['inserts'], $info['inserts']));
            if ($info['updates'] > 0) setEventMessage($langs->trans("Cuentas actualizada ".$info['updates'], $info['updates']));
            if ($info['warnings'] > 0) setEventMessage($langs->trans("Cuentas en compras ", $info['warnings']), 'warnings');
            //if ($info['errors'] > 0) setEventMessage($langs->trans("Error al importar cuentas ", $info['errors']), 'errors');
...
$transAreaType = $langs->trans("Importación cuentas por pagar externas");
