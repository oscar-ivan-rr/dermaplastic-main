...
$sql =	 '( '."\r\n"
		...
		.'			,f.ticketnumber '."\r\n"
		.'			,f.type '."\r\n"
		.'			,if(f.type=0, f.total_ttc, (f.total_ttc * -1) ) '."\r\n"
		.'			,f.paye '."\r\n"
		.'			,f.fk_statut '."\r\n"
		.'			,pf.amount '."\r\n"
		.'			,s.nom as name '."\r\n"
		.'			,s.rowid as socid '."\r\n"
		.'			,1 AS is_ticket '."\r\n"
		.'FROM	 '.MAIN_DB_PREFIX.'pos_paiement_ticket as pf '."\r\n"
		.'		,'.MAIN_DB_PREFIX.'pos_ticket as f '."\r\n"
		.'		,'.MAIN_DB_PREFIX.'societe as s '."\r\n"
		.'WHERE pf.fk_ticket = f.rowid '."\r\n"
		.'  AND f.fk_soc = s.rowid '."\r\n"
		.'	AND f.entity IN ('.getEntity('invoice').') '."\r\n"
		.'	AND pf.fk_paiement = '.$object->id.' '."\r\n"
		.'  AND f.fk_facture IS NULL '."\r\n"
		.') '."\r\n"
		.'';
...
if ($resql)
{
	...
	print '<td>'.$langs->trans('Documento').'</td>';
	...
	if ($num > 0)
	{
		while ($i < $num)
		{
			...
			if (get_class($invoice) == 'Ticket' && $invoice->type==1)
			{
				$invoice->total_ttc = $invoice->total_ttc *-1; 
			}
			...
			// Status
			print '<td class="right">'.$invoice->getLibStatut((($objp->is_ticket)?1:5), $alreadypayed).'</td>';

			print "</tr>\n";
			if ($objp->paye == 1 && $objp->is_ticket == 0)	// If at least one invoice is paid, disable delete
			{
				$disable_delete = 1;
