		...
		if ($resql)
		{
			...
			foreach ($this->amounts as $key => $amount)
			{
				...
				if (is_numeric($amount) && $amount <> 0)
				{
					...
					if ($resql)
					{
#						$rc_sql= 'SELECT * FROM llx_element_element '."\r\n"
#								.'WHERE  '."\r\n"
#								.''
						...
						$invoice->fetchObjectLinked('', '', $invoice->id, 'facture');

						if (is_array($invoice->linkedObjects['ticket']))
						{
							$tamount = $amount;
							foreach($invoice->linkedObjects['ticket'] as $lticket)
							{
								break;
								if($lticket->diff_payment >= $tamount)
								{
									$ttamount = $tamount;
									$tamount = 0;
								}
								...
								{
									$ttamount = $lticket->diff_payment;
									$tamount -= $lticket->diff_payment;
								}
								$ttquery =	 "INSERT INTO llx_pos_paiement_ticket "
											.'(fk_paiement,fk_ticket,amount) '
											.'VALUES '
											.'('
												.$this->id
												.','
												...
												.')'
											;
								if (!$this->db->query($ttquery))
								{
									dol_print_error($this->db);
									die();
								}
								$lticket->diff_payment -= $ttamount;
								$lticket->update($user);
							}
						}
					    ...
					    if ($closepaidinvoices)
					    {
                            ...
                            */
                            ...
                            elseif ($remaintopay < -0.109 || $remaintopay > 0.101) dol_syslog("Remain to pay for invoice ".$facid." not null. We do nothing more.");
                        ...
                        if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
                        {
                            ...
                            $result = $invoice->generateDocument('crabe', $outputlangs);
                            if ($result < 0) {
                            	var_dump($facid);
    ...
    public function delete($notrigger = 0)
	{
		...
		if ($result)
		{
			$sql =   'SELECT fk_ticket,amount'." \r\n"
					.'FROM '.MAIN_DB_PREFIX.'pos_paiement_ticket'." \r\n"
					.'WHERE fk_paiement = '.$this->id.''." \r\n"
					.''
					;
			if ($res1 = $this->db->query($sql))
			{
				while ($row = $this->db->fetch_object($res1))
				{
					...
					$static_ticket->fetch($row->fk_ticket);
					$ticket_total_ttc = 0;
					$ticket_total_ht = 0;
					$ticket_total_tva = 0;
					foreach($static_ticket->lines as $line)
					{
						if ($line->desc == 'Bonificación por descuento CxC')
						{
							$static_ticket->deleteline($line->rowid);
						}
						else
						{
							$ticket_total_ttc += $line->total_ttc;						
							$ticket_total_ht += $line->total_ht;
							$ticket_total_tva += $line->total_tva;
						}
					}
					$sql =	 'UPDATE llx_pos_ticket '
							.'SET	 total_ttc='.$ticket_total_ttc.' '
							.'		,total_ht='.$ticket_total_ht.' '
							.'		,tva = '.$ticket_total_tva.' '
							.'		,customer_pay = (customer_pay - '.$row->amount.') '
							.'		,difpayment = (difpayment + '.$row->amount.') '
							.'		,paye = 0 '
							.'		,fk_statut = 2 '
							.'WHERE rowid = '.$row->fk_ticket.' '
							;
					...
					//$static_pos->cronFixSaldos($row->fk_ticket);
			...
			$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'pos_paiement_ticket';
			$sql.= ' WHERE fk_paiement = '.$this->id;
			dol_syslog($sql);
			$result = $this->db->query($sql);
		}
	...
	/**
	 * Save amount paid to print on the POS receipt grouped by type of payment and calculate the change 
	 */
    public function addAmount($ref, $paiementid = 0, $amount = 0){
		$error = 0;
		
		$this->db->begin();
		if(!empty($ref)){
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."amounts_paid (num_paiement, fk_paiement, amount) VALUES('".$ref."', '".$paiementid."', '".$amount."')";
