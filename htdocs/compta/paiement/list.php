...
else
{
    ...
    $sql.= " if (s.rowid IS NOT NULL, s.rowid, st.rowid) as socid,";
	$sql.= " IF (s.nom IS NOT NULL, s.nom, st.nom) as name ,";
	$sql.= " IF(s.email IS NOT NULL, s.email,st.email) as email";
	// Add fields from hooks
	$parameters=array();
	$reshook=$hookmanager->executeHooks('printFieldListSelect', $parameters);    // Note that $action and $object may have been modified by hook
	$sql.=$hookmanager->resPrint;
    ...
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."paiement_facture as pf ON p.rowid = pf.fk_paiement";
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pos_paiement_ticket as pt ON p.rowid = pt.fk_paiement";    
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."facture as f ON pf.fk_facture = f.rowid";
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."pos_ticket as t ON pt.fk_ticket = t.rowid";
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."societe as s ON f.fk_soc = s.rowid";
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."societe as st ON t.fk_soc = st.rowid";
    ...
    $sql.=" AND p.datep >= '".$date_start_factureyear."-".$date_start_facturemonth."-".$date_start_factureday."'";
    else if (!$date_start_factureyear && $date_end_factureyear)
        $sql.=" AND p.datep <= '".$date_end_factureyear."-".$date_end_facturemonth."-".$date_end_factureday."'";
    else if ($date_start_factureyear && $date_end_factureyear)
        $sql.=" AND p.datep BETWEEN '".$date_start_factureyear."-".$date_start_facturemonth."-".$date_start_factureday."' AND '".$date_end_factureyear."-".$date_end_facturemonth."-".$date_end_factureday."'";
    ...
    if ($search_company)     		$sql .= ' AND ((s.nom LIKE "%'.$search_company.'%") OR (st.nom LIKE "%'.$search_company.'%"))';
	// Add where from hooks
	$parameters=array();
	$reshook=$hookmanager->executeHooks('printFieldListWhere', $parameters);    // Note that $action and $object may have been modified by hook
	$sql.=$hookmanager->resPrint;
}
// $sql.= $db->order($sortfield, $sortorder); // FIXME: disable this and use standard $param for pagination because it make slow request with lot of data

$nbtotalofrecords = '';
$conf->global->MAIN_DISABLE_FULL_SCANLIST = 1; // FIXME: disable full scan to speed up request with lot of data
...
if ($resql)
{
    ...
    print_liste_field_titre("Cliente", $_SERVER["PHP_SELF"], "s.nom", "", $param, "", $sortfield, $sortorder);
