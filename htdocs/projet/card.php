...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->projet->creer)
	{
		...
		{
			setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentities("Nombre de proyecto")), null, 'errors');
			$error++;
		}
...
if ($action == 'create' && $user->rights->projet->creer)
{
	...
	// Label
	print '<tr><td><span class="fieldrequired">'.$langs->trans("Nombre de Proyecto").'</span></td><td><input class="minwidth500" type="text" name="title" value="'.dol_escape_htmltag(GETPOST("title", 'none')).'" autofocus></td></tr>';
	...
	if ($conf->societe->enabled)
	{
		...
		print $langs->trans("Nombre de Cliente");
...
elseif ($object->id > 0)
{
	...
	else
	{
		...
		include('./card_resumen_calculos.php');
		// Saldo
		print '<td class="titlefield tdtop">'.$langs->trans("Saldo").'</td><td>';
		print '$'.number_format(($t_cobros-(($t_envio + $t_pos)+($t_gastos+$t_nomina))),2,'.',',');
		print '</td></tr>';
...
?>
<style>
	.sub-tabla{background-color: #e0e0e0;
    padding: 10px;
    margin: 10px;
    width: 20%;
...
<div style="background-color: #eee; width: 100%;display: block;overflow: hidden;">
	<div class="sub-tabla">
		<h3><i class="fa fa-bank"></i> Financiero 
			<p>&nbsp;</p>
		</h3>
		<div class="sub_sub_tabla">
			<h4>Facturas: <span><a href="<?php echo DOL_URL_ROOT;?>/compta/facture/list.php?mainmenu=billing&leftmenu=&search_type=0&search_project_ref=<?php echo $object->ref;?>&search_status=1,2">$<?php echo number_format($t_facturas,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>Presupuestos: <span><a href="<?php echo DOL_URL_ROOT;?>/comm/propal/list.php?mainmenu=billing&leftmenu=&search_refproject=<?php echo $object->ref;?>">$<?php echo number_format($t_presup,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>Cobros: <span><a href="<?php echo DOL_URL_ROOT;?>/compta/facture/list.php?mainmenu=billing&leftmenu=&search_type=0&search_project_ref=<?php echo $object->ref;?>&search_status=1,2">$<?php echo number_format($t_cobros,2,'.',','); ?></a></span></h4>
		</div>
	</div>
	<div class="sub-tabla">
		<h3><i class="fa fa-shopping-cart"></i> Salidas de Material <br>(Con IVA)
			<span class="right">
				$<?php echo number_format(($t_envio + $t_pos),2,'.',',');?>
			</span>
		</h3>
		<span style="float: right;">Costo | Venta</span>
		<br>
		<div class="sub_sub_tabla">
			<h4>Envios: 
				<span><a href="<?php echo DOL_URL_ROOT;?>/expedition/list.php?mainmenu=products&leftmenu=&viewstatut=2&search_project=<?php echo $object->ref;?>">$<?php echo number_format($t_envio,2,'.',','); ?></a></span>
				<span><a href="<?php echo DOL_URL_ROOT;?>/expedition/list.php?mainmenu=products&leftmenu=&viewstatut=2&search_project=<?php echo $object->ref;?>">$<?php echo number_format($t_envio_cost,2,'.',','); ?> | </a></span>
			</h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>Pedidos: 
				<span><a href="<?php echo DOL_URL_ROOT;?>/commande/list.php?mainmenu=commercial&leftmenu=&viewstatut=-3&search_project_ref=<?php echo $object->ref;?>">$<?php echo number_format($t_co,2,'.',','); ?></a></span>
				<span><a href="<?php echo DOL_URL_ROOT;?>/commande/list.php?mainmenu=commercial&leftmenu=&viewstatut=-3&search_project_ref=<?php echo $object->ref;?>">$<?php echo number_format($t_co_cost,2,'.',','); ?> | </a></span>
			</h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>POS: <span><a href="<?php echo DOL_URL_ROOT;?>/compta/facture/list.php?mainmenu=billing&leftmenu=&search_project_ref=<?php echo $object->ref;?>&search_status=1,2">$<?php echo number_format($t_pos,2,'.',','); ?></a></span></h4>
		</div>
	</div>
	<div class="sub-tabla">
		<h3><i class="fa fa-cart-plus"></i> Entradas de Material
			<p class="right">
				$<?php echo number_format(($t_epres + $t_compras),2,'.',',');?>
			</p>
		</h3>
		<div class="sub_sub_tabla">
			<h4>Presupuestos: <span><a href="<?php echo DOL_URL_ROOT;?>/supplier_proposal/list.php?search_project=<?php echo $object->ref;?>">$<?php echo number_format($t_epres,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>Compras: <span><a href="<?php echo DOL_URL_ROOT;?>/fourn/facture/list.php?search_project=<?php echo $object->ref;?>">$<?php echo number_format($t_compras,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla" style="visibility: hidden;">
			<h4>&nbsp;</h4>
		</div>
	</div>
	<div class="sub-tabla">
		<h3><i class="fa fa-money"></i> Gastos
			<p class="right">
				$<?php echo number_format(($t_gastos + $t_nomina),2,'.',',');?>
			</p>
		</h3>
		<div class="sub_sub_tabla">
			<h4>Gastos: <span><a href="<?php echo DOL_URL_ROOT;?>/expensereport/list.php?search_status=2,5,6&search_projet=<?php echo $object->ref;?>">$<?php echo number_format($t_gastos,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla">
			<h4>Nómina: <span><a href="#">$<?php echo number_format($t_nomina,2,'.',','); ?></a></span></h4>
		</div>
		<div class="sub_sub_tabla" style="visibility: hidden;">
			<h4>&nbsp;</h4>
		</div>
