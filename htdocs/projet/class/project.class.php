...
class Project extends CommonObject
{
    ...
    public function get_element_list($type, $tablename, $datefieldname = '', $dates = '', $datee = '', $projectkey = 'fk_projet')
    {
        ...
        elseif ($type == 'tickets_projet')
        {
            $sql = 'SELECT t.rowid, t.fk_user_author as fk_user FROM '.MAIN_DB_PREFIX."pos_ticket as t WHERE  t.fk_projet IN (".$ids.")";
            $sql .=" AND t.fk_statut <> 0 AND (SELECT COUNT(ee.rowid) FROM ".MAIN_DB_PREFIX."element_element as ee WHERE ee.fk_source=t.rowid AND ee.sourcetype='ticket') = 0";
        }
        else
		{
    ...
    public function load_board($user, $mode)
    {
        ...
        if ($mode == 'draft') $sql.= " WHERE p.fk_statut = 0";
        ...
        if ($resql)
        {
            ...
            if ($mode == 'opened') {
                ...
                $response->warning_delay = $conf->projet->warning_delay / 60 / 60 / 24;
                $response->label = $langs->trans("DraftProjects");
                $response->labelShort = $langs->trans("Draft");
                if ($user->rights->projet->all->lire) $response->url = DOL_URL_ROOT . '/projet/list.php?search_status=0&mainmenu=project';
                else $response->url = DOL_URL_ROOT . '/projet/list.php?search_project_user=-1&search_status=0&mainmenu=project';
                $response->img = img_object('', "projectpub");
        }
