...
class Commande extends CommonOrder
{
    ...
    /**
	 * @var int Origin commande fournisseur
	 */
    ...
    public function valid($user, $idwarehouse = 0, $notrigger = 0)
    {
        ...
        if (! $error)
        {
        	$static_comm = new Commande($this->db);
        	$static_comm->fetch ($this->id);
        	$static_comm->fetchObjectLinked();
        	$hasTicket = false;
        	if (is_array($static_comm->linkedObjectsIds) && count($static_comm->linkedObjectsIds))
        	{
	        	foreach($static_comm->linkedObjectsIds as $tk => $tv)
    	    	{
        			if($tk == 'ticket' && $tv > 0)
        			{
        				$hasTicket = true;
            ...
            // Stock is handled in POS tickets
            if (!$hasTicket && $result >= 0 && ! empty($conf->stock->enabled) && $conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER == 1)
            {
            ...
            //Añadir precio de compra a cada producto
            $cpt=count($this->lines);
            for ($i = 0; $i < $cpt; $i++)
            {
                if ($this->lines[$i]->fk_product > 0)
                {
                    $prodPC = new Product($this->db);
                    $prodPC->fetch($this->lines[$i]->fk_product);
                    $precioCompra = 0;

                    if(is_null($prodPC->cost_price)){
                        $precioCompra = 0;
                    }else{
                        $precioCompra = $prodPC->cost_price;
                    }

                    $sqlPC = "UPDATE ".MAIN_DB_PREFIX."commandedet";
                    $sqlPC.= " SET cost_price = ".$precioCompra;
                    $sqlPC.= " WHERE rowid = ".$this->lines[$i]->rowid;
                    $sqlPC.= " AND fk_product = ".$this->lines[$i]->fk_product;

                    $resqlPC=$this->db->query($sqlPC);
                    if (! $resqlPC)
                    {
                        dol_print_error($this->db);
                        $this->error=$this->db->lasterror();
        ...
        //Verificar Saldo por Facturar y si tiene Facturas Asociadas
        if(!$error){
            //Cambiar estado de Pedido a Facturado
            $sqlComm = "SELECT ee.rowid
            FROM ".MAIN_DB_PREFIX."element_element AS ee, ".MAIN_DB_PREFIX."facture AS f
                WHERE 
                    (ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_source = f.rowid AND f.fk_statut > 0 AND ee.fk_target = ".$this->id.") 
                    OR 
                    (ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_target = f.rowid AND f.fk_statut > 0 AND ee.fk_source = ".$this->id.")";
            $resComm = $this->db->query($sqlComm);
            if ($resComm){
                $numComm = $this->db->num_rows($resComm);
                if ($numComm > 0){
                    $slqUpComm = "UPDATE ".MAIN_DB_PREFIX."commande SET facture = 1 WHERE rowid = ".$this->id;
                                    ...
                                    (
                                        SELECT c.total_ht AS TOTAL_PEDIDO,  SUM(f.total) AS TOTAL_FACTURA
                                            FROM llx_element_element AS ee, llx_facture AS f, llx_commande AS c
                                                WHERE 
                                                    (ee.sourcetype = 'facture' AND ee.targettype = 'commande' AND ee.fk_target = c.rowid AND c.fk_statut > 0 AND f.fk_statut > 0 AND ee.fk_source = f.rowid AND c.rowid = ".$this->id.") 
                                                    OR 
                                                    (ee.sourcetype = 'commande' AND ee.targettype = 'facture' AND ee.fk_source = c.rowid AND c.fk_statut > 0 AND f.fk_statut > 0 AND ee.fk_target = f.rowid AND c.rowid = ".$this->id.")
                                    )saldo
                            ) AS resta
                                SET cm.billedRest = resta.devolucion
                                WHERE cm.rowid = ".$this->id;
    ...
    public function classifyShipped($user, $idwarehouse)
    {
        global $langs;
        $num = $this->ref;
        require_once DOL_DOCUMENT_ROOT.'/product/stock/class/mouvementstock.class.php';
    ...
    public function setDraft($user, $idwarehouse = -1)
    {
        ...
        if ($this->db->query($sql))
        {
            ...
            // Stock is handled in POS tickets
            if (!$hasTicket && !empty($conf->stock->enabled) && $conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER == 1)
            {
                $result = 0;
            ...
            || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->commande->order_advance->validate)))
        {
            ...
            if ($this->db->query($sql))
            {
                ...
                if (! $error)
                {
					...
					if (is_array($static_comm->linkedObjectsIds) && count($static_comm->linkedObjectsIds))
					{
						foreach($static_comm->linkedObjects as $tk => $tv)
						{
							if($tk == 'ticket')
							{
								foreach($tv as $stv)
								{
									foreach($stv->lines as $tvLine)
									{
										foreach($this->lines as $ll)
										{
											if ($ll->fk_product == $tvLine->fk_product)
											{
												$tvLine->updateEstadoV(6,$ll->qty);
												break;
											}
										}
									}
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        if (!is_numeric($this->distance)) $this->distance = 'NULL';
        ...
        $sql.= ", ".$this->distance;
        $sql.= ", ".($this->fk_commande_fourn > 0 ? $this->fk_commande_fourn : 'NULL');
        $sql.= ", ".($this->guide_number ? "'".$this->db->escape($this->guide_number)."'":"null");
        $sql.= ", ".($this->pack_id ? "'".$this->db->escape($this->pack_id)."'":"null");
        $sql.= ")";
    ...
    public function createFromClone(User $user, $socid = 0)
    {
        ...
        $this->user_valid         = '';
        $this->date = dol_now();
        $this->date_commande = dol_now('tzuser');
        $this->date_creation      = '';
    ...
    public function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $fk_product = 0, $remise_percent = 0, $info_bits = 0, $fk_remise_except = 0, $price_base_type = 'HT', $pu_ttc = 0, $date_start = '', $date_end = '', $type = 0, $rang = -1, $special_code = 0, $fk_parent_line = 0, $fk_fournprice = null, $pa_ht = 0, $label = '', $array_options = 0, $fk_unit = null, $origin = '', $origin_id = 0, $pu_ht_devise = 0)
    {
        ...
        if ($this->statut == self::STATUS_DRAFT)
        {
            ...
            $this->line->fk_commandefourndet=$origin_id;
    ...
    public function fetch($id, $ref = '', $ref_ext = '', $ref_int = '')
    {
        ...
        $sql.= ", c.distance, c.guide_number, c.pack_id";
        $sql.= ", i.libelle as label_incoterms";
        $sql.= ', p.code as mode_reglement_code, p.libelle as mode_reglement_libelle';
        $sql.= ', cr.code as cond_reglement_code, cr.libelle as cond_reglement_libelle, cr.libelle_facture as cond_reglement_libelle_doc';
        $sql.= ', ca.code as availability_code, ca.label as availability_label';
        $sql.= ', dr.code as demand_reason_code, c.fk_commande_fourn';
        ...
        if ($result)
        {
            ...
            if ($obj)
            {
                ...
                $this->guide_number          = $obj->guide_number;
                $this->pack_id               = $obj->pack_id;
    ...
    public function fetch_lines($only_product = 0, $loadalsotranslation = 0)
    {
        ...
        $sql .= ' p.ref as product_ref, p.description as product_desc, p.fk_product_type, p.label as product_label, p.tobatch as product_tobatch, p.tosell, p.tobuy,';
        $sql .= ' p.weight, p.weight_units, p.volume, p.volume_units, l.fk_commandefourndet';
        ...
        if ($result)
        {
            ...
            while ($i < $num)
            {
                ...
                $line->fk_commandefourndet 	    = $objp->fk_commandefourndet;
    ...
    public function update(User $user, $notrigger = 0)
    {
        ...
        $sql .= " guide_number=".(isset($this->guide_number) ? "'".$this->db->escape($this->guide_number)."'" : "null").",";
    ...
    public function LibStatut($status, $billed, $mode, $donotshowbilled = 0)
    {
        ...
        elseif ($status==self::STATUS_SENT || $status==self::STATUS_CLOSED && ($billed && empty($conf->global->WORKFLOW_BILL_ON_SHIPMENT))){
            $labelStatus = $langs->trans('StatusOrderFacture');
            $labelStatusShort = $langs->trans('StatusOrderFacture');
            $statusType='status6';
        }
    ...
    /**
     * Set commande freight cost with shipment type and depreciation
     * @return <0 KO | 0 Nothing happened | 1 OK
     */
    public function setFreightCost() {
        global $conf, $langs, $user;

        $service = Product::getFreightServiceID();
        ...
        if ($service > 0) {
            ...
            if (!is_null($this->shipping_method_id)) {
                $sql = "SELECT performance, price_gasoline, depreciation FROM ".MAIN_DB_PREFIX."c_shipment_mode WHERE rowid = $this->shipping_method_id";
                $res = $this->db->query($sql);
        
                if ($res) {
                    $obj = $this->db->fetch_object($res);
                    $distance_client = (is_numeric($this->distance)) ? $this->distance : 0;
    
                    $freight_cost = ($obj->price_gasoline / $obj->performance) * $distance_client + $obj->depreciation;
                    // $freight_cost = 100 * round($freight_cost / 100);
    
                    // Update line
                    if (sizeof($obj_serv) > 0) {
                        $s = array_pop($obj_serv);
                        $success = $this->updateline($s->id, $s->desc, $freight_cost, 1, 0, 16, 0.0, 0.0, 'TTC', 0, '', '', 1);
                    } 
                    // Insert line
                    else {
                        $success = $this->addline("", $freight_cost, 1, 16, 0, 0, $service, 0, 0, 0, 'TTC', $freight_cost, '', '', 1);
                    }
                    ...
                    if ($success > 0) {
                        ...
                        if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) {
                            $outputlangs = $langs;
                            $this->generateDocument($this->modelpdf, $outputlangs);
                        }
            ...
            elseif (sizeof($obj_serv) > 0) {
                // Remove line
                $s = array_pop($obj_serv);
                $this->deleteline($user, $s->id);
    ...
    /**
	 * Update thirdparty distance
	 * @param	float	$distance	Societe's distance
     * @return  int     <0 KO | 0 OK but setFreightCost did nothing | 1 OK full process
    ...
    public function updateDistance($distance) {
        ...
        if (!empty($distance)) {
            if (!is_numeric($distance)) {
                $this->error = "ErrorFormatDistance";
                return -3;
            }
        }
		...
		if ($res) {
            ...
            // Set freight cost with new distance
            $result = $this->setFreightCost();

            // If failed, retrieve old distance
            if ($result < 0) {
                $this->db->rollback();
	...
	public function deleteLinesMassive($linesIDs){
		$sqlDelLines = "DELETE FROM ".MAIN_DB_PREFIX."commandedet WHERE rowid IN (".implode(",", $linesIDs).")";
    ...
    //Facturar Todos los Pedidos que Estén en NO Facturado, NO Proyecto Asignado y Estado Validado-Emitido-Validación en Curso
    public function factureMassive(){
        $getFactures = "SELECT rowid, ref, ref_client, fk_soc, fk_cond_reglement FROM ".MAIN_DB_PREFIX.$this->table_element." WHERE fk_projet IS NULL AND facture = 0 AND ( fk_statut IN (1,2) OR (fk_statut = 3 AND (facture = 0 OR (facture = 1 AND billedRest IS NOT NULL)))) ORDER BY rowid DESC";
        
        if($resFactures = $this->db->query($getFactures)){

            require_once DOL_DOCUMENT_ROOT.'/compta/facture/class/facture.class.php';
            global $user;

            //Contador Facturas Creadas
            $contFacturas = 0;

            //Total de Facturas a Crear
            $totalFacturas = $this->db->num_rows($resFactures);

            while($obj = $this->db->fetch_object($resFactures)){

                $facture = new Facture($this->db);
                $facture->date = strtotime('now');
                $facture->ref_client = $obj->ref_client;
                $facture->socid = $obj->fk_soc;
                $facture->cond_reglement_id = $obj->fk_cond_reglement;
                $facture->type = '0';
                ...
                if($res > 0){
                    ...
                    $factureLines = "SELECT rowid FROM ".MAIN_DB_PREFIX.$this->table_element_line." WHERE fk_commande = ".$obj->rowid;

                    //Funcionalidad de Lines
                    if($resFactLines = $this->db->query($factureLines)){

                        $errorLine = 0;

                        while($objLine = $this->db->fetch_object($resFactLines)){

                            $orderLigne = new OrderLine($this->db);
                            $orderLigne->fetch($objLine->rowid);

                            $resLine = $facture->addline($orderLigne->desc, $orderLigne->subprice, $orderLigne->qty, $orderLigne->tva_tx, 
                                                             $orderLigne->localtax1_tx, $orderLigne->localtax2_tx, $orderLigne->fk_product,
                                                                 $orderLigne->remise_percent, $orderLigne->date_start, $orderLigne->date_end, 
                                                                     $orderLigne->fk_code_ventilation, $orderLigne->info_bits, $orderLigne->fk_remise_except,
                                                                         'HT', $orderLigne->total_ttc, $facture->type, -1, $orderLigne->special_code,
                                                                             $orderLigne->origin, $orderLigne->origin_id, $orderLigne->fk_parent_line,
                                                                                 $orderLigne->fk_fournprice, $orderLigne->pa_ht, '', 0, 100, 0, 
                                                                                     $orderLigne->fk_unit);
                        ...
                        if($errorLine == 0){
                            ...
                            //Ponemos Factura como Pte. de Pago
                            $resPtePago = $facture->validate($user, '', 0, 0, false);
                            
                            //Ponemos Pedido a Cliente como Facturado
                            $updateCommande = "UPDATE ".MAIN_DB_PREFIX.$this->table_element." SET fk_statut = 3, facture = 1 WHERE rowid = ".$obj->rowid;
                            $resUpdateCommande = $this->db->query($updateCommande);

                            if($resPtePago <= 0){
                                setEventMessage("Error al Actualizar Estado de Factura ".$facture->ref, "errors");
                            }

                            if(!$resUpdateCommande){
                                setEventMessage("Error al Actualizar Estado de Pedido a Cliente ".$obj->ref, "errors");
                            }

                            if($resPtePago > 0 && $resUpdateCommande){
                                //Relación de Objetos
                                $sqlRel = "INSERT INTO ".MAIN_DB_PREFIX."element_element (fk_source, sourcetype, fk_target, targettype)";
                                $sqlRel .= " VALUES (".$obj->rowid.", 'commande', ".$facture->id.", 'facture')";

                                if(!$this->db->query($sqlRel)){
                                    setEventMessage("Error al Crear Relación entre Pedido y Factura", "errors");
                                }
                            }
                        }
                    }
                    ...
                    setEventMessage("Error al Crear Factura ".$obj->ref, "errors");
                }

            }

            setEventMessage($contFacturas." Facturas creadas de ".$totalFacturas, "mesgs");
        }
    }

    
    /**
     *	Print Shipping Label for this order
     */
    public function printShippingLabel(string $marketplace)
    {
        $url = '';
        switch ($marketplace) {
            case 'ML':
                $url = 'http://backend:8080/api/mercadolibre/orders/'.$this->pack_id.'/shipping_label';
                break;
            default:
                break;
        }
        ...
        if (curl_errno($ch)) {
            ...
            if ($httpCode == 404) {
                setEventMessages('Orden no Encontrada', null, 'errors');
            } elseif ($httpCode == 403) {
                setEventMessages('Forbidden (403) error', null, 'errors');
            } elseif ($httpCode == 401) {
                setEventMessages('Unauthorized (401) error', null, 'errors');
            } elseif ($httpCode == 400) {
                list($headers, $jsonContent) = explode("\r\n\r\n", $response, 2);
                $b = json_decode($jsonContent,true);
                setEventMessages($b, null, 'errors');
            } elseif ($httpCode >= 500) {
                setEventMessages('Server error: ' . $httpCode, null, 'errors');
            } elseif ($httpCode >= 400) {
                setEventMessages('Other client error: ' . $httpCode, null, 'errors');
            } else {
                $filename = 'Label-'.$this->ref_client.'.pdf';
...
class OrderLine extends CommonOrderLine
{
    ...
    public function fetch($rowid)
    {
        ...
        $sql .= ' cd.date_start, cd.date_end, cd.fk_commandefourndet';
        ...
        if ($result)
        {
            ...
            $this->fk_commandefourndet = $objp->fk_commandefourndet;
    ...
    public function insert($user = null, $notrigger = 0)
    {
        ...
        $sql.= ' fk_unit';
        $sql.= ', fk_multicurrency, multicurrency_code, multicurrency_subprice, multicurrency_total_ht, multicurrency_total_tva, multicurrency_total_ttc, fk_commandefourndet';
        $sql.= ')';
        ...
        $sql.= ", ".(! empty($this->fk_commandefourndet) ? $this->fk_commandefourndet : 'NULL');
        $sql.= ')';
