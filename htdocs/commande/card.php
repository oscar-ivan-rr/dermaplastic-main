...
if (empty($reshook))
{
	...
	elseif ($action == 'print_label_ml') {
		try {
			$object->printShippingLabel("ML");
		}
		catch (Exception $e) {
    ...
    elseif ($action == 'reopen' && $usercancreate) {
        if ($object->statut == Commande::STATUS_CANCELED || $object->statut == Commande::STATUS_CLOSED) {
			$query = "SELECT fk_warehouse FROM ".MAIN_DB_PREFIX."commande WHERE rowid = ".$object->id;
            ...
            if ($result > 0) {
				...
				foreach ($object->lines as $line) {
					sendUpdateStockNotification($db, $line->fk_product, $obj->fk_warehouse);
				}
            } else {
                setEventMessages($object->error, $object->errors, 'errors');
            }
    ...
    elseif ($action == 'add' && $usercancreate) {
        ...
        if (!$error) {
            ...
            $object->distance = $object->thirdparty->distance;
            $object->guide_number = GETPOST('guide_number', 'alpha');
            $object->pack_id = GETPOST('pack_id', 'alpha');
    ...
    } // Edit distanche third
    elseif ($action == 'setdistance_third') {
        $result = $object->updateDistance(GETPOST('distance_third'));
        if ($result < 0)
            setEventMessages($object->error, $object->errors, 'errors');
    ...
    elseif ($action == 'setshippingmethod' && $usercancreate) {
        ...
        if ($result < 0) {
            ...
            if ($object->setFreightCost() < 0) setEventMessages($object->error, $object->errors, 'errors');
        }
    } // warehouse
    ...
    //Apply discount general
    else if($action =='apply_line_discount' && $object->statut==Commande::STATUS_DRAFT && $usercancreate){
        if(GETPOST('discount_percent')!=null){
            if($user->posdesc < GETPOST('discount_percent')){
                ...
                foreach ($object->lines as $line) {
                    $result = $object->updateline($line->id, $line->desc, $line->subprice, $line->qty, GETPOST('discount_percent'), $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, 'HT', $line->info_bits, $line->date_start, $line->date_end, $line->product_type, $line->fk_parent_line, 0, $line->fk_fournprice, $line->pa_ht, $line->label, $line->special_code, $line->array_options, $line->fk_unit, $line->multicurrency_subprice);
                }
            }
        }else{
	...
	//Delete Massive Lines
		...
		if(!empty($linesURL)){
			...
			foreach($lines as $line){
				$res = $object->deleteline($user, $line);
			}
	...
	elseif ($action == 'confirm_shipped' && $confirm == 'yes' && $usercanclose) {
		...
		$idwarehouse = GETPOST('idwarehouse');
		// $result1 = $object->classifyShipped($user, $idwarehouse); Se evita realizar decrementos de stock
		$result1 = 0;
...
if ($action == 'create' && $usercancreate)
{
	...
	if ($socid > 0) {
		...
		print '<td>';
		print $form->select_company(GETPOST('filtrosoicd'), 'socid', '(s.client = 1 OR s.client = 3)', 'SelectThirdParty', 0, 0, null, 0, 'minwidth300');
	...
	if (!empty($cond_reglement_id)){
        ...
        $form->select_conditions_paiements(GETPOST('cond_reglement_id'), 'cond_reglement_id', - 1, 1);
    }
	...
	// Guide number
	print '<tr class="fielddeliverydelay"><td>'.$langs->trans('GuideNumber').'</td><td>';
	print '<input name="guide_number" value="'.GETPOST('guide_number').'"/>';
	print '</td></tr>';

	// ID de orden
	print '<tr class="fielddeliverydelay"><td>ID de orden</td><td>';
	print '<input name="pack_id" value="'.GETPOST('pack_id').'"/>';
	print '</td></tr>';
	...
	if (!empty($conf->expedition->enabled)) {
		...
		if($user->rights->stock->show_all_warehouses){
			print '<tr><td class="fieldrequired">' . $langs->trans("Warehouse") . '</td><td colspan="3">';
			print $formproduct->selectWarehouses($user->fk_warehouse, 'warehouse_id', 'warehouseopen', 0);
			print '</td></tr>';
	
			// Script para obtener el RFC asociado al almacén seleccionado (en evento change)
			print '<script type="text/javascript">
				$(document).ready(function() {
					// Función para realizar la solicitud AJAX y actualizar el campo #rfc
					function updateRFC() {
						var fk_entrepot = $("#warehouse_id").val();
			...
			print '<tr><td class="fieldrequired">' . $langs->trans("Warehouse") . '</td><td colspan="3">';
			print $formproduct->selectWarehouses($user->fk_warehouse, 'warehouse_id', 'warehouseopen', 0, 1);
			print '</td></tr>';
			
		}
		// RFC
		print '<tr><td>' . $langs->trans("RFC") . '</td><td colspan="3">';
		print '<input readOnly= "true" name="rfc" id="rfc"></input>';
		print '</td></tr>';
	}
	...
	if (!empty($conf->projet->enabled))
	{
        ...
        print '<script type="text/javascript">
			$(document).ready(function() {
				$("#socid").change(function() {
        			...
        			$.ajax("'.DOL_URL_ROOT.'/core/ajax/ajaxcommande.php", {
                        ...
                        success: function (result) {
                           ...
                           window.location.href = "'.$_SERVER["PHP_SELF"].'?action=create&leftmenu=orders&filtrosoicd="+socid+"&ref_client="+$("input[name=ref_client]").val()+"&cond_reglement_id="+$("select[name=cond_reglement_id]").val();
                        }
				    });
			    });
			});
		...
		$numprojet = $formproject->select_projects(($soc->id > 0 ? $soc->id : -1), $projectid, 'projectid', 0, 0, 1, 0, 0, 0, 0, '', 0, 0,'','',GETPOST('filtrosoicd'));
	...
	print '<tr><td style="display: none;">'.$langs->trans('DefaultModel').'</td>';
	print '<td>';
	include_once DOL_DOCUMENT_ROOT.'/core/modules/commande/modules_commande.php';
	$liste = ModelePDFCommandes::liste_modeles($db);
	print $form->selectarray('model', $liste, $conf->global->COMMANDE_ADDON_PDF,'','','','','','','','','"style="display: none;');
	print "</td></tr>";
	...
	if ($object->id > 0) {
		...
		if($action == 'continue'){
            $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id,'Cliente con Credito Excedido','¿Desea continuar?','validate','',0,1,200);
        }
		...
		*/
		if ($action == 'shipped') {
			...
			// }
			// $formquestion = array();
			// if (!empty($conf->stock->enabled) && $qualified_for_stock_change)
			// {
			// 	$langs->load("stocks");
			// 	require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
			// 	$formproduct = new FormProduct($db);
		...
		$morehtmlref .= '<br>'.$langs->trans('RefCustomer').' : '.$object->ref_client;
		// Thirdparty
		...
		// Distance thirdparty
		$morehtmlref .= '<br>' . $form->editfieldkey("DistanceThird", 'distance_third', $object->distance, $object, $usercancreate, 'string', '', 0, 1);
		$morehtmlref .= $form->editfieldval("DistanceThird", 'distance_third', $object->distance, $object, $usercancreate, 'string', '', null, null, '', 1);
		...
		if (!empty($conf->projet->enabled))
		{
			...
			if ($usercancreate && $user->rights->commande->change_project)
			{
				if ($action != 'classify')
		...
		*/
		...
		if ($action == 'edit_entrepot')
		{
			...
			require_once DOL_DOCUMENT_ROOT.'/product/class/html.formproduct.class.php';
			$formproduct = new FormProduct($db);
			print $formproduct->selectWarehouses($object->warehouse_id, 'warehouse_id', 'warehouseopen', 0);
			print '<input type="submit" class="button" value="'.$langs->trans('Modify').'">';
			print '</form>';
			print '<input value="" readOnly= "true" name="rfc" id="rfc"></input>';
		...
		else
		{
			...
			FROM llx_commande lc JOIN llx_entrepot le ON lc.fk_warehouse = le.rowid 
			JOIN llx_c_rfc lcr ON le.fk_rfc = lcr.rowid WHERE lc.rowid = ".$object->id."";
		...
		// Guide number
		print '<tr><td>'.$langs->trans('GuideNumber').'</td><td>'.$object->guide_number.'</td></tr>';
		
		// ID de orden
		print '<tr><td>ID de orden</td><td>'.$object->pack_id.'</td></tr>';
		...
		if (!empty($object->lines) && ($object->statut == Commande::STATUS_DRAFT) && ($object->getNbOfShipments() == 0)) {
			//Eliminar Productos
						...
						buttons: [
							{
								text: \"Si\",
								click: function() {
										var l = '".DOL_URL_ROOT."/commande/card.php?id=".$id."&action=confirmDelete&lines=';
										var q = [];
        ...
        if($action == 'addline')
        {
            ...
            //print '    $("#idprod").select2("open");';
            print '    $("#search_idprod").focus();';
            print '});';
            print '</script>';
        }
		...
		if (!empty($conf->multicurrency->enabled) && ($object->multicurrency_code != $conf->currency))
		{
			...
			print '<tr><td class="titlefieldmiddle class="center"">'.$form->editfieldkey('MulticurrencyAmountHT', 'multicurrency_total_ht', '', $object, 0).'</td>';
		...
		// echo '<pre>';
		// print_r([$subtotal, $discount]);
		// echo '</pre>';

		print '<tr><td class="titlefieldmiddle">'.$langs->trans('AmountHT').'</td>';
		print '<td class="center">'.price($subtotal, '', '', 0, - 1, - 1, $conf->currency).$alert.'</td>';
		...
		// Total VAT
		print '<tr><td>'.$langs->trans('VAT').' (16%)'.'</td><td class="center">'.price($object->total_tva, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';

		// Amount Local Taxes
		if ($mysoc->localtax1_assuj == "1" || $object->total_localtax1 != 0) 		// Localtax1
		{
			print '<tr><td class="center">'.$langs->transcountry("AmountLT1", $mysoc->country_code).'</td>';
			print '<td class="center">'.price($object->total_localtax1, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
		}
		if ($mysoc->localtax2_assuj == "1" || $object->total_localtax2 != 0) 		// Localtax2 IRPF
		{
			print '<tr><td class="center">'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td>';
			print '<td class="center">'.price($object->total_localtax2, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
		}

		// Total TTC
		print '<tr><td>'.$langs->trans('AmountTTC').'</td><td class="center">'.price($object->total_ttc, 1, '', 1, - 1, - 1, $conf->currency).'</td></tr>';
		...
		if ($action != 'presend' && $action != 'editline') {
			...
			if (empty($reshook)) {
				...
				if ($soc->name == "MercadoLibre" && $object->statut > Commande::STATUS_DRAFT){
					print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=sync_guide">'.$langs->trans('SynchronizeGuide').'</a>';
				}
				//Print Label
				if ($soc->name == "MercadoLibre" && $object->statut > Commande::STATUS_DRAFT){
				...
				{
                    $arrayoutstandingbills = $soc->getOutstandingBills();
                    $cant_exedida=price($arrayoutstandingbills['opened']);
                    $limite_credito= price($soc->outstanding_limit);
                    $cant_exedida= str_replace(',','',$cant_exedida);
                    $limite_credito= str_replace(',','',$limite_credito);
                    if($cant_exedida>$limite_credito){
                        print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=continue">'.$langs->trans('Validate').'</a>';
                    }else{
					...
					if ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED && ($object->getNbOfProductsLines() > 0 || !empty($conf->global->STOCK_SUPPORTS_SERVICES))) {
						if (($conf->expedition_bon->enabled && $user->rights->expedition->creer) || ($conf->livraison_bon->enabled && $user->rights->expedition->livraison->creer)) {
							if ($user->rights->expedition->creer) {
								$ship_params = "action=create";
								$ship_params .= "&origin=$object->element";
								$ship_params .= "&origin_id=$object->id";
								$ship_params .= "&shipping_method_id=$object->shipping_method_id";
								$ship_params .= "&projectid=$object->fk_project";
								if (is_numeric($object->warehouse_id)) $ship_params .= "&entrepot_id=$object->warehouse_id";

								print '<a class="butAction" href="'.DOL_URL_ROOT.'/expedition/card.php?'.$ship_params.'">'.$langs->trans('CreateShipment').'</a>';
							} else {
