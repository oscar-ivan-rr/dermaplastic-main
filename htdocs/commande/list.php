...
$search_guide_number = GETPOST('search_guide_number', 'alpha');
$search_pack_id = GETPOST('search_pack_id', 'alpha');
...
$arrayfields = array(
	...
	'c.guide_number'=>array('label'=>$langs->trans("GuideNumber"), 'checked'=>1, 'position'=>1000),
	'c.pack_id'=>array('label'=>$langs->trans("ID de orden"), 'checked'=>1, 'position'=>1000)
);
// Extra fields
...
$sql .= ' c.rowid, c.ref, c.total_ht, c.tva as total_tva, c.total_ttc, c.ref_client, c.guide_number, c.pack_id,';
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."entrepot as le ON c.fk_warehouse = le.rowid";
...
if($user->fk_warehouse == $conf->global->CEDIS_WAREHOUSE){
	 if ($search_company > 0) $sql .= " AND s.rowid = ".$search_company;
} else{
...
if ($search_warehouse > 0) $sql .= " AND c.fk_warehouse = '".$search_warehouse."' ";
if ($search_rfc > 0) $sql .= " AND lcr.rowid = '".$search_rfc."' ";
if ($search_guide_number ==1) $sql .= " AND c.guide_number IS NULL";
if ($search_guide_number == 2) $sql .= " AND c.guide_number IS NOT NULL";
if ($search_pack_id ==1) $sql .= " AND c.pack_id IS NULL";
if ($search_pack_id == 2) $sql .= " AND c.pack_id IS NOT NULL";
if(!$user->rights->stock->show_all_warehouses) $sql .= " AND c.fk_warehouse = '".$user->fk_warehouse."' ";
...
if ($resql)
{
	...
	if ($search_rfc != '')   $param .= "&search_rfc=".urlencode($search_rfc);
	if ($search_guide_number != '')   $param .= "&search_guide_number=".$search_guide_number;
	if ($search_pack_id != '')   $param .= "&search_pack_id=".$search_pack_id;
	...
	if ($user->fk_warehouse == $conf->global->CEDIS_WAREHOUSE) $arrayofmassactions['order_pdf'] = img_picto('', 'download', 'class="pictofixedwidth"').$langs->trans("Creación de reporte de pedidos");
	...
	if (!empty($arrayfields['s.nom']['checked']))
	{
		print '<td class="liste_titre" align="left">';
		if($user->fk_warehouse == $conf->global->CEDIS_WAREHOUSE){
			print $form->select_company(GETPOST('search_company'), 'search_company', 's.shipment = 1', 'SelectThirdParty', 0, 0, null, 0, 'minwidth300');
		} else {
	...
	if (!empty($arrayfields['rfc']['checked']))
	{
		...
		if($user->rights->stock->show_all_warehouses){
			...
			while ($rfc = $db->fetch_object($requery2)){
				$rfclist[$rfc->rowid] = $rfc->code;
			}
			print $form->selectarray('search_rfc', $rfclist, ($search_rfc > 0 ? $search_rfc : -1), 1, 0, 0, '', 0, 0, 0, '', 'maxwidth200 widthcentpercentminusx', 1);
		}	
		print '</td>';
	}
	$options =  array(1 => 'No', 2 => 'Sí');
	// Guide number
	if (!empty($arrayfields['c.guide_number']['checked']))
	{
		print '<td class="liste_titre">';
		print $form->selectarray('search_guide_number', $options, $search_guide_number, 1, 0, 0, '', 0, 0, 0, '', 'maxwidth200 widthcentpercentminusx', 1);
		print '</td>';
	}
	// Guide number
	if (!empty($arrayfields['c.pack_id']['checked']))
	{
		print '<td class="liste_titre">';
		print $form->selectarray('search_pack_id', $options, $search_pack_id, 1, 0, 0, '', 0, 0, 0, '', 'maxwidth200 widthcentpercentminusx', 1);
		print '</td>';
	}
	// Town
	...
	if (!empty($arrayfields['c.guide_number']['checked']))   print_liste_field_titre($arrayfields['c.guide_number']['label'], $_SERVER["PHP_SELF"], "c.guide_number", "", $param, '', $sortfield, $sortorder);
	if (!empty($arrayfields['c.pack_id']['checked']))   	 print_liste_field_titre($arrayfields['c.pack_id']['label'], $_SERVER["PHP_SELF"], "c.pack_id", "", $param, '', $sortfield, $sortorder);
	...
	while ($i < min($num, $limit))
	{
		...
		if (! empty($arrayfields['c.guide_number']['checked']))
		{
			print "<td>".$obj->guide_number."</td>\n";
			if (! $i) $totalarray['nbfield']++;
		}
		// ID de orden
		if (! empty($arrayfields['c.pack_id']['checked']))
		{
			print "<td>".$obj->pack_id."</td>\n";
			if (! $i) $totalarray['nbfield']++;
		}
		// Town
	...
	print '<form method="POST" id="FormularioExportacion2" action="order_report_pdf.php">';
	print '<input type="hidden" id="arrayofselected" name="arrayofselected" value=\''.json_encode($arrayofselected).'\'/>';
    print '</form>';
