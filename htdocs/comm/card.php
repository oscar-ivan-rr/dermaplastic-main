...
*/
    ...
    if (!empty($conf->facture->enabled))
    {
        ...
        $pos = $object->getOutstandingTickets();
        $pedidos = $object->getOutstandingOrders();
        //$outstandingOpened = $tmp['opened']+$pos['opened']+$pedidos['opened'];
		$outstandingOpened = $tmp['opened']+$pos['opened'];
        ...
        $text = $langs->trans("Saldo");
		$link = DOL_URL_ROOT.'/compta/facture/list.php?socid='.$object->id.'&search_status=1';
        $icon = 'bill';
        if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
        $boxstat .= '<div class="boxstats" style="width: 20%;" title="'.dol_escape_htmltag($text).'">';
       $boxstat .= '<span class="boxstatstext" style=" font-weight: bold; font-size: 20px;color: #0E0E0E">'.img_object("", $icon).' '.$text.'</span><br>';
        $boxstat .= '<span style="font-weight: bold;"  class="boxstatsindicator'.($outstandingOpened > 0 ? ' amountremaintopay' : '').'">'.price($outstandingOpened, 1, $langs, 1, -1, -1, $conf->currency).$warn.'</span>';
        $boxstat .= '</div>';
        if ($link) $boxstat .= '</a>';
    }
	...
	if (!empty($conf->propal->enabled))
	{
		...
		$boxstat .= '<span class="boxstatsindicator">'.price($outstandingOpened, 1, $langs, 1, -1, -1, $conf->currency).'</span>';
		$boxstat .= '</div>';
		if ($link) $boxstat .= '</a>';
	}
    ...
    {
        // Box proposals
        $tmp = $object->getOutstandingTickets();
        $outstandingOpened = $tmp['opened'];
        $outstandingTotal = $tmp['total_ht'];
        $outstandingTotalIncTax = $tmp['total_ttc'];
        $text = $langs->trans("Ventas POS");
        //$link = DOL_URL_ROOT.'/custom/pos/backend/liste.php?socid='.$object->id;
		$link = DOL_URL_ROOT.'/compta/facture/list.php?socid='.$object->id.'&search_status=1&search_type=9999';
        $icon = 'bill';
        if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
        $boxstat .= '<div class="boxstats" title="'.dol_escape_htmltag($text).'">';
        $boxstat .= '<span class="boxstatstext">'.img_object("", $icon).' '.$text.'</span><br>';
	...
	if (!empty($conf->facture->enabled))
	{
		...
		$link = DOL_URL_ROOT.'/compta/facture/list.php?socid='.$object->id.'&search_status=1&search_type=99';
		$icon = 'bill';
		if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
		$boxstat .= '<div class="boxstats" title="'.dol_escape_htmltag($text).'">';
		$boxstat .= '<span class="boxstatstext">'.img_object("", $icon).' '.$text.'</span><br>';
	...
	if (!empty($conf->projet->enabled))
	{
		// Saldo Proyectos
		$saldoProyecto = 0;
		$tempoObjectRef = $object->ref;

		$sqlProyecto = "SELECT ref FROM llx_projet WHERE fk_soc = ".$object->id;
		$resqlProyecto = $db->query($sqlProyecto);
		if ($resqlProyecto){
			while ($objProyecto = $db->fetch_object($resqlProyecto)){
				$object->ref = $objProyecto->ref;
				include DOL_DOCUMENT_ROOT.'/projet/card_resumen_calculos.php';
				$saldoProyecto += ($t_cobros-(($t_envio + $t_pos)+($t_gastos+$t_nomina)));
			}
		}
		$object->ref = $tempoObjectRef;

		$text = $langs->trans("Projects");
		$link = DOL_URL_ROOT.'/projet/list.php?search_societe='.$object->nom;
		$icon = 'bill';
		if ($link) $boxstat .= '<a href="'.$link.'" class="boxstatsindicator thumbstat nobold nounderline">';
		$boxstat .= '<div class="boxstats" title="'.dol_escape_htmltag($text).'">';
		$boxstat .= '<span class="boxstatstext">'.img_object("", $icon).' '.$text.'</span><br>';
		$boxstat .= '<span class="boxstatsindicator">'.price($saldoProyecto, 1, $langs, 1, -1, -1, $conf->currency).'</span>';
		$boxstat .= '</div>';
		if ($link) $boxstat .= '</a>';
	}
     ...
     *   Last tickets
     */
    if (!empty($conf->pos->enabled) && $user->rights->pos->backend)
    {
        $sql = 'SELECT f.rowid as tickeid, f.ticketnumber as ref, f.type';
        $sql .= ', f.total_ht';
        $sql .= ', f.tva as total_tva';
        $sql .= ', f.total_ttc';
        $sql .= ', f.date_ticket as df, f.date_creation as dc, f.paye as paye, f.fk_statut as statut';
        $sql .= ', s.nom, s.rowid as socid';
        $sql .= ', SUM(pf.amount) as am';
        $sql .= " FROM ".MAIN_DB_PREFIX."societe as s,".MAIN_DB_PREFIX."pos_ticket as f";
        $sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'pos_paiement_ticket as pf ON f.rowid=pf.fk_ticket';
        $sql .= " WHERE f.fk_soc = s.rowid AND s.rowid = ".$object->id;
        $sql .= " AND f.entity IN (".getEntity('ticket').")";
        $sql .= ' GROUP BY f.rowid, f.ticketnumber, f.type, f.total_ht, f.tva, f.total_ttc,';
        $sql .= ' f.date_ticket, f.date_creation, f.paye, f.fk_statut,';
        $sql .= ' s.nom, s.rowid';
        $sql .= " ORDER BY f.date_ticket DESC, f.date_creation DESC";
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                print '<tr class="liste_titre">';
                print '<td colspan="5"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastCustomersTickets", ($num <= $MAXLIST ? "" : $MAXLIST)).'</td><td class="right"><a class="notasortlink" href="'.DOL_URL_ROOT.'/custom/pos/backend/liste.php?socid='.$object->id.'">'.$langs->trans("AllTickets").'<span class="badge marginleftonlyshort">'.$num.'</span></a></td>';
                print '</tr></table></td>';
                print '</tr>';
            }
            ...
            while ($i < $num && $i < $MAXLIST)
            {
                ...
                $ticketstatic->id = $objp->tickeid;
                $ticketstatic->ref = $objp->ref;
                $ticketstatic->type = $objp->type;
                $ticketstatic->total_ht = $objp->total_ht;
                $ticketstatic->total_tva = $objp->total_tva;
                $ticketstatic->total_ttc = $objp->total_ttc;
                ...
                print '<td class="nowrap right" style="min-width: 60px">'.($ticketstatic->LibStatut($objp->statut, 1)).'</td>';
                print "</tr>\n";
                $i++;
            }
            $db->free($resql);
