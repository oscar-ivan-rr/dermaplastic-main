...
foreach($linkedObjectBlock as $key => $objectlink)
{
    ...
    $sql1 = "SELECT * FROM ".MAIN_DB_PREFIX."element_element where fk_source = ".$objectlink->id." and sourcetype = 'propal' and fk_target = ".$object->id." and targettype = 'propal'";
    $resql1=$db->query($sql1);
    if ($resql1) {
        $num1 = $db->num_rows($resql1);
        if($num1 > 0 ){
            print '<td class="linkedcol-element" >'.$langs->trans("PropalOrigin");
        }else{
            $sql2 = "SELECT * FROM ".MAIN_DB_PREFIX."element_element where fk_source = ".$object->id." and sourcetype = 'propal' and fk_target = ".$objectlink->id." and targettype = 'propal'";
            $resql2=$db->query($sql2);
            if ($resql2) {
                $num2 = $db->num_rows($resql2);
                if($num2 > 0 ){
                    print '<td class="linkedcol-element" >'.$langs->trans("PropalClone");
                }else{
