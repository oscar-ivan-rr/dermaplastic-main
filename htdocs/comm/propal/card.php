...
$canCreateFournPropal = false;

if (!empty($user->rights->propal->client_to_fourn))
{
	$canCreateFournPropal = true;
}
...
$precioCompra = GETPOST('selectPC', 'int');
$producto = GETPOST('producto','int');

if($precioCompra && $producto){
	$prueba = $object->update_precios($producto, $precioCompra);
	if($prueba < 0)
		setEventMessages($object->error, $object->errors, 'errors');
}
...
if (empty($reshook))
{
	...
	elseif ($action == 'addline' && $usercancreate) {
		...
		if (!$error && ($qty >= 0) && (!empty($product_desc) || !empty($idprod))) {
			...
			if (!empty($idprod)) {
				...
				if (empty($conf->global->MAIN_PRODUCT_DISABLE_CUSTOMCOUNTRYCODE) && (!empty($prod->customcode)))
				{
					// Define output language
					if (!empty($conf->global->MAIN_MULTILANGS) && !empty($conf->global->PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE)) {
						$outputlangs = $langs;
			...
			if ((empty($user->rights->produit->ignore_price_min_advance) || empty($conf->global->MAIN_USE_ADVANCED_PERMS)) && (!empty($price_min) && (price2num($pu_ht) * (1 - price2num($remise_percent) / 100) < price2num($price_min)))) {
				$mesg = $langs->trans("CantBeLessThanMinPrice", price(price2num($price_min, 'MU'), 0, $langs, 0, 0, - 1, $conf->currency));
				setEventMessages($mesg, null, 'errors');
			} else {
				$result = $object->addline($desc, $pu_ht, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, $idprod, $remise_percent, $price_base_type, $pu_ttc, $info_bits, $type, - 1, 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $date_start, $date_end, $array_options, $fk_unit, '', 0, $pu_ht_devise, 0, $unit);
	...
	elseif ($action == 'updateline' && $usercancreate && GETPOST('save'))
	{
		...
		if (!empty($productid)) {
			...
			if ((empty($user->rights->produit->ignore_price_min_advance) || empty($conf->global->MAIN_USE_ADVANCED_PERMS)) && ($price_min && (price2num($pu_ht) * (1 - price2num(GETPOST('remise_percent')) / 100) < price2num($price_min)))) {
		...
		if (!$error) {
			...
			$result = $object->updateline(GETPOST('lineid'), $pu_ht, GETPOST('qty'), GETPOST('remise_percent'), $vat_rate, $localtax1_rate, $localtax2_rate, $description, 'HT', $info_bits, $special_code, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $type, $date_start, $date_end, $array_options, $_POST["units"], $pu_ht_devise, 0, $unit);
	...
	//Eliminar Lines de Forma Masiva
	...
	// Process CSV
	elseif ($action == 'processcsv') {
		// Permitir UTF8 en csv
		setlocale(LC_ALL, 'es_MX.iso88591');
		
		$file = $_FILES["csvToUpload"]["tmp_name"];

		$csv = fopen($file, "r");
		$datos = fgetcsv($csv);

		$relation = array('descripción' => 'desc', 'precio u' => 'subprice', 'cantidad' => 'qty', 'descuento %'=>'remise_percent', 'ud' => 'unit');
		$columns = array();
		foreach ($datos as $index => $data) {
            // Clean
			$value = trim(dol_strtolower($data));
            if (array_key_exists($value, $relation))
				$columns[$index] = $relation[$value];
		}

		$error = 0;
		$lines = array();
		while (($register = fgetcsv($csv)) !== FALSE) {
			$propaldet = new stdClass();

            // Asignación de propiedades
			foreach($columns as $index => $col)
				$propaldet->{$col} = $register[$index];
			
			// Required fields
			if (!empty($propaldet->desc) && !empty($propaldet->subprice) && !empty($propaldet->qty))
				$lines[] = $propaldet;
			else {
				$error++;
				break;
		...
		if (!$error) {
			// Insert records
			if (sizeof($lines) > 0) {
				$res = $object->loadLinesFromCSV($lines);
				// Success
				if ($res > 0) {
				    setEventMessage($langs->trans("AddedProducts", $res));
                    $object->update_price('','',0,'');
                }
				// Error
				elseif ($res < 0) setEventMessage($object->error, 'errors');
			}

			// No data
			else setEventMessage($langs->trans("NoData"), 'errors');
		}
		// Missed fields
		else setEventMessage($langs->trans("MissedFields"), 'warnings');
	}
    //Apply discount general
    else if($action =='apply_line_discount' && $object->statut==Propal::STATUS_DRAFT && $usercancreate ){
        if(GETPOST('discount_percent')!=null){
            if($user->posdesc < GETPOST('discount_percent')){
                setEventMessages('El descuento no puede ser mayor al descuento que se puede asignar por el usuario ('.$user->posdesc.'%)', null, 'errors');
            }else {
                foreach ($object->lines as $line) {
                    $result = $object->updateline($line->id, $line->subprice, $line->qty, GETPOST('discount_percent'), $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, $line->desc, 'HT', $line->info_bits, $line->special_code, $line->fk_parent_line, $line->skip_update_total, $line->fk_fournprice, $line->pa_ht, $line->label, $line->product_type, $line->date_start, $line->date_end, $line->array_options, $line->fk_unit, $line->multicurrency_subprice, 0, $Line->unit);
                }
            }
        }else{
    ...
    //Update Prices of Lines
    else if($action == 'updatelinesprices' && $object->statut==Propal::STATUS_DRAFT){
        //SQL
        $res = $object->updatePricesLines();
        if($res < 0) {
            setEventMessages($object->error, $object->errors, 'errors');
        }else{
            header("Location: ".$_SERVER['PHP_SELF']."?id=".$object->id);
...
llxHeader('', $langs->trans('Cotización'), 'EN:Commercial_Proposals|FR:Proposition_commerciale|ES:Presupuestos');
...
if ($action == 'create')
{
	...
	print load_fiche_titre($langs->trans("Nueva cotización"));
	...
	if (!empty($conf->global->BANK_ASK_PAYMENT_BANK_DURING_PROPOSAL) && !empty($conf->banque->enabled)) {
		print '<tr style="display:none;"><td>'.$langs->trans('BankAccount').'</td><td>';
	...
	// Delivery delay
	print '<tr class="fielddeliverydelay" style="display:none;"><td>'.$langs->trans('AvailabilityPeriod').'</td><td>';
	$form->selectAvailabilityDelay('', 'availability_id', '', 1);
	print '</td></tr>';

	// Shipping Method
	if (!empty($conf->expedition->enabled)) {
		print '<tr style="display:none;"><td>'.$langs->trans('SendingMethod').'</td><td>';
		print $form->selectShippingMethod($shipping_method_id, 'shipping_method_id', '', 1);
		print '</td></tr>';
	}

	// Delivery date (or manufacturing)
	print '<tr style="display:none;"><td>'.$langs->trans("DeliveryDate").'</td>';
	print '<td>';
    ...
    elseif ($action == 'create_ticket'){
        $aryTicket=array();
        $aryTicket['data']['type']=0;
        $aryTicket['data']['state']=0;
        $aryTicket['data']['proyectId']=$object->fk_project;
        $aryTicket['data']['customerId']=$object->socid;
        $aryTicket['data']['discount_percent']=$soc->remise_percent;
        $aryTicket['data']['discount_qty']=0;
        $aryTicket['data']['customerpay1']=0;
        $aryTicket['data']['customerpay2']=0;
        $aryTicket['data']['id_place']=0;
        $aryTicket['data']['note']=$object->note_public;
        $aryTicket['data']['note']=$object->note_public;
        $aryTicket['data']['customerpay']=0;
        $aryTicket['data']['difpayment']=0;
        $aryTicket['data']['idsource']=0;
        $aryTicket['data']['employeeId']=$user->id;
        $aryTicket['data']['lines'] = $object->lines;
        $aryTicket['data']['mode'] = 0;
        $aryTicket['data']['id'] = 0;
        $aryTicket['data']['originid'] = $object->id;
        $aryTicket['data']['origintype'] = $object->element;
        $sql="select rowid from llx_pos_cash where fk_warehouse=".$user->fk_warehouse." LIMIT 1";
        $resql=$db->query($sql);
        $warehouse=$db->fetch_object($resql);
        $aryTicket['data']["cashId"] = $warehouse->rowid;
        POS::CreateTicketPropal($aryTicket);
        //$action = 'view';
    }
    ...
    if (!empty($object->lines) && ($object->statut == Propal::STATUS_DRAFT)) {
		//Eliminar Productos
					...
					buttons: [
						{
							text: \"Si\",
							click: function() {
									var l = '".DOL_URL_ROOT."/comm/propal/card.php?id=".$id."&action=confirmDelete&lines=';
									var q = [];
		...
		//Actualizar precios
		print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=updatelinesprices">'.$langs->trans('UpdatePrices').'</a>';
		print '</div>';
    }
	...
	if (!empty($conf->multicurrency->enabled) && ($object->multicurrency_code != $conf->currency))
	{
		...
		print '<td class="nowrap center">'.price($object->multicurrency_total_tva, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';

		// Multicurrency Amount TTC
		print '<tr><td class="center">'.$form->editfieldkey('MulticurrencyAmountTTC', 'multicurrency_total_ttc', '', $object, 0).'</td>';
		print '<td class="nowrap center">'.price($object->multicurrency_total_ttc, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';
	}

    // $discount = $object->getDiscount();

	// recorremos las lineas del objeto para sumar el precio por la cantidad
	$subtotal = 0;
	$discount = 0;
	...
	print '<td class="nowrap center">'.price($subtotal, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	print '</tr>';

	// Discount
	print '<tr><td class="titlefieldmiddle ">'.$langs->trans('Reductions').'</td>';
	print '<td class="center">'.price($discount, '', '', 0, - 1, - 1, $conf->currency).$alert.'</td>';

    print '<tr><td class="titlefieldmiddle ">'.$langs->trans('Subtotal con Descuento').'</td>';
    print '<td class="nowrap center">'.price($subtotal-$discount, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	// Amount VAT
	print '<tr><td>'.$langs->trans('VAT').'</td>';
	print '<td class="nowrap center">'.price($object->total_tva, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	print '</tr>';
	...
	if ($mysoc->localtax1_assuj == "1" || $object->total_localtax1 != 0) 	// Localtax1
	{
		...
		print '<td class="nowrap center">'.price($object->total_localtax1, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
		print '</tr>';
	}
	if ($mysoc->localtax2_assuj == "1" || $object->total_localtax2 != 0) 	// Localtax2
	{
		print '<tr><td>'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td>';
		print '<td class="nowrap center">'.price($object->total_localtax2, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
		print '</tr>';
	}

	// Amount TTC
	print '<tr><td>'.$langs->trans('Importe Total').'</td>';
	print '<td class="nowrap center">'.price($object->total_ttc, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	print '</tr>';
	...
	if ($action != 'presend' && $action != 'loadCSV') {
		print '<div class="tabsAction">';
		...
		if (empty($reshook))
		{
            ...
            {
                print '<a class="butAction" target="_blank" href="'.DOL_URL_ROOT.'/comm/propal/propal_ticket.php?id='.$object->id.'">'.$langs->trans('PrintTicket').'</a>';
            }
			if ($action != 'editline')
			{
				if ($object->statut == Propal::STATUS_DRAFT) 
				{
                	if (!empty($user->rights->propal->client_to_fourn) && count($object->lines) > 0)
                	{
                		print '<a class="butAction" target="_blank" href="#" onclick="return generaCotizacionAProveedor();">'.$langs->trans('Cot. a Proveedor').'</a>';
                		?>
                		<script>
                			function generaCotizacionAProveedor()
                			{
                				...
                				if (ctt == 0)
                				{
                					alert ('Seleccione al menos un producto a cotizar');
                				}
                				else
                				{
                					$( "#cot_a_prov_dialog" ).dialog('open');
                				}
					...
					print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=loadCSV#formcsv">'.$langs->trans('LoadCSV').'</a>';
				}
				// Validate
				...
				if ($object->statut != Propal::STATUS_DRAFT && $usercancreate) {
					print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=exportexcel">'.$langs->trans('Exportar a Excel').'</a>';
				}
				// Edit
				...
				//TICKET EN POS
                //Create an pos ticket
                if($object->statut == Propal::STATUS_SIGNED)
                {
                    if(!empty($conf->pos->enabled) && $user->rights->pos->frontend)
                    {
						$sql1 = "SELECT a.rowid FROM ".MAIN_DB_PREFIX."element_element AS a, ".MAIN_DB_PREFIX."pos_ticket AS b";
						$sql1 .= " WHERE a.fk_source = ".$object->id." AND a.sourcetype = 'propal' AND a.targettype = 'ticket'";
						$sql1 .= " AND a.fk_target = b.rowid";
						$resql1=$db->query($sql1);
						if ($resql1) {
							$num1 = $db->num_rows($resql1);
							if($num1 > 0 ){
								print '<span id ="btnVenta" class="butAction" style="background-color:#73E540;">'.$langs->trans("Crear Venta en POS").'</span>';
								print '	<div id="dialogConfirm" title="Alerta">
											<p>No se puede crear la Venta en el POS, ya que existe una Cotizacion relacionada.</p>
											<center>
								...
								print '<a class="butAction" href="'.DOL_URL_ROOT.'/comm/propal/card.php?id='.$object->id.'&amp;action=create_ticket">'.$langs->trans("Crear Venta en POS").'</a>';
							}
						}
                    }
                }
	...
	if ($action != 'presend' && $action != 'loadCSV')
	{
		...
		$temporal = $object->element;
		$object->element = 'propal';

		$somethingshown = $form->showLinkedObjectBlock($object, $linktoelem, $compatibleImportElementsList);
		$object->element = $temporal;
	...
	if($action == 'exportexcel'){
	    ...
	    $table.= '<td colspan="7"><img src="'.$logo.'" width="20" height="20"></td>';
	    $table.= '</tr>';
	    $table.= '<tr>';

        $socex = new Societe($db);
        $socex->fetch($object->socid);
        $table.= '<td> Ref:'.$object->ref.'</td>';
        $table.= '<td> Cliente:'.$socex->nom.'</td>';
        $proj = 'select ref from llx_projet where rowid = '.$object->fk_project;
        $resqlpro = $db->query($proj);
        if($resqlpro){
            $project = $db->fetch_object($resqlpro);
            $table.= '<td> Proyecto:'.$project->ref.'</td>';
        }else{
            $table.= '<td> Proyecto: NA</td>';
        }
        $table.= '<td> Fecha:'.$object->date.'</td>';
        $table.= '<td> Fecha fin de Validez:'.$object->fin_validite.'</td>';
        $table.= '<td> Codigo del Cliente:'.$socex->code_client.'</td>';
        $table.= '</tr>';
        $table.= '</table>';
        $table.= '<table>';
        $table.= '<tr><td>&nbsp;</td></tr>';
        ...
        $table.= '<tr>';
        $table.= '<td style="border: 1px solid black;">Codigo</td>';
        $table.= '<td style="border: 1px solid black;">Descripcion</td>';
        $table.= '<td style="border: 1px solid black;">Cantidad</td>';
        $table.= '<td style="border: 1px solid black;">Precio Unitario (P.U.)</td>';
        $table.= '<td style="border: 1px solid black;">Descuento (Dto%)</td>';
        $table.= '<td style="border: 1px solid black;">Importe</td>';
        $table.= '</tr>';
        //Productos
        $i = 3;
        $prod1 = new Product($db);
        foreach ($object->lines as $key => $line){
            $prod1->fetch($line->fk_product);
            $table.= '<tr>';
            $table.= '<td style="border: 1px solid black;">'.$prod1->ref.'</td>';
            $table.= '<td style="border: 1px solid black;">'.$prod1->label.'</td>';
            $table.= '<td style="border: 1px solid black;">'.$line->qty.'</td>';
            $table.= '<td style="border: 1px solid black;">'.$line->subprice.'</td>';
            $table.= '<td style="border: 1px solid black;">'.$line->remise_percent.'</td>';
            $table.= '<td style="border: 1px solid black;">'.$line->total_ht.'</td>';
            $table.= '</tr>';
        }
        $discount = $object->getDiscount();
        $table.= '<tr><td colspan="5" align="right">Subtotal</td><td style="border: 1px solid black;">'.($object->total_ht+$discount).'</td></tr>';
        $table.= '<tr><td colspan="5" align="right">Descuentos</td><td style="border: 1px solid black;">'.$discount.'</td></tr>';
        $table.= '<tr><td colspan="5" align="right">Subtotal con Descuento</td><td style="border: 1px solid black;">'.$object->total_ht.'</td></tr>';
        $table.= '<tr><td colspan="5" align="right">IVA 16%</td><td style="border: 1px solid black;">'.$object->total_tva.'</td></tr>';
        $table.= '<tr><td colspan="5" align="right">Importe Total</td><td style="border: 1px solid black;">'.$object->total_ttc.'</td></tr>';
        $table.= '<tr><td colspan="6" align="right">'.$object->num2letras($object->total_ttc).'</td></tr>';
        $table.= '</table>';
        $table.= '</div>';
        print '<script language="javascript">
                $(document).ready(function() {
                    ...
                    var tabla_html = html.replace(/ /g, \'%20\');
                    tmpElemento.href = data_type + \', \' + tabla_html;
                    //Asignamos el nombre a nuestro EXCEL
                    tmpElemento.download = \''.$object->ref.'.xls\';
                    // Simulamos el click al elemento creado para descargarlo
                    tmpElemento.click();
                    setTimeout(function(){
                           location.href= \''.$_SERVER['PHP_SELF'].'?id='.$object->id.'\';
                        }, 200);
                });
                </script>';
        //header('Location:'.$_SERVER['PHP_SELF'].'?id='.$object->id);
	...
	// Form to load CSV
	if ($action == 'loadCSV') {
		print load_fiche_titre($langs->trans('LoadCSV'), '', 'title_document');

		print '<div class="tabBar">';
		print '<form action="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'" method="post" enctype="multipart/form-data" id="formcsv">';
		print '<input type="hidden" name="action" value="processcsv">';
		print '<table class="border centpercent">';
		print '<tr><td>Selecciona el archivo CSV:</td><td><input type="file" name="csvToUpload" id="xmlToUpload" accept=".csv"></td></tr>';
		print '</table><br>';
		print '<div class="center">';
		print '<input type="submit" class="button" name="bouton" value="' . $langs->trans('Load') . '">';
		print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
		print '<input type="button" class="button" value="' . $langs->trans("Cancel") . '" onClick="javascript:history.go(-1)">';
		print '</div>';
...
?>
<div style="display: none;" id="cot_a_prov_dialog" title="Crear Cotización a Proveedor">
	<select id="select_proveedor" class="select2" multiple="multiple" style="width: 100%;"></select>
	<p>¿Desea crear Cotización a Proveedor?</p>
	<table>
	</table>
</div>
<div  style="display: none;" id="delete_line_dialog" name="delete_line_dialog" title="Eliminar Productos">
...
$(document).ready(function() {
    $('#select_proveedor').select2({
    	ajax: {
    	 	url: '<?php echo DOL_URL_ROOT;?>/comm/propal/providers_ajax.php',
			dataType: 'json',
		...
		buttons: [
			{
				text: "Si",
				click: function() {
						var l = '<?php echo DOL_URL_ROOT; ?>/comm/propal/auto_prov_propal.php?lines=';
						var q = [];
						$('.ptp_checkbox').each(function(i,e){
							if($(e).is(':checked'))
							{
						...
						l += '&provs='+$('#select_proveedor').val();
						l += '&source=<?php echo GETPOST('id'); ?>';
						window.location.href = l;
				}
			},
			{
    ...
    $( "#cot_a_prov_dialog" ).dialog('close');
