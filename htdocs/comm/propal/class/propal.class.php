...
class Propal extends CommonObject
{
     ...
     *    	@see       	add_product()
     */
    public function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1 = 0.0, $txlocaltax2 = 0.0, $fk_product = 0, $remise_percent = 0.0, $price_base_type = 'HT', $pu_ttc = 0.0, $info_bits = 0, $type = 0, $rang = -1, $special_code = 0, $fk_parent_line = 0, $fk_fournprice = 0, $pa_ht = 0, $label = '', $date_start = '', $date_end = '', $array_options = 0, $fk_unit = null, $origin = '', $origin_id = 0, $pu_ht_devise = 0, $fk_remise_except = 0, $unit = '')
	{
	 ...
	 *  @return     int     		        		0 if OK, <0 if KO
	 */
    public function updateline($rowid, $pu, $qty, $remise_percent, $txtva, $txlocaltax1 = 0.0, $txlocaltax2 = 0.0, $desc = '', $price_base_type = 'HT', $info_bits = 0, $special_code = 0, $fk_parent_line = 0, $skip_update_total = 0, $fk_fournprice = 0, $pa_ht = 0, $label = '', $type = 0, $date_start = '', $date_end = '', $array_options = 0, $fk_unit = null, $pu_ht_devise = 0, $notrigger = 0, $unit = '')
	{
		global $mysoc, $langs;
    ...
    public function fetch($rowid, $ref = '')
	{
		...
		$sql .= ", p.date_cloture as date_cloture";
	...
	public function fetch_lines($only_product = 0, $loadalsotranslation = 0)
	{
		...
		$sql .= ' p.weight, p.weight_units, p.volume, p.volume_units, p.unit_entrada, p.unit_salida, p.tobuy, p.tosell,';
		$sql .= ' d.date_start, d.date_end, d.stock_sign,';
		...
		if ($result)
		{
			...
			while ($i < $num)
			{
				...
				$line->stock_sign	= $objp->stock_sign;
				...
				$line->unit_entrada = $objp->unit_entrada;
				$line->unit_salida  = $objp->unit_salida;
    ...
    public function cloture($user, $statut, $note = "", $notrigger = 0)
	{
		...
		if ($resql)
		{
			...
			if ($statut == self::STATUS_SIGNED)
			{
				...
				//Guardar stock a la fecha de firmado de cada producto
				foreach($this->lines as $line){
					//Obtenemos stock total de cada producto
					$sqlStock = "SELECT SUM( IF(reel < 0 , 0, reel ) ) as stock FROM ".MAIN_DB_PREFIX."product_stock WHERE fk_product = ".$line->fk_product;
					$result = $this->db->query($sqlStock);
					if ($result){
						$obj = $this->db->fetch_object($result);
						//Guardamos valor de stock a la fecha de FIRMADO
						$sql = "UPDATE ".MAIN_DB_PREFIX."propaldet SET stock_sign = ".( $obj->stock ? $obj->stock : 0);
	...
	/**
	 * Save lines into DB from CSV file.
	 * @param	array	$lines	Array with services information to put into DB.
	 * @return	int		-1 KO, 0 Nothing happened, > 0 Records inserted
	 */
	public function loadLinesFromCSV($lines) {
		...
		if ($num > 0) {
			...
			$sql .= ' (fk_propal, description, product_type, qty,remise_percent, tva_tx, subprice, rang, unit,';
			$sql .= ' total_ht, total_tva, total_ttc,';
			$sql .= 'multicurrency_code, multicurrency_subprice, multicurrency_total_ht, multicurrency_total_tva, multicurrency_total_ttc)';
			$sql .= ' VALUES ';
			...
			foreach($lines as $line) {
                ...
                if(empty($line->remise_percent)){
                    ...
                    $x=$line->remise_percent;
                }
				// Calculate prices
				$tabprice = calcul_price_total($line->qty, $line->subprice, $x, 16, 0, 0, 0, 'HT', 0, 1, $mysoc);
				...
				$pu_ht_devise = $tabprice[19];
				$sql .= " (".$this->id.",";
				$sql .= " '".utf8_encode($this->db->escape($line->desc))."',";
				$sql .= " '".$this->db->escape(1)."',"; // TODO: Later, not only 1.
				$sql .= " ".price2num($line->qty).",";
                $sql .= " ".$x.",";
				$sql .= " ".price2num(16).",";
				$sql .= " ".(price2num($pu_ht) !== '' ?price2num($pu_ht) : "null").",";
				$sql .= ' 1,';
				$sql .= " '".$this->db->escape($line->unit)."',";
			...
			if ($res) {
				$inserts = $this->db->affected_rows($res);
				$this->db->commit();
				return $inserts;
			}
			else {
	...
	public function printObjectLine($action, $line, $var, $num, $i, $dateSelector, $seller, $buyer, $selected = 0, $extrafields = null, $defaulttpldir = '/core/tpl', &$relationLines = null)
	{
		...
		if ($action != 'editline' || $selected != $line->id)
		{
			...
			if ($line->fk_product > 0)
			{
				...
				$text = $product_static->getNomUrl(1, '', 0, -1, 0, 1);
			...
			foreach ($dirtpls as $module => $reldir)
			{
				if (!empty($module))
				{
					$tpl = dol_buildpath($reldir.'/propalline_view.tpl.php');
				}
				else
				{
					$tpl = DOL_DOCUMENT_ROOT.$reldir.'/propalline_view.tpl.php';
				}
	...
	*/
	//UPDATE PRECIO VENTA DESDE LISTA PROVEDORES
	public function update_precios($line,$precio){
		$this->db->begin();

		$sql = "SELECT p.gain,";
		$sql.= " pd.rowid, pd.subprice, pd.qty, pd.remise_percent ,pd.tva_tx,";
		$sql.= " pd.localtax1_tx, pd.localtax2_tx, pd.description, pd.info_bits ,pd.special_code,";
		$sql.= " pd.fk_parent_line, pd.label, pd.product_type, pd.date_start ,pd.date_end,";
		$sql.= " pd.fk_unit, pd.multicurrency_subprice, pd.unit";
		$sql.= " FROM ".MAIN_DB_PREFIX."product as p";
		$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."propaldet as pd ON pd.fk_product = p.rowid";
		$sql.= " WHERE pd.rowid=".$line;
		...
		if ($resql){
			...
			$result = $this->updateline($obj->rowid,$precio,$obj->qty,$obj->remise_percent,$obj->tva_tx,$obj->localtax1_tx,$obj->localtax2_tx,$obj->description,'HT',$obj->info_bits,$obj->special_code,$obj->fk_parent_line,0,0,0,$obj->label,$obj->product_type,$obj->date_start,$obj->date_end,0,$obj->fk_unit,$obj->multicurrency_subprice,0,$obj->unit);
	...
	public function deleteLinesMassive($linesIDs){
		$sqlDelLines = "DELETE FROM ".MAIN_DB_PREFIX."propaldet WHERE rowid IN (".implode(",", $linesIDs).")";
