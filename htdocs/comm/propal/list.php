...
$date_start_endday		=GETPOST("date_start_endday");
$date_start_endmonth	=GETPOST("date_start_endmonth");
$date_start_endyear	=GETPOST("date_start_endyear");
$date_end_endmonth		=GETPOST("date_end_endmonth");
$date_end_endday		=GETPOST("date_end_endday");
$date_end_endyear		=GETPOST("date_end_endyear");
...
if($date_start_creationyear && !$date_end_creationyear)
    $sql.=" AND p.datec >= '".$date_start_creationyear."-".$date_start_creationmonth."-".$date_start_creationday."'";
else if (!$date_start_creationyear && $date_end_creationyear)
    $sql.=" AND p.datec <= '".$date_end_creationyear."-".$date_end_creationmonth."-".($date_end_creationday+1)."'";
else if ($date_start_creationyear && $date_end_creationyear)
    $sql.=" AND p.datec BETWEEN '".$date_start_creationyear."-".$date_start_creationmonth."-".$date_start_creationday."' AND '".$date_end_creationyear."-".$date_end_creationmonth."-".($date_end_creationday+1)."'";

if($date_start_endyear && !$date_end_endyear)
    $sql.=" AND p.fin_validite >= '".$date_start_endyear."-".$date_start_endmonth."-".$date_start_endday."'";
else if (!$date_start_endyear && $date_end_endyear)
    $sql.=" AND p.fin_validite <= '".$date_end_endyear."-".$date_end_endmonth."-".($date_end_endday+1)."'";
else if ($date_start_endyear && $date_end_endyear)
    $sql.=" AND p.fin_validite BETWEEN '".$date_start_endyear."-".$date_start_endmonth."-".$date_start_endday."' AND '".$date_end_endyear."-".$date_end_endmonth."-".($date_end_endday+1)."'";
...
if ($resql)
{
 	...
 	if ($socid > 0)
	{
		$soc = new Societe($db);
		$soc->fetch($socid);
		$title = $langs->trans('Listado de cotizaciones').' - '.$soc->name;
		if (empty($search_societe)) $search_societe = $soc->name;
	}
	else
	{
		$title = $langs->trans('Listado de cotizaciones');
	}
	...
	llxHeader('', $langs->trans('Cotizaciones'), $help_url);
	...
	if ($date_start_endday)         	$param .= '&date_start_endday='.urlencode($date_start_endday);
    if ($date_start_endmonth)       	$param .= '&date_start_endmonth='.urlencode($date_start_endmonth);
    if ($date_start_endyear)        	$param .= '&date_start_endyear='.urlencode($date_start_endyear);
	if ($date_end_endday)         		$param .= '&date_end_endday='.urlencode($date_end_endday);
    if ($date_end_endmonth)       		$param .= '&date_end_endmonth='.urlencode($date_end_endmonth);
    if ($date_end_endyear)        		$param .= '&date_end_endyear='.urlencode($date_end_endyear);
	...
	if (!empty($arrayfields['p.ref']['checked']))
	{
		...
		print '<input class="flat" size="18" type="text" name="search_ref" value="'.dol_escape_htmltag($search_ref).'">';
 		print '</td>';
	}
	...
	if (!empty($arrayfields['s.nom']['checked']))
	{
		...
		print '<input class="flat" type="text" size="25" name="search_societe" value="'.dol_escape_htmltag($search_societe).'">';
	    print '</td>';
	}
	...
	{
		print '<td class="liste_titre nowraponall" align="center">';
        if($date_start_endyear)$date_start_end=dol_mktime(0,0,0,$date_start_endmonth,$date_start_endday,$date_start_endyear);
        else $date_start_end=null;
        if($date_end_endyear)$date_end_end=dol_mktime(0,0,0,$date_end_endmonth,$date_end_endday,$date_end_endyear);
        else $date_end_end=null;
        print '<table>';
        print '<tr>';
        print '<td>';
		print "Desde<br>".$form->select_date($date_start_end,'date_start_end',0,0,0,'',1,0,1);
        print "<br>Hasta<br>".$form->select_date($date_end_end,'date_end_end',0,0,0,'',1,0,1);
        print '</td>';
        print '</tr>';
        print '</table>';
        print '</td>';
