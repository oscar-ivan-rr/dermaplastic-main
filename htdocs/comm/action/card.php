...
$eventCobranza = GETPOST('eventCobranza', 'int');
$cliente = GETPOST('fk_soc', 'int');
	...
	if (!$error)
	{
		...
		if ($idaction > 0)
		{
			if (!$object->error)
			{
				...
				if(GETPOSTISSET('promise')){
					$eventPromise = dol_clone($object,1);
					$eventPromise->id = 0;
					$datePromiseP = dol_mktime($fulldayevent ? '00' : GETPOST("prohour", 'int'), $fulldayevent ? '00' : GETPOST("promin", 'int'), $fulldayevent ? '00' : GETPOST("prosec", 'int'), GETPOST("promonth", 'int'), GETPOST("proday", 'int'), GETPOST("proyear", 'int'));
					$datePromiseF = dol_mktime($fulldayevent ? '00' : GETPOST("prohour", 'int'), $fulldayevent ? '00' : GETPOST("promin", 'int') + 5, $fulldayevent ? '00' : GETPOST("prosec", 'int'), GETPOST("promonth", 'int'), GETPOST("proday", 'int'), GETPOST("proyear", 'int'));
					$eventPromise->datep = $datePromiseP;
					$eventPromise->datef = $datePromiseF;

					$idPromise = $eventPromise->create($user);
					
					if($idPromise <= 0){
						setEventMessages("No se pudo crear el Evento Promesa de Pago", null, 'errors');
					}
				}
...
if ($action == 'create')
{
	...
	if($eventCobranza && $cliente){
		print '<input type="hidden" name="promise" value="1">';
	}
	...
	// Title
	print '<tr><td'.(empty($conf->global->AGENDA_USE_EVENT_TYPE) ? ' class="fieldrequired titlefieldcreate"' : '').'>'.$langs->trans("Label").'</td>';
	if($eventCobranza && $cliente){
		print '<td><input type="text" id="label" name="label" class="soixantepercent" value="Seguimiento de Cobranza"></td></tr>';
	}else{
		print '<td><input type="text" id="label" name="label" class="soixantepercent" value="'.GETPOST('label').'"></td></tr>';
	}
	print '</tr>';
	...
	if (GETPOST("afaire") == 1) {
		...
		if($eventCobranza && $cliente){
			print $form->selectDate(dol_now(), 'ap', 1, 1, 1, "action", 1, 2, 0, 'fulldaystart');
		}else{
	...
	if (GETPOST("afaire") == 1) {
		...
		if($eventCobranza && $cliente){
			print $form->selectDate(dol_now()+300, 'p2', 1, 1, 1, "action", 1, 1, 0, 'fulldayend');
		}else{
        	print $form->selectDate($datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 'fulldayend');
		}
    }
	...
	if($eventCobranza && $cliente){
		print $form->select_dolusers_forevent(($action == 'create' ? 'add' : 'update'), 'assignedtouser', 1, '', 0, '', '', 0, 0, 0, 'AND u.statut != 0', 1, $listofuserid, $listofcontactid, $listofotherid,$user->id);
	}else{
	...
	if ($conf->societe->enabled)
	{
		...
		else
		{
			...
			if (!empty($user->socid)) {
				...
				if($eventCobranza && $cliente){
					print $form->select_company($cliente, 'socid', '', 'SelectThirdParty', 1, 0, $events, 0, 'minwidth300');
				}else{
	...
	if($eventCobranza && $cliente){
		...
		print '<tr><td class="nowrap"><span class="fieldrequired">'.$langs->trans("DatePromise").'</span></td><td>';
	  	print $form->selectDate('', 'pro', 1, 1, 1, "promiseDate", 1, 1);
		print '</td></tr>';
	}
