...
class Expedition extends CommonObject
{
		...
		if ($resql)
		{
			...
			if ($this->db->query($sql))
			{
				...
				for ($i = 0; $i < $num; $i++)
				{
					...
					{	// no batch management
						if (! $this->create_line($this->lines[$i]->entrepot_id, $this->lines[$i]->origin_line_id, $this->lines[$i]->qty, $this->lines[$i]->rang, $this->lines[$i]->array_options, $this->lines[$i]->fk_lote, $this->lines[$i]->fk_lote_ficha, $this->lines[$i]->qty_asked_per_product) > 0)
						{
							$error++;
						}
					}
	 ...
	 */
	public function create_line($entrepot_id, $origin_line_id, $qty, $rang = 0, $array_options = 0, $fk_lote = 0, $fk_lote_ficha = 0, $qty_asked_per_product)
	{
		//phpcs:enable
		global $user;
		...
		$expeditionline->fk_lote = $fk_lote;
		$expeditionline->fk_lote_ficha = $fk_lote_ficha;
		$expeditionline->qty_asked_per_product = $qty_asked_per_product;
	...
	public function valid($user, $notrigger = 0)
	{
		...
		{
			$st_cm = new Commande($this->db);
			$st_cm->fetch($this->origin_id);
			$st_cm->fetchObjectLinked();
			if (
					property_exists($st_cm,'linkedObjects')
				&&	isset($st_cm->linkedObjects['ticket'])
				&&	is_array($st_cm->linkedObjects['ticket'])
				&&	count($st_cm->linkedObjects['ticket'])	
				)
			{
				foreach($st_cm->linkedObjects['ticket'] as $tk => $tv)
				{
					if (strtolower(get_class($tv)) == 'ticket')
					{
						$hasTicket = true;
						break;
					}
        ...
        // El almacén del usuario es el que define cambios en la referencia
		...
		{
			$numref = $this->getNextNumRef($soc, $user_warehouse);
		}
		else
		{
			$numref = "EXP".$this->id;
		...
		if (!$error && !$hasTicket && !empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_SHIPMENT))
		{
			require_once DOL_DOCUMENT_ROOT.'/product/stock/class/mouvementstock.class.php';
			...
			if ($resql)
			{
				...
				for ($i = 0; $i < $cpt; $i++)
				{
					...
					//CONSULTA PARA OBTENER STOCK ANTES DE MNODIFICARLO
					$sqlStock = "SELECT ps.reel 
									FROM ".MAIN_DB_PREFIX."product_stock AS ps 
										WHERE ps.fk_product = ".$obj->fk_product." 
											AND ps.fk_entrepot = ".$obj->fk_entrepot;
					$resqlStock = $this->db->query($sqlStock);
					if ($resqlStock)
					{
						$objStock = $this->db->fetch_object($resqlStock);
						$oldstock = $objStock->reel;
					}
					...
					//CONSULTA PARA STOCK NEGATIVO Y PERMISO ACTIVADO
					$sqlStock = "SELECT ps.reel 
									FROM ".MAIN_DB_PREFIX."product_stock AS ps 
										WHERE ps.fk_product = ".$obj->fk_product." 
											AND ps.fk_entrepot = ".$obj->fk_entrepot;
					...
					{
						$objStock = $this->db->fetch_object($resqlStock);
						if($objStock->reel < 0 && $user->rights->expedition->expeditionNoStock){
							$result=$mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, ($qty - $oldstock), $obj->subprice, $langs->trans("ShipmentIncrement", $numref), $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch,'',0,'',0);
							if ($result < 0) {
								$error++;
								$this->error = $mouvS->error;
								$this->errors = array_merge($this->errors, $mouvS->errors);
	...
	/**
	 * Decrementar el stock de los lotes cuando se realice el envio
	 * 
	 * 
	 * @param	int		$lote_id	Id del lote
	 * @param	int		$qty		Qty to move
	 * @param	int		$increment	1 to increment, 0 to decrement
	 */
	public function move_stock_lote($lote_id, $qty, $increment = 1, $idAlmacen, $idPrd){
		if($increment){
			$sql = "UPDATE ".MAIN_DB_PREFIX."product_batch SET qty = qty + ".$qty;
			$sql.= " WHERE rowid = ".$lote_id;
		}else{
			$sql = "UPDATE ".MAIN_DB_PREFIX."product_batch SET qty = qty - ".$qty;
			$sql.= " WHERE rowid = ".$lote_id;
		}
		...
		if($increment){
			...
			$sql.= " WHERE fk_product = ".$idPrd." AND fk_entrepot = ".$idAlmacen;
		...
		$sql1 = "DELETE FROM ".MAIN_DB_PREFIX."product_stock WHERE reel = 0 AND rowid NOT IN (SELECT fk_product_stock FROM ".MAIN_DB_PREFIX."product_batch as pb)";
	 ...
	 * @param 	int		$qty				Quantity
	 * @param	array	$array_options		extrafields array
	 * @param	array	$lote_id			Id if lote of warehouse
	 * @return	int							<0 if KO, >0 if OK
	 */
	public function addline($entrepot_id, $id, $qty, $array_options = 0, $lote_id = 0, $idLote_ficha = 0, $qty_asked_per_product)
	{
		...
		$line->qty = $qty;
		$line->fk_lote = $lote_id;
		$line->fk_lote_ficha = $idLote_ficha;
		$line->qty_asked_per_product = $qty_asked_per_product;
	...
	public function fetch_lines()
	{
		...
		$sql.= ", ed.rowid as line_id, ed.qty as qty_shipped, ed.fk_origin_line, ed.fk_entrepot, ed.date_devolucion, ed.fk_lote_ficha, ed.qty_asked_per_product";
		$sql.= ", p.ref as product_ref, p.label as product_label, p.fk_product_type, p.tosell, p.tobuy";
		$sql.= ", p.weight, p.weight_units, p.length, p.length_units, p.surface, p.surface_units, p.volume, p.volume_units, p.tobatch as product_tobatch, ed.fk_lote";
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				$line->fk_lote    		= $obj->fk_lote; // id of lote
				$line->fk_lote_ficha    		= $obj->fk_lote_ficha; // id of ficha lote
				$line->qty_asked_per_product   = $obj->qty_asked_per_product; // id of ficha lote
...
class ExpeditionLigne extends CommonObjectLine
{
	...
	/**
     * @var int $fk_lote_ficha
     */
    public $fk_lote_ficha;
	
	/**
     * @var int $qty_asked_per_product
     */
    public $qty_asked_per_product;
	...
	{
		$sql = 'SELECT ed.rowid, ed.fk_expedition, ed.fk_entrepot, ed.fk_origin_line, ed.qty, ed.rang, ed.fk_lote, ed.fk_lote_ficha, ed.qty_asked_per_product';
		...
		if ($result)
		{
			...
			$this->qty = $objp->qty;
			$this->rang = $objp->rang;
			$this->fk_lote = $objp->fk_lote;
			$this->fk_lote_ficha = $objp->fk_lote_ficha;
			$this->qty_asked_per_product = $objp->qty_asked_per_product;
	...
	public function insert($user, $notrigger = 0)
	{
		...
		$sql.= ", ".$ranktouse;
		$sql.= ", ".$this->fk_lote;
		$sql.= ", ".$this->fk_lote_ficha;
		$sql.= ", ".$this->qty_asked_per_product;
		$sql.= ")";
