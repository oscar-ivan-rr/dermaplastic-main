...
$arrayfields = array(
	...
	'importeTotal'=>array('label'=>$langs->trans("AmountTotal"), 'checked'=>1)
);
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."projet as p ON p.rowid = e.fk_projet";
...
if ($resql)
{
	...
	if ($massaction == 'createbills'){
		...
		$shipments = GETPOST('toselect', 'array');

		//Validación de Que solo Existan Pedidos en Estado: PROCESADO
		$sqlStatut = "SELECT ref FROM ".MAIN_DB_PREFIX."expedition";
		$sqlStatut .= " WHERE rowid IN (".implode(",", $shipments).") AND fk_statut != 2";

		if($resStatut = $db->query($sqlStatut)){
			if($db->num_rows($resStatut) > 0){
				setEventMessages('Solo pueden Seleccionarse Pedidos en Estado PROCESADO', '', 'errors');
				$error++;
				$massaction == '';
			}
		}
	...
	if (!empty($arrayfields['e.ref']['checked']))
	{
		...
		print '<input class="flat" type="text" name="search_ref_exp" value="'.$search_ref_exp.'">';
		print '</td>';
	}
	// Ref customer
	...
	{
		print '<td class="liste_titre">';
		print '<input class="flat" type="text" name="search_ref_customer" value="'.$search_ref_customer.'">';
		print '</td>';
	}
	// Thirdparty
	...
	{
		print '<td class="liste_titre left">';
		print '<input class="flat" type="text" name="search_project" value="'.dol_escape_htmltag($search_project).'">';
		print '</td>';
	}
	// Town
	...
	// Zip
	if (!empty($arrayfields['s.zip']['checked'])) print '<td class="liste_titre"><input class="flat" type="text" name="search_zip" value="'.$search_zip.'"></td>';
	// State
	if (!empty($arrayfields['state.nom']['checked']))
	{
		print '<td class="liste_titre">';
		print '<input class="flat" type="text" name="search_state" value="'.dol_escape_htmltag($search_state).'">';
		print '</td>';
	}
	// Country
	...
	if (!empty($arrayfields['l.ref']['checked']))
	{
		// Delivery ref
		print '<td class="liste_titre">';
		print '<input class="flat"  type="text" name="search_ref_liv" value="'.$search_ref_liv.'"';
		print '</td>';
	}
	...
	// ImporteTotal
	if (!empty($arrayfields['importeTotal']['checked']))
	{
	...
	if (!empty($arrayfields['importeTotal']['checked']))    print_liste_field_titre($arrayfields['importeTotal']['label'], "", "", "", $param, '', $sortfield, $sortorder, 'center ');
			...
			$sqlTotal = "SELECT SUM(ed.qty*(cd.subprice*(1-(cd.remise_percent/100))))*1.16 as total_ttc
						FROM llx_commande AS c, llx_commandedet AS cd, llx_expedition AS e, llx_expeditiondet AS ed, llx_element_element AS ee
							WHERE ee.targettype = 'shipping' AND ee.sourcetype = 'commande' AND ee.fk_source = c.rowid AND ee.fk_target = e.rowid
								...
								AND e.rowid = ".$obj->rowid;
			$resTotal = $db->query($sqlTotal);
			$objTotal = $db->fetch_object($resTotal);

			print '<td class="right nowrap">'.price($objTotal->total_ttc).'</td>';
			if (!$i) $totalarray['nbfield']++;
			if (!$i) $totalarray['pos'][$totalarray['nbfield']] = 'importeTotal';
			$totalarray['val']['importeTotal'] += $objTotal->total_ttc;
		}
