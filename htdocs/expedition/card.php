...
if (empty($reshook))
{
	...
	if ($action == 'reopen' && $user->rights->expedition->creer)
	{
		...
		if(!isset($_SESSION['devolucion_'.$object->id])) {
			$_SESSION['devolucion_'.$object->id] = array();
		}
	}
	...
	if ($action == 'add' && $user->rights->expedition->creer) {
		...
		$total_lotes = intval(GETPOST("total_lotes", 'int'));

		if ($total_lotes > 0) {
			// Validacion para que la suma de los productos a enviar por lote no supere la cantidad 
			$prod_cmp = GETPOST("prod1");
			$suma = 0;

			for ($i = 0; $i <= $total_lotes; $i++) {
				$qtylote = GETPOST("qtylote" . $i, 'int');
				if (empty($qtylote) || $qtylote <= 0) {
					continue;
				}

				$qty_data = GETPOST("qtydata_" . $i);
				$data_parts = explode('_', $qty_data);
				$qty_lote_real = (int) $data_parts[0];
				$qty_asked_per_product = (int) $data_parts[2];

				$batch_data = GETPOST("batchdata_" . $i);
				$data_parts = explode('_', $batch_data);
				$origin_line = (int) $data_parts[4];

				$quantityDelivered = $objectsrc->expeditions[$origin_line] ?: 0;

				if ($prod_cmp == GETPOST("prod$i")) {
					$suma += $qtylote;
					if ($suma > $qty_asked_per_product) {
						$error++;
						setEventMessages("Cantidad a enviar exceden la cantidad pedida", null, 'errors');
					}
					if ($qtylote > $qty_lote_real) {
						$error++;
						setEventMessages("Cantidad a enviar excede la cantidad del Lote", null, 'errors');
					}
				} else {
					$suma = $qtylote;
					$prod_cmp = GETPOST("prod$i");
				}
				if (($suma + $quantityDelivered) > $qty_asked_per_product) {
					$error++;
					setEventMessages("Cantidad a enviar más la cantidad enviada excede la cantidad pedida", null, 'errors');
				}
			}
			...
			if (!$error) {
				for ($i = 0; $i <= $total_lotes; $i++) {
					...
					$idAlmacen = (int) $data_parts[0];
					$idPrd = (int) $data_parts[1];
					$idLote = (int) $data_parts[2];
					$idLote_ficha = (int) $data_parts[3];
					$origin_line = (int) $data_parts[4];
					$eatby = $data_parts[5]; // Asumiendo que estas fechas no necesitan conversión a entero
					$sellby = $data_parts[6]; // Asumiendo que estas fechas no necesitan conversión a entero
					$batch = $data_parts[7]; // Asumiendo que este valor no necesita conversión a entero
					$fk_commandefourndet = (int) $data_parts[8];

					$qty_data = GETPOST("qtydata_" . $i);
					$data_parts = explode('_', $qty_data);
					$qty_asked_per_product = (int) $data_parts[2];

					$qtylote = (int) GETPOST("qtylote" . $i, 'int');

					if ((!empty($qty_asked_per_product) && !empty($idAlmacen) && !empty($idPrd) && !empty($idLote) && !empty($qtylote) && !empty($idLote_ficha)) && ($qtylote > 0)) {
						// Se añade el producto en el envio
						$ret = $object->addline($idAlmacen, $origin_line, $qtylote, 0, $idLote, $idLote_ficha, $qty_asked_per_product);
						$product = new Product($db);
						$product->fetch($idPrd);
						$result = $product->correct_stock_batch($user, $idAlmacen, $qtylote, 1, 'Envio de pedido a cliente: ' . $objectsrc->ref, 0, $eatby, $sellby, $batch, '', $objectsrc->element, $objectsrc->id);
						if ($ret < 0 || $result < 0) {
							setEventMessages($object->error, $object->errors, 'errors');
							$error++;
						} else {
						...
						setEventMessages($object->error, $objectsrc->errors, 'errors');
					}
				}
			}
			...
			if (!$error) {
				//VERIFICAR SI NO SE ESCEDE DE LIMITE DE CREDITO
				//Obtener Saldo Disponible del Cliente
				$cliente = new Client($db);
				$cliente->fetch($object->socid);
				$facturas = $cliente->getOutstandingBills();
				$tickets = $cliente->getOutstandingTickets();
				$saldo = $facturas['opened'] + $tickets['opened'];

				$banCredit = true;
				$clienteTemp = new Societe($db);
				$clienteTemp->fetch($object->socid);
				//Verificar si el Cliente es de CREDITO
				if ($clienteTemp->typent_id == 235) {
					$totalTemp = 0;
					foreach ($object->lines as $lineaExp) {
						$pedido = new Commande($db);
						$pedido->fetch($object->origin_id);
						$pedido->fetch_lines();
						foreach ($pedido->lines as $lineaComm) {
							if ($lineaExp->origin_line_id == $lineaComm->rowid) {
								//Total del Envio
								$totalTemp += $lineaExp->qty * ($lineaComm->subprice * (1 - ($lineaComm->remise_percent / 100)));
							}
						}
					}
					$totalTemp = $totalTemp * 1.16;
					$totalTemp = str_replace(',', '', price($totalTemp));
					$credito_disponible = $clienteTemp->outstanding_limit - $saldo;
					$credito_disponible = str_replace(',', '', price($credito_disponible));
					//Comparamos Credito Dispoble contra el Total del Envio
					if ($credito_disponible > $totalTemp) {
						$banCredit = true;
					} else {
						$banCredit = false;
					}
				}
				//VERIFICAR SI EXCEDE EL LIMTE DE CREDITO O ES UN CLIENTE DIFERENTE A CREDITO
				if ($banCredit == true) {
					// Se comenta esta parte para que no decremente desde stock
					...
					setEventMessages("El Envío no puede ser creado porque excede el Crédito Disponible.", "Limite de crédito: $" . price($clienteTemp->outstanding_limit) . "/Crédito Disponible: $" . price($credito_disponible) . "", 'errors');
					$error++;
				}
			}
		} else {
	...
	{
		$sql = "SELECT cd.fk_product, ed.fk_entrepot, ed.rowid as expeditiondet_id";
		$sql .= " FROM ".MAIN_DB_PREFIX."commandedet as cd,";
		$sql .= " ".MAIN_DB_PREFIX."expeditiondet as ed";
		$sql .= " WHERE ed.fk_expedition = ".$object->id;
	...
	elseif ($action == 'setQty' && $user->rights->expedition->creer)
	{		
		$productRef = GETPOST('productRef', 'int');
		$entrepotId = GETPOST('entrepotId', 'int');

		$currentQty = GETPOST('currentQty', 'int');
		$loteId = GETPOST('loteId', 'int');
		$newQty = GETPOST('newQty', 'int');
		$rowidDet = GETPOST('rowidDet', 'int');

		$totalQtyReal = $currentQty - $newQty;

		$totalQty = $newQty;

		$banT = false;

		for ($it = 0; $i < sizeof($_SESSION['devolucion_'.$object->id]); $i++){
			if($_SESSION['devolucion_'.$object->id][$it][0] == $productRef){

				$_SESSION['devolucion_'.$object->id][$it][1] = $entrepotId;
				$_SESSION['devolucion_'.$object->id][$it][2] = $currentQty;
				$_SESSION['devolucion_'.$object->id][$it][3] = $totalQty;
				$_SESSION['devolucion_'.$object->id][$it][4] = $loteId;
				$banT = true;
			}
		}
		...
		if(!$banT){
			$datos = array();
			array_push($datos,$productRef);
			array_push($datos,$entrepotId);
			array_push($datos,$currentQty);
			array_push($datos,$totalQty);
			array_push($datos,$loteId);

			array_push($_SESSION['devolucion_'.$object->id],$datos);
		}

		$devolucion = $totalQty;
		$sqlDev = "SELECT devolucion FROM ".MAIN_DB_PREFIX."expeditiondet WHERE rowid = ".$rowidDet."";
		$resqlDev = $db->query($sqlDev);
		if ($resqlDev){
			$objDev = $db->fetch_object($resqlDev);
			$devolucion = $objDev->devolucion + $totalQty;
		}
		date_default_timezone_set("America/Mexico_City");
		$now = dol_now();
		$sql = "UPDATE ".MAIN_DB_PREFIX."expeditiondet SET qty = ".$totalQtyReal.", devolucion = ".$devolucion.", date_devolucion = '".$db->idate($now)."' WHERE rowid = ".$rowidDet."";
		$resql = $db->query($sql);
    	if (! $resql){
			setEventMessages($db->error, $db->errors, 'errors');
		}else{
			//header("Location: card.php?id=".$object->id."&action=reopen&productRef=".$productRef."&entrepotId=".$entrepotId."&totalQty=".$totalQty);
			header("Location: card.php?id=".$object->id."&action=classifyclosed");
			exit;
		}
	}
	...
	{
		// Vars to know if the action comes from a digital signature. Person 1 = Customer (default), 0 = Driver
		$signed = GETPOSTISSET('saved') ? GETPOST('saved', 'int') : 1;
		$person = GETPOSTISSET('person') ? GETPOST('person', 'int') : 1;

		if(isset($_SESSION['devolucion_'.$object->id])){
			$incrementoQty = new MouvementStock($db);
			$incrementoQty->origin = $object;
			$productIncremento = new Product($db);
			$errorRec = 0;

			$sql = 'UPDATE '.MAIN_DB_PREFIX.'expedition SET fk_statut= 2 ';
			$sql .= ' WHERE rowid = '.$object->id.' AND fk_statut > 0';
			$resql = $db->query($sql);
			if ($resql){
				$url = BACKEND_URL."api/anymarket/orders/".$object->ref_customer;
				$ch = curl_init();
				...
				foreach($_SESSION['devolucion_'.$object->id] as $key => $dato){
					$productIncremento->fetch($dato[0]);
					// $result = $incrementoQty->reception($user, $dato[0], $dato[1], $dato[3], $productIncremento->price_ttc, "Devolución de Envio", '', '', '', '', 0, '', $dato[4]);
					
					$mv_res = $object->move_stock_lote($dato[4], $dato[3], 1, $dato[1], $dato[0]);
					if($result <= 0){
						$errorRec ++;
					}
				}
				if($errorRec > 0){
					setEventMessages($incrementoQty->error, $incrementoQty->errors, 'errors');
				}else{
					if(!empty(($_SESSION['devolucion_'.$object->id])))
						$object->generateDocument('devolucion', $outputlangs, $hidedetails, $hidedesc, $hideref);
					unset($_SESSION['devolucion_'.$object->id]);
					header('Location: '.$_SERVER["PHP_SELF"].'?id='.$object->id);
					exit();
				}
			}
		}else{
			if ($signed == 1 && $person == 1) {
				$object->fetch($id);
	...
	// Signature form
	elseif ($action == 'digital_sign' && $conf->global->MAIN_SIGNATURE_MODULE) {
		$ref = $object->ref;
		$element = "Expedition";
		$objectfile = "/expedition/class/expedition.class.php";
		$dir = $conf->expedition->dir_output."/sending/" . $object->ref;

		$person = GETPOSTISSET('person', 'int') ? GETPOST('person', 'int') : 0;
		$params = array('action=classifyclosed', "person=$person");
		$moreinputs = array('person' => $person);
...
$(document).ready(function() {
    // Manejar el evento keyup para el input de código de barras si el valor no es vacio
    ...
    // Obtener el valor del input oculto y asignarlo al span
    var totalAskedValue = $('input[name="totalasked"]').val();
    $('#totalOrdered').text(totalAskedValue);

	var totalDelivered = 0;
	$("input[name^='qtydelivered']").each(function() {
	...
	$('#scannedCount').text(totalDelivered);
});
...
function handleAjaxResponse(data) {
    if (data.status === 'error') {
        ...
        const inputId = `${data.fk_product}_${data.id}`;
        const inputElement = $(`#${inputId}`);
		if (!inputElement.length) {
			showNotification("El producto no se encuentra en la lista de productos a enviar.", "error");
			$("#barcodeScan").val("");
			$("#barcodeScan").focus();
			$("#barcodeScan").select();
			return;
		}
        let currentValue = parseInt(inputElement.val(), 10) || 0;
        const qty = parseInt(data.qty, 10) || 0;
        if (currentValue < qty) {
            inputElement.val(Math.min(currentValue + 1, qty));
...
function sumaTotal() {
	var total = 0;
    $("input[name^='qtylote']").each(function() {
		var value = parseInt($(this).val(), 10);
		if (!isNaN(value)) {
			total += value;
		}
...
function AddExpedition() {
    const barcode = $("#barcodeScan").val().replace(/ /g, "");
    const whid = <?php echo GETPOST('entrepot_id', 'int'); ?>;
    console.log("AddExpedition Call ajax search to replace products");
    $.getJSON(`<?php echo DOL_URL_ROOT ?>/expedition/ajax/ajax.php?action=search&warehouse=${whid}&barcode=${barcode}`, handleAjaxResponse);
}

function validarInput(input) {
    try {
        // Ensure the input element is usable
        ...
        console.error("Error in validarInput:", error);
    }
}
...
function handleChange(input, idPrd, index) {
	const oldValue = parseInt(input.dataset.oldValue, 10) || 0;
	validarCantidad(idPrd, index, oldValue);
}

function validarCantidad(idPrd, index, oldValue) {
    // Obtener el valor de los inputs
    const qtyInputElement = $(`input[name='qtylote${index}']`);
    let qtyInput = parseFloat(qtyInputElement.val());

    // Verificar si qtyInput no es un número entero
    if (!Number.isInteger(qtyInput) && qtyInput !== "") {
		showNotification("La cantidad ingresada no debe tener decimales.", "error");
        qtyInput = Math.floor(qtyInput);
        qtyInputElement.val(qtyInput);
    }

    // Convertir qtyInput a entero
    qtyInput = parseInt(qtyInput, 10);

    // Obtener los datos de cantidad
    const qtyData = $(`input[name='qtydata_${index}']`).val();
    const [_, qtyDelivered = 0, qtyAsked = 0] = qtyData.split("_").map(Number);

    // Calcular la cantidad máxima a entregar
    const maxQtyToDeliver = qtyAsked - qtyDelivered;

    // Calcular la suma total de las cantidades para el producto
    let qtyTotal = 0;
    $(`input[id^='${idPrd}_']`).each(function() {
    ...
    if (qtyInput < 0) {
        qtyInput = 0;
    } else if (qtyInput > maxQtyToDeliver) {
		qtyInput = Math.max(0, maxQtyToDeliver - (qtyTotal - qtyInput));
		showNotification("La cantidad ingresada excede el máximo permitido.", "error");
    } else if (qtyTotal > maxQtyToDeliver) {
        qtyInput = Math.max(0, maxQtyToDeliver - (qtyTotal - qtyInput));
        qtyTotal = maxQtyToDeliver;
		showNotification("La cantidad ingresada excede el máximo permitido.", "error");
    }else if (qtyInput > oldValue) {
		showNotification("Cantidad actualizada correctamente", "ok");
	}

    qtyInputElement.val(qtyInput);

    // Actualizar el contador de escaneados
    $('#scannedCount').text(sumaTotal());

    // Quitar el foco del elemento #barcodeScan
    $("#barcodeScan").blur();
}

function showNotification(message, type) {
    // Show the new notification
    $.jnotify(message, type, 1000, false, { remove: function (){} });
}
</script>
...
if ($action == 'create') {
	...
	if ($origin) {
		...
		if ($object->fetch($origin_id))	// This include the fetch_lines
		{
			...
			print '<form id="formCreate" action="' . $_SERVER["PHP_SELF"] . '" method="post">';
			...
			$i = 0;
			while ($i < $numAsked) {
				print 'if(!jQuery("#qtyl' . $i . '").hasClass("notDis' . $i . '")){';
			...
			print '<input type="text" name="searchbarcode" id="barcodeScan" placeholder="Código de barras" style="font-size: 20px;">';
			print '<button type="button" class="button" onclick="AddExpedition()">Agregar</button>';
			print '</div>';
			print '<div style="display: flex; align-items: center;">';
			print '<span>Cantidad escaneada: <span id="scannedCount">0</span>/<span id="totalOrdered"/></span>';
			print '</div>';
			print '</div>';
			...
			if ($numAsked) {
				...
				if (!empty($conf->stock->enabled)) {
					...
					print '<td class="left">' . $langs->trans("Qty") . '</td>';
			...
			$indiceAsked = 0;
			$index = 0;
			$identificador_product = 0;
			$totalasked = 0;

			$total_lotes = 0;
			while ($indiceAsked < $numAsked) {
				$identificador_product++;
				$product = new Product($db);
				...
				if (empty($reshook)) {
					...
					if ($warehouse_id > 0 || !($line->fk_product > 0) || empty($conf->stock->enabled))     // If warehouse was already selected or if product is not a predefined, we go into this part with no multiwarehouse selection
					{
						...
						if ($deliverableQty < 0)
							$deliverableQty = 0;
						// Product need lot
						print '<td colspan="5"></td></tr>'; // end line and start a new one for lot/serial
						...
						if (is_object($product->stock_warehouse[$warehouse_id]) && count($product->stock_warehouse[$warehouse_id]->detail_batch)) {
							...
							{
								$productlotObject->fetch($dbatch->lotid);
								$batchStock = +$dbatch->qty; // To get a numeric
								$deliverableQty = min($quantityToBeDelivered, $batchStock);

								print '<td colspan="3" ></td><td class="left">';

								print '<input name="prod' . $index . '" type="hidden" value="' . $identificador_product . '">';

								$batch_data = implode('_', [
									$warehouse_id, // ID del almacén
									$line->fk_product, // ID del producto
									$dbatch->id, // ID del lote
									$dbatch->lotid, // ID del lote
									$line->id, // ID de la línea
									$dbatch->eatby,	// Fecha de caducidad
									$dbatch->sellby, // Fecha de venta
									$dbatch->batch, // Lote
									$line->fk_commandefourndet // ID de la línea de pedido
								]);

								print '<input name="batchdata_' . $index . '" type="hidden" value="' . htmlspecialchars($batch_data, ENT_QUOTES, 'UTF-8') . '">';

								$qty_data = implode('_', [
									$dbatch->qty, // Cantidad disponible
									$quantityDelivered, // Cantidad entregada
									$line->qty // Cantidad solicitada
								]);

								print '<input name="qtydata_' . $index . '" type="hidden" value="' . htmlspecialchars($qty_data, ENT_QUOTES, 'UTF-8') . '">';

								print '<input id="' . $line->fk_product . '_' . $dbatch->id . '" name="qtylote' . $index . '" type="number" min="0" max="' . $dbatch->qty . '" value="' . (GETPOST("qtylote" . $index) ?: '0') . '" size="4" onfocus="storeOldValue(this)" onchange="handleChange(this, ' . $line->fk_product . ',' . $index . ')" oninput="validarInput(this)" >';
								print '</td>';
								print '<!-- Show details of lot -->';
								print '<td class="left">';

								// print '<input name="batchl' . $indiceAsked . '_' . $subj . '" type="hidden" value="' . $dbatch->id . '">'; // ID del lote

								print $productlotObject->getNomUrl(0) . ' ';
								...
								$subj++;
								print '</td>';
								print '<td class="left">' . $dbatch->qty . '</td>';
								print '<td class="left">' . dol_print_date($dbatch->eatby, "day") . '</td>';
								print '<td class="left">' . dol_print_date($dbatch->sellby, "day") . '</td>';
								print '</td></tr>';
								$index++;
							}
							$total_lotes += count($product->stock_warehouse[$warehouse_id]->detail_batch);
							// print '<input name="total_lotes" type="hidden" value="'.count($product->stock_warehouse[$warehouse_id]->detail_batch).'">';
						} else {
			...
			print '<input name="total_lotes" type="hidden" value="' . $total_lotes . '">';

			print '<input type="hidden" name="totalasked" value="' . $totalasked . '">';

			print '<br>';

			print '<div class="center">';
			print '<input id="btnConfirm" type="button" class="button" name="add" value="' . dol_escape_htmltag($langs->trans("Create")) . '">';
			...
			print '<div id="dialogConfirm" title="Confirmación">
						<p>¿Está seguro de crear este envío?</p>
						<center>
							<button id="confirmSI">SI</button>
			...
			//CONFIRMACION A LA HORA DE CREAR EL ENVIO
			print '<script>
...
/* *************************************************************************** */
{
	...
	if ($object->id > 0)
	{
        ...
        if (!empty($conf->projet->enabled)) {
            ...
            if (0) {    // Do not change on shipment
				...
				if(!empty($objectsrc->fk_project)){
					$sqlPro = 'UPDATE '.MAIN_DB_PREFIX.'expedition SET fk_projet = '.$objectsrc->fk_project;
					$sqlPro .= ' WHERE rowid = '.$object->id;
					$resqlPro = $db->query($sqlPro);
					if(!$resqlPro){
						setEventMessages('No se pudo actualizar el Proyecto', null, 'errors');
					}
				}
		...
		// Dimensions
		$opendim = GETPOST('opendim', 'int');
		
		if ($opendim) print '<tr class="open"><td class="dropdown-toggle" id="dimensions-toggle">';
		else print '<tr><td class="dropdown-toggle" id="dimensions-toggle">';
		print $langs->trans("Dimensions");
		print '</td><td colspan="3">&nbsp;</td></tr>';

		if ($opendim) print '<tbody id="dimensions-area">';
		else print '<tbody id="dimensions-area" style="display: none;">';

		// Weight
		print '<tr><td>';
		print $form->editfieldkey("Weight", 'trueWeight', $object->trueWeight, $object, $user->rights->expedition->creer, 'string', '&opendim=1');
		print '</td><td colspan="3">';
		...
		// Width
		print '<tr><td>'.$form->editfieldkey("Width", 'trueWidth', $object->trueWidth, $object, $user->rights->expedition->creer, 'string', '&opendim=1').'</td><td colspan="3">';
		...
		// Height
		print '<tr><td>'.$form->editfieldkey("Height", 'trueHeight', $object->trueHeight, $object, $user->rights->expedition->creer, 'string', '&opendim=1').'</td><td colspan="3">';
		...
		// Depth
		print '<tr><td>'.$form->editfieldkey("Depth", 'trueDepth', $object->trueDepth, $object, $user->rights->expedition->creer, 'string', '&opendim=1').'</td><td colspan="3">';
		...
		print '</tbody>';
		print '
        <!-- Toggle dimensions area -->
        <script>
        $(document).ready(function() {
            $("#dimensions-toggle").on("click", function(event) {
				$("#dimensions-area").fadeToggle("slow");
		...
		else
		{
			...
			if (!empty($conf->stock->enabled))
			{
				...
				print '<td class="left linecollote">'.$langs->trans("Lote").'</td>';
			}
		...
		for ($i = 0 ; $i < $num_prod ; $i++)
		{
			...
			if(empty($reshook))
			{
				...
				if ($lines[$i]->fk_product > 0)
				{
					...
					$red_ref = false;
					print '<td ';
					if($lines[$i]->tosell == "0" && $lines[$i]->tobuy == "0"){
						print 'style="color: red;" ';
						$red_ref = true;
					}
					print 'class="linecoldescription">';
					// Show product and description
					...
					$text=$product_static->getNomUrl(1,'',0,-1,0,0,$red_ref);
					$text.= ' - '.$label;
					...
					$sqlDev = "SELECT devolucion FROM ".MAIN_DB_PREFIX."expeditiondet WHERE rowid = ".$lines[$i]->id."";
					$resqlDev = $db->query($sqlDev);
					if ($resqlDev){
						$objDev = $db->fetch_object($resqlDev);
						if($objDev->devolucion > 0){
							print '<span style="color:red;">&nbsp;&nbsp;(Con Devolución de: '.$objDev->devolucion.')</span>';
						}
					}
					print "</td>\n";
				}
				...
				// Qty ordered
				print '<td class="center linecolqty">'.$lines[$i]->qty_asked_per_product.'</td>';
				...
				{
					// Qty to ship or shipped
					print '<td class="linecolqtytoship center">';

					if ($action == 'editQty'.$i)
					{
						print '<form name="setQty" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
						print '<input type="hidden" name="token" value="'.newToken().'">';
						print '<input type="hidden" name="action" value="setQty">';

						print '<input type="hidden" id="productRef" name="productRef" value="'.$lines[$i]->fk_product.'">';
						print '<input type="hidden" id="entrepotId" name="entrepotId" value="'.$lines[$i]->entrepot_id.'">';
						print '<input type="hidden" id="currentQty" name="currentQty" value="'.$lines[$i]->qty_shipped.'">';
						print '<input type="hidden" id="currentQty" name="loteId" value="'.$lines[$i]->fk_lote.'">';
						print '<input type="hidden" id="rowidDet" name="rowidDet" value="'.$lines[$i]->id.'" />';

						print '<input type="number" id="newQty'.$i.'" name="newQty" value="'.$lines[$i]->qty_shipped.'" />';
						print '<input type="submit" class="button" value="'.$langs->trans('Cant. a Devolver').'">';
						print '<a class="button" href="'.$_SERVER["PHP_SELF"].'?action=reopen&amp;id='.$object->id.'">'.$langs->trans('Cancel').'</a>';
						print '</form>';

						print '<script>
								$("#newQty'.$i.'").on("input", function () {
									if(this.value > '.$lines[$i]->qty_shipped.'){
										this.value = '.$lines[$i]->qty_shipped.';
									}
						...
						if($action == 'reopen'){
							print '<a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editQty'.$i.'&amp;id='.$object->id.'">'.img_edit('Editar cantidad').'</a>';
						}
						print $lines[$i]->qty_shipped.'</td>';
					}
				...
				$product_lot_static->fetch($lines[$i]->fk_lote_ficha);
	...
	if (($user->socid == 0) && ($action!='presend'))
	{
		...
		if (empty($reshook))
		{
			...
			if ($object->statut == 1 && $conf->global->MAIN_SIGNATURE_MODULE) {
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=digital_sign">'.$langs->trans("DriverSignature").'</a>';
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=digital_sign&person=1">'.$langs->trans("UserSignature").'</a>';
			}
			...
			if ($object->statut == Expedition::STATUS_CLOSED && $user->rights->expedition->creer)
			{
				...
				{
			    	print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=reopen">'.$langs->trans("Reject").'</a>';
				}
			}
