			...
			while ($i < $num)
			{
				...
				if (empty($reshook))
				{
					...
					if ($objp->fk_product > 0)
					{
						...
						$sqlDev = "SELECT devolucion FROM ".MAIN_DB_PREFIX."expeditiondet WHERE fk_origin_line = ".$objp->rowid."";
		...
		if (!empty($conf->stock->enabled) && ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED))
		{
			if ($user->rights->expedition->creer)
			{
				...
				if (!empty($conf->stock->enabled))
				{
					...
					//print '</td>';
					//print '<td>';
					$slAttr = empty($object->warehouse_id) ? '0' : '1';
					//print $formproduct->selectWarehouses(!empty($object->warehouse_id) ? $object->warehouse_id : 'ifone', 'entrepot_id', '', 1, $slAttr, 0, '', 0, 0, array(), 'minwidth200');
					/*
