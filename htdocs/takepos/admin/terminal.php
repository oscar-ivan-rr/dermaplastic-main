...
if (GETPOST('action', 'alpha') == 'set')
{
	...
	$res = dolibarr_set_const($db, "CASHDESK_NAME".$terminaltouse, GETPOST('terminal_name', 'alpha'), 'chaine', 0, '', $conf->entity);
	...
	$res = dolibarr_set_const($db, "CASHDESK_ID_BANKACCOUNT_FINALTRANSFER".$terminaltouse, (GETPOST('CASHDESK_ID_BANKACCOUNT_FINALTRANSFER'.$terminaltouse, 'alpha') > 0 ? GETPOST('CASHDESK_ID_BANKACCOUNT_FINALTRANSFER'.$terminaltouse, 'alpha') : ''), 'chaine', 0, '', $conf->entity);
    ...
    $res = dolibarr_set_const($db, "TAKEPOS_NUM_TICKETS_TO_PRINT".$terminaltouse, GETPOST('NUM_TICKETS_TO_PRINT', 'int'), 'int', 1, '', $conf->entity);
...
print '<tr class="oddeven"><td>'.$langs->trans("Nombre de terminal").'</td>';
print '<td>';
print '<input type="text" name="terminal_name" value="'.(empty($conf->global->{'CASHDESK_NAME'.$terminaltouse}) ? '' : $conf->global->{'CASHDESK_NAME'.$terminaltouse}).'">';
print '</td></tr>';
...
if (!empty($conf->banque->enabled))
{
	...
	print '<tr class="oddeven"><td>Cuenta para Transferencia de corte de caja</td>';
	print '<td>';
	$form->select_comptes($conf->global->{'CASHDESK_ID_BANKACCOUNT_FINALTRANSFER'.$terminaltouse}, 'CASHDESK_ID_BANKACCOUNT_FINALTRANSFER'.$terminaltouse, 0, "courant=1", 1);
	if (!empty($conf->global->{'CASHDESK_ID_BANKACCOUNT_FINALTRANSFER'.$terminaltouse})) $atleastonefound++;
	print '</td></tr>';
}
...
// Imprimir más de un ticket por venta
$terminalvariable = "TAKEPOS_NUM_TICKETS_TO_PRINT".$terminaltouse;
print '<tr class="oddeven"><td>'.$langs->trans("NumberOfTicketsToPrint").'</td>';
print '<td>';
print '<input type="number" name="NUM_TICKETS_TO_PRINT" value="'.(empty($conf->global->{$terminalvariable}) ? '1' : $conf->global->{$terminalvariable}).'" min="1">';
print '</td></tr>';
