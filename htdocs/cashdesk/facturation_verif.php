...
switch($action)
{
	default:
		...
		{
			$sql = "SELECT p.rowid, p.ref, p.price, p.tva_tx, p.default_vat_code, p.recuperableonly, lcp.fk_categorie";
			if (! empty($conf->stock->enabled) && !empty($conf_fkentrepot)) $sql.= ", ps.reel";
			$sql.= " FROM ".MAIN_DB_PREFIX."product as p JOIN llx_categorie_product lcp ON p.rowid = lcp.fk_product";
			if ($_SESSION['use_condition'] == 1)
			{
				$sql.= " JOIN llx_condition_product cp ON p.rowid = cp.fk_product";
			}
			...
			if ($_SESSION['use_condition'] == 1)
			{
				$sql.= " AND cp.fk_condition = '".$_SESSION['condition_id']."'";
			}
			// Recuperation des donnees en fonction de la source (liste deroulante ou champ texte) ...
			if ( $_POST['hdnSource'] == 'LISTE' )
			{
				if(empty($db->escape(GETPOST('txtRef', 'alpha'))))
					$sql.= " AND p.rowid = ".((int) GETPOST('selProduit', 'int'));
				else
					$sql.= " AND p.rowid = ".((int) GETPOST('selProduit', 'int'))." AND lcp.fk_categorie =".((int) GETPOST('txtRef', 'int'));
			}
	...
	case 'change_thirdparty':	// We have clicked on button "Modify" a thirdparty
		...
		print '<div style="text-align: center; position: fixed; top: 50%; left: 50%; -webkit-transform: translate(-50%, -50%); transform: translate(-50%, -50%);">
			<h1 style="color: #00A65E;">PACIENTE SELECCIONADO CON ÉXITO <br> &#10004;</h1>
			<div>';
		//$redirection = DOL_URL_ROOT.'/cashdesk/affIndex.php?menutpl=facturation';
        break;

	case 'change_condition':	// We have clicked on button "Padecimiento"
		$condition = GETPOST('id', 'int');
		if ($condition > 0)
		{
			$_SESSION["condition_id"] = $condition;
		}
		print '<div style="text-align: center; position: fixed; top: 50%; left: 50%; -webkit-transform: translate(-50%, -50%); transform: translate(-50%, -50%);">
			<h1 style="color: #00A65E;">PADECIMIENTO SELECCIONADO CON ÉXITO <br> &#10004;</h1>
			<div>';
		break;
	
	// Seleccionar listado de productos de borrador existente y agregarlos al poscart 
	case 'change_invoice':	
		unset($_SESSION['serObjFacturation']);
		...
		if ($newthirdpartyid > 0 && $invoice_id > 0)
		{
			$_SESSION["CASHDESK_ID_THIRDPARTY"] = $newthirdpartyid;
			$_SESSION["invoice"] = $invoice_id;
			$invoice = new Facture($db);
			$res_invoice = $invoice -> fetch($invoice_id);
			if($res_invoice > 0){
				$_SESSION["condition_id"] = $invoice->medical_condition;
				for ($i=0; $i < sizeof($invoice->lines); $i++) {
					$obj_facturation = new Facturation();
					
					$obj_facturation->id($invoice->lines[$i]->fk_product);
					$obj_facturation->qte($invoice->lines[$i]->qty);
					$obj_facturation->tva($invoice->lines[$i]->tva_tx);
					$obj_facturation->remisePercent($invoice->lines[$i]->remise_percent);
					$obj_facturation->prix($invoice->lines[$i]->subprice);
					$obj_facturation->note($invoice->lines[$i]->note);
					$obj_facturation->ajoutArticle();
				}
				
				print '<div style="text-align: center; position: fixed; top: 50%; left: 50%; -webkit-transform: translate(-50%, -50%); transform: translate(-50%, -50%);">
				<h1 style="color: #00A65E;">REGISTRO SELECCIONADO CON ÉXITO <br> &#10004;</h1>
				<div>';
	...
	// Agregar nota a cada producto
	case 'add_note_article':
		$obj_facturation->addNoteArticle($_POST['id'], $_POST['note']);
