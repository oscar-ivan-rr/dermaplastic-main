...
class Societe extends CommonObject
{
    ...
    /**
     * @var int Discount for early payment
     */
    public $earlypayment_discount;

    /**
     * @var int Validity in days for early payment discount
     */
	public $earlypayment_validity;

    /**
     * @var int Flag for automatic invoicing CFDI
     */
    ...
    public function create(User $user)
	{
		...
		if (empty($this->distance)) $this->distance = 'NULL';
		...
		// checking for Early payment discounts
		if (empty($this->earlypayment_discount)) $this->earlypayment_discount = 0;
		if (empty($this->earlypayment_validity)) $this->earlypayment_validity = 0;

		// checking automatic invoicing
        if (empty($this->automatic_invoicing)) $this->automatic_invoicing = 0;
		...
		if ($result >= 0)
		{
			...
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."societe (nom, name_alias, entity, datec, fk_user_creat, canvas, status, ref_int, ref_ext, fk_stcomm, fk_incoterms, location_incoterms ,import_key, fk_multicurrency, multicurrency_code, earlypayment_discount, earlypayment_validity, distance, automatic_invoicing, shipment)";
            ...
            $sql .= ", '".$this->db->escape($this->earlypayment_discount)."'";
			$sql .= ", '".$this->db->escape($this->earlypayment_validity)."'";
			$sql .= ", $this->distance";
			$sql .= ", ".$this->db->escape($this->automatic_invoicing);
			$sql .= ", ".(int) $this->shipment.")";
    ...
    public function update($id, $user = '', $call_trigger = 1, $allowmodcodeclient = 0, $allowmodcodefournisseur = 0, $action = 'update', $nosyncmember = 1)
	{
        ...
        $this->earlypayment_discount = ($this->earlypayment_discount)? dol_sanitizeFileName($this->earlypayment_discount) : 'null';
        $this->earlypayment_validity = ($this->earlypayment_validity)? dol_sanitizeFileName($this->earlypayment_validity) : 'null';

        // Automatic invoicing
        $this->automatic_invoicing = price2num($this->automatic_invoicing);
		...
		if ($result >= 0)
		{
            ...
            $sql .= ", earlypayment_discount = ".$this->db->escape($this->earlypayment_discount);
			$sql .= ", earlypayment_validity = ".$this->db->escape($this->earlypayment_validity);
			$sql .= ", distance = ".$this->distance;
			$sql .= ", automatic_invoicing = ".$this->automatic_invoicing;
			$sql .= ", shipment = ".(int) $this->shipment;
    ...
    public function fetch($rowid, $ref = '', $ref_ext = '', $ref_int = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '')
	{
        ...
        $sql .= ', s.earlypayment_discount';
		$sql .= ', s.earlypayment_validity';
		$sql .= ', s.distance';
		$sql .= ', s.automatic_invoicing, s.shipment';
		...
		if ($resql)
		{
			...
			elseif ($num)   // $num = 1
			{
                ...
                $this->earlypayment_discount = $obj->earlypayment_discount;
				$this->earlypayment_validity = $obj->earlypayment_validity;
				
				// Distance
				$this->distance = $obj->distance;

				// Automatic invoicing
                $this->automatic_invoicing = $obj->automatic_invoicing;

                $this->shipment = $obj->shipment;
    ...
    public function getNomUrl($withpicto = 0, $option = '', $maxlen = 0, $notooltip = 0, $save_lastsearch_value = -1)
	{
		...
		if ($this->client)
		{
			$pic = $this->_getCreditDocsStatus($notooltip, $label, $withpicto);
		}
		else
		{
			$pic = img_object(($notooltip ? '' : $label), ($this->picto ? $this->picto : 'generic'), ($notooltip ? (($withpicto != 2) ? 'class="paddingright"' : '') : 'class="'.(($withpicto != 2) ? 'paddingright ' : '').'classfortooltip valignmiddle"'), 0, 0, $notooltip ? 0 : 1);
		}
		$result .= $linkstart;
		if ($withpicto) $result .= $pic;
		if ($withpicto != 2) $result .= '<span style="/*display:block;margin-left:35px;*/ margin-top:-16px;">'.($maxlen ?dol_trunc($name, $maxlen) : $name).'</span>';
		$result .= $linkend;
		...
		$result = '<div style="display:inline-block;">'.$result.'</div>';
    ...
    public function LibStatut($status, $mode = 0)
	{
		...
		{
			$this->labelStatus[0] = $langs->trans("Suspendido");
			$this->labelStatus[1] = $langs->trans("Activo");
			$this->labelStatusShort[0] = $langs->trans("Suspendido");
			$this->labelStatusShort[1] = $langs->trans("Activo");
		}
     ...
     *  @return    array				array('opened'=>Amount, 'total'=>Total amount)
     */
    public function getOutstandingTickets($mode = 'customer')
    {
        $table = 'pos_ticket';
        //if ($mode == 'supplier') $table = 'supplier_proposal';

        $sql  = "SELECT pt.rowid,pt.total_ht,pt.total_ttc,pt.paye, pt.fk_statut, pt.type,pt.fk_facture,";
        $sql  .= "IF(pt.fk_facture is not null,(SELECT IF(f.fk_statut=3,0,1) FROM ".MAIN_DB_PREFIX."facture as f WHERE f.rowid=pt.fk_facture),0) as is_facture";
        $sql .= " FROM ".MAIN_DB_PREFIX.$table." as pt";
        $sql .= " WHERE pt.fk_soc = ".$this->id;
        if ($mode == 'supplier') {
            $sql .= " AND pt.entity IN (".getEntity('supplier_proposal').")";
        } else {
            $sql .= " AND pt.entity IN (".getEntity('propal').")";
        }
        //$sql .= "AND fk_facture IS NULL ";

        dol_syslog("getOutstandingTickets", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql)
        {
			$outstandingOpened = 0;
			...
			$tmpobject = new Ticket($this->db);
			
			$result = $resql->fetch_all(MYSQLI_ASSOC);
			//* Se cambia el metodo de recorrido de los datos para que sea mas rapido. Se cambia de fetch_object a fetch_all y se recorre con un foreach. Antes se usaba fetch_object y se recorria con un while lo que hacia que el sistema estuviera lento al momento de guardar un pago
			foreach ($result as $obj) {
				$obj = (Object)$obj;
				//! se quita la asignacion de id para que no se guarde en la tabla de pagos ya que esto estaba que el sistema estuviera lento al momento de guardar un pago
				// $tmpobject->id = $obj->rowid;
				$paiements = $tmpobject->getSommePaiement();
				$outstandingTotal += $obj->total_ht;
				$outstandingTotalIncTax += $obj->total_ttc;
				if ( ($obj->fk_statut == 1 || $obj->fk_statut == 2) && !$obj->is_facture && $obj->paye == 0)    // PROCESADO O PENDIENTE DE PAGO / SIN FACTURA / SN PAGO
				{
					$paiement = 0;
					if(isset($paiements[$obj->rowid])){
					$paiement = $paiements[$obj->rowid];
					}
					if($obj->type == 0) {
						$outstandingOpened += $obj->total_ttc-$paiement;
					}else{
						$outstandingOpened += (-1*$obj->total_ttc)+$paiement;
					}
				}
			}
            return array('opened'=>$outstandingOpened, 'total_ht'=>$outstandingTotal, 'total_ttc'=>$outstandingTotalIncTax); // 'opened' is 'incl taxes'
    ...
    public function getOutstandingOrders($mode = 'customer')
	{
		...
		$sql  = "SELECT f.rowid, f.total_ht, f.total_ttc, f.fk_statut";
		//$sql .= "if((SELECT ee.rowid from ".MAIN_DB_PREFIX."element_element as ee WHERE (ee.sourcetype='facture' or ee.targettype='facture') AND (ee.sourcetype='commande' or ee.targettype='commande') AND (ee.fk_source=f.rowid or ee.fk_target=f.rowid)) is null,0,1) is_facture";
		$sql .=" FROM llx_commande as f ";
		$sql .= " WHERE f.fk_statut IN (1,2) AND f.fk_soc = ".$this->id;
		if ($mode == 'supplier') {
			$sql .= " AND f.entity IN (".getEntity('supplier_order').")";
		} else {
		...
		if ($resql)
		{
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				$outstandingTotal += $obj->total_ht;
				$outstandingTotalIncTax += $obj->total_ttc;
				//if ($obj->fk_statut != 0 && !$obj->is_facture)
				$outstandingOpened += $obj->total_ttc;
			}
			...
			{
				/*
				$sql =	 'SELECT f.total_ttc,f.total,f.fk_statut '."\r\n" 
						.'FROM llx_commande AS c '."\r\n" 
						.'LEFT JOIN llx_element_element AS ee '."\r\n" 
						.'  ON (c.rowid = ee.fk_source AND ee.sourcetype=\'commande\') '."\r\n" 
						.'LEFT JOIN llx_facture AS f '."\r\n" 
						.'  ON ee.fk_target = f.rowid AND ee.targettype=\'facture\' '."\r\n" 
						.'WHERE c.rowid IN ('.implode(',',$rowids).') '."\r\n" 
						.'  AND f.fk_statut = 3'."\r\n"
						//.'  AND c.fk_statut = 1'."\r\n"
						;
				*/						
				$sql = "SELECT f.total_ttc,f.total,f.fk_statut, c.total_ttc as tPed";
				$sql .= " FROM llx_commande AS c , llx_facture AS f ";
				$sql .= " WHERE c.rowid IN (".implode(',',$rowids).") AND f.fk_statut NOT IN (0,3)";
				$sql .= "	AND (c.rowid IN (SELECT e.fk_source FROM llx_element_element AS e WHERE e.targettype = 'facture' AND e.sourcetype = 'commande' AND e.fk_target = f.rowid) ";
				$sql .= "	OR c.rowid IN (SELECT e.fk_target FROM llx_element_element AS e WHERE e.sourcetype = 'facture' AND e.targettype = 'commande'   AND e.fk_source = f.rowid) )";

				if ($resq2 = $this->db->query($sql))
				{
					while ($obj = $this->db->fetch_object($resq2)) 
					{
						$outstandingTotal -= $obj->total;

						if($obj->tPed <= $obj->total_ttc){
							$outstandingTotalIncTax -= $obj->tPed;
							$outstandingOpened -= $obj->tPed;
						}else{
							$outstandingTotalIncTax -= $obj->total_ttc;
							$outstandingOpened -= $obj->total_ttc;
						}
	...
	public function setCategories($categories, $type_categ)
	{
		...
		foreach ($to_add as $add) {
			if ($c->fetch($add) > 0)
			{
				...
				// Agregar descuento fijo si se registra tercero como cliente leal
				if (strtolower($c->label) == 'cliente leal') {
					$loyaldiscount = 10;
					$loyalnote = 'Primera Visita';

					$this->set_remise_client($loyaldiscount, $loyalnote, $user);
				}
				
				// Agregar descuento fijo si se registra tercero como cliente Especial
				if (strtolower($c->label) == 'cliente especial') {
					$loyaldiscount = 20;
					$loyalnote = 'Cliente especial';
	...
	/**
	 * Function to get supplier id with given RFC.
	 * @param	string	$rfc	Supplier RFC
	 * @return	int		-1 KO | > 0 Supplier ID.
	 */
	public static function getSupplierID($rfc) {
		global $db;

		$id = -1;
		$sql = "SELECT rowid as id FROM ".MAIN_DB_PREFIX."societe WHERE siren = '$rfc'";
    ...
    public function load_board($user, $mode)
    {
        ...
        if ($mode == 'cliente_excedido') {
            $response->warning_delay = null;
            $response->label = 'Con crédito excedido';
            $response->labelShort = 'Crédito excedido';
            $response->url = DOL_URL_ROOT . '/societe/list.php?mainmenu=companies&leftmenu=customers&action=client_excedido';
            $response->img = img_object('', "projectpub");
            $response->nbtodo = sizeof($this->GetAllWithDebt());
        }if ($mode == 'proveedor_excedido') {
            require_once DOL_DOCUMENT_ROOT.'/fourn/class/fournisseur.class.php';
            $fourn = new Fournisseur($this->db);
            $response->warning_delay = null;
            $response->label = 'Con crédito excedido';
            $response->labelShort = 'Crédito excedido';
            $response->url = DOL_URL_ROOT . '/societe/list.php?mainmenu=companies&leftmenu=suppliers&action=prov_excedido';
            $response->img = img_object('', "projectpub");
            $response->nbtodo = sizeof($fourn->GetAllWithDebt());
        }
    ...
    public function GetAllWithDebt(){
        $sql =" SELECT sub.rowid, sub.outstanding_limit as limite, SUM(sub.total_ttc), SUM(sub.amount), 
                    SUM(sub.total_ttc) - SUM(sub.amount) as deuda, SUM(sub.total_ttc) - SUM(sub.amount) > sub.outstanding_limit as excede
                FROM (
                    SELECT s.rowid, s.outstanding_limit, f.total_ttc, SUM(pf.amount ) AS amount
                    FROM
                        llx_societe s,
                        llx_facture f,
                    ...
                    group by s.rowid, f.rowid
                ) AS sub
                Group by sub.rowid";
        // Ya tenemos todos los clientes con sus facturas asociadas, su deuda, limite de credito y si excede
        $ids_exceden = array();
        $resql = $this->db->query($sql);
        if ($resql) {
            while ($obj = $this->db->fetch_object($resql)) {
                if ($obj->excede == 1)
                    array_push($ids_exceden, $obj->rowid);
            }
        }
        return $ids_exceden;
    }
    
    private function _getCreditDocsStatus($notooltip, $label, $withpicto, $mode=0)
    {
		require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
		$files = array(
		...
		#					 htdocs/societe/admin/societe_expiration.php
		$vals	= array(
						 '-1'			=> 'No Expira'
						,'+1 month'		=> '1 Mes'
						,'+2 months'	=> '2 Meses'
		...
		# Leer los archivos del disco
		$credit_dir = $conf->societe->multidir_output[$this->entity] . "/credit/" . get_exdir($this->id, 0, 0, 0, $this, 'thirdparty');
		...
		# Clasificar los archivos leidos
		...
		foreach($files as $k => $v)
		{
			...
			$tmpSelName = 'SOCIETE_CREDIT_DOCS_EXPIRATION_'.strtoupper($k).'_NOTIFICATION';
			foreach($files[$k] as $disk_file)
			{
				$days = ( is_numeric($conf->global->$tmpSelName) && $conf->global->$tmpSelName > 0 ) ? $conf->global->$tmpSelName : '30';
				if(strlen($disk_file['expires']) && $disk_file['expires'] < time())
				{
					$expired[$k][] = $disk_file['name'];
					$hasExpried = true;
				}
				elseif(strlen($disk_file['expires']) && strtotime('-'.$days.' days', $disk_file['expires']) < time())
				{
					$toExpire[$k][] = $disk_file['name'];
					$hasToExpire = true;
				}
				else
				{
					$okDocs[$k][] = $disk_file['name'];
				}
			}
		}
		$return = null;
		switch($mode)
		{
			...
			default:
				if ($hasExpried)
				{
					$msg =	 '<strong>Documentos de Crédito Expirados</strong>'
							.'<ul>'
							;
					...
					$msg .= '</ul>';
					$return = img_picto($msg, 'error.png', 'style="color:red;"' , 0, 0, 0);
				}
				elseif ($hasToExpire)
				{
					$msg =	 '<strong>Documentos de Crédito POR Expirar</strong>'
							.'<ul>'
							;
					foreach($toExpire as $k => $v)
					{
						$msg .= '<li>'.$titles[$k].'</li>';
					}
					$msg .= '</ul>';
					$return = img_picto($msg, 'warning.png', 'style="color:#fc0;"' , 0, 0, 0);
				}
				else
				{
					$return = img_object(($notooltip ? '' : $label), ($this->picto ? $this->picto : 'generic'), ($notooltip ? (($withpicto != 2) ? 'class="paddingright"' : '') : 'class="'.(($withpicto != 2) ? 'paddingright ' : '').'classfortooltip valignmiddle"'), 0, 0, $notooltip ? 0 : 1);
				}
				if ($hasPending)
				{
					$msg =	 '<strong>Documentos de Crédito Faltantes</strong>'
							.'<ul>'
							;
					...
					$msg .= '</ul>';
					$return .= img_picto($msg, 'statut8.png', 'style="color:#fc0;width:10px;height:auto;"' , 0, 0, 0);
				}
				break;
		}
		return $return.' ';
	...
	function reassignFixedDiscounts(){
        
        try {
            global $db;
			global $user;
			$currentDate = date('Y-m-d');
			$refdate = strtotime('-6 months', strtotime($currentDate));
			$refdate = date('Y-m-d' , $refdate);

			//Se selecciona el id de los clientes leales
			$sql = "SELECT cs.fk_soc AS id FROM llx_categorie_societe cs 
			JOIN llx_categorie lc ON lc.rowid  = cs.fk_categorie WHERE lc.label = 'Cliente Leal'";
			$resql = $db->query($sql);

			if (empty($resql))
            {
                $this->error = 'Error al obtener los clientes leales';
                return -6;
            }
            
			$count = 0;
			// Recorremos el resultado para validar el descuento fijo actual
            while ($obj = $db->fetch_object($resql))
            {
                $sql = "SELECT lsr.fk_soc AS id, lsr.remise_client as discount FROM llx_societe_remise lsr  WHERE lsr.fk_soc = ".$obj->id." ORDER BY lsr.rowid DESC 
				limit 1";
				$resql2 = $db->query($sql);

				if (empty($resql2))
				{
					$this->error = 'Error al obtener descuento fijo';
					return -7;
				}

				$obj2 = $db->fetch_object($resql2);

				// Solo se toman en cuenta a los clientes que tengan 14% o menos de descuento fijo
				if($obj2 -> discount > 10 and  $obj2 -> discount <= 14 ){
					// Se selecciona la factura más reciente del cliente
					$sql = "SELECT lf.fk_soc AS id, lf.datef AS dateFacture, lf.date_valid AS valid, lf.paye AS pay FROM llx_facture lf WHERE lf.fk_soc = ".$obj2->id." ORDER BY lf.datef DESC LIMIT 1";
					$resql3 = $db->query($sql);

					if (empty($resql3))
					{
						$this->error = 'Error al obtener factura del cliente';
						return -8;
					}

					while($obj3 = $db->fetch_object($resql3)){
						// Se valida si la fecha de facturación es de 6 meses atrás o más
						if($obj3 -> dateFacture <= $refdate and $obj3 -> valid != null and $obj3 -> pay == 1){
							// Si es de hace más de 6 meses, se reasignará el descuento fijo al 10%
							$this->fetch($obj3->id);
							$this->set_remise_client(10, 'Reasignación automática', $user);
							$count++;
						}
					}				
				}
			...
			if ($count == 1){
				$this->output = "Se reasignó el descuento fijo de: " . $count . " cliente.";
			}else {
				$this->output = "Se reasignó el descuento fijo de: " . $count . " clientes.";
			}

			
            return 0;
        } catch (\Throwable $th) {
            $this->error = 'Error al reasignar descuentos fijos a clientes leales ' . $th->getMessage();
            return -9;
        }
       
    }

	function discountIncrease(){
        ...
        try {
            ...
            while ($obj = $db->fetch_object($resql))
            {
				...
				// Solo se toman en cuenta a los clientes que tengan 14% o menos de descuento fijo
				if($obj2 -> discount != null and $obj2 -> discount <= 14 ){
					// Se seleccionan las facturas del día actual
					$sql = "SELECT lf.fk_soc AS id, lf.datef AS dateFacture, lf.date_valid AS valid, lf.paye AS pay FROM llx_facture lf WHERE lf.fk_soc = ".$obj2->id."  AND lf.datef = CURDATE()";
					$resql3 = $db->query($sql);
					if (empty($resql3))
					{
						$this->error = 'Error al obtener facturas del cliente';
						return -6;
					}
					
					$disc = $obj2 -> discount;
					$aux = 0;
					// Se recorre el resultado para aumentar el descuento fijo por cada cuenta por cobrar
					while($obj3 = $db->fetch_object($resql3) and $disc <= 14){
						// Se valida que la factura esté pagada en su totalidad
						if($obj3 -> dateFacture == $currentDate and $obj3 -> valid != null and $obj3 -> pay == 1){
							$disc ++;
							$this->fetch($obj3->id);
							$this->set_remise_client($disc ++, 'Incremento del 1% por nueva compra', $user);
							$aux ++;
						}
			...
			if ($count == 1){
				$this->output = "Se incrementó en 1% el descuento fijo de: " . $count . " cliente.";
			}else {
				$this->output = "Se incrementó en 1% el descuento fijo de: " . $count . " clientes.";
			}

            return 0;
        } catch (\Throwable $th) {
            $this->error = 'Error al incrementar descuentos fijos a clientes leales ' . $th->getMessage();
            return -9;
        }
       
    }

	function reassignAnnualDiscounts(){
        
        try {
            global $db;
			global $user;
			$currentDate = date('Y-m-d');
			$refdate = strtotime('-1 year', strtotime($currentDate));
			$refdate = date('Y-m-d' , $refdate);
            ...
            while ($obj = $db->fetch_object($resql))
            {
				...
				// Solo se toman en cuenta a los clientes que tengan el 15% de descuento fijo
				if($obj2 -> discount == 15 ){
					// Se selecciona la fecha del descuento más reciente del cliente
					$sql = "SELECT rowid, DATE_FORMAT(datec, '%Y-%m-%d') as datec, fk_soc as id FROM llx_societe_remise WHERE fk_soc = ".$obj2 -> id." ORDER BY rowid DESC LIMIT 1";
					$resql3 = $db->query($sql);

					if (empty($resql3))
					{
						$this->error = 'Error al obtener último descuento del cliente';
						return -8;
					}

					while($obj3 = $db->fetch_object($resql3)){
						// Se valida si la fecha de la última asignación de descuento es de hace un año o más
						if($obj3 -> datec <= $refdate){
							// Si es de hace 1 año o más, se reasignará el descuento fijo al 10%
							$this->fetch($obj3->id);
							$this->set_remise_client(10, 'Reinicio de descuento leal', $user);
							$count++;
						}
					}				
				}
