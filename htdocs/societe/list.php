...
$arrayfields = array(
	...
	's.nom'=>array('label'=>($type == 'f'?"Proveedor":"Cliente"), 'position'=>2, 'checked'=>1),
	...
	's.remise'=>array('label'=>"Descuento Fijo", 'position'=>28, 'checked'=>0),
	...
	's.siren'=>array('label'=>"RFC", 'position'=>40, 'checked'=>$checkedprofid1),
...
$sql.= " region.code_region as region_code,s.remise_client as remise_client, region.nom as region_name ";
...
if ($action == 'prov_excedido') {
    require_once DOL_DOCUMENT_ROOT.'/fourn/class/fournisseur.class.php';
    $fourn = new Fournisseur($db);
    $fourn_exceded = $fourn->GetAllWithDebt();
    $list_ids = '-1'; // to not show any other row if we don't have any exceded providers
    if (sizeof($fourn_exceded) >0)
        $list_ids = implode(', ', $fourn_exceded);
    $sql .= ' AND s.rowid IN ('. $list_ids .')';
}
if ($action == 'client_excedido') {
    $client_exceded = $companystatic->GetAllWithDebt();
    $list_ids = '-1'; // to not show any other row if we don't have any exceded clients
    if (sizeof($client_exceded) >0)
        $list_ids = implode(', ', $client_exceded);
    $sql .= ' AND s.rowid IN ('. $list_ids .')';
}
...
if ($user->rights->societe->creer)
{
	...
	}else $newcardbutton .= dolGetButtonTitle($langs->trans($label), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/societe/card.php?action=create&type=c&contextpage=poslist'.$typefilter);
}
...
// Excluir filtro para categorías de proveedor en POS
if ($contextpage != 'poslist' && (empty($type) || $type == 'f'))
{
	if (! empty($conf->categorie->enabled))
	{
		require_once DOL_DOCUMENT_ROOT . '/categories/class/categorie.class.php';
...
// Excluir filtro para comerciales en POS
if ( $contextpage != 'poslist' && ($user->rights->societe->client->voir || $socid))
{
 	$moreforfilter.='<div class="divsearchfield">';
...
if (! empty($arrayfields['s.nom']['checked']))
{
	...
	print '<input style="width:90%" class="flat searchstring maxwidth300imp" type="text" name="search_nom" value="'.dol_escape_htmltag($search_nom).'">';
	print '</td>';
}
if (! empty($arrayfields['s.name_alias']['checked']))
{
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth100imp" type="text" name="search_alias" value="'.dol_escape_htmltag($search_alias).'">';
	print '</td>';
}
// Barcode
...
{
	print '<td class="liste_titre" style="max-width:'.$cc_widht.' !important; width:'.$cc_widht.' important;">';
	print '<input class="flat searchstring" style="max-width:'.intval(intval($cc_widht)*0.8).'px ! important" type="text" name="search_customer_code" value="'.dol_escape_htmltag($search_customer_code).'">';
	print '</td>';
}
// Supplier code
...
if (! empty($arrayfields['s.remise']['checked']))
{  	
	print '<td class="liste_titre center">';
    print '</td>'; 
...
if (!empty($arrayfields['s.status']['checked']))
{
	...
	print $form->selectarray('search_status', array('0'=>$langs->trans('Suspendido'), '1'=>$langs->trans('Activo')), $search_status, 1);
	print '</td>';
}
...
if (!empty($arrayfields['s.code_client']['checked']))             print_liste_field_titre($arrayfields['s.code_client']['label'], $_SERVER["PHP_SELF"], "s.code_client", "", $param, ' style="max-width:'.$cc_widht.' !important;"', $sortfield, $sortorder);
...
if (!empty($arrayfields['s.remise']['checked'])) 		 print_liste_field_titre($arrayfields['s.remise']['label'], $_SERVER["PHP_SELF"], "s.remise", "", $param, '', $sortfield, $sortorder);
...
if (!empty($arrayfields['s.siren']['checked']))          print_liste_field_titre($form->textwithpicto($langs->trans("RFC"), $textprofid[1], 1, 0), $_SERVER["PHP_SELF"], "s.siren", "", $param, '', $sortfield, $sortorder, 'nowrap ');
...
while ($i < min($num, $limit))
{
	...
	$companystatic->remise_client= $obj->remise_client;
	...
	if (!empty($arrayfields['s.code_client']['checked']))
	{
		print '<td class="nowraponall " style="max-width:'.$cc_widht.' !important; width:'.$cc_widht.' !important;">'.$obj->code_client.'</td>';
		if (!$i) $totalarray['nbfield']++;
	}
	// Supplier code
	if (!empty($arrayfields['s.code_fournisseur']['checked']))
	...
	if (!empty($arrayfields['s.remise']['checked']))
	{
		print "<td>".($obj->remise_client + 0)."% </td>\n";
		if (!$i) $totalarray['nbfield']++;
	}
	if (!empty($arrayfields['s.url']['checked']))
	{
