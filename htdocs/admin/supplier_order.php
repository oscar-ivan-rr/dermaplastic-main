...
elseif ($action == 'set_SUPPLIER_ORDER_OTHER')
{
	...
	$res3 = dolibarr_set_const($db, "CEDIS_SUPPLIER", GETPOST('CEDIS_SUPPLIER', 'int'), 'chaine', 0, '', $conf->entity);
...
print "<tr class='oddeven'><td>Proveedor de CEDIS</td>";
print '<td width="100" class="right">';
print $form->select_company($conf->global->{'CEDIS_SUPPLIER'}, 'CEDIS_SUPPLIER', 's.fournisseur=1', 'SelectThirdParty', 0, 0, null, 0, 'minwidth200');
print '</td><td class="right">';
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print "</td></tr>\n";

// Option for select supplier for OC from a list of suppliers 
print '<tr class="oddeven">';
print '<td>' . $langs->trans('Proveedor para crear OC en estado validado') . '</td>';
print '<td></td>';
print '<td class="center">';

print $form->select_company($conf->global->SUPPLIER_ORDER_DEFAULT_SUPPLIER, 'socid', 's.fournisseur=1', 'SelectThirdParty', 0, 0, null, 0);

print "<script> 
$(document).ready(function() {
    $('#socid').change(function() {
        var socid = $(this).val();
        // set const with socid value
        $.ajax({
            url: '" . dol_buildpath('/core/ajax/constantonoff.php', 1) . "',
            type: 'POST',
            data: {
                action: 'set',
                name: 'SUPPLIER_ORDER_DEFAULT_SUPPLIER',
                value: socid,
                entity: " . $conf->entity . "
            },
