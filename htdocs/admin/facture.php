...
elseif ($action == 'setbankcxp')
{
    $bankcxp = GETPOST('bankcxp', 'alpha');

    $res = dolibarr_set_const($db, "FACTURE_BANK_CXP", $bankcxp, 'chaine', 0, '', $conf->entity);
...
// Cuenta de banco para cuentas por pagar
print '<form action="'.$_SERVER["PHP_SELF"].'" method="POST">';
print '<input type="hidden" name="token" value="'.newToken(). '" />';
print '<input type="hidden" name="action" value="setbankcxp" />';
print '<tr class="oddeven"><td>';
print $langs->trans("Cuenta de banco para cuentas por pagar");
print '</td><td width="60" class="center">';
$form->select_comptes($conf->global->FACTURE_BANK_CXP, 'bankcxp', 0, '', 1);
