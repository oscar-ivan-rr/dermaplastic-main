...
elseif ($action == 'setmail')
{
	$sql = "UPDATE ".MAIN_DB_PREFIX."const SET value = ".$value." WHERE name = 'EXPENSEREPORT_EMAIL'";
...
print load_fiche_titre($langs->trans("ExpenseReportsAutoMail"), '', '');
...
foreach ($dirmodels as $reldir){
	$dir = dol_buildpath($reldir."core/modules/expensereport/email/");
	if (is_dir($dir))
	{
		$handle = opendir($dir);
		if (is_resource($handle))
			...
			while (($file = readdir($handle))!==false)
			{
				if (substr($file, 0, 18) == 'mod_expensereport_' && substr($file, dol_strlen($file)-3, 3) == 'php')
				{
					...
					if ($module->isEnabled())
					{
						...
						print '<td class="center">';
						if ($conf->global->EXPENSEREPORT_EMAIL == '1')
						{
							print '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?action=setmail&amp;value=0">';
							print img_picto($langs->trans("Activated"), 'switch_on');
							print '</a>';
						}
						else if($conf->global->EXPENSEREPORT_EMAIL == '0')
						{
							print '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?action=setmail&amp;value=1">';
							print img_picto($langs->trans("Disabled"), 'switch_off');
							print '</a>';
						}
						print '</td>';
