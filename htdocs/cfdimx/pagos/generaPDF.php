...
$pdf2=new PDF();
$pdf->SetFont('Arial','',8);
$pdf->Cell(190,4,"Fecha de Pago: ".$pagos["fechaPago"],"LR",0,'L'); //cliente
$pdf->Ln();

$pdf->Cell(190,4,"Forma de Pago: ".strtoupper($pagos["formaDePago"]),"LR",0,'L'); //cliente
$pdf->Ln();

$pdf->Cell(190,4,"Moneda de Pago: ".$pagos["monedaP"] ,"LR",0,'L'); //cliente
$pdf->Ln();

if($pagos["tipoCambioP"]!=NULL && $pagos["tipoCambioP"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Tipo de Cambio: ".$pagos["tipoCambioP"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
//$pagos["monto"]=str_replace(",", "", number_format($respag->monto,2));
$pdf->Cell(190,4,"Monto: ".number_format($pagos["monto"],2) ,"LR",0,'L'); //cliente
$pdf->Ln();
if($pagos["numOperacion"]!=NULL && $pagos["numOperacion"]){
	//$pagos["numOperacion"]=$respag->numOperacion;
	$pdf->Cell(190,4,"Numero de Operacion: ".$pagos["numOperacion"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["rfcEmisorCtaOrd"]!=NULL && $pagos["rfcEmisorCtaOrd"]){
	//$pagos["rfcEmisorCtaOrd"]=$respag->rfcEmisorCtaOrd;
	$pdf->Cell(190,4,"RFC Emisor Cuenta Ordenante: ".$pagos["rfcEmisorCtaOrd"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["nomBancoOrdExt"]!=NULL && $pagos["nomBancoOrdExt"]){
	//$pagos["nomBancoOrdExt"]=$respag->nomBancoOrdExt;
	$pdf->Cell(190,4,"Nombre Banco Ordenante: ".$pagos["nomBancoOrdExt"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["ctaOrdenante"]!=NULL && $pagos["ctaOrdenante"]){
	//$pagos["ctaOrdenante"]=$respag->ctaOrdenante;
	$pdf->Cell(190,4,"Cuenta Ordenante: ".$pagos["ctaOrdenante"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["rfcEmisorCtaBen"]!=NULL && $pagos["rfcEmisorCtaBen"]){
	//$pagos["rfcEmisorCtaBen"]=$respag->rfcEmisorCtaBen;
	$pdf->Cell(190,4,"RFC Emisor Cuenta Beneficiario: ".$pagos["rfcEmisorCtaBen"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["ctaBeneficiario"]!=NULL && $pagos["ctaBeneficiario"]){
	//$pagos["ctaBeneficiario"]=$respag->ctaBeneficiario;
	$pdf->Cell(190,4,"Cuenta Beneficiario: ".$pagos["ctaBeneficiario"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["tipoCadPago"]!=NULL && $pagos["tipoCadPago"]){
	//$pagos["tipoCadPago"]=$respag->tipoCadPago;
	$pdf->Cell(190,4,"Tipo Cadena Pago: ".$pagos["tipoCadPago"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["certPago"]!=NULL && $pagos["certPago"]){
	//$pagos["certPago"]=$respag->certPago;
	$pdf->Cell(190,4,"Certificado del Pago: ".$pagos["certPago"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["cadPago"]!=NULL && $pagos["cadPago"]){
	//$pagos["cadPago"]=$respag->cadPago;
	$axscad_pago=str_split($pagos["cadPago"],110);
...
if($pagos["selloPago"]!=NULL && $pagos["selloPago"]){
	//$pagos["selloPago"]=$respag->selloPago;
	$axsello_pago=str_split($pagos["selloPago"],110);
...
$pdf2=new PDF();
$pdf->SetFont('Arial','',8);
$pdf->Cell(190,4,"ID Documento: ".$pagos["idDocumento"],"LR",0,'L'); //cliente
$pdf->Ln();

if($pagos["serie"]!=NULL && $pagos["serie"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Serie: ".$pagos["serie"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["folio"]!=NULL && $pagos["folio"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Folio: ".$pagos["folio"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
$pdf->Cell(190,4,"Moneda: ".$pagos["monedaDR"],"LR",0,'L'); //cliente
$pdf->Ln();
if($pagos["tipoCambioDR"]!=NULL && $pagos["tipoCambioDR"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Tipo de Cambio: ".$pagos["tipoCambioDR"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
$pdf->Cell(190,4,"Metodo de Pago: ".$pagos["metodoDePagoDR"],"LR",0,'L'); //cliente
$pdf->Ln();
if($pagos["numParcialidad"]!=NULL && $pagos["numParcialidad"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Numero de Parcialidad: ".$pagos["numParcialidad"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["impSaldoAnt"]!=NULL && $pagos["impSaldoAnt"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Importe Saldo Anterior: ".$pagos["impSaldoAnt"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["impPagado"]!=NULL && $pagos["impPagado"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Importe Pagado: ".$pagos["impPagado"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["impSaldoInsoluto"]!=NULL && $pagos["impSaldoInsoluto"]){
	//$pagos["tipoCambioP"]=$respag->TipoCambioP;
	$pdf->Cell(190,4,"Importe Saldo Insoluto: ".$pagos["impSaldoInsoluto"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
$pdf->Cell(190,4,"","LBR",0,'L'); //cliente
$pdf->Ln(5);
...
QRcode::png($data_cbb,$conf->facture->dir_output."/".$facture->ref."/Pago_".$prmsnd["uuid"].'.png');
// $pdf->Cell(150,5,'',0,0,'L');
// $pdf->CheckPageBreak(24);
$pdf->Image($conf->facture->dir_output."/".$facture->ref."/Pago_".$prmsnd["uuid"].'.png',162,$pdf->GetY()+10,42);
