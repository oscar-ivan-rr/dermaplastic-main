...
$labelp = (!empty($line->product_label) ? $line->product_label : $label);
if(strcmp($line->element, "facturedet") == 0){

	if ($line->fk_product > 0) {
		$producto_cfdimx = new Product($this->db);
		$producto_cfdimx->fetch($line->fk_product);
		$tipo_producto = ($producto_cfdimx->type == 0 ? 'Producto' : 'Servicio');

		$label = "";

		$label .= "<strong>Información del Cátalogo</strong><br>";

		if($producto_cfdimx->type == 0){
			$label .= img_picto('', 'product').' <u class="paddingrightonly">'.$langs->trans("Product").'</u><br>';
		}

		if($producto_cfdimx->type == 1){
			$label .= img_picto('', 'service').' <u class="paddingrightonly">'.$langs->trans("Service").'</u><br>';
		}

		if($producto_cfdimx->array_options["options_claveprodserv"] != "" && $producto_cfdimx->array_options["options_claveprodserv"] != -1){
			$label .= " ClaveProdServ: <b>".$producto_cfdimx->array_options["options_claveprodserv"]."</b><br>";
		}

		if($producto_cfdimx->array_options["options_umed"] != "" && $producto_cfdimx->array_options["options_umed"] != -1){
			$label .= " Unidad de Medida: <b>".$producto_cfdimx->array_options["options_umed"]."</b><br>";
		}

		if($producto_cfdimx->array_options["options_noidenticfdi"] != ""){
			$label .= " No. Identificación: <b>".$producto_cfdimx->array_options["options_noidenticfdi"]."</b><br>";
		}

		if($producto_cfdimx->array_options["options_aplicar_ret_individual"] != ""){
			$label .= " Aplicar Retención Individual: <b>".$producto_cfdimx->array_options["options_aplicar_ret_individual"]."</b><br>";
		}

		if($producto_cfdimx->array_options["options_exentoiva"] != ""){
			$label .= " Exento de I.V.A.: <b>".$producto_cfdimx->array_options["options_exentoiva"]."</b><br>";
		}

		if($producto_cfdimx->array_options["options_objimp"] != "" && $producto_cfdimx->array_options["options_objimp"] != -1){
			$label .= " Objeto de Impuesto: <b>".$producto_cfdimx->array_options["options_objimp"]."</b><br>";
		}
		...
		$ruta_cfdimx = "../../product/card.php?id=".$line->fk_product;

		$linkstart = '<a href="'.$ruta_cfdimx.'"';
		$linkstart .= $linkclose.' target="_blank">';
		$linkend = '</a>';
		...
		if ($withpicto != 2) {
			...
			$result .= '<b>';
				$result .= 'CFDIMX';
				$result .= '<span class="fa fa-info-circle valignmiddle btnTitle-icon"></span>&nbsp;';
			$result .= '</b>';
		}
		$result .= $linkend;
...
<?php if ($this->element == 'facture' && $this->statut == 0) { //Facturas Checkbox?>
	<input type="checkbox" class="product_discount ptp_checkbox" name="line_<?=$line->id?>" value="<?=$line->id?>">
<?php } ?>
</td>
<td class="dropdown-toggle" id="extra-toggle-<?=$line->id?>" style="cursor: pointer;"></td>
...
$coldisplay++;
print '<td class="nowrap">'.$labelp.'</td>';
...
print '</td>';
?>
	<td class="linecoluht nowrap right"><?php $coldisplay++; ?><?php print price($line->pa_ht); ?></td>
...
print '<script>
		$(document).ready(function() {
			$("#extra-toggle-'.$line->id.'").on("click", function(event) {
				...
				$("#extrarow-facturedet_batch_'.$line->id.'").toggle("slow");
				$("#extrarow-facturedet_expdate_'.$line->id.'").toggle("slow");
				$("#extrarow-facturedet_medico_'.$line->id.'").toggle("slow");	
