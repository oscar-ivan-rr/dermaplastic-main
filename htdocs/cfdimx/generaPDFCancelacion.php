...
$sql = "SELECT * FROM ".MAIN_DB_PREFIX."cfdimx c WHERE c.fk_facture = ".$id;
$resql = $db->query($sql);
if($resql) {
    if($row = $db->fetch_object($resql)) {
        $selloSAT = $row->sello;
        $dateS = $row->fecha_emision.' '.$row->hora_emision;
        $dateC = $row->fecha_timbrado.' '.$row->hora_timbrado;
        $uuid = $row->uuid;
    }
}
...
ob_get_clean();
$pdf->Output(DOL_DATA_ROOT."/facture/".$object->ref."/acuse_cancelacion.pdf", 'F');
