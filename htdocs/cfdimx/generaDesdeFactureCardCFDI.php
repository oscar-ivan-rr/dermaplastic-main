	...
	if( $id!="" ){
		...
		if( $existe_tabla>0 ){
			...
			if( $existe_umed > 0 ){
				...
				$resql=$db->query($sql);
				$obj = $db->fetch_object($resql);
				if( $obj->umed!="" && $obj->umed != 0){ $umed = limpiar($obj->umed); }else{ $umed = "NA"; }
			}else{
				$umed = "ZZ";
			}
		}else{
	...
	if( $id!="" ){
		...
		if( $existe_tabla>0 ){
			...
			if( $existe_umed > 0 ){
				...
				$resql=$db->query($sql);
				$obj = $db->fetch_object($resql);
				if( $obj->claveprodserv!="" && $obj->claveprodserv!=0 ){ $claveprodserv = limpiar($obj->claveprodserv); }else{ $claveprodserv = "01010101"; }
			}else{
...
function getProducto( $id, $db ){
	...
	$resql=$db->query($sql);
	$obj = $db->fetch_object($resql);
	$producto['ref'] = limpiar($obj->ref);
	$producto['label'] = limpiar($obj->label);
	$producto['description'] = limpiar($obj->description);
...
$fecha_factura_actual = date("Y-m-d")."T".$hora_factura;
$resql=$db->query($sql);
if ($resql){
	...
	if($num_fact){
		while ($i < $num_fact){
			...
			$fecha_factura = $fecha_factura_actual;
	...
	if($num_detalle_fact){
		while ($i < $num_detalle_fact){
		    ...
		    if($description_add == 1 && $obj->description != ""){
		    	if($desc != ""){$desc .= ' - ';}
		    	if(strpos(limpiar($obj->description),"'")){
			...
			if($obj->fk_product){
				...
				if( $existe_tabla>0 ){
					...
					if( $existe_umed > 0 ){
						...
						if( $objm->umed!="" && $objm->umed!=0){ $unidad = limpiar($objm->umed); }else{ $unidad = "NA"; }
					}else{
						$unidad = "ZZ";
					}
				}else{
			...
			if(false && $obj->fk_product){
				...
				if( $existe_tabla>0 ){
					...
					if( $existe_claveprodserv > 0 ){
						...
						$resqlm=$db->query($sqlm);
						$objm = $db->fetch_object($resqlm);
						if( $objm->claveprodserv!="" && $objm->claveprodserv!=0 ){ $claveprodserv = limpiar($objm->claveprodserv); }else{ $claveprodserv = "01010101"; }
					}else{
						$claveprodserv = "01010101";
					}
				}else{
			...
			if ($obj->fk_product && $claveprodserv == '01010101')
			{
				$claveprodserv = getclaveprodserv( $db, $obj->fk_product );
			}
			if($obj->fk_product){
				...
				if( $existe_tabla>0 ){
					...
					if( $existe_noidenticfdi > 0 ){
						...
						$resqlm=$db->query($sqlm);
						$objm = $db->fetch_object($resqlm);
						if( $objm->noidenticfdi!="" && $objm->noidenticfdi!=NULL && $objm->noidenticfdi!=null){ $noIdentificacion = limpiar($objm->noidenticfdi); 
						}else{ $noIdentificacion = NULL; }
					}else{
						$noIdentificacion = NULL;
					}
			...
			if($entraconcepto=='SI'){
				...
				$conceptos[$i] = array(
						...
						'base'=>str_replace($vowels, "", number_format($obj->total_ht,4)),
						'retenBase'=>$retenBase,
		 ...
		 while ($i < $num_emisor_datacomp){
			 ...
			 if ($obj){
				...
				WHERE fk_facture=".$facid;
				 ...
				 $emisor_delompio = limpiar($obj->emisor_delompio); // Convertir a Colonia
				 $col_emisor = limpiar(html_entity_decode($obj->emisor_colonia));
			 	}
				 $emisor_calle = limpiar($obj->emisor_calle);
				 $emisor_noint = limpiar($obj->emisor_noint);
				 $emisor_noext = limpiar($obj->emisor_noext);
	 ...
	 if ($num_emisor_datacomp){
		 while ($i < $num_emisor_datacomp){
			 ...
			 if ($obj){
				 ...
				 $receptor_colonia = limpiar($obj->receptor_colonia);
				 $receptor_calle = limpiar($obj->receptor_calle);
				 $receptor_noint = limpiar($obj->receptor_noint);
				 $receptor_noext = limpiar($obj->receptor_noext);
			 }
			 $i++;
		 }
	 }
...
$estado_emisor = getState(explode(':',$conf->global->MAIN_INFO_SOCIETE_STATE)[0]);
...
if( $valida_rfc_emisor && $valida_rfc_receptor  ){
	...
	if( $num_llx_cfdimx<1){
		...
		if(1/* $db->num_rows($sql_control) < 1 */){
			...
			//{
			//	if (!in_array($v['claveProdServ'],$cps))
			//	{
			//		$cps[] = $v['claveProdServ'];
			//	}
			//}
			//sort($cps);
			//print '<pre>CPSs<br>'; print_r($cps); print '</pre>';
			//die;
			...
			// $file_xml = fopen ($conf->facture->dir_output."/".strtoupper($serie).$guion.$folio."/".strtoupper($serie).$guion.$folio.".xml", "w");
...
// $prmsnd["coccds"] = "||".$prmsnd["version"]."|".$prmsnd["uuid"]."|".$prmsnd["fechaTimbrado"]."|".$prmsnd["selloCFD"]."|".$prmsnd["selloSAT"]."||";
				...
				*/
				...
				print '<script>
				location.href="card.php?facid='.$_REQUEST["facid"].'&cfdi_commit=1";
				</script>';
				}
			}else{
