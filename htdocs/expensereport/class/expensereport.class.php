...
class ExpenseReport extends CommonObject
{
    ...
    public function __construct($db)
    {
        ...
        $this->statuts_short = array(0 => 'Draft', 2 => 'ValidatedPay', 4 => 'Canceled', 5 => 'Approved', 6 => 'Paid', 99 => 'Refused');
        $this->statuts = array(0 => 'Draft', 2 => 'ValidatedPay', 4 => 'Canceled', 5 => 'Approved', 6 => 'Paid', 99 => 'Refused');
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        $sql .= ", ".$this->accountid;
        $sql .= ", ".($this->fk_projet ? $this->fk_projet : "null");
        $sql .= ", ".$this->fk_c_type_fees;
        $sql .= ", ".(!empty($this->referencia) ? "'".$this->referencia."'" : "null");
        $sql .= ", ".($this->fk_warehouse > 0 ? $this->fk_warehouse : 0);
        $sql .= ")";
    ...
    public function update($user, $notrigger = 0, $userofexpensereport = null)
    {
        ...
        $sql .= " , fk_projet = ".(!empty($this->fk_projet) ? "'".$this->db->escape($this->fk_projet)."'" : "null");
        $sql .= " , fk_c_type_fees = ".$this->fk_c_type_fees;
        $sql .= " , fk_typepayment = ".$this->fk_typepayment;
        $sql .= " , accountid = ".$this->accountid;
        $sql .= " , referencia = ".(!empty($this->referencia) ? "'".$this->referencia."'" : "null");
        $sql .= " , fk_warehouse = ".(!empty($this->fk_warehouse) ? "'".$this->db->escape($this->fk_warehouse)."'" : 0);
    ...
    public function fetch($id, $ref = '')
    {
        ...
        $sql .= " d.fk_typepayment, d.accountid, d.fk_projet, d.fk_c_type_fees, d.referencia, d.fk_warehouse";
        ...
        if ($resql)
        {
            ...
            if ($obj)
            {
                ...
                $this->fk_c_type_fees                = $obj->fk_c_type_fees;
...
function select_expensereport_statut($selected = '', $htmlname = 'fk_statut', $useempty = 1, $useshortlabel = 0)
{
    ...
    {
        if ($selected != '' && $selected == $key && $selected != '2,5,6')
        {
