...
//Variable para editar Referencia desde ficha
$referenciaEdit = GETPOST('referenciaEdit');
...
$path=trim(DOL_DOCUMENT_ROOT,'htdocs');
$path.="documents/expensereport/".$object->ref;
$path.="/signature.png";
unlink($path);
...
if (empty($reshook))
{
    ...
    if ($action == 'add' && $user->rights->expensereport->creer)
    {
		...
		//AÑADIR TIPO DE PAGO Y CUENTA
		if (!GETPOST('fk_typepayment', 'int'))
    	{
    		setEventMessages($langs->trans("ErrorFieldRequired", "Tipo de pago"), null, 'errors');
			$error++;
			$action = 'create';
    	}else{
			$object->fk_typepayment = GETPOST('fk_typepayment', 'int');
		}
		if (GETPOST('accountid', 'int') < 0)
    	{
    		setEventMessages($langs->trans("ErrorFieldRequired", "Cuenta de débito"), null, 'errors');
			$error++;
			$action = 'create';
    	}else{
    	...
    	{
    		setEventMessages($langs->trans("ErrorFieldRequired", "Tipo de Gasto"), null, 'errors');
			$error++;
			$action = 'create';
    	}else{
			$object->fk_c_type_fees = GETPOST('fk_c_type_fees', 'int');
		}

		$object->fk_projet = GETPOST('projectid', 'int');
    	$object->fk_statut = 1;
    	$object->fk_c_paiement = GETPOST('fk_c_paiement', 'int');
    	$object->fk_user_validator = GETPOST('fk_user_validator', 'int');
    	$object->note_public = GETPOST('note_public', 'none');
    	$object->note_private = GETPOST('note_private', 'none');
		$object->referencia = GETPOST('referencia');
    	...
    	if (!$error)
    	{
    		...
    		if (!$error)
    		{
				...
				$date_line = dol_mktime(0, 0, 0, $month, $day, $year);
				$res_line = $object->addline(1, GETPOST('commission'), 0, 0.000, $date_line, 'Comisión');
				if($res_line > 0){
					$ids = explode(",", GETPOST('ids'));
					$objecttmp = new Facture($db);
					foreach($ids as $id_cxc){
						$res_id = $objecttmp->fetch($id_cxc);
						$objecttmp->commission = 0;
						$objecttmp->commission_porcent = 0;
    ...
    if ($action == 'update' && $user->rights->expensereport->creer)
    {
		...
		//MODIFICAR TIPO DE PAGO Y CUENTA
		if (!GETPOST('fk_typepayment', 'int'))
    	{
    		setEventMessages($langs->trans("ErrorFieldRequired", "Tipo de pago"), null, 'errors');
			$action = 'edit';
    ...
    if ($action == "confirm_validate" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->expensereport->creer)
    {
		...
		*/
			...
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."payment_expensereport ";
			$sql.= " (fk_expensereport, datec, datep, amount, fk_typepayment, fk_bank)";
			$sql.= " VALUES ";
			$sql.= " (".$object->id.", '".$db->idate(dol_now())."','".$db->idate(dol_now())."',";
			$sql.= " ".$object->total_ttc.",".$object->fk_typepayment.", 0)";

			$resql=$db->query($sql);
			if ($resql)
			{
				$paymentid= $db->last_insert_id(MAIN_DB_PREFIX."payment_expensereport");
				$db->commit();
			...
			if (! $error)
            {
				$result=$payment->addPaymentToBank($user, 'payment_expensereport', '(ExpenseReportPayment)', $object->accountid, '', '');
                if (! $result > 0)
                {
    ...
    if ($action == "confirm_approve" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->expensereport->approve)
    {
    	...
    	if ($result > 0)
    	{
			...
			if ($conf->global->EXPENSEREPORT_EMAIL == '0')
			{
				// Send mail
    	...
    	else
    	{
	    	...
	    	if ($user->id == $object->fk_user_valid || $user->id == $object->fk_user_author)
	    	{
	    		...
	    		if ($result > 0)
	    		{
					// Eliminar pago
					$errorPago = 0;
					$sqlPago = "SELECT p.rowid ";
					$sqlPago .= " FROM ".MAIN_DB_PREFIX."expensereport as e, ".MAIN_DB_PREFIX."payment_expensereport as p";
					$sqlPago .= " WHERE e.rowid = '".$id."'";
					$sqlPago .= " AND p.fk_expensereport = e.rowid";
					$resqlPago = $db->query($sqlPago);
					if ($resqlPago){
						while($objpPago = $db->fetch_object($resqlPago)){
							$paymentPago = new PaymentExpenseReport($db);
							$paymentPago->fetch($objpPago->rowid);
							$sqlDeleteBank = "DELETE FROM llx_bank WHERE rowid = ".$paymentPago->fk_bank;
							$resqlDeleteBank = $db->query($sqlDeleteBank);
							if ($resqlDeleteBank){
								$resPagoDelete = $paymentPago->delete($user);
								if($resPagoDelete < 0){
									$errorPago++;
								}
							}
					...
					if($errorPago){
						setEventMessages("No se pudieron eliminar los pagos", null, 'errors');
					}
    ...
    if ($action == "addline" && $user->rights->expensereport->creer)
    {
		...
		*/
    	...
    	if (!$error)
    	{
			...
			// Insert line
    		$result = $object->addline($qty, $value_unit, 0, $vatrate, $date, $comments, $fk_project, $fk_c_exp_tax_cat, $type, $fk_ecm_files);
			if ($result > 0)
			{
    ...
    if ($action == "updateline" && $user->rights->expensereport->creer)
    {
		...
		*/
    	...
    	if (!$error)
    	{
    	    ...
    	    $result = $object->updateline($rowid, 0, $projet_id, $vatrate, $comments, $qty, $value_unit, $date, $id, $fk_c_exp_tax_cat, $fk_ecm_files);
    		if ($result >= 0)
    		{
    			if ($result > 0)
    			{
...
//Editar la referencia desde la ficha
	if ($action == 'setreferenciaEdit' && $user->rights->expensereport->creer)
	{
		$object = new ExpenseReport($db);
		$result = $object->fetch($id);
		$object->referencia = $referenciaEdit;
		$result = $object->update($user);
		if ($result < 0)
			setEventMessages($object->error, $object->errors, 'errors');
		$action = '';
...
if ($action == 'create')
{
	...
	print '<td>';
	print $form->selectDate(time() + ((7 * $conf->global->DATE_LIVRAISON_WEEK_DELAY) * 24 * 60 * 60), 'date_debut', 0, 0, 0, '', 1, 1);
	print '</td>';
	print '</tr>';
	...
	//TIPO DE PAGO
	$sql = "SELECT * FROM ".MAIN_DB_PREFIX."usergroup_user WHERE fk_usergroup = 4 AND fk_user = ".$user->id;
	$resql = $db->query($sql);
	$num = $db->num_rows($resql);
	print '<tr><td class="fieldrequired">'.$langs->trans("PaymentMode").'</td><td colspan="2">';
	print '<select name="fk_typepayment">';
	//SI PERTENECE AL GRUPO DE CAJAS, SOLO PODRA SELECCIONAR EFECTIVO
	if($num > 0){
		print '<option value="4">Efectivo</option>';
	}else{
		print '<option value=""></option>';
		print '<option value="4">Efectivo</option>';
		print '<option value="2">Transferencia bancaria</option>';
	}
    print '</select>';
	print "</td>\n";
	print '</tr>';
	...
	{
		// Get warehouse id
		$wh_id = isset($_POST["accountid"]) ? $_POST["accountid"] : (!empty($user->fk_warehouse) ? $user->fk_warehouse : $expensereport->accountid);
		$warehouse = Account::getBankWithWarehouse($wh_id);

    	print '<tr>';
    	print '<td class="fieldrequired">'.$langs->trans('AccountToDebit').'</td>';
    	print '<td colspan="2">';
    	$form->select_comptes($warehouse, "accountid", 0, '', 1);  // Show open bank account list
    	print '</td></tr>';
	}
	...
	print '<td colspan="2">';
	select_type_fees_id('', 'fk_c_type_fees');
	print '</td></tr>';
	if(is_numeric(GETPOST('commission'))){
		print '<tr>';
		print '<td>Comisión</td>';
		print '<td>';
		print '<input id="commission" name="commission" value="'.GETPOST('commission').'" text="'.GETPOST('commission').'" rows="1" style="margin-top: 5px; width: 20%" class="flat"></input>';
		print '</td></tr>';
		print '<input type="hidden" name="ids" value="'.GETPOST('ids').'">';
	}
	...
	print '<td colspan="2">';
	if($user->rights->stock->show_all_warehouses) print $formproduct->selectWarehouses($user->fk_warehouse, 'fk_warehouse', 'warehouseopen', 0);
	...
	print '<td>';
	print '<textarea id="referencia" name="referencia" rows="1" style="margin-top: 5px; width: 20%" class="flat"></textarea>';
	print '</td></tr>';
	...
	if (!empty($conf->projet->enabled))
	{
		...
		$numprojet = $formproject->select_projects(($soc->id > 0 ? $soc->id : -1), ( (GETPOST('projectid', 'int')) ? GETPOST('projectid', 'int'):''), 'projectid', 0, 0, 1, 1);
	...
	if ($id > 0 || $ref)
	{
		...
		if ($result > 0)
		{
			...
			if ($action == 'edit' && ($object->fk_statut < 3 || $object->fk_statut == 99))
			{
				...
				print '<tr '.($banProy ? 'style="display:none;"' : '').' >';
				...
				$resql = $db->query($sql);
				$num = $db->num_rows($resql);
				print '<tr '.($banProy ? 'style="display:none;"' : '').'><td class="fieldrequired">'.$langs->trans("PaymentMode").'</td><td colspan="2">';
				print '<select name="fk_typepayment">';
				//SI PERTENECE AL GRUPO DE CAJAS, SOLO PODRA SELECCIONAR EFECTIVO
				if($num > 0){
					print '<option value="4">Efectivo</option>';
				}else{
					if($object->fk_typepayment == 4){
						print '<option value=""></option>';
						print '<option value="4" selected>Efectivo</option>';
						print '<option value="2">Transferencia bancaria</option>';
					}
					elseif($object->fk_typepayment == 2){
						print '<option value=""></option>';
						print '<option value="4">Efectivo</option>';
						print '<option value="2" selected>Transferencia bancaria</option>';
					}else{
						print '<option value=""></option>';
						print '<option value="4">Efectivo</option>';
						print '<option value="2">Transferencia bancaria</option>';
				...
				$temp = ($user->fk_account?$user->fk_account:'0');
				$temp2 = ($object->accountid?$object->accountid:'0');
				print '<script>
							$(document).ready(function () {
								if('.$temp.' != "0"){
									$("#selectaccountid option[value='.$temp.']").attr("selected", true);
								...
								if('.$temp2.' != "0"){
									$("#selectaccountid option[value='.$temp2.']").attr("selected", true);
								}
							});
						</script>';		
				...
				print '<td class="fieldrequired">'.$langs->trans('Type').'</td>';
				print '<td colspan="2">';
				select_type_fees_id($object->fk_c_type_fees, 'fk_c_type_fees');
				print '</td></tr>';
				...
				print '<td class="fieldrequired">'.$langs->trans('Warehouse').'</td>';
				print '<td colspan="2">';
				if($user->rights->stock->show_all_warehouses) print $formproduct->selectWarehouses($object->fk_warehouse, 'fk_warehouse', 'warehouseopen', 0);
				else print $formproduct->selectWarehouses($object->fk_warehouse, 'fk_warehouse', 'warehouseopen', 0, 1);
				print '</td></tr>';
				...
				print '<td>'.$langs->trans('Reference').'</td>';
				print '<td>';
				print '<textarea id="referencia" name="referencia" rows="1" style="margin-top: 5px; width: 20%" class="flat">'.$object->referencia.'</textarea>';
				print '</td></tr>';
				...
				if (!empty($conf->projet->enabled))
				{
					...
					print '<tr>';
					print '<td>'.$langs->trans("Project").'</td><td>';
					$numprojet = $formproject->select_projects(($soc->id > 0 ? $soc->id : -1), $object->fk_projet, 'projectid', 0, 0, 1, 1);
					print ' <a href="'.DOL_URL_ROOT.'/projet/card.php?socid='.$soc->id.'&action=create&status=1&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create&socid='.$soc->id).'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("AddProject").'"></span></a>';
					print '</td>';
					print '</tr>';
				}
			...
			else
			{
				...
				// Referencia
				print '<tr><td>'.$form->editfieldkey($langs->trans("Reference"), 'referenciaEdit', $object->referencia, $object,1).'</td><td>';
				print $form->editfieldval($langs->trans("Reference"), 'referenciaEdit', $object->referencia, $object, 1, 'string');
				print '</td></tr>';

				//Tipo
				print '<tr>';
				print '<td class="titlefield">'.$langs->trans("Type").'</td>';
				$sqlTipo = "SELECT label FROM llx_c_type_fees WHERE id = ".$object->fk_c_type_fees;
				$resqlTipo = $db->query($sqlTipo);
				print '<td>';
				if($resqlTipo){
					$objTipo = $db->fetch_object($resqlTipo);
					print $langs->trans($objTipo->label);
				}
				print '</td>';
				print '</tr>';
				...
				//Almacén
				print '<tr>';
				print '<td class="titlefield">'.$langs->trans("Warehouse").'</td>';
				$sqlWarehouse = "SELECT ref FROM llx_entrepot WHERE rowid = ".$object->fk_warehouse;
				$resqlWarehouse= $db->query($sqlWarehouse);
				print '<td>';
				if($resqlWarehouse){
					$objWarehouse = $db->fetch_object($resqlWarehouse);
					print $langs->trans($objWarehouse->ref);
				}
				print '</td>';
				print '</tr>';
				...
				print '<td>';
				$sqlProyecto = "SELECT ref, title FROM ".MAIN_DB_PREFIX."projet WHERE rowid = ".$object->fk_projet;
				$resqlProyecto = $db->query($sqlProyecto);
				if($resqlProyecto){
					$objProyecto = $db->fetch_object($resqlProyecto);
					$projectstatic = new Project($db);
					$projectstatic->id = $object->fk_projet;
					$projectstatic->ref = $objProyecto->ref;
					$projectstatic->title = $objProyecto->title;
				...
				print '<tr>';
				print '<td class="titlefield">'.$langs->trans("PaymentMode").'</td>';
				print '<td>';
				if($object->fk_typepayment == 4){
					print 'Efectivo';
				}elseif($object->fk_typepayment == 2){
				...
				if( !is_null($object->accountid) ){
					$sql = "SELECT label FROM ".MAIN_DB_PREFIX."bank_account WHERE rowid = ".$object->accountid;
					$resql = $db->query($sql);
					$objp = $db->fetch_object($resql);
				}
				print '<tr>';
				print '<td class="titlefield">'.$langs->trans("AccountToDebit").'</td>';
				print '<td>';
				if( !is_null($object->accountid) ){
					print $objp->label;
				}
				    ...
				    && $user->rights->expensereport->creer)
				{
					...
					// Select date
					print '<td class="center">';
					print $form->selectDate(time() + ((7 * $conf->global->DATE_LIVRAISON_WEEK_DELAY) * 24 * 60 * 60), 'date', 0, 0, 0, '', 1, 1);
					print '</td>';
	...
	*/
	if ($user->rights->expensereport->creer && $object->fk_statut == ExpenseReport::STATUS_DRAFT)
	{
		if (in_array($object->fk_user_author, $user->getAllChildIds(1)) || !empty($user->rights->expensereport->writeall_advance))
		{
			...
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=save&id='.$object->id.'">'.$langs->trans('ValidateAndPay').'</a></div>';
			}
		}
	}
	...
	if ($user->rights->expensereport->approve && $object->fk_statut == ExpenseReport::STATUS_VALIDATED)
	{
		...
		//{
			//Modificar Proyecto
			if(!$object->fk_projet){
				print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=edit&banProy=1&id='.$object->id.'">'.$langs->trans('Modify').' Proyecto</a></div>';
			}
			// Validate
	...
	//Imprimir ticket
	//if($object->fk_statut >= ExpenseReport::STATUS_VALIDATED) {
	if($object->fk_statut >= ExpenseReport::STATUS_APPROVED) {
        print '<div class="inline-block divButAction"><a class="butAction" target="_blank" href="' . DOL_URL_ROOT . '/expensereport/aux_firma.php?id=' . $object->id . '">' . $langs->trans('PrintTicket') . '</a></div>';
    }
...
if($object->fk_statut == 2){
	$defaulttopic = 'Link para la Aprobación de Gasto: '.DOL_MAIN_URL_ROOT.'/expensereport/card.php?id='.$object->id;
}else{
