...
{
	$robject = json_decode($request_body);
...
$row =	'<tr>'
			.'<td style="vertical-align:top;width:145px;">'
				.date('Y-m-d H:i:s e').'<br />'.gmdate('Y-m-d H:i:s e')
			.'</td>'
...
$file_bitacora = DOL_DATA_ROOT.'/bitacoras/bitacora_'.gmdate('Y_m_d').'.html';
if (!file_exists($file_bitacora))
{
	$row = '<style>td{width: 200px !important;word-break: break-all;}</style><table>'.$row;
}
if($fp = fopen($file_bitacora,'a+'))
{
	fwrite($fp,$row);
	fclose($fp);
}
...
if (isset($_SESSION['no_duplicados']))
{
	if($_SESSION['no_duplicados']['fecha'] + 5 >= time()){		
        if($_SESSION['no_duplicados']['url'] == $_SERVER['REQUEST_URI'].'?'.$_SERVER['QUERY_STRING']){
            if($_SESSION['no_duplicados']['datos'] === $_POST){
				dol_syslog("DUPLICADOS: ".$_SESSION['no_duplicados'], LOG_WARNING);
				//setEventMessages("Error en petición.", "Petición realizada en un lapso de tiempo inadecuado.", 'errors');
...
if (!defined('NOLOGIN'))
{
	...
	if ($user->admin)
	{
		...
		$user->rights->user->user->modify_warehouse = 1;
...
{
	global $user, $conf, $langs, $db, $mysoc, $hookmanager;

	// Ensure variable is defined
	if (empty($conf->css)) $conf->css = '/theme/eldy/style.css.php';	// If not defined, eldy by default
	
	// Add custom alerts CSS as a default CSS for all pages
	if (!is_array($arrayofcss)) $arrayofcss = array();
	array_unshift($arrayofcss, '/theme/custom_alerts.css');
	
	// Add custom alerts JS handler for all pages
	if (!is_array($arrayofjs)) $arrayofjs = array();
	array_push($arrayofjs, '/theme/alert_handler.js');
	top_httphead();
