...
class SupplierProposal extends CommonObject
{
     ...
     *  @return     int     		        		0 if OK, <0 if KO
     */
    public function updateline($rowid, $pu, $qty, $remise_percent, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $desc = '', $price_base_type = 'HT', $info_bits = 0, $special_code = 0, $fk_parent_line = 0, $skip_update_total = 0, $fk_fournprice = 0, $pa_ht = 0, $label = '', $type = 0, $array_options = 0, $ref_supplier = '', $fk_unit = '', $pu_ht_devise = 0, $unit = '')
    {
    ...
    public function createPriceFournisseur($product, $user)
    {
        ...
        $sql .= '(datec, fk_product, fk_soc, ref_fourn, price, quantity, unitprice, tva_tx, fk_user, remise_percent, multicurrency_code,reason) VALUES ('.implode(',', $values).',"Cotización")';
    ...
    public function getLinesArray()
    {
        ...
        $sql.= ' pt.fk_multicurrency, pt.multicurrency_code, pt.multicurrency_subprice, pt.multicurrency_total_ht, pt.multicurrency_total_tva, pt.multicurrency_total_ttc, pt.fk_unit, pt.unit, p.tosell, p.tobuy';
        ...
        if ($resql)
        {
            ...
            while ($i < $num)
            {
                ...
                $this->lines[$i]->tosell		    = $obj->tosell;
                $this->lines[$i]->tobuy		        = $obj->tobuy;
	...
	public function loadLinesFromCSV($lines) {
		...
		if ($num > 0) {
			...
			$sql .= ' (fk_supplier_proposal, description, product_type, qty, tva_tx, subprice, rang, unit,';
			...
			foreach($lines as $line) {
				...
				// Calculate prices
				$tabprice = calcul_price_total($line->qty, $line->subprice, 0, 16, 0, 0, 0, 'HT', 0, 1, $mysoc);
...
class SupplierProposalLine extends CommonObjectLine
{
    ...
    public function fetch($rowid)
    {
        ...
        $sql.= ' pd.fk_multicurrency, pd.multicurrency_code, pd.multicurrency_subprice, pd.multicurrency_total_ht, pd.multicurrency_total_tva, pd.multicurrency_total_ttc, pd.fk_unit, pd.unit';
