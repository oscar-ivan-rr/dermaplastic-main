...
if (empty($reshook))
{
	...
	if ($action == 'addline' && $user->rights->supplier_proposal->creer)
	{
		...
		if (!$error && ($qty >= 0)) {
			...
			elseif ((GETPOST('price_ht') !== '' || GETPOST('price_ttc') !== '') && empty($error))    // Free product.  // $price_ht is already set
			{
				...
				$result = $object->addline($desc, $pu_ht, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, $idprod, $remise_percent, $price_base_type, $pu_ttc, $info_bits, $type, - 1, 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $array_options, $ref_supplier, $fk_unit, '', 0, 0, 0, 0, $unit);
    ...
    if($action =='apply_line_discount' && $object->statut == SupplierProposal::STATUS_DRAFT && $user->rights->supplier_proposal->creer){
        if(GETPOST('discount_percent')!=null){
            foreach($object->lines as $line){
                $result = $object->updateline($line->id, $line->subprice, $line->qty, GETPOST('discount_percent'), $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx,$line->desc, 'HT', $line->info_bits,$line->special_code,$line->fk_parent_line,0,$line->fk_fournprice,$line->pa_ht,$line->label,$line->product_type,$line->array_options,$line->ref_supplier,$line->fk_unit,$line->pu_ht,$line->unit);
            }
        }else{
	...
	elseif ($action == 'processcsv') {
		...
		$relation = array('descripción' => 'desc', 'precio u' => 'subprice', 'cantidad' => 'qty', 'ud' => 'unit');
		$columns = array();
		...
		while (($register = fgetcsv($csv)) !== FALSE) {
			$supplier_propaldet = new stdClass();

            // Asignación de propiedades
			foreach($columns as $index => $col)
				$supplier_propaldet->{$col} = $register[$index];

			// Required fields
			if (!empty($supplier_propaldet->desc) && !empty($supplier_propaldet->subprice) && !empty($supplier_propaldet->qty))
				$lines[] = $supplier_propaldet;
			else {
				$error++;
				break;
			}

			unset($supplier_propaldet);
		}
		...
		if (!$error) {
			...
			if (sizeof($lines) > 0) {
				...
				// Success
				if ($res > 0) setEventMessage($langs->trans("AddedProducts", $res));
				// Error
...
if ($action == 'create')
{
	...
	// Model
	print '<tr>';
	print '<td style="display: none;">'.$langs->trans("DefaultModel").'</td>';
	print '<td colspan="2">';
	$liste = ModelePDFSupplierProposal::liste_modeles($db);
	print $form->selectarray('model', $liste, ($conf->global->SUPPLIER_PROPOSAL_ADDON_PDF_ODT_DEFAULT ? $conf->global->SUPPLIER_PROPOSAL_ADDON_PDF_ODT_DEFAULT : $conf->global->SUPPLIER_PROPOSAL_ADDON_PDF),'','','','','','','','','"style="display: none;');
	print "</td></tr>";
	...
	// Thirdparty
	$morehtmlref .= $langs->trans('Supplier').' : '.$object->thirdparty->getNomUrl(1);
					...
					buttons: [
						{
							text: \"Si\",
							click: function() {
									var l = '".DOL_URL_ROOT."/supplier_proposal/card.php?id=".$id."&action=confirmDelete&lines=';
									var q = [];
	...
	if (!empty($conf->multicurrency->enabled) && ($object->multicurrency_code != $conf->currency))
	{
		...
		print '<td class="center">'.price($object->multicurrency_total_ht, '', $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $conf->currency)).'</td>';
		print '</tr>';
	...
	// Amount HT
    // $total = number_format($object->total_ht+$discount,2);
	print '<tr><td class="titlefieldmiddle">'.$langs->trans('AmountHT').'</td>';
	print '<td class="center">'.price($subtotal, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	print '</tr>';
	...
	print '<td class="center">'.price($object->total_tva, '', $langs, 0, - 1, - 1, $conf->currency).'</td>';
	print '</tr>';
	...
	{
		print '<tr><td class="center" height="10">'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td>';
			...
			if ($action != 'statut' && $action != 'editline')
			{
				...
				if (!empty($conf->fournisseur->enabled) && $object->statut == SupplierProposal::STATUS_SIGNED) {
					if ($user->rights->fournisseur->commande->creer) {
						print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/fourn/commande/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'">'.$langs->trans("Crear OC").'</a></div>';
					}
				}
