...
class Categorie extends CommonObject
{
	...
	public function fetch($id, $label = '', $type = null, $ref_ext = '')
	{
		...
		$sql = "SELECT rowid, fk_parent, entity, label, description, color, fk_soc, visible, type, ref_ext, samePrice";
	...
	public function create($user)
	{
		...
		if (!is_numeric($this->samePrice)) $this->samePrice = 0;
		...
		$sql .= $this->db->escape($this->samePrice);
		$sql .= ")";
	...
	public function update(User $user)
	{
		...
		$sql .= " samePrice = ".$this->db->escape($this->samePrice);
		if (!empty($conf->global->CATEGORY_ASSIGNED_TO_A_CUSTOMER))
		{
	 ...
	 */
	public function get_full_arbo($type, $markafterid = 0, $include = 0,$action='')
	{
        // phpcs:enable
	 ...
	 *                      	    labels, 'id'= Get array of category IDs, 'samePrice' Get array of category ids with samePrice = 1 (only for product)
	...
	public function containing($id, $type, $mode = 'object')
	{
        ...
        {
    		$sql = "SELECT ct.fk_categorie, c.label, c.rowid, c.samePrice";
			...
			if ($mode == 'samePrice') $sql .= " AND c.samePrice = 1";
	...
	/**
	 * Select id of category if exist.
	 * @param	string	$label			Category label
	 * @param	const	$type_element	Category type
	 * @return	int		Id if Ok | < 0 KO
	 */
	public function getId($label = '', $type_element) {
		// Map id type
		$type = $this->MAP_ID[$type_element];
		...
		$sql .= " WHERE c.entity IN (".getEntity('category').")";
		$sql .= " AND c.type = ".$type;
		$sql .= " AND c.label = '".$this->db->escape($label)."'";
		$sql .= " AND c.fk_parent = ".$this->fk_parent;
		...
		if ($resql) {
			...
			dol_syslog(get_class($this)."::getId with name=".$label." exists: rowid=".$obj->id, LOG_DEBUG);
			return $obj->id;
		}
		else {
			$this->error = $this->db->error();
	...
	/**
	 * Update products prices of this category
	 * @param	array	$prices		Array with price_min, price, price_min_ttc, price_ttc and base_price_type
	 * @param	int		$copyBases	Update price bases.
	 * @return	int		-1 KO, 0 Nothing happened, 1 OK
	 */
	public function setPriceToProducts($prices, $copyBases = 1) {
		global $user;

		$prods = $this->getObjectsInCateg('product', 1);

		// Checar que la categoría tenga más de un producto.
		if (sizeof($prods) > 1) {
			$this->db->begin();

			$sql = "UPDATE ".MAIN_DB_PREFIX."product SET";
            $sql .= " price_base_type='".$prices['base_price']."',";
            $sql .= " price=".$prices['price'].",";
            $sql .= " price_ttc=".$prices['price_ttc'].",";
            $sql .= " price_min=".$prices['price_min'].",";
			$sql .= " price_min_ttc=".$prices['price_min_ttc'];

			if ($copyBases) {
                $sql .= ", base_price=".$prices['price'];
                $sql .= ", base_price_ttc=".$prices['price_ttc'];
                $sql .= ", base_price_min=".$prices['price_min'];
                $sql .= ", base_price_min_ttc=".$prices['price_min_ttc'];
			}
			
			$sql_record = 'INSERT INTO '.MAIN_DB_PREFIX.'product_price (fk_product, date_price, price_level, price, price_ttc, price_min, price_min_ttc, price_base_type, fk_user_author, tva_tx) VALUES ';

			// Iteración sobre todos los productos de la categoría para actualizar al mismo tiempo
			$i = 0;
			foreach($prods as $prodid) {
				if ($prodid != $prices['prodid']) {
					if ($i++ == 0)
						$sql .= " WHERE rowid = ".$prodid;
					else {
						$sql .= " AND rowid = ".$prodid;
						$sql_record .= ', ';
					}

					$sql_record.= '('.$prodid.' , "'.date("Y-m-d H:i:s").'" ,1 ,'.$prices['price'].','.($prices['price'] * (1 + $prices['tva_tx']/100) ).', '.$prices['price_min'].','.($prices['price_min'] * (1 + $prices['tva_txt']/100) ).', "HT" , '.$user->id.', 16)';
				}
			}
			
			$res = $this->db->query($sql);
			if ($res) {
				if ($this->db->query($sql_record)) {
					$this->db->commit();
					return 1;
				}
				$this->db->rollback();
