...
class User extends CommonObject
{
	...
	public function fetch($id = '', $login = '', $sid = '', $loadpersonalconf = 0, $entity = -1, $email = '')
	{
		...
		$sql .= " u.admin,u.pos_pass, u.pos_desc, u.login, u.note as note_private, u.note_public,";
		...
		if ($result)
		{
			...
			if ($obj)
			{
				...
				$this->pospass		= $obj->pos_pass;
				$this->posdesc		= $obj->pos_desc;
				...
				$this->fk_warehouse = $obj->fk_warehouse ? $obj->fk_warehouse : -1;//Siempre cargar un almacén para cumplir validaciones
    ...
    /*
    * Fetch all users pos password checkpoint 130117
    */
    function fetch_all(){
        $sql = 'SELECT rowid, pos_pass, admin,pos_desc FROM '.MAIN_DB_PREFIX.'user';
        $res = $this->db->query($sql);
        if ($res) {
            $i=0;
            while ($users[$i] = $this->db->fetch_array($res)) {
                $i++;
            }
            return $users;
        }else{
	...
	public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
	{
		...
		if (!is_numeric($this->posdesc))
		{
			if (empty($this->posdesc))
			{
				$this->posdesc = 'NULL';
			}
			else
			{
				$this->error = 'El campo "Descuento para aplicar en POS" debe ser numérico.';
				return -1;
			}
		}
		...
		$sql .= ", pos_pass = '".$this->db->escape($this->pospass)."'";
		$sql .= ", pos_desc = ".$this->posdesc."";
		...
		$sql .= ", fk_account = ".($this->fk_account > 0 ? "'".$this->db->escape($this->fk_account)."'" : "null");
	...
	// Cronjob para limpiar terminales
	public function clean_terminals()
	{
		$sql = "UPDATE llx_user SET terminal = NULL WHERE terminal IS NOT NULL";
		$this->db->query($sql);
		return 0;
	}
}
