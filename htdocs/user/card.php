...
if (empty($reshook)) {
	...
	if ($action == 'add' && $canadduser) {
		...
		if (!$error) {
			...
			$object->pospass = GETPOST("pospass", 'alphanohtml');
			$object->posdesc = GETPOST("posdesc", 'alphanohtml');
...
if ($action == 'create' || $action == 'adduserldap')
{
    ...
    // Contraseña POS
    print '<tr><td class="tdtop">Contraseña POS para descuentos</td><td>';
    print '<input class="minwidth100" type="text" name="pospass" class="flat" value="'.GETPOST('pospass').'">';
    print "</td></tr>\n";

    // Descuento POS
    print '<tr><td class="tdtop">Descuento para aplicar en POS</td><td>';
    print '<input class="minwidth100" type="text" name="posdesc" class="flat" value="'.GETPOST('posdesc').'">';
    print "</td></tr>\n";
...
else
{
	...
	if ($id > 0)
	{
		...
		if ($action != 'edit')
		{
            ...
            // Contraseña POS
            print '<tr><td class="tdtop">Contraseña POS para descuentos</td><td>';
            print $object->pospass;
            print "</td></tr>\n";

            // Descuento POS
            print '<tr><td class="tdtop">Descuento para aplicar en POS</td><td>';
            print $object->posdesc."%";
            print "</td></tr>\n";

            // Habilitado para validar descuentos
            print '<tr><td class="tdtop">Habilitado para validar descuentos</td><td>';
            print $object->admin?"SI":"NO";
            print "</td></tr>\n";
		...
		if ($action == 'edit' && ($canedituser || $caneditfield || $caneditpassword || ($user->id == $object->id)))
		{
            ...
            // Contraseña POS
            print '<tr><td class="tdtop">Contraseña POS para descuentos</td><td>';
            print '<input class="minwidth100" type="text" name="pospass" class="flat" value="'.$object->pospass.'">';
            print "</td></tr>\n";

            // Descuento POS
            print '<tr><td class="tdtop">Descuento para aplicar en POS</td><td>';
            print '<input class="minwidth100" type="text" name="posdesc" class="flat" value="'.$object->posdesc.'">';
            print "</td></tr>\n";
			...
			//Bank Account
			print '<tr><td>'.$langs->trans("Account").'</td><td>';
			$form->select_comptes($object->fk_account, 'accountid', 0, '', 2);
			print '</td></tr>';
