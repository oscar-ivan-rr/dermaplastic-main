...
if (empty($reshook))
{
	...
	if ($action == 'create')
	{
	    ...
	    if (!$error)
	    {
		    ...
		    $date_debut = dol_mktime(GETPOST('date_debut_hour'), GETPOST('date_debut_min'), 0, GETPOST('date_debut_month'), GETPOST('date_debut_day'), GETPOST('date_debut_year'));
		    $date_fin = dol_mktime(GETPOST('date_fin_hour'), GETPOST('date_fin_min'), 0, GETPOST('date_fin_month'), GETPOST('date_fin_day'), GETPOST('date_fin_year'));
            ...
            if(GETPOST('date_debut_hour')<12){
                $starthalfday = "morning";
            }else{
                $starthalfday = "afternoon";
            }
            if(GETPOST('date_fin_hour')<12){
                $endhalfday = "morning";
            }else{
	...
	if ($action == 'confirm_delete' && GETPOST('confirm') == 'yes' && $user->rights->holiday->delete)
	{
		...
		$object->fetch($id);
        $files = $conf->holiday->dir_output."/" . $object->ref;
        dol_delete_dir_recursive($files);
	...
	if($action == "doc"){
        ...
        $res=$object->generateDocument("dias",$outputlangs);
        if($res){
            setEventMessages('Documento Firmado', null, 'mesgs');
        }
    }

    if ($action == 'digital_sign1' && $conf->global->MAIN_SIGNATURE_MODULE) {
        $id=$object->id;
        $ref = $object->ref;
        $element = "Holiday";
        $objectfile = "/holiday/class/holiday.class.php";
        $model_pdf = "dias";
        $dir = $conf->holiday->dir_output."/" . $object->ref."/firma1";
        $moreinputs = array('force_redirect' => 1, 'url' => DOL_URL_ROOT . '/holiday/card.php?id= '.$object->id.'&action=doc');
        include (DOL_DOCUMENT_ROOT .'/custom/signature/index.php');
        die();
    }
    if ($action == 'digital_sign2' && $conf->global->MAIN_SIGNATURE_MODULE) {
        $id=$object->id;
        $ref = $object->ref;
        $element = "Holiday";
        $objectfile = "/holiday/class/holiday.class.php";
        $dir = $conf->holiday->dir_output."/" . $object->ref."/firma2";
        $model_pdf = "dias";
        $moreinputs = array('force_redirect' => 1, 'url' => DOL_URL_ROOT . '/holiday/card.php?id= '.$object->id.'&action=doc');
        include (DOL_DOCUMENT_ROOT .'/custom/signature/index.php');
        die();
...
if ((empty($id) && empty($ref)) || $action == 'add' || $action == 'request' || $action == 'create')
{
    ...
    else
    {
        ...
        if (!GETPOST('date_debut_')) {
            print $form->selectDate(-1, 'date_debut_', 1, 1, 1, '', 1, 1);
        } else {
            $tmpdate = dol_mktime(GETPOST('date_debut_hour', 'int'), GETPOST('date_debut_minute', 'int'), 0, GETPOST('date_debut_month', 'int'), GETPOST('date_debut_day', 'int'), GETPOST('date_debut_year', 'int'));
            print $form->selectDate($tmpdate, 'date_debut_', 1, 1, 1, '', 1, 1);
        }
        //print ' &nbsp; &nbsp; ';
...
else
{
        ...
        if (($id > 0) || $ref)
        {
            ...
            if ($cancreate)
            {
                ...
                else
                {
                    ...
                    print '<td class="nowrap">'.$langs->trans('DateDebCP').' ('.$langs->trans("FirstDayOfHoliday").')</td>';
                    print '<td>';
                    print $form->selectDate($object->date_debut, 'date_debut_',1,1);
                    //print ' &nbsp; &nbsp; ';
                ...
                else
                {
                    ...
                    print '<td class="nowrap">'.$langs->trans('DateFinCP').' ('.$langs->trans("LastDayOfHoliday").')</td>';
                    print '<td>';
                    print $form->selectDate($object->date_fin, 'date_fin_',1,1);
                    //print ' &nbsp; &nbsp; ';
                ...
                if (!$edit)
                {
                    ...
                    if ($object->statut == Holiday::STATUS_APPROVED && $conf->global->MAIN_SIGNATURE_MODULE) {
                        if($user->id == $object->fk_user_create){
                            print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=digital_sign1">'.$langs->trans("Firma  Solicitante").'</a>';
                        }else{
                            print '<a href="#" class="butActionRefused classfortooltip" title="'.$langs->trans("Usuario no autorizado").'">'.$langs->trans("Firma  Solicitante").'</a>';
                        }
                        if($user->id == $object->fk_validator){
                            print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=digital_sign2">'.$langs->trans("Firma quien Aprueba").'</a>';
                        }else{
                            print '<a href="#" class="butActionRefused classfortooltip" title="'.$langs->trans("Usuario no autorizado ").'">'.$langs->trans("Firma  quien Aprueba").'</a>';
                        }
    ...
    if ($action != 'presend' && $action != 'editline')
    {
        ...
        $filedir = $conf->holiday->dir_output."/".$objectref;

        $urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;

        $genallowed = $user->rights->holiday->read;
        $delallowed = $user->rights->holiday->delete;


        print $formfile->showdocuments('holiday', $objectref, $filedir, $urlsource, 1, 1, 'Dias', 1, 0, 0, 28, 0, '', '', '', $soc->default_lang);
    }
}
