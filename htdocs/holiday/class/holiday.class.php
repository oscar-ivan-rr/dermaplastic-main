...
class Holiday extends CommonObject
{
	...
	public function create($user, $notrigger = 0)
	{
        ...
        $sql .= " ".(!empty($this->last_main_doc)?"'".$this->db->escape($this->last_main_doc)."'":"null");
		$sql .= ")";
	...
	public function fetchByUser($user_id, $order = '', $filter = '')
	{
		...
		if ($resql) {
			...
			while ($i < $num) {
                ...
                $tab_result[$i]['model_pdf'] = $obj->model_pdf;
                $tab_result[$i]['last_main_doc'] = $obj->last_main_doc;
	...
	public function approve($user = null, $notrigger = 0)
	{
        ...
        if (!empty($this->last_main_doc)) {
            $sql .= " last_main_doc = '".$this->db->escape($this->last_main_doc)."'";
        } else {
            $sql .= " last_main_doc = NULL";
        }
    ...
    public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
    {
        ...
        $modelpath = "core/modules/holiday/doc/";
        $this->origin='holiday';
        $this->fetch_origin();
