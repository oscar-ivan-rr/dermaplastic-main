...
if ($action == 'digital_sign' && $conf->global->MAIN_SIGNATURE_MODULE) {
	$ref = $object->ref;
	$element = "Livraison";
	$objectfile = "/livraison/class/livraison.class.php";
	$dir = $conf->expedition->dir_output."/receipt/" . $object->ref;
            ...
            */
			...
			if ($user->socid == 0)
			{
				...
				if ($object->statut == 1 && $conf->global->MAIN_SIGNATURE_MODULE) {
					print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=digital_sign">'.$langs->trans("UserSignature").'</a>';
				}
